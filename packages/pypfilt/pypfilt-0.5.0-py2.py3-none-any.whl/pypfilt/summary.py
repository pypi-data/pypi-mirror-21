...
class HDF5(object):
    ...
    def __init__(self, params, obs_list, meta=None, first_day=False,
        ...
        if using_cache and not only_fs:
            ...
            only_fs = True
            # TODO -- remove only_fs entirely, and always treat it as False?
            # logger.warning('Leaving only_fs=False ...')
        ...
        # TODO --- this means we always need a dummy observation.
        self.__obs_types = sorted(set((o['unit'], o['period'])
                                      for o in self.__all_obs))
        # TODO --- can instead query each observation model, as per
        #          epifx.summary.obs_types()
        # TODO --- move obs_types() and obs_units() into pypfilt?
        #      --- NO, epifx enforces a particular structure ...
    ...
    def save_state(self, grp):
        ...
        :param grp: The h5py Group object in which to save the state.
        """
        # TODO --- accept kwargs for create_dataset?
        grp_names = set()
        for mon in self.__monitors:
            grp_name = mon.__class__.__name__
            if grp_name in grp_names:
    ...
    def save_forecasts(self, fs, filename):
        ...
        with h5py.File(filename, 'w') as f:
            ...
            # TODO: add a "save_tables" method?
            # e.g., summary.save_tables(tbl_dict, path='/data')
            # and use require_group
