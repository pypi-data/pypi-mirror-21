...
class SliceUpload(object):
    ...
    def record_upload_progress(self, result, uploadBatch):
        ...
        if record_data['code'] == 200:
            ...
            debug('current size: {0:d}, total upload progress: {1:.2f}%'.format(int(blocksize), self.progress * 100))
    ...
    def result_analysis(self, results):
        ...
        for offset in self.offsetlist:
            for result in results:
                if offset == result['offset'] and result['code'] != 200:
                    fail_list.append(result['offset'])
        return fail_list
    ...
    def slice_upload(self):
        ...
        if len(offsets) != 0:
            ...
            if uploadBatch == 'Null':
                uploadBatch = GetUuid()
                self.progress = 0
                debug('current upload progress: %d' % (self.progress))
            else:
                ...
                debug('current upload progress: %d' % (current))
        ...
        if self.iscomplet(results):
            ...
            debug('Sorry, These block %s upload fail' % (fail_list ))
