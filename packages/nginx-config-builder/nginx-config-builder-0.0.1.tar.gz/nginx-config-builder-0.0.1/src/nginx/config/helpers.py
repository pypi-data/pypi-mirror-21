...
from .api import Config, Section, Location
...
def default_configuration(port=8080):
    """ Returns a standard default nginx config.
...
def option_generator(**kwargs):
    """ Supply key/value pairs and this function will yield appropriate option objects """
    for key, value in six.iteritems(kwargs):
        if isinstance(value, list):
            obj = KeyMultiValueOption
        else:
            obj = KeyValueOption
        yield obj(key, value=value)
