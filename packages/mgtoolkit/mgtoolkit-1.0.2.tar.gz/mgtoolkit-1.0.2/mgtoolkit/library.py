...
class Metagraph(object):
            ...
            for edge2 in edge_list:
                if edge1 != edge2:
                    if edge1.invertex == edge2.invertex and edge1.outvertex == edge1.outvertex:
                        match = True
                        comp1 = MetagraphHelper().extract_label_components(edge1.label)
                        comp2 = MetagraphHelper().extract_label_components(edge2.label)
    ...
    def dominates(self, metagraph2):
        """Checks if current metagraph dominates that provided"""
        if metagraph2 is None:
            raise MetagraphException('metagraph2', resources['value_null'])
...
class ConditionalMetagraph(Metagraph):
    ...
    def get_all_metapaths_from(self, source, target):
        ...
        return self.mg.get_all_metapaths_from(source.union(self.propositions_set), target)  # source
    ...
    @staticmethod
    def get_active_times(attributes):
        if attributes is None:
            raise MetagraphException('attributes', resources['value_null'])
        active_times = []
        max_mins = 60*24
...
class MetagraphHelper:
    ...
    def get_triples(self, triples_list):
    ...
    @staticmethod
    def extract_label_components(label):
