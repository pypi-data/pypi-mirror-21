...
def read_html(filepath, table=0, mode='r', **kwargs):
    ...
    def reader(f, **kwargs):
        ...
        if horizontal or len(ths) == 1:
            # the headers are vertical instead of horizontal
            horizontal = True
            rows = list(it.chain([first_row], rows))
            names = (row.find('th').string for row in rows)
        else:
            # TODO: figure out if I can just do (th.string for th in ths)
        ...
        if horizontal:
            for num, _ in enumerate(first_row.find_all('td')):
                row = (tr.find_all('td')[num].string for tr in rows)
