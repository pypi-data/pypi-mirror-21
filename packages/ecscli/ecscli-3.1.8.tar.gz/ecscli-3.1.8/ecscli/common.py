...
def getToken():
    ...
    try:
        ...
        raise SOSERROR(SOSError.NOT_FOUND_ERR,
                           cookiefile + " : Cookie file could not be opened")
    return token
        ...
        if (http_method == 'GET'):
            ...
            if(filename):
                if(requests.__version__ == "2.3.0"):
                    response = requests.get(url, data=body, stream=True, headers=headers,
                                        verify=False, cookies=cookiejar)
                else:
                    response = requests.get(url, data=body, prefetch=False, headers=headers,
                                        verify=False, cookies=cookiejar)
