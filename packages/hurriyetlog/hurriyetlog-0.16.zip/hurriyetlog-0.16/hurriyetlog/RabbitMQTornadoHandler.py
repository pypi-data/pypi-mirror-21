...
class RabbitMQTornadoHandler(logging.Handler):
    """
     ...
     Example setup::
        ...
        handler = RabbitMQTornadoHandler("amqp://guest:guest@localhost/%2f", queue="HurLogQueue","application name")
    """
    def __init__(self, uri="", queue="", appname=""):
        ...
        try:
            self.connection = pika.TornadoConnection(pika.URLParameters(uri))
            self.queue = queue
    ...
    def emit(self, record):
        try:
            log_obj = hurriyetlog.HurriyetLog(record, self.appname)
