...
.. |export_supersample_description| replace:: Controls the amount of antialiasing used in the image.
            ...
            Antialiasing smooths jagged edges on text, lines, and edges of
            image output formats by the process of supersampling. A large
            intermediate image is rendered and then reduced to the final image
            size. Each pixel on the final image is created from multiple
            rendered pixels. The width and height of the intermediate image
            are the width and height of the final image times some scale factor.
            This scale factor is the Supersample Factor. You can use values
            from **2** to **16**. Factors greater than **3** are seldom necessary.
            Large scale factors take a lot more time and memory. Some graphics
            cards limit the dimensions of rendered images to a maximum of
            2048x2048 or 4096x4096 pixels, and thus Tecplot cannot
            antialias if the intermediate image would be larger than this limit.
            Antialiasing uses many colors. Antialiasing with 256 colors wastes
            time and may decrease plot quality. (default: `None`)
.. |export_width_description| replace:: Specify a width in pixels for the generated image.
            ...
            and the larger the exported file.
...
.. |export_convert256_description| replace:: Pass `True` to generate an image with no more than 256 colors
            ...
            or antialiasing may result in poor image quality. (default: `None`)
