...
#from .tecutil_remote import TecUtilRemote
...
class TecUtilConnector(object):
    ...
    def connect(self, host='localhost', port=4222, timeout=3):
        '''
        if self.socket is None:
            ...
            if self.socket is None:
                msg = 'could not connect to {}:{}'.format(host,port)
                raise TecplotConnectionError(msg)
            self.tecutil_remote = TecUtilRemote(self.socket)
            msg = 'connected to Tecplot 360 instance on {}:{}'
            log.info(msg.format(host, port))
        '''
        ...
        if not hasattr(self, '_tecsdkhome'):
            try:
                ...
                if self.libbatch_path is not None:
                    ...
                    self.tecsdkhome = path.dirname(tecutilbatch_dir)
                else:
                    self.tecsdkhome = ''
    ...
    def load_windows(self):
        ...
        if self.handle is None:
            ...
            for libfmt in ['{name}d.dll', '{name}.dll']:
                ...
                self.libbatch_path = find_file([libinterproc], os.environ['PATH'])
                if self.libbatch_path is not None:
                    try:
                        # Temporarily place 360EX first in the path so that we can
                        # be sure the right Qt dlls are loaded.
