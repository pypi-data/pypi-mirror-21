...
class TestFrame(unittest.TestCase):
    ...
    def test_load_stylesheet(self):
        # stylesheet for examples/3D_Volume/orb.lpk
        data = dedent('''\
            #!MC 1410
        ...
        try:
            ...
            tp.new_layout()
            tp.load_layout(path.join(exdir,'3D_Volume','orb.lpk'))
            ...
            self.assertEqual(plot.contour(0).colormap_name, 'Modern')
    ...
    def test_save_stylesheet(self):
        ...
        try:
            ...
            with open(fstycopy.name) as f:
                ...
                self.assertRegex(datacopy, "\$\!PLOTTYPE  = CARTESIAN3D")
                self.assertRegex(datacopy, "COLORMAPNAME = 'Modern'")
                self.assertRegex(datacopy, "SHOWMESH = NO")
                self.assertRegex(datacopy, "SHOWCONTOUR = YES")
