...
if _swig_python_version_info >= (2, 7, 0):
    def swig_import_helper():
        ...
        mname = '.'.join((pkg, '_bmc_utils')).lstrip('.')
        try:
            return importlib.import_module(mname)
        except ImportError:
            return importlib.import_module('_bmc_utils')
    _bmc_utils = swig_import_helper()
    ...
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_bmc_utils', [dirname(__file__)])
        except ImportError:
            import _bmc_utils
            return _bmc_utils
        if fp is not None:
            try:
                _mod = imp.load_module('_bmc_utils', fp, pathname, description)
            finally:
                fp.close()
            return _mod
    _bmc_utils = swig_import_helper()
...
def MEMOIZER_key(formula: 'node_ptr', time: 'int', k: 'int', l: 'int', offset: 'int') -> "node_ptr":
    """MEMOIZER_key(node_ptr formula, int time, int k, int l, int offset) -> node_ptr"""
