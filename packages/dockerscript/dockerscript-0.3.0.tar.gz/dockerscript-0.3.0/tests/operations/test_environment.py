...
class TestEnvironment(unittest.TestCase):
    ...
    def test_key_value(self):
        """
        Tests that variables can be set as a key/value pair
        """
        ...
        dockerscript.operations.environment(image, key, value)

        self.assertListEqual(
            [
                dockerscript.operations.EnvironmentOperation({key: value}),
            ],
            image.operations,
            'Operation for a single key/value pair was added',
        )

    def test_key_no_value(self):
        """
        Tests that an error is raised when missing a value
        """

        image = dockerscript.Image()

        with self.assertRaises(ValueError):
            dockerscript.operations.environment(image, 'key')
