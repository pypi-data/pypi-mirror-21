...
class NotProjectAssistantError(Exception):
  ...
  def __init__(self):
    self.message = "fatal: Not an udacity project assistant directory (no .udacity-pa folder)"

try:
  d = os.path.join(os.path.realpath(os.getcwd()),'.udacity-pa')
  sys.path.append(os.path.join(os.path.realpath(os.getcwd()),'.udacity-pa'))
  import projects
  projects.nanodegree
  projects.projects
  projects.submit
