...
class RequestActive(Request):
    ...
    async def _read(self, lines):
        ...
        async for line in lines:
            ...
            try:
                parser.send(line[:-1].decode('utf-8'))
...
class RequestCommandLine(RequestActive):
    ...
    def _parser(self, success=SUCCESS):
        ...
        while True:
            ...
            if match:
                self.set_exception(errors.ReplyError(int(match.group(1)), int(match.group(2)), match.group(3), match.group(4)))
                return
            elif line == success:
                self.set_result(lines if self._transform is None else self._transform(lines))
                return
            else:
                lines.append(line.split(DELIM, 1))
...
Event = enum.IntEnum('Event', EVENTS)
del EVENTS
...
class RequestPassive(Request):
    ...
    def __init__(self, executor, event_mask, *, timeout=None):
        ...
        if timeout is None:
            ...
            self._timeout = asyncio.Task(self._timeout_task(timeout))
            self._event_mask |= Event.TIMEOUT
            self.add_done_callback(lambda self_: self_._timeout.cancel())

    def __await__(self):
        if self._timeout:
            asyncio.ensure_future(self._timeout)
        return (yield from super().__await__())

    async def _timeout_task(self, timeout):
        try:
            await asyncio.sleep(timeout)
            self.set_result(Event.TIMEOUT)
