...
class Client:
    ...
    def send_file(self, file):
        ...
        if os.path.exists(file):
            ...
            if os.path.isfile(file):
                ...
                header_l = pack(">I", len(header))
                # return header_l + header + data
                # loger.debug("start send: len: %d" % len(header))
    ...
    def check(self):
        ...
        # cprint("remote: {}".format(hash_set),"red")
        hash_set_local = set(Dir(Client.root, Client.io).all())
        
        # cprint("local:{}".format(hash_set_local), "green")
        # cprint(Client.root, "blue")
