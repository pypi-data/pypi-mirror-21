...
def export(bld):
	...
	targets = wtools.deps.get_targets(bld)
...
class CDTProject(EclipseProject):
	...
	*Eclipse* and used to run and/or debug C/C++ programs. 
	...
	def cconfig_external_settings_update(self, module):
		deps = wtools.deps.get_deps(self.bld, self.tgen.get_name())
		tgens = wtools.deps.get_tgens(self.bld, deps)
		for tg in tgens:
	...
	def compiler_add_includes(self, compiler, language):
		if self.language != language:
			return
		uses = wtools.deps.get_deps(self.bld, self.tgen.get_name())
...
class CDTLaunch(Project):
	...
	def get_content(self):
		...
		deps = wtools.deps.get_deps(self.bld, self.tgen.get_name())
		for tg in wtools.deps.get_tgens(self.bld, deps):
