...
from .constants import INFINITE_COMMAND, MAX_API_ATTEMPTS
...
class DockerClient(Client):
    ...
    def run_container_infinitely(self, image, infinite_command=None, volumes=None, container_limits=None):
        infinite_command = infinite_command or INFINITE_COMMAND
        volumes = volumes or {}
    ...
    def commit_container(self, container, conf, author, message, tags=None):
        tags = tags or []
        if IS_OLD_API:
            image = self.client.commit(container=container,
                                       conf=conf,
                                       author=author,
                                       message=message).get('Id')
            for repo, tag in tags:
                self.client.tag(image=image,
                                repository=repo,
                                tag=tag)
            return image
        else:
            container = self.client.containers.get(container)
            image = container.commit(conf=conf,
                                     author=author,
                                     message=message)
            for repo, tag in tags:
                image.tag(repo, tag)
