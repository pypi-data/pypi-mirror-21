...
def get_postgresql(provider_url):
    ...
    if not this.psycopg2_available:
        raise Exception('You must install the psycopg2 python package to use a database.')
...
def get(provider_name, provider_key=None):
    ...
    if not found_resource_path:
        data = get_postgresql(provider_url)
        if data is None:
            raise cherrypy.HTTPError(404, "Unable to locate data provider for \'%s\'" % provider_url)
