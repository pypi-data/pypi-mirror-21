...
- Added: Introduced new ``__Resource.requirements`` class attribute where all Redmine plugins
...
- Added: Introduced new ``__Resource._members`` class attribute where all instance attributes
...
- Changed: ``__Resource.attributes`` renamed to ``__Resource._attributes``
...
- Added: ``Redmine.auth()`` shortcut for the case if we just want to check if user provided
  ...
  Redmine user database (see `docs <https://python-redmine.com/advanced/
...
- Added: An ability to create custom resources which allow to easily redefine the behaviour
  of existing resources (see `docs <https://python-redmine.com/advanced/
