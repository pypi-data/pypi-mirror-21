...
class Mounts(Jsonizable):
    ...
    def _validate_entry(self, entry):
        return copy.deepcopy(entry) if isinstance(entry, MountEntry) else MountEntry(entry)
...
class ModifyClusterDescription(Jsonizable):
    ...
    def _validate_group(self, group):
        return copy.deepcopy(group) if isinstance(group, ModifyGroupDescription) else ModifyGroupDescription(group)
