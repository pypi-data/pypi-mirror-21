...
class FakeResponsePackages(object):
    ...
    def read(self):
        ...
        """
        arcbase = 'https://download.fedoraproject.org/pub/archive/fedora/linux/'
...
class TestRelease:
    ...
    @pytest.mark.parametrize(
        ("release", "milestone", "compose", "respin", "expclass",
        ...
        [
            # Archive stable.
            (15, '', '', None, fedfind.release.ArchiveRelease, '15', None, None, None),
            # Fedora Core stable (release string).
            ('6', '', '', None, fedfind.release.CoreRelease, '6', None, None, None),
            # 'Final' treated as stable.
            ('6', 'Final', '', None, fedfind.release.CoreRelease, '6', None, None, None),
            # Milestone.
            (23, 'Beta', '', None, fedfind.release.Milestone, '23', 'Beta', None, None),
            # Branched nightly.
            (23, 'Branched', 'datestr', 1,
            ...
            # Atomic nightly.
            (24, 'Atomic', 'datestr', 0,
             fedfind.release.AtomicNightly, '24', 'Atomic', 'datestr', '0'),
            # Docker nightly.
            (25, 'Docker', 'datestr', 0,
             fedfind.release.DockerNightly, '25', 'Docker', 'datestr', '0'),
            # Cloud nightly.
            (25, 'Cloud', 'datestr', 0,
             fedfind.release.CloudNightly, '25', 'Cloud', 'datestr', '0'),
            # Wikitcms format Rawhide.
            ('24', 'Rawhide', 'datestr.n.0', None,
            ...
            (25, 'Docker', '', 0, fedfind.release.DockerNightly, '25', 'Docker', 'datestr', '0'),
            (25, 'Cloud', '', 0, fedfind.release.CloudNightly, '25', 'Cloud', 'datestr', '0'),
            # Default, check None is handled.
            (None, None, None, 0, fedfind.release.RawhideNightly, 'Rawhide', '', 'datestr', '0'),
        ]
    )
    def test_get_release_simple(self, release, milestone, compose, respin, expclass, exprel,
    ...
    def test_get_release_url_cid(self):
        """Getting a release based on a correct nightly compose URL
        should work.
        """
    ...
    def test_get_stable_current(self):
        """Ensure that if ArchiveRelease doesn't exist, get_release
        for a recent stable returns CurrentRelease. Tested number must
        be higher than the 'just return ArchiveRelease' cutoff, so
        bump this test when bumping that cutoff.
        """
    ...
    def test_get_stable_archive(self):
        """Ensure that if ArchiveRelease does exist, get_release for
        a recent stable returns it. Tested number must be higher than
        the 'just return ArchiveRelease' cutoff (or else this test
        will pass even if the feature is broken), so bump this test
        when bumping that cutoff.
        """
        ret = fedfind.release.get_release(23)
    ...
    @pytest.mark.parametrize(
        ...
        [
            # Branched nightly.
            (23, 'Branched', 'datestr', 1, 'Fedora-23-datestr.n.1'),
            # Rawhide nightly.
            ('Rawhide', '', 'datestr', 2, 'Fedora-Rawhide-datestr.n.2'),
            # Atomic nightly.
            (24, 'Atomic', 'datestr', 0, 'Fedora-Atomic-24-datestr.0'),
            # Docker nightly.
            (25, 'Docker', 'datestr', 0, 'Fedora-Docker-25-datestr.0'),
            # Cloud nightly.
            (25, 'Cloud', 'datestr', 0, 'Fedora-Cloud-25-datestr.0'),
        ]
    )
    def test_get_release_cid_passthru(self, release, milestone, compose, respin, cid):
        """Check compose ID is passed through from get_release to the
        Release instance in various cases.
        """
        datestr = datetime.date.today().strftime('%Y%m%d')
        compose = compose.replace('datestr', datestr)
        cid = cid.replace('datestr', datestr)
        got = fedfind.release.get_release(release, milestone, compose, respin, cid=cid)
        assert got._cid == cid
    ...
    def test_images_respinrelease(self, http):
        ...
        exp = ['Cinnamon', 'KDE', 'LXDE', 'Mate', 'Workstation', 'Xfce']
