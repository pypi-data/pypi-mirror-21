...
_Config = NamedTuple('_Config', [('edp_url', str), ('bearer_token', str)])
...
class EdpClient(object):
    ...
    def __init__(self,
        ...
        Args:
            profile: The name of a profile to use. If not provided then the
    ...
    def _read_edp_auth(profile_name):
        ...
        config_path = os.path.expanduser(os.path.join('~', '.edp_auth'))
    ...
    def _upload_population_as_generator(
            self, generator, name, parent_pid=None):
        ...
        if parent_pid is None:
            url = edp_url + '/rpc/population_model/upload_generator'
        else:
