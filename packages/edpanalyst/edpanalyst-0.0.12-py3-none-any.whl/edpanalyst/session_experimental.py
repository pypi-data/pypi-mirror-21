...
class PopulationModelExperimental(object):
        ...
        for model in resp['models']:
            for view in model['views']:
                for cluster in view['clusters']:
                    ...
                    cluster['rowids'] = [rowids[rowNum]
                                         for rowNum in cluster['rowNums']]
                    del cluster['rowNums']
        return resp
    ...
    def infer_plot(self, target, rowid):  # type: (str, int) -> plt
        ...
        def pdf(x):
            return self._popmod.joint_probability(
                targets={target: list(x)}, given=given)['p']

        sel = self._popmod.select(targets=[target], limit=100)[target]
        ...
        inf_x = self._popmod.infer(
            targets=[target], rowids=[rowid], infer_present=True)[target]
        inf_y = pdf(inf_x)[0]
        inf_plt, = ax.plot(
            inf_x, inf_y,
            marker='o', markersize=10, color='red', label='Inferred')
        handles = [inf_plt]
        ...
        if real_x:
            ...
            real_plt, = ax.plot(
                real_x, real_y,
                marker='o', markersize=10, color='black', label='Real')
