...
from configdeck._compat import (
    DEFAULTSECT,
...
class TestIncludes(unittest.TestCase):
    ...
    def test_basic_include(self):
        ...
        parser.read_file(config, 'my.cfg')
        self.assertEqual({'__main__': {'foo': 'bar'}}, parser.values())
    ...
    @patch('configdeck.parser.logger.warning')
    @patch('configdeck.parser.codecs.open')
    ...
    def test_relative_include(self):
        ...
        def setup_config():
            ...
            f = codecs.open(
                "%s/first.cfg" % folder, 'w', encoding=CONFIG_FILE_ENCODING)
            ...
            f = codecs.open(
                "%s/second.cfg" % folder, 'w', encoding=CONFIG_FILE_ENCODING)
            f.write(textwrap.dedent("""\
                [__main__]
                bar=2
            ...
            f = codecs.open(
                "%s/sub/third.cfg" % folder,
                'w', encoding=CONFIG_FILE_ENCODING)
            f.write(textwrap.dedent("""\
                [__main__]
                includes=../fourth.cfg
                """))
            ...
            f = codecs.open(
                "%s/fourth.cfg" % folder, 'w', encoding=CONFIG_FILE_ENCODING)
            f.write(textwrap.dedent("""\
                [__main__]
                baz=3
                """))
    ...
    def test_multiple_includes(self):
        ...
        def setup_config():
            ...
            f = codecs.open(
                "%s/third.cfg" % folder, 'w', encoding=CONFIG_FILE_ENCODING)
            f.write(textwrap.dedent("""\
                [__main__]
                foo = 3
                bar = 3
...
class TestInterpolation(unittest.TestCase):
    ...
    @patch('configdeck.parser.os')
    @patch('configdeck.parser.re.compile')
    ...
    def test_interpolate_value_no_keys(self):
        ...
        parser.read_file(config)
        with patch.object(
                parser, '_get_interpolation_keys',
...
class TestSchemaConfigParser(unittest.TestCase):
    def setUp(self):
        ...
        self.config = BytesIO(textwrap.dedent("""\
            [__main__]
            foo = bar
            """).encode(CONFIG_FILE_ENCODING))
    ...
    def test_init_invalid_schema(self):
        ...
        self.assertRaises(
            SchemaValidationError,
            SchemaConfigParser, MyInvalidSchema())

    def test_items(self):
        self.parser.read_file(self.config)
    ...
    def test_multi_file_dict_config(self):
        ...
        config1 = BytesIO(textwrap.dedent("""\
            [__main__]
            foo=mydict
            [mydict]
            bar=1
            baz=1
            """).encode(CONFIG_FILE_ENCODING))
        config2 = BytesIO(textwrap.dedent("""\
            [mydict]
            baz=2
            """).encode(CONFIG_FILE_ENCODING))
        ...
        parser = SchemaConfigParser(MySchema())
        parser.read_file(config1)
        parser.read_file(config2)
        self.assertEqual(parser.values(), expected_values)
    ...
    def test_multi_file_dict_list_config(self):
        ...
        config3 = BytesIO(textwrap.dedent("""\
            [otherdict]
            baz=3
            """).encode(CONFIG_FILE_ENCODING))
        expected_values = {'__main__': {'foo': [{'bar': 2, 'baz': 3}]}}
        parser.read_file(config3)
        self.assertEqual(parser.values(), expected_values)

        # reuse existing dict
        config4 = BytesIO(textwrap.dedent("""\
            [__main__]
            foo=mydict
                otherdict
            """).encode(CONFIG_FILE_ENCODING))
    ...
    def test_read_utf8_encoded_file(self):
        ...
        f = codecs.open(filename, 'w', encoding=CONFIG_FILE_ENCODING)
        f.write(textwrap.dedent("""\
            [__main__]
            foo=€
            """))
    ...
    def test_read_file_with_utf8_encoded_text(self):
        config = BytesIO(textwrap.dedent("""\
            [__main__]
            foo=€
            """).encode(CONFIG_FILE_ENCODING))
    ...
    def test_write(self):
        ...
        parser.write(
            codecs.open(filename, 'w', encoding=CONFIG_FILE_ENCODING))
        result = codecs.open(
            filename, 'r', encoding=CONFIG_FILE_ENCODING,
            ).read().strip()
        self.assertEqual(result, expected)
    ...
    def _check_save_file(self, expected, read_config=True):
        ...
        with codecs.open(
                filename, 'w', encoding=CONFIG_FILE_ENCODING) as configfile:
            self.parser.save(configfile)
        with codecs.open(
                filename, 'r', encoding=CONFIG_FILE_ENCODING) as configfile:
            result = configfile.read().strip()
        self.assertEqual(result, expected)
    ...
    def test_save_config_same_files(self):
        ...
        data = codecs.open(
            "%s/first.cfg" % folder, encoding=CONFIG_FILE_ENCODING,
            ).read()
        ...
        data = codecs.open(
            "%s/second.cfg" % folder, encoding=CONFIG_FILE_ENCODING,
            ).read()
    ...
    def test_save_config_last_location_nested_includes(self):
        ...
        with codecs.open(
                "%s/first.cfg" % folder,
                encoding=CONFIG_FILE_ENCODING) as configfile:
            data = configfile.read()
        expected_data = textwrap.dedent("""\
            [__main__]
            foo=1""")
        self.assertEqual(data.strip(), expected_data)
        with codecs.open(
                "%s/third.cfg" % folder,
                encoding=CONFIG_FILE_ENCODING) as configfile:
            data = configfile.read()
...
class TestParserIsValid(unittest.TestCase):
    ...
    def test_basic_is_not_valid_with_report(self):
        ...
        config = BytesIO(textwrap.dedent("""\
            ...
            """).encode(CONFIG_FILE_ENCODING))
        errors = [
            "Configuration includes invalid options"
            " for section '__main__': bar"]
...
class SchemaConfigParser_readfp_TestCase(unittest.TestCase):
    """ Test cases for method `SchemaConfigParser.readfp`. """

    def test_raises_deprecationwarning(self):
        """ Should raise DeprecationWarning and recommend ‘read_file’. """
        if PY2:
            self.skipTest(
                "'ConfigParser.readfp' is not deprecated in Python 2")
        ...
        self.assertIn("read_file", str(warning.message))
