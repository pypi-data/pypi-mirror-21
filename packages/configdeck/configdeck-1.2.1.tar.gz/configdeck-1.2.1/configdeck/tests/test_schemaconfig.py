...
class TestSchemaConfigDeck(unittest.TestCase):
    ...
    def test_deck_environ_precedence(self):
        with patch.object(
                os, 'environ', {'CONFIGDECK_FOO_BAR': '42', 'BAR': '1'}):
    ...
    def test_help(self):
        ...
        with patch('sys.stdout', new_callable=new_callable) as mock_stdout:
            ...
            self.assertRaises(
                SystemExit,
                schemaconfigdeck, self.parser, argv=['--help'])
    ...
    def test_parser_set_with_encoding(self):
        ...
        self.assertEqual(
            parser.get('__main__', 'foo', parse=False),
            'fóobâr')
    ...
    def test_option_conflicting_short_name(self):
        ...
        self.assertRaises(
            OptionConflictError,
            schemaconfigdeck, parser, argv=['-f', '42'])
...
class ConfigdeckTestCase(unittest.TestCase):
    ...
    def test_configdeck_no_errors(
            self,
            mock_schemaconfigdeck,
            mock_schema_parser,
    ):
        ...
        expected_args = Mock()
        mock_schemaconfigdeck.return_value = (
            expected_option_parser, expected_options, expected_args)
        ...
        mock_schemaconfigdeck.assert_called_with(
            expected_schema_parser, op=None)
    ...
    @patch('configdeck.deck.SchemaConfigParser')
    @patch('configdeck.deck.schemaconfigdeck')
    def test_configdeck_with_errors(
            self,
            mock_schemaconfigdeck,
            mock_schema_parser,
    ):
    ...
    @patch('configdeck.deck.SchemaConfigParser')
    @patch('configdeck.deck.schemaconfigdeck')
    def test_configdeck_with_options(
            self,
            mock_schemaconfigdeck,
            mock_schema_parser,
    ):
