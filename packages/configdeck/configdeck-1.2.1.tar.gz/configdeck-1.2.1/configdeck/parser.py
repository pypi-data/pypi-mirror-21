...
class SchemaConfigParser(BaseConfigParser, object):
    ...
    def __init__(self, schema, *args, **kwargs):
        super(SchemaConfigParser, self).__init__(*args, **kwargs)
        # validate schema
    ...
    def is_valid(self, report=False):
        ...
        try:
            ...
            if unmatched_sections:
                error_msg = (
                    "Sections in configuration are missing from schema: %s")
                error_value = ', '.join(unmatched_sections)
                errors.append(error_msg % error_value)
                valid = False
            ...
            for name in config_sections.union(schema_sections):
                if name not in skip_sections:
                    ...
                    fatal_options = set(
                        opt.name for opt in schema_options
                        if opt.fatal)
                    # all fatal options are included
                    fatal_included = parsed_options.issuperset(fatal_options)
                    if not fatal_included:
                        error_msg = (
                            "Configuration missing required options"
                            " for section '%s': %s")
                        error_value = ', '.join(
                            list(fatal_options - parsed_options))
                    ...
                    if not schema_options:
                        error_msg = (
                            ...
                            " for section '%s': %s")
                        error_value = ', '.join(
                            list(other_options - schema_opt_names))
                        errors.append(error_msg % (name, error_value))
    ...
    def read(self, filenames, already_read=None, *args, **kwargs):
        ...
        for filename in filenames:
            ...
            sub_parser._read(
                fp, path, *args, already_read=already_read, **kwargs)
    ...
    def read_file(self, f, source=None):
        """ Like ConfigParser.read_file, but consider the encoding. """
        decoded_file = codecs.getreader(CONFIG_FILE_ENCODING)(f)
        super(SchemaConfigParser, self).read_file(decoded_file, source)
    ...
    def _update(self, fp, fpname, *args, **kwargs):
        # remember current values
        old_sections = copy.deepcopy(self._sections)
        # read in new file
        super(SchemaConfigParser, self)._read(fp, fpname, *args, **kwargs)
        ...
        for section, options in sections.items():
            ...
            if old_section is not None:
                ...
                for option, value in options.items():
                    ...
                    option_changed = (
                        option not in old_section
                        or value != old_section[option])
    ...
    def _extract_interpolation_keys(self, item):
        ...
        if isinstance(item, (list, tuple)):
            item_collection = set(item)
        elif isinstance(item, dict):
            item_collection = set(item.values())
        if item_collection is not None:
            keys = [
                self._extract_interpolation_keys(x) for x in item_collection]
    ...
    def _get_interpolation_keys(self, section, option):
        ...
        try:
            ...
            if isinstance(opt, (DictOption, ListOption)):
                value = opt.parse(rawval, parser=self, raw=True)
            else:
        ...
        for key in keys:
            ...
            try:
                ...
                for section in ('__main__', '__noschema__'):
                    try:
                        value = super(SchemaConfigParser, self).get(
                            section, key)
                    except (NoSectionError, NoOptionError):
                        continue
                    else:
    ...
    def get(
            self,
            section, option,
            raw=False, vars=None, parse=True,
            *args,
            **kwargs):
    ...
    def set(self, section, option, value, *args, **kwargs):
        ...
        if not option_obj.validate(value):
            raise TypeError(
                "{0} is not a valid {1} value.".format(
        ...
        super(SchemaConfigParser, self).set(
            section, option, str_value,
            *args, **kwargs)
            ...
            for filename, sections in self._dirty.items():
                ...
                if filename is None:
                    ...
                    if not getattr(self, '_last_location', None):
                        ...
                        raise ValueError(
                            "No config files where read and no location"
                            " was specified for writing the configuration.")
                    else:
                ...
                with codecs.open(
                        "%s.new" % filename, 'w',
                        encoding=CONFIG_FILE_ENCODING) as outfile:
