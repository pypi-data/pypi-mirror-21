...
class DevServerSchema(Schema):
    """Configdeck schema for django-devserver."""
    ...
    class devserver(Section):
        devserver_args = ListOption(
            ...
            help=(
                ...
                ' to the runserver command (as defaults).'))
        ...
        devserver_wsgi_middleware = ListOption(
            ...
            default=[],
            help=(
                'A list of additional WSGI middleware to apply'
        ...
        devserver_ignored_prefixes = ListOption(
            ...
            help=(
                'List of prefixes to supress and skip process on.'
                ' By default, ADMIN_MEDIA_PREFIX, MEDIA_URL and STATIC_URL'
                ' (for Django >= 1.3) will be ignored'
                ' (assuming MEDIA_URL and STATIC_URL is relative).'))
        ...
        devserver_sql_min_duration = IntOption(
            default=None,
            help=(
                'Minimum time a query must execute to be shown,'
                ' value is in ms.'))
