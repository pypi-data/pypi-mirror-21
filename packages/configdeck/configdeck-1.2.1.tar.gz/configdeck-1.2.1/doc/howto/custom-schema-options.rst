...
..  currentmodule:: configdeck.schema

Introduction
============

The :doc:`schema reference </topics/schemas>` documentation explains how to
use configdeck's standard option classes --
:class:`~configdeck.schema.BoolOption`,
:class:`~configdeck.schema.IntOption`, etc. For many purposes,
those classes are all you'll need. Sometimes, though, the configdeck version
won't meet your precise requirements, or you'll want to use a option that is
entirely different from those shipped with configdeck.

configdeck's built-in option types don't cover every possible data type --
...
own configdeck :class:`~configdeck.schema.Option` subclasses.

Writing an option subclass
==========================

When planning your :class:`~configdeck.schema.Option` subclass,
first give some thought to which existing
:class:`~configdeck.schema.Option` class your new option
is most similar to. Can you subclass an existing configdeck option and save
yourself some work? If not, you should subclass the
:class:`~configdeck.schema.Option` class, from which everything
is descended.

Initializing your new option is a matter of separating out any arguments that are
specific to your case from the common arguments and passing the latter to the
:meth:`~configdeck.schema.Option.__init__` method of
:class:`~configdeck.schema.Option` (or your parent class).

In our example, we'll call our option ``UpperCaseDictOption``. (It's a
good idea to call your :class:`~configdeck.schema.Option`
subclass ``<Something>Option``, so it's easily identifiable as a
:class:`~configdeck.schema.Option` subclass.) It behaves
mostly like a :class:`~configdeck.schema.DictOption`, so we'll
subclass from that::
    ...
    class UpperCaseDictOption(schema.DictOption):
        ...
        def parse(self, section, parser=None, raw=False, *args, **kwargs):
            parsed = super(UpperCaseDictOption, self).parse(
                section, parser, raw, *args, **kwargs)
            result = {}
...
..  note::
    ...
    to :class:`~configdeck.schema.StringOption`.
