...
class RavenSchema(Schema):
    ...
    class raven(Section):
        ...
        sentry_exclude_paths = ListOption(
            item=StringOption(),
            help=(
                'Ignore module prefixes when attempting to'
                ' discover which function an error comes from.'))
        ...
        sentry_name = StringOption(
            null=True,
            help=(
                'This will override the server_name value'
                ' for this installation.'))
        sentry_auto_log_stacks = BoolOption(
            default=False,
            help=(
                'Should raven automatically log frame stacks'
        ...
        sentry_max_length_list = IntOption(
            ...
            help=(
                ...
                ' a list-like container should store.'))
        sentry_site = StringOption(
            null=True,
            help=(
                'An optional, arbitrary string to'
                ' identify this client installation.'))
        sentry_public_key = StringOption(
            null=True,
            help=(
                'Public key of the project member which will'
                ' authenticate as the client.'))
        sentry_private_key = StringOption(
            null=True,
            help=(
                'Private key of the project member which will'
                ' authenticate as the client.'))
        sentry_project = IntOption(
            default=1,
            help=(
                'Sentry project ID.'
                ' The default value for installations is 1.'))
        sentry_processors = ListOption(
            ...
            help=(
                ...
                ' before sending them to the Sentry server.'))
