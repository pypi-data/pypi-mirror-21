...
from configdeck.app.base import (
    App,
    Config,
    )
...
def make_app(
        name=None, schema=None, plugin_manager=None, validate=False,
        parser=None):
    ...
    with patch('configdeck.deck.sys', mock_sys):
        with patch('configdeck.app.base.sys.stderr'):
            app = App(
                name=name, schema=schema, plugin_manager=plugin_manager,
                parser=parser)
    return app
...
class ConfigTestCase(TestCase):
    ...
    @patch('configdeck.app.base.merge')
    @patch('configdeck.app.base.Config.get_config_files')
    @patch('configdeck.app.base.configdeck')
    def test_constructor(
        ...
        self.assertEqual(config.deck, mock_configdeck.return_value)
        mock_configdeck.assert_called_with(
            mock_merge.return_value, mock_get_config_files.return_value,
            op=app.parser)

    def test_deck_valid_config(self):
        config = make_config()
        self.assertEqual(config.deck.schema_parser.is_valid(), True)

    def test_deck_validate_invalid_config(self):
        class MySchema(Schema):
            foo = IntOption(fatal=True)

        self.assertRaises(
            SystemExit,
            make_app, schema=MySchema, validate=True)

    def test_deck_no_validate_invalid_config(self):
        class MySchema(Schema):
            foo = IntOption(fatal=True)
    ...
    @patch('xdg.BaseDirectory.os.path.exists')
    def test_get_config_files_with_plugins_full_hierarchy(
            self,
            mock_path_exists,
    ):
        mock_path_exists.return_value = True
...
class AppTestCase(TestCase):
    ...
    @patch('configdeck.app.base.Config')
    ...
    def test_default_parser(self):
        ...
        self.assertEqual(app.config.deck.option_parser, app.parser)
    ...
    def test_custom_parser(self):
        ...
        self.assertEqual(app.config.deck.option_parser, custom_parser)
