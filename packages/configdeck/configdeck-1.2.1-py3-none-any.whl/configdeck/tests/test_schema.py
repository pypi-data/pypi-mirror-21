...
class TestDictOption(unittest.TestCase):
    ...
    def test_parse_raw(self):
        ...
        parser = SchemaConfigParser(schema)
        parser.read_file(config)
        parsed = schema.foo.parse('mydict', parser=parser, raw=True)
        self.assertEqual(parsed, expected)

    def test_parse_json_raw_with_interpolation_marks(self):
        """
        Should parse JSON using raw=True when data has interpolation marks.
        """
        class MySchema(Schema):
    ...
    def test_parse_no_json_raw_with_interpolation_marks(self):
        """
        Should parse non-json using raw=True when data has interpolation marks.
        """
        class MySchema(Schema):
            class logging(Section):
                formatters = self.cls(raw=True, item=self.cls())
    ...
    def test_parse_dict_with_dicts(self):
        ...
        expected_values = {
            '__main__': {
                'foo': {
                    'name': '', 'size': 0,
                    'options': {'bar': 'something', 'baz': 42, 'bla': False},
                    }}}
        schema = MySchema()
        parser = SchemaConfigParser(schema)
        parser.read_file(config)
...
class TestSection(unittest.TestCase):
    ...
    def test_repr(self):
        ...
        self.assertEqual(repr(section2), '<{0} foo>'.format(self.cls.__name__))
...
class MultiSchemaTestCase(unittest.TestCase):
    def test_merge_schemas_no_conflicts(self):
        ...
        self.assertEqual(
            set(s.name for s in schema.sections()),
            set(['__main__']))
        self.assertEqual(
    ...
    def test_merge_schemas_same_section(self):
        ...
        self.assertEqual(
            ...
            set(['foo']))
        self.assertEqual(
            set(o.name for o in schema.options('foo')),
            set(['bar', 'baz']))
