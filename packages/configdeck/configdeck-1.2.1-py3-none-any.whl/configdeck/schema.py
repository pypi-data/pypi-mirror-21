...
class Schema(object):
    ...
    def _add_option(self, name, option):
        ...
        section = self._sections.setdefault(
            '__main__', Section(name='__main__'))
    ...
    def is_valid(self):
        ...
        explicit_default_section = isinstance(
            getattr(self, '__main__', None), Section)
    ...
    def options(self, section=None):
        ...
        if section is None:
            ...
            options = [
                getattr(self, att) for att, _ in class_config_objects
...
class Option(object):
    ...
    def __init__(
            self,
            name='', raw=False, default=NO_DEFAULT, fatal=False,
...
class ListOption(Option):
    ...
    def __init__(
            self, name='', item=None, raw=False, default=NO_DEFAULT,
            fatal=False, help='', action='store', remove_duplicates=False,
            short_name='', parse_json=True):
        super(ListOption, self).__init__(
            name=name, raw=raw, default=default, fatal=fatal, help=help,
            action=action, short_name=short_name)
        if item is None:
            item = StringOption()
        self.item = item
    ...
    def __eq__(self, other):
        ...
        if equal:
            ...
            equal &= (
                ...
                and self.require_parser == other.require_parser
                and self.raw == other.raw
                and self.remove_duplicates == other.remove_duplicates)
...
class StringOption(Option):
    ...
    def __init__(
            self, name='', raw=False, default=NO_DEFAULT, fatal=False,
...
class TupleOption(Option):
    ...
    def __init__(
            self, name='', length=0, raw=False, default=NO_DEFAULT,
            fatal=False, help='', action='store', short_name=''):
        super(TupleOption, self).__init__(
            name=name, raw=raw, default=default, fatal=fatal, help=help,
            action=action, short_name=short_name)
        self.length = length
...
class DictOption(Option):
    ...
    def __init__(
            self,
            name='', spec=None, strict=False, raw=False,
    ...
    def parse(self, value, parser, raw=False):
        ...
        for key, value in parsed.items():
            if self.strict and key not in self.spec:
                raise ValueError(
                    "Invalid key %s in section %s" % (key, value))
        ...
        for key in self.spec:
            if key not in parsed:
                ...
                if option.fatal:
                    raise ValueError(
                        "No option '%s' in section '%s'" % (key, value))
                else:
                    if not raw:
    ...
    def get_extra_sections(self, section, parser):
        ...
        for option in parser.options(section):
            ...
            is_dict_lines_item = (
                hasattr(option_obj, 'item')
                and isinstance(option_obj.item, DictOption))
