...
datastore_namespace = os.environ.get('GCP_DATASTORE_NAMESPACE', 'Fulfil')
...
class Many2OneType(BaseType):
    ...
    def to_native(self, value, *args, **kwargs):
        ...
        if isinstance(value, datastore.key.Key):
            if value.kind != self.target.__name__:
                raise Exception(
                    "Expected key of %s kind, found %s instead." % (
                        self.target.__name__, value.kind)
                )
            return self.target(id=value.id_or_name)
    ...
    def validate_entity(self, value):
        if value and not isinstance(value, self.target):
            raise ValidationError(
                "Value must be an instance of %s" % self.target.__name__)
        ...
        if type(value) is list:
            ...
            for elm in value:
                ...
                if isinstance(elm, datastore.key.Key):
                    if elm.kind != self.target.__name__:
                        raise Exception(
                            "Expected key of %s kind, found %s instead." % (
                                self.target.__name__, elm.kind)
                        )
                    keys_to_read.append(elm)
            if keys_to_read:
                items += map(
                    lambda e: self.target(entity=e),
