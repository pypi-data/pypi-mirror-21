...
class FieldFamily(GObject.GObject):
    ...
    def set_config(self, config):
        if self.config is not None:
            self.disconnect_by_func(config_notify_cb, self.config)
    ...
    def set_field_config(self, field):
        if field.config is not None:
            field.disconnect_by_func(config_notify_cb, field.config)
        field.config = self.config['info'].setdefault(field.name, self.old_info.get(field.name, {}))
        field.connect('notify', config_notify_cb, field.config)
        field.width = field.config.setdefault('width', field.width)
        field.visible = field.config.setdefault('visible', field.visible)
