...
class Current(module.Module):
    ...
    def __init__(self, *args):
        super(Current, self).__init__(*args)
        ...
        self.shared.ampd_server_properties.bind_property('current-song', self.current, 'visible', GObject.BindingFlags.SYNC_CREATE, self.current_visible)
        self.shared.bind_property('restricted', self.current, 'visible', GObject.BindingFlags(0), self.current_visible)
    ...
    def current_visible(self, *args):
        song = self.shared.ampd_server_properties.current_song
        if not song:
            return False
        if not self.shared.restricted:
            return True
        return song.get('Genre') != 'Cortina' and int(song.get('Time', '10')) > 5
