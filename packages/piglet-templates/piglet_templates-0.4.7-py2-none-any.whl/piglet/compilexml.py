...
from piglet import parse, intermediate as im, interpolate
...
class Directive(object):
    ...
    def _parse(self, tagname, attrname, el):
        ...
        pos = el.pos
        if el.qname == tagname:
            remaining = Fragment(children=el.children,
                                 pos=el.pos,
                                 attrs=el.attrs)
            if not self.empty:
                try:
                    ...
                    if self.default is self.REQUIRED:
                        raise PigletParseError(
                            "Missing attribute {!r} in element {}"
                            .format(self.q(self.attr), el),
                            lineno=el.pos.line)
                    else:
...
directives = [
    Directive(name='comment',
              attr='comment',
              empty=True,
              factory=im.Comment.factory),
    ...
    Directive(name='switch', attr='test', default='True',
              factory=im.ChooseNode.factory),
    Directive(name='choose', attr='test', default='True',
              factory=im.ChooseNode.factory),
...
class Element(object):
    ...
    def __init__(self, qname='', space='', attrs='', pos=None, end='',
                 close_tag_pos=None, children=None):
        self.qname = qname
        self.space = space
        self.attrs = attrs
        self.pos = pos
...
def get_directives(element):
    ...
    for name in chain([remaining.qname], remaining.attrs):
        if name.startswith('py:'):
            raise PigletParseError('Unrecognized directive {!r} in element {}'
                                   .format(name, element),
...
def make_dom(parse_result):
    ...
    for item in parse_result:
        if isinstance(item, parse.OpenCloseTag):
            head.append(Element(qname=item.qname,
                                space=item.space,
                                attrs=item.attrs,
                                pos=item.pos,
                                end='/>',
            ...
            newhead = Element(qname=item.qname,
                              space=item.space,
                              attrs=item.attrs,
                              pos=item.pos,
                              end='>')
            ...
            head.append(item)
    if len(stack) > 1:
        raise parse.PigletParseError("Missing closing tag for <{}>, "
                                     "opened at {}:{}"
                                     .format(stack[-1].qname,
                                             stack[-1].pos.line,
                                             stack[-1].pos.char))
    return stack[0]


def strip_spaces_between_directives(node,
                                    element_cls=Element,
                                    is_directive=(
                                        lambda n: isinstance(n, Element) and
                                        n.qname.startswith('py:'))
                                    ):
    st = None
    ws = []
...
def pp_move_else_inside_if(elsenode, parent):
    ...
    for ix, item in siblings_before:
        ...
        raise PigletParseError('py:else without an py:if',
                               lineno=elsenode.pos.line)
...
def _compile_node(node):
    ...
    if isinstance(node, Fragment):
        ...
        if directives:
            return _compile_element_with_directives(
                node, directives, remaining)
        ...
        if node.target == 'python':
            return im.InlineCodeNode(pysrc=textwrap.dedent(node.content),
                                     pos=node.pos)
        else:
...
def _compile_text(node):
    ...
    for i in items:
        if isinstance(i, (str, ustr)):
            ...
            container.append(im.InterpolateNode.factory(
...
def _compile_open_tag(node,
    ...
    wn.append(im.TextNode.factory('<{}{}'.format(node.qname, node.space), pos=node.pos))
    if extra_attrs:
        wn.append(im.InlineCodeNode(pysrc='__piglet_attrs = {}', pos=node.pos))
        for item in extra_attrs:
            wn.append(im.InlineCodeNode(pysrc='__piglet_attrs.update({})'
                                        .format(item),
                                        pos=node.pos))
    ...
    for a in node.attrs.values():
        if node.attrs and extra_attrs:
            wn.append(im.IfNode(
                test="'{}' not in __piglet_attrs".format(a.name),
                pos=a.pos))
    ...
    if extra_attrs:
        ...
        if_ = im.IfNode(test='__piglet_attr_v is not None',
                        pos=node.end_pos,
                        children=[im.TextNode(' ', pos=node.end_pos),
                                  im.InterpolateNode(
                                      value='__piglet_attr_k', pos=node.end_pos),
                                  im.TextNode('="', pos=node.end_pos),
                                  im.InterpolateNode(
                                      value='__piglet_attr_v', pos=node.end_pos),
                                  im.TextNode('"', pos=node.end_pos)])
...
def _compile_attr(a, container):
    ...
    """
    intro_text = im.TextNode.factory('{0.name}{0.space1}='
                                        '{0.space2}{0.quote}'.format(a),
                                     pos=a.pos)
       ...
       isinstance(items[0], interpolate.Interpolation):
            ...
            im.WithNode(
                ...
                children=[
                    im.IfNode(test='__piglet_tmp is not None',
                              ...
                              children=[
                                  intro_text,
                                  im.InterpolateNode(value='__piglet_tmp',
                                                     pos=a.value_pos),
                                  outro_text
                              ])
                ])
        ...
        for i in items:
            if isinstance(i, (str, ustr)):
                ...
                container.append(im.InterpolateNode(value=unescape(i.value),
                                                    pos=pos))
                pos = pos.advance(i.source)
        outro_text.pos = pos
...
def _compile_close_tag(node, strip_condition=None):
    if isinstance(node, Fragment) or not node.close_tag:
...
def _compile_element_with_directives(orig, directives, remaining):
    ...
    for d, dd, pos in directives:
        ...
        strip_condition = (d.get_strip_condition(dd, remaining) or
                           strip_condition)

    working_node.append(_compile_open_tag(remaining,
    ...
    working_node.append(_compile_close_tag(remaining,
                                           strip_condition=strip_condition))
