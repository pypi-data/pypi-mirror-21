...
class MultiKeyDict:
    ...
    """
    from functools import lru_cache
    def __init__(self, dc={}, conv=None, iskey=None, dtype=dict):
        """
        ...
        """
        self._lst, self._dct = [], {}
        self._conv, self._iskey, self._dtype = conv, iskey, dtype
        if dtype==dict or isinstance(dc,dict):
            for k, v in dc.items():
                if iskey and iskey(k):
                    self._dct[k] = MultiKeyDict(v, conv, iskey, dtype=dtype)
                else:
                    self._lst.append((k, conv(v) if conv else v))
        elif dtype==list and iterable(dc):
            self._lst.extend(conv(v) if conv else v for v in dc)
        elif dtype=='extend':
            self._lst.extend(conv(dc) if conv else dc)
        else:
            self._lst.append(conv(dc) if conv else dc)
    def getitem_iter(self, keys):
        """
        該当木の全直系親、直系子の情報
        keys: キーリスト
        """
        ...
        if not keys:
            for v in self._dct.values():
                yield from v.getitem_iter(keys)
        elif keys[0] in self._dct:
            yield from self._dct[keys[0]].getitem_iter(keys[1:])
    @lru_cache(maxsize=2048)
    def __getitem__(self, keys):
        ...
        keys: キーリスト
        """
        r = self.getitem_iter(keys if isinstance(keys, tuple) else (keys,))
        return dict(r) if self._dtype == dict else list(r)
    def __repr__(self):
        """表現"""
