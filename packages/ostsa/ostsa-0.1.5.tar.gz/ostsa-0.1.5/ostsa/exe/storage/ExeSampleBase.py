...
class ExeSampleBase(ExeSample):
    ...
    def assembly_bytes(self, max_len = 800):
        
        instructions = iter(self.instructions())
        line = b''
        while len(line) < max_len:
            instruction = next(instructions)
            line += bytes(instruction[1])
            line += b''.join(instruction[2:])
        line = line[:max_len]
        
        return [ord(line[i:i+1]) for i in range(len(line))]
...
#        mode.detail = True
    ...
    def four_gram_frequencies(self):
        ...
        # combinations of 7 selected opcodes: mov, add, sub, call, push, pop, and
        # ret.
    ...
    def two_gram_assembly_frequencies(self):
        
        # The two gram assembly features include 169 combination sets of manually
        # chosen opcodes: mov, add, sub, call, pop, ret, push, popf, retn, wait, 
        # jmp, pushf, and xor.
        
        instructions = self.instructions()
        instructions = [i[2] for i in instructions]
        file_two_grams = (instructions[i:i+2] for i in range(len(instructions)-2))
        file_two_grams = (b','.join(g) for g in file_two_grams)
        
        # Count occurrences.
        two_grams = ExeSample.two_gram_assembly_list()
...
###############################################################################
    ...
    def save_image(self, file_name, color_bound1=0x000000, 
        ...
        aspect_ratio does not work as expected. Changing its value is not
        recommended.
        """
        ...
        total_weight = aspect_ratio + 1
        width = int(len(pixels) ** (aspect_ratio / total_weight))
        height = int(len(pixels) ** (1 / total_weight))
