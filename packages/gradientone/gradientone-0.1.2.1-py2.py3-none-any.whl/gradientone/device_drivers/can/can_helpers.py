from numpy import int16, int32, uint8, uint16, uint32
...
def lookup_trace_variable_address(input_name):
    cfg = ConfigParser()
    cfg.read(join(dirname(abspath(__file__)), "trace_variables.cfg"))
    return int(cfg.get(input_name, "address"))
...
def lookup_trace_variable_units(input_name):
    ...
    cfg.read(join(dirname(abspath(__file__)), "trace_variables.cfg"))
    try:
        return float(cfg.get(input_name, "units").split(" ")[0])
...
def lookup_trace_variable_unit_name(input_name):
    ...
    cfg.read(join(dirname(abspath(__file__)), "trace_variables.cfg"))
    try:
        return unit_getter(cfg.get(input_name, "units").split(" "))
    except Exception as e:
        return "Unknown"
...
def lookup_hardware_type(input_int):
    cfg = ConfigParser()
    cfg.read(join(dirname(abspath(__file__)), "drive_hardware_type.cfg"))
    address = "%0.4X" % input_int
    return str(cfg.get(address, "name"))
...
def properties_from_cfg():
    maps = {"INT16": int16, "INT32": int32, "INT32*": int32,
            "String": bytes_to_string,
            "STRING": bytes_to_string, "U8": uint8, "U16": uint16,
            "U32": uint32, "HARDWARE_TYPE": lookup_hardware_type,
            "OCTET": read_in_data}

    width_dict = {"INT16": 2, "INT32": 4, "INT32*": 4, "String": 40,
                  "STRING": 40, "HARDWARE_TYPE": 4, "OCTET": 40,
                  "U8": 1, "U16": 2, "U32": 4}
    cfg = ConfigParser()

    cfg.read(join(dirname(abspath(__file__)), "parameter_dictionary.cfg"))
    out_dictionary = {}
    for section in cfg.sections():
        _parameter_name = cfg.get(section, "parametername")
        _type = cfg.get(section, "datatype")
        ...
        try:
            _units = cfg.get(section, "units").split(" ")
