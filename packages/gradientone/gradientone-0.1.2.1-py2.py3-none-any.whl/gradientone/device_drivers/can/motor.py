...
class Motor(CanCard):
    ...
    def property_getter(self, property_name='current'):
        ...
        try: 
            ...
            print(property_name, "not in", self._last_values)
            return None
    ...
    def do_trace(self, max_counts, max_time, properties, poll=True):
        if poll:
            data = []
            start = time()
            ...
            for row_index in range(len(data_array)):
                ...
                for col_index in range(len(self._trace_properties)):
                    to_int = c_short(data_array[row_index][col_index]).value
...
if __name__ == "__main__":
    ...
    if card.unplugged:
        print("card unplugged. aborting")
        sys.exit()
    print(get_hardware_info(card))
    print(card.status)
    start = time()
    data = card.do_trace(10000, 1, ['actual_position', 'actual_velocity'])
    #data = card.do_trace(20000, 2, ['actual_load_position', 'actual_motor_velocity'], poll=False)
    print(time()-start)
    print(len(data))
