...
from controllers import MultiClientController, ClientInfo
...
class ScopeClient(object):
    ...
    def check_config_url(self):
        ...
        params = {'status': 'pending', 'tags': ['Scope']}
...
def run():
    ...
    The targets and names are used to create ClientInfo objects to pass
    to the MultiClientController to run. The keep_alive_interval will
    be the seconds allowed to pass between updates to the activity file
    ...
    # activity file. Then append the object to client_infos before
    # creating the MultiClientController

    multi_ctrl = MultiClientController(client_infos=client_infos)
    multi_ctrl.run()
