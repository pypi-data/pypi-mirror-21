...
class MotorClient(TestRunner):
    ...
    def run(self):
        try:
            self.busy = True
            logger.info("in run motor, config type is: "+self.test_run["label"]+" "+str(self.test_run["label"] == "trace"))  # noqa
            if self.test_run["label"] == "trace":
                self.run_motor_trace()
            if self.test_run["label"] == "start_canbus":
                self.run_canbus()
            self.busy = False
        except Exception:
            SENTRY_CLIENT.captureException()
    ...
    def run_canbus(self):
        ...
        for retry_numb in range(MAX_RETRIES):
            if incoming_frame:
                ...
                sleep(BREATH_TIME)
                print("going for get frame retry number:"+str(retry_numb))
                incoming_frame = self.get_frame(frame_index=None)
        if not incoming_frame:
            # no frames received
            self.update_status("No Frames Received")
            raise ValueError("No frames received after " +
        ...
        for current_frame in range(len(frames_to_write)):
            ...
            for response_frame in self.card.frames_list:
                self.update_frame(response_frame,
                                  index=insert_index)
                insert_index += 1
        _data = {"command_id": self.setup["id"], "status": "complete"}
    ...
    def run_motor_trace(self):
        ...
        while True:
            if self.card.unplugged:
                return
            logger.info("Motor state is: "+str(state) +
                        " setup is: "+str(self.setup))
            ...
            if state == MotorState.MOVING:
                ...
                print("doing trace")
                data_points = self.card.do_trace(max_counts=7000,
                                                 max_time=test_order[
                                                     "time_window"],
