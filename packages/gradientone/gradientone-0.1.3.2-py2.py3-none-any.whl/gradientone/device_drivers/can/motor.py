...
class Motor(CanCard):
    ...
    def property_getter(self, property_name='current'):
        ...
        try: 
            ...
            gateway_helpers.logger.warning(property_name+ "not in"+ str(self._last_values))
            return None
...
if __name__ == "__main__":
    ...
    if trace:
        data = card.do_trace(0x00FF, 7000, 1, ['actual_load_position', 'commanded_position', "actual_motor_velocity"],
                         poll=False)
    else:
    ...
    min_velocity = 10000000
    max_velocity = -10000000
    for i in range(len(data)):
        if data[i]["actual_motor_velocity"] < min_velocity:
            min_velocity = data[i]["actual_motor_velocity"]
        if data[i]["actual_motor_velocity"] > max_velocity:
            max_velocity = data[i]["actual_motor_velocity"]
