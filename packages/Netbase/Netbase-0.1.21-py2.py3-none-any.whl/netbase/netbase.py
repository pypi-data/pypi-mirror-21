...
file_caches = expanduser("/tmp/netbase/")
abstracts_cache = expanduser("/tmp/netbase/all/")
file_caches += lang + "/"
...
def query(edges):
	return net.get(edges,api=api_query_short,use_cache=True)
...
class Edges(extensions.xlist):
	...
	# def __init__(self):
	# 	self.all = lambda: self._load_edges(all=True)
	...
	def of(self,property):
		if self.complete:
			return self.filter(lambda x:x.has(property))
		else:
			query((self.node.id or self.node.name)+"."+property)
...
class Node:
	def __init__(self, *args, **kwargs):
		...
		if 'statements' in kwargs:
			...
			self.loaded = True
			self.edges.all = lambda :self._load_edges(all=True) # todo : replace? no extend!
			self.edges.node = self
			self.edges.size = self.count
			self.edges.complete= self.count == self.edges.count()
	...
	def _load_edges(self, all=False):
		if self.loaded:
			return self.edges
		id = str(self.id)
		if all:id+=" limit 1000"
		file = file_caches + id + ".json"
		...
		self.edges.size = self.count
		self.edges.complete = self.count == self.edges.count()
		self.edges.all=lambda :self._load_edges(all=True)
		return self.edges
	...
	def getProperty(self, property, strict=False):
		...
		if property == 'edges':
			return self._load_edges(all=False)
		if property == 'all':
...
class Netbase:
	...
	def _all(self, name, instances=False, deep=False, reload=False):
		...
		if "__" in name: return  # ?
		if "pytest" in name: return  # ?
		if is_plural(name):
		...
		# find_best(nodes)
		nodes.size=max(nodes.count(), nodes[0].statementCount) # instance count todo!
		return nodes
	...
	def get(self, name, get_the=False,use_cache=True, api=api):
		...
		if '/' in name: file = file_caches + str(hash(name))+ ".json"
