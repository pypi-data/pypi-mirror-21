...
from ntabuildtools.repos import (
    CPP_EXTENSIONS_PATH, DEFAULT_REPOS, HAS_CPP, REQUIREMENTS)
from ntabuildtools.utils import subprocessCall



def installRequirements(repo):
  ...
  for repo in repos:
    if repo not in DEFAULT_REPOS.keys():
      print "Unknown repo: '{}'. Specify one of: {}".format(
          repo, ", ".join(HAS_CPP + ["all"]))
      sys.exit(-1)
    if repo not in HAS_CPP:
