...
from ntabuildtools.repos import DEFAULT_REPOS, DEFAULT_ORDER, REQUIREMENTS
...
def executeInstall(repo, version, dryrun):
  ...
  subprocessCall(["pip", "install", "-r", REQUIREMENTS[repo]],
                 dryrun, checkCall=True)

  # Install the package
  if version == "latest":
    subprocessCall(["ARCHFLAGS='-arch x86_64'", "pip", "install", package],
                   dryrun, checkCall=True)
...
def main():
  ...
  for repo in repos:
    if repo not in DEFAULT_REPOS.keys():
      print "Unknown repo: '{}'. Specify one of: {}".format(
          repo, ", ".join(DEFAULT_REPOS.keys() + ["all"]))
      sys.exit(-1)
