...
class ProductsFilterManager:
    ...
    def set_sort(self, sort=pcart_settings.PCART_COLLECTION_DEFAULT_ORDERING):
        self.ordering_type = sort
    ...
    def get_product_types(self):
        return ProductType.objects.filter(products__collections=self.collection).distinct()
    ...
    def get_products_kwargs_all(self):
        return {
    ...
    def calculate_counters(self, tags_expr, v_tags_expr):
        ...
        nonfiltered_vendor_counters = {
            ...
            for x in Product.objects.filter(
                **self._products_kwargs_without_variants).values('vendor__title').annotate(Count('id')).order_by()}
        nonfiltered_v_vendor_counters = {
            x['product__vendor__title']: x['id__count']
            for x in ProductVariant.objects.filter(
                **self._variants_kwargs).values('product__vendor__title').annotate(Count('id')).order_by()}
        ...
        _vendors_titles = [v.title for v in self.vendors]
        vendor_tags = sorted([{
            'tag': t.lower(),
            'label': t,
            ...
            'url': '' if t_vendor_counter[t] == 0 else self.collection.get_absolute_url() + '/'.join(
                normalize_filter_tags(
                    self.collection,
                    (
                        list(self.vendors)+[t]
                    ) if t not in _vendors_titles else [v.title for v in self.vendors if v.title != t],
                    self.prices,
                    self.filter_tags)),
        ...
        for f in self.properties_for_filters:
            ...
            all_tags.append({
                ...
                'tags': sorted([
                {
                    ...
                    'url': '' if t_counter[t] == 0 else self.collection.get_absolute_url() + '/'.join(
                        normalize_filter_tags(
                            self.collection,
                            self.vendors,
                            self.prices,
