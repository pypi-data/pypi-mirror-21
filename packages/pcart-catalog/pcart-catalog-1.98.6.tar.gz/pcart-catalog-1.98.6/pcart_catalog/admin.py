...
class CollectionAdmin(MPTTModelAdmin):
    ...
    list_display = ('title_preview', 'id', 'changed', 'published', 'site', 'slug')
...
class ProductVariantInline(admin.TabularInline):
    ...
    def get_fields(self, request, obj=None):
        ...
        fields = ('title', 'slug', 'tags', 'sku', 'barcode', 'status', 'price', 'compare_at_price') + exclude_prop_fields
        return fields
    ...
    def get_fieldsets(self, request, obj=None):
        if obj is None:
            ...
            return (
                (None, {
                    'fields': (
                        'title', 'slug', 'product_type', 'vendor', 'description'),
                }),
                (_('SEO'), {
                    'fields': ('page_title', 'meta_description'),
                    'classes': ('collapse',),
                ...
                (_('Tags'), {
                    'fields': ('tags', 'sku', 'barcode', 'status'),
                }),
                (_('Properties'), {
                    'fields': exclude_prop_fields,
                }),
                (_('Price'), {
                    'fields': ('price', 'compare_at_price', 'max_variant_price', 'min_variant_price'),
                }),
                (_('Shipping'), {
                    'fields': ('weight',),
                    'classes': ('collapse',),
