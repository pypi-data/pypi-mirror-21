...
class DeployvCLI(click.MultiCommand):
    ...
    def _callback(self, log_level, log_file, config):
        ...
        parsed_config = configuration.DeployvConfig(config)
...
def create(ctx, config_file, backup_dir, external, key_file, without_db, database, without_demo):
    ...
    if backup_dir is not False:
        backup = backup_dir
    elif "database_folder" in config_dict.get("container_config"):
        backup = config_dict.get("container_config").get("database_folder")
    elif config.has_option("deployer", "database_folder"):
        backup = config.get("deployer", "database_folder")
    res_restore = command.restore(backup, database_name=database)
...
def backupdb(config_file, backup_dir, external, reason, tmp_dir, database, cformat,
             container, prefix):
...
def restore(ctx, config_file, backup_dir, database, container, admin_pass,
    ...
    if not backup:
        logger.error(
            'You "must" specify the database folder. '
            'You can do it using the --backup_dir (-z) option. '
            'Also in the json inside container_config > database_folder. '
            'Or in the deployv config files inside deployer > database_folder.')
        sys.exit(1)
    res = command.restore(backup, database_name=database)
    if res.get('result', False):
...
def deactivate_backup(ctx, prefix, store_path, backup, admin_password, cformat):
    config = ctx.parent.command.config
...
def build(config_file, repo, branch, image_name, working_folder, version, force, odoo_repo, tag):
    if config_file:
        config_dict = utils.load_json(config_file)
    else:
        config_dict = {}
