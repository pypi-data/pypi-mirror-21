...
from django.contrib.admin.validation import (
    check_isseq, check_isdict, get_field, fetch_attr)
                    ...
                    if not hasattr(model, field):
                        try:
                            ...
                            raise ImproperlyConfigured(
                                "%s.list_display[%d], %r is not a callable "
                                "or an attribute of %r or found in the "
                                "model %r." % (
                        ...
                        # getattr(model, field) could be
                        # an X_RelatedObjectsDescriptor
                        f = fetch_attr(cls, model, opts,
                                       "list_display[%d]" % idx, field)
                        if isinstance(f, models.ManyToManyField):
                            raise ImproperlyConfigured(
                                "'%s.list_display[%d]', '%s' is a "
    ...
    if hasattr(cls, 'inlines'):
        ...
        for idx, compose in enumerate(cls.compositions):
            if not issubclass(compose, BaseModel):
                raise ImproperlyConfigured(
                    "'%s.compositions[%d]' does not inherit "
            ...
            if not issubclass(compose.model, models.Model):
                raise ImproperlyConfigured(
                    "'%s.compositions[%d].model' does not "
...
def validate_compose(cls, parent, parent_model):
    ...
    if cls.fk_name:
        ...
        if not isinstance(f, models.ForeignKey):
            raise ImproperlyConfigured(
                "'%s.fk_name is not an instance of "
