...
from mads.time import to_time, today, elapsed
...
startdate = "2012-10-23 00:00:00"
starttime = to_time(startdate)
source = "https://bitbucket.org/thatebart/evrm"

def init(event):
    args = event._parsed.args
    for name, obj in wanted.items():
        if name in kernel.cfg.init.split(",") or args and name in args:
            for key, val in obj.items():
                sec = seconds(val)
                repeater = Repeater(sec, stat, event, name="Stats.%s" % key)
                kernel.launch(repeater.start)
...
def stat(event, **kwargs):
    e = Event(kwargs, _default="")
    cfg = cfgs.get("stats", None)
    if not cfg:
       cfgs.stats = cfg = kernel.last("cfg", "stats")
    name = event._parsed.rest or e.name or "suicide" 
    if "." in name:
        name = name.split(".")[-1]
    name = name.lower()
    delta = time.time() - starttime
