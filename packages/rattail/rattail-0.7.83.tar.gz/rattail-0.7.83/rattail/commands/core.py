    ...
    def get_runas_user(self, session=None, username=None):
        ...
        if username is None:
            username = self.config.get('rattail', 'runas.default',
                                       default=self.runas_username)
        if not username:
            # TODO: this seems a little hacky, but gets 'er done
            self.config.require('rattail', 'runas.default')
        with short_session(session) as s:
            return s.query(self.model.User).filter_by(username=username).one()
        ...
        while count:
            try:
                ...
                if args.timeout and (now - started).seconds >= (args.timeout * 60):
                    log.warning("datasync wait timed out after {} minutes, with {} changes in queue".format(args.timeout, count))
                    sys.exit(1)

                if (now - last_logged).seconds >= 60:
                    log.debug("still waiting, {} changes in the datasync queue".format(count))
                    last_logged = now
