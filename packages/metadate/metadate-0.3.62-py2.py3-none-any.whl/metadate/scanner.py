...
class Scanner():
    ...
    def __init__(self, locale, re_flags=re.IGNORECASE | re.MULTILINE):
        ...
        self.scanner = re.Scanner([
            ...
            (pipe(self.BETWEEN, post=r'\b'), lambda y,
             x: MetaBetween(None, None, None, span=y.match.span())),
    ...
    def season(self, y, x):
        return MetaRelative(month=self.SEASONS[x.lower()], day=21, level=Units.SEASON,
                            span=y.match.span())

    def quarter(self, y, x):
        return MetaRelative(month=self.QUARTERS[x.lower()], day=1, level=Units.QUARTER,
                            span=y.match.span())
    ...
    def ordinal_apm(self, y, x):
        ...
        return MetaRelative(hour=hour, minute=minute, second=second, level=Units.MINUTE,
                            span=y.match.span())
    ...
    @staticmethod
    def ambiguous_dd_dd_yyyy(y, x):
        if len(set(re.findall("[ ./-]", x))) > 1:
            return None
        day, month, year = [int(x) for x in re.split("[ ./-]", x)]
        if not validate(year=year):
            # let's remind ourselves that this is not going to matter in some years
            year = int("20" + str(year))
        if not validate(day=day, month=month, year=year):
            if validate(day=month, month=day, year=year):
                day, month = month, day
            else:
                return None
        return MetaRelative(year=year, month=month, day=day, span=y.match.span())
