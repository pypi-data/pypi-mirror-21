...
class Options(object):
    ...
    def __init__(self, disable_sjts=False, disable_msgpack=False, point_serializer='shooju_point'):
...
def serialize_points(points):
    """Serializes array of points into array of objects according options.point_serializer setting"""
    if options.point_serializer == 'shooju_point':
        point = Point
    else:
        point = lambda *a: a
    return list(imap(lambda p: point(*p), points))
...
try:
    ...
    datetime.datetime.utcfromtimestamp(-50000)  # test
    ...
    def milli_to_datetime(milli):
        return epoch + datetime.timedelta(milliseconds=milli)
...
def to_milli(dt):
    ...
    if isinstance(dt, (int, float, long)):
        return dt
    elif not isinstance(dt, (datetime.datetime, date)):
...
class Connection(object):
    ...
    def search(self, query='', date_start=None, date_finish=None, fields=None, max_points=0, size=None,
               per_page=10, sort_on=None, sort_order=None, query_size=None, sort=None):
        """
        ...
        if sort_on or sort_order:
            warnings.warn('"sort_on" and "sort_order" parameters are deprecated. use "sort" parameter instead',
                          DeprecationWarning)
    ...
    def scroll(self, query='', date_start=None, date_finish=None, fields=None, max_points=0, size=None,
               scroll_batch_size=2500, sort_on=None, sort_order=None, operators=None, query_size=None, sort=None):
        """
        ...
        r = self.shooju_api.search_series(query, date_start, date_finish, max_points=max_points,
                                          ...
                                          sort_on=sort_on, sort_order=sort_order, operators=operators, sort=sort)
    ...
    def get_points(self, series_id, date_start=None, date_finish=None, max_points=10,
        ...
        return serialize_points(data.get('points', [])) if data is not None else None
    ...
    def get_point(self, series_id, dt, snapshot_job_id=None, snapshot_date=None):
        ...
        if points is None:
            return None
        return points[0] if len(points) == 1 else serialize_points([(int(to_milli(dt)), None)])[0]
    ...
    def _process_search_response(self, data):
        ...
        for s in data['series']:
            s.setdefault('fields', {})
            if 'points' in s:
                s['points'] = serialize_points(s['points'])
            results.append(s)
        return results
...
class BaseJob(object):
    ...
    def _init_get_series_dict(self, series, delete=False):
        if series not in self._requests:
            self._requests.setdefault(series, {})
            self._cur_batch_size += 1

        if not delete and self._requests[series].get('op') != 'post':
            self._requests[series] = {'fields': {}, 'points': {}, 'op': 'post'}
        elif delete:
            self._requests[series] = {'force': False, 'op': 'delete'}

        return self._requests[series]
...
class RemoteJob(BaseJob):
    ...
    def submit(self):
        ...
        bulk_data = []
        for series, values in self._requests.items():
            # here we will construct a BULK API call
            if values['op'] == 'delete':
                tmp_dict = {"type": "DELETE", "id": series, 'force': values['force']}
            else:  # post
...
class GetBulk(object):
    ...
    def _process_response(self, get_type, response, index):
        ...
        if get_type == "get_points":
            return serialize_points(response.get('points', []))
        elif get_type == "get_fields" or get_type == "get_field":
            return response.get('fields', {})
        elif get_type == "get_point":
            if 'points' not in response:
                return serialize_points([(float(self._reqs[index]['df']), None)])[0]
            pdata = response['points'][0]
            return serialize_points([(pdata[0], pdata[1])])[0]
        else:
            return []
