# Copyright (C) 2012, 2013, 2014 Johannes Spielmann
#
...
def cli(with_help=True, default_command='help', config_file_name=None):
    ...
    if config_file_name is not None:
        switches = _update_args_with_config_file(switches, command, config_file_name)
...
def _update_args_with_config_file(switches, command_name, config_file_name):
    """
    Update the given switches with the configuration from the given file.

    Only those switches which are not already present are being updated.
    """
    ...
    # load the switches from the config file
    if command_name in config:
        section = config[command_name]

        ini_switches = dict(section)

        # update the switches from the config file with the already present switches
        ini_switches.update(switches)
        switches = ini_switches
...
def _help_for_command(name, command, command_func):
    ...
    shortdoc, longdoc, params = _parse_docstring(command_func)
...
def _parse_docstring(fn):
    ...
    # first, augment our param documentation with the additional kwarg
    # parameters
    try:
        extra_params = {}
        for param in fn.params:
            try:
                extra_params[param[0]] = param[2]
            except IndexError:
                extra_params[param[0]] = ''
    ...
    for line in lines:
        ...
        line = line.strip()
        if line.startswith('\param') or line.startswith('@param') or line.startswith(':param '):
            spl = line.split(' ', 2)
            spl.extend(('', '', ''))
            commandname = spl[1]
...
def _funcspec(fn):
    ...
    try:
        additional_params = fn.params
    except Exception:
        additional_params = []
    if fnkwname:
        for param in additional_params:
            fnkwonlydefaults[param[0]] = param[1]
