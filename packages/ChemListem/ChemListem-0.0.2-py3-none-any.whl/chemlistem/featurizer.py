...
def mi(joint, a, b, total):
	...
	for i in range(4):
		try:
			if o[i] != 0: score += (o[i]/total) * math.log(o[i]/e[i])
		except:
			print(joint, a, b, total, file=sys.stderr)
			print(o, file=sys.stderr)
			print(e, file=sys.stderr)
			raise Exception()
	if o[0] < e[0]: score = -score
	return score
...
class Featurizer(object):
	...
	def _init_train(self, train):		
		...
		for i in range(0,len(train),2):
			...
			for tok in btok:
				if tok not in self.tokcounts: self.tokcounts[tok] = 0
		...
		strfeats = set()
		for f in self.feats: strfeats.update(f[1])
		...
		for i in range(len(lstrfeats)):
			...
			fv.append((sf.encode("charmap", errors="replace").decode("charmap"), sf, e, o, m))
		...
		fctr = Counter()
		for i in range(len(self.feats)):
			n = self.tokcounts[self.toks[i]] if self.toks[i] in self.tokcounts else 0
	...
	def _xval_and_train_rf(self):
		...
		for fold in range(5):
			...
			for i in range(len(self.toks)):
				if i % 5 == fold:
					...
					trainset.append(i)
			# gather
			x = np.array([self._sfeats_to_bits(self.feats[i][1]) for i in trainset])
			y = np.array([self.isne[i] for i in trainset])
			test_x = np.array([self._sfeats_to_bits(self.feats[i][1]) for i in testset])
			test_y = np.array([self.isne[i] for i in testset])
			# build the RF
			rf = self._make_rf(x, y)
			# Memorize the scores
			ppreds = rf.predict_proba(test_x)
			for i in range(len(testset)): self.cachescores[testset[i]] = ppreds[i][1]
		...
		# How's the score?
		if(tp+fp+fn+tn > 0): print(tp, fp, fn, tp*2/(tp+tp+fp+fn), (tp+tn)/(tp+fp+fn+tn), sep="\t", file=sys.stderr)
		# OK, enough with the cross-validation, let's generate the classifier for unknown tokens.
		x = np.array([self._sfeats_to_bits(self.feats[i][1]) for i in range(len(self.toks))])
	...
	def _feats_for_tok(self, tok):
		...
		num_feats.extend([1 if featres[featre].match(tok) else 0 for featre in frk])
