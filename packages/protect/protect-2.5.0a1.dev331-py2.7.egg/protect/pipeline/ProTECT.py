...
def _ensure_patient_group_is_ok(patient_object, patient_name=None):
    """
    Ensure that the provided entries for the patient groupd is formatted properly.
    ...
    >>> _ensure_patient_group_is_ok({'tumor_dna_fastq_1', 'normal_dna_fastq_1', \
                                     'tumor_rna_fastq_1'}, 'test')
    >>> _ensure_patient_group_is_ok({'tumor_dna_fastq_1', 'normal_dna_fastq_1', 'tumor_rna_bam'}, \
                                    'test')
    Traceback (most recent call last):
    ...
    ParameterError: The patient entry for sample test does not contain a haplotype_txt entry.
    Cannot haplotype patient if all the input sequence files are not fastqs.
    >>> _ensure_patient_group_is_ok({'tumor_dna_fastq_1', 'normal_dna_fastq_1', 'tumor_rna_bam', \
                                     'haplotype_txt'}, 'test')
    >>> _ensure_patient_group_is_ok({'tumor_dna_fastq_1', 'tumor_rna_bam', 'haplotype_txt'}, \
                                     'test')
    Traceback (most recent call last):
    ...
    ParameterError: The patient entry for sample test does not contain a mutation_vcf entry. If
    both tumor and normal DNA sequences (fastqs or bam) are not provided, a pre-computed vcf must
    be provided.
    >>> _ensure_patient_group_is_ok({'tumor_dna_fastq_1', 'tumor_rna_bam', 'mutation_vcf', \
                                     'haplotype_txt'}, 'test')
    >>> _ensure_patient_group_is_ok({'tumor_dna_fastq_1', 'tumor_dna_bam', 'tumor_rna_bam', \
                                     'haplotype_txt'}, 'test')
    Traceback (most recent call last):
    ...
    ParameterError: The patient entry for sample test does not contain a mutation_vcf entry. If
    ...
    if {'tumor_dna_fastq_1', 'normal_dna_fastq_1', 'tumor_rna_fastq_1'}.issubset(test_set):
        # Best case scenario, we get alll fastqs
        pass
    else:
        # We have less than 3 fastqs so we have to have a haplotype.
        if 'haplotype_txt' not in test_set:
            raise ParameterError(('The patient entry for sample %s ' % patient_name) +
                                 'does not contain a haplotype_txt entry.\nCannot haplotype patient '
                                 'if all the input sequence files are not fastqs.')
        ...
        if (({re.search('tumor_dna_((bam)|(fastq_1)).*', x) for x in test_set} == {None} or
            ...
            raise ParameterError(('The patient entry for sample %s ' % patient_name) +
                                 ...
                                 'vcf must\nbe provided.')
        # We have toe be given a tumor rna fastq or bam
...
def parse_patients(patient_dict):
    """
    Parse a dict of patient entries to retain on ly the useful entries (Teh user may provide more
    ...
    >>> parse_patients({'ssec_encrypted': 'False', \
                        'tumor_dna_fastq_1': 'tdf1', \
                        'tumor_dna_fastq_2': 'tdf2', \
                        'tumor_dna_bam': 'tdb', \
                        'normal_dna_fastq_1': 'ndf1', \
                        'normal_dna_fastq_2': 'ndf2', \
                        'normal_dna_bam': 'ndb', \
                        'tumor_rna_fastq_1': 'trf1', \
                        'tumor_rna_fastq_2': 'trf2', \
                        'tumor_rna_bam': 'trb', \
                        'haplotype_txt': 'ht', \
                        'mutation_vcf': 'mv'}) == {'haplotype_txt': 'ht', \
                                                   'mutation_vcf': 'mv', \
                                                   'ssec_encrypted': False, \
                                                   'tumor_rna_bam': 'trb'}
    True
    >>> parse_patients({'tumor_dna_fastq_1': 'tdf1', \
                        'tumor_dna_bam': 'tdb', \
                        'normal_dna_bam': 'ndb', \
                        'tumor_rna_fastq_1': 'trf1', \
                        'tumor_rna_fastq_2': 'trf2', \
                        'tumor_rna_bam': 'trb', \
                        'haplotype_txt': 'ht'}) == {'haplotype_txt': 'ht', \
                                                    'normal_dna_bam': 'ndb', \
                                                    'ssec_encrypted': False, \
                                                    'tumor_dna_bam': 'tdb', \
                                                    'tumor_rna_bam': 'trb'}
    True
    >>> parse_patients({'ssec_encrypted': 'True', \
                        'tumor_dna_fastq_2': 'tdf2', \
                        'tumor_dna_fastq_1': 'tdf1', \
                        'tumor_dna_bam': 'tdb', \
                        'normal_dna_fastq_1': 'ndf1', \
                        'normal_dna_bam': 'ndb', \
                        'tumor_rna_fastq_1': 'trf1', \
                        'tumor_rna_bam': 'trb'}) == {'tumor_dna_fastq_2': 'tdf2', \
                                                     'tumor_dna_fastq_1': 'tdf1', \
                                                     'normal_dna_fastq_1': 'ndf1', \
                                                     'tumor_rna_fastq_1': 'trf1', \
                                                     'ssec_encrypted': True}
    ...
    out_keys = []
    if {'mutation_vcf', 'haplotype_txt'}.issubset(patient_dict):
        # We have a haplotype and a vcf so do the minimum work.
        out_keys.extend(['mutation_vcf', 'haplotype_txt'])
    ...
    elif 'haplotype_txt' in patient_keys:
        out_keys.extend(['haplotype_txt'])
        ...
        # Can never have vcf without haplotype so the only remaining case is all fastqs.
        for stype in 'tumor_dna', 'normal_dna', 'tumor_rna':
            out_keys.extend([x for x in patient_keys if x.startswith(stype + '_fastq')])
    for key in out_keys:
    ...
    for key in input_config.keys():
        if key == 'patients':
            ...
            for sample_name in input_config[key]:
                ...
                sample_set[sample_name] = parse_patients(input_config['patients'][sample_name])
        else:
