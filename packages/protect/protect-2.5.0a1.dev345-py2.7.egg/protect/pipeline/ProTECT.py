...
def _ensure_patient_group_is_ok(patient_object, patient_name=None):
    ...
    if {'tumor_dna_fastq_1', 'normal_dna_fastq_1', 'tumor_rna_fastq_1'}.issubset(test_set):
        ...
        if 'mhc_haplotype_files' not in test_set:
            raise ParameterError(('The patient entry for sample %s ' % patient_name) +
                                 'does not contain a mhc_haplotype_files entry.\nCannot haplotype '
...
def parse_patients(job, patient_dict):
    ...
    out_keys = []
    if {'mutation_vcf', 'mhc_haplotype_files'}.issubset(patient_dict):
        # We have a haplotype and a vcf so do the minimum work.
        out_keys.extend(['mutation_vcf', 'mhc_haplotype_files'])
    ...
    elif 'mhc_haplotype_files' in patient_keys:
        out_keys.extend(['mhc_haplotype_files'])
...
def launch_protect(job, patient_data, univ_options, tool_options):
    ...
    if 'mhc_haplotype_files' in patient_data:
        haplotype_patient = job.wrapJobFn(get_patient_mhc_haplotype, patient_data)
    ...
    if 'mutation_vcf' in patient_data:
        get_mutations = job.wrapJobFn(get_patient_vcf, patient_data)
...
def get_patient_bams(job, patient_dict, sample_type, univ_options, bwa_options):
    ...
    if 'dna' in sample_type:
        prefix = sample_type + '_fix_pg_sorted'
    else:
        prefix = 'rna_genome_sorted'
    if sample_type + '_bai' in patient_dict:
        ...
        output_job = job.wrapJobFn(index_bamfile, patient_dict[sample_type + '_bam'], sample_type,
...
def get_patient_mhc_haplotype(job, patient_dict):
    ...
    :rtype: toil.fileStore.FileID
    """
    haplotype_archive = job.fileStore.readGlobalFile(patient_dict['mhc_haplotype_files'])
    haplotype_archive = untargz(haplotype_archive, os.getcwd())
    output_dict = {}
    for filename in 'mhci_alleles.list', 'mhcii_alleles.list':
        output_dict = job.fileStore.writeGlobalFile(os.path.join(haplotype_archive, filename))
    return output_dict
