...
def usage():
    print('python KLdivergence-InverseMap-InverseConstruction.py -t <test_number> -s <span_type> -q <quad_type> -n <quad_n_points> [--tol=<tolerance> --with-reg=<alpha> --ord-list=<comma_sep_order_list> --no-plotting --no-l2 --nprocs=<nprocs>]')
...
def print_example():
    print('python KLdivergence-InverseMap-InverseConstruction.py -t 2 -s total -q 3 -n 30 --tol=1e-4 --with-reg=1e-3 --ord-list=1,3,5,7,9')
...
try:
    ...
    if PLOTTING and PLOT_SUPPORT:
        ...
        ax_kde_1d.hist(T_samp[:,0], bins=20, histtype='step', normed=True, color='k')
        if setup['dim'] >= 2:
            ...
            for i,order in enumerate(ORDERS):
                ...
                ax_pdf2d.contour(xx2d,yy2d,pdf2d.reshape(xx2d.shape),
                                 levels=levels_pdf2d)
                nscatter = min(100,N_samp)
                ax_pdf2d.scatter(T_samp[:nscatter,0],T_samp[:nscatter,1])
    ...
    for i_ord,order in enumerate(ORDERS):
        ...
        tm_distribution = DIST.PushForwardTransportMapDistribution(tm_approx, target_distribution)

        # SOLVE
        start = time.clock()
        log_entry_solve = tm_distribution.minimize_kl_divergence(test['base_distribution'], qtype=qtype,
                                                            qparams=qparams,
                                                            regularization=REG,
                                                            tol=TOL, ders=ders,
        ...
        approx_distribution_1 = DIST.PullBackTransportMapDistribution(tm_approx, test['base_distribution'])
        # Construct approximate distribution L_\sharp T^\sharp \pi
        approx_distribution = DIST.PushForwardTransportMapDistribution(test['support_map'],
                                                             approx_distribution_1)
        ...
        if L2_ERR or PLOTTING:
            ...
            tmp = TM.mpi_eval("map_samples_base_to_target", scatter_tuple=scatter_tuple,
                              obj=approx_distribution_1, mpi_pool=mpi_pool)
            scatter_tuple = (['x'],[tmp])
