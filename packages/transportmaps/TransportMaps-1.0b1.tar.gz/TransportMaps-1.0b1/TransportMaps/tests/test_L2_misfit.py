...
class L2_misfit_DerivativeChecks(unittest.TestCase):
    ...
    def test_grad_a_l2_misfit(self):
        ...
        def L2_misfit(a, params):
            f1.set_coeffs(a)
            out = TM.L2_misfit(f1, f2, d=d, qtype=qtype, qparams=qparams,
                               mpi_pool=mpi_pool)
            return out

        def grad_a_L2_misfit(a, params):
            f1.set_coeffs(a)
            out = TM.grad_a_L2_misfit(f1, f2, d=d, qtype=qtype, qparams=qparams,
                                      mpi_pool=mpi_pool)
            return out
    ...
    def test_hess_a_l2_misfit(self):
        ...
        def hess_a_L2_misfit(a, params):
            f1.set_coeffs(a)
            out = TM.hess_a_L2_misfit(f1, f2, d=d, qtype=qtype, qparams=qparams,
                                      mpi_pool=mpi_pool)
            return out
    ...
    def test_action_storage_hess_a_l2_misfit(self):
        ...
        def action_storage_hess_a_L2_misfit(a, v, params):
            f1.set_coeffs(a)
            params1 = {}
            (_, H) = TM.storage_hess_a_L2_misfit(
                f1, f2, d=d, params1=params1, qtype=qtype, qparams=qparams,
                mpi_pool=mpi_pool)
            out = TM.action_hess_a_L2_misfit(H, v)
            return out
    ...
    def test_batch_grad_a_l2_misfit(self):
        ...
        def L2_misfit(a, params):
            f1.set_coeffs(a)
            out = TM.L2_misfit(f1, f2, x=x, w=w, batch_size=batch_size,
                               mpi_pool=mpi_pool)
            return out

        def grad_a_L2_misfit(a, params):
            f1.set_coeffs(a)
            out = TM.grad_a_L2_misfit(f1, f2, x=x, w=w, batch_size=batch_size,
                                      mpi_pool=mpi_pool)
            return out
    ...
    def test_batch_hess_a_l2_misfit(self):
        ...
        def hess_a_L2_misfit(a, params):
            f1.set_coeffs(a)
            out = TM.hess_a_L2_misfit(f1, f2, x=x, w=w, batch_size=batch_size,
                                      mpi_pool=mpi_pool)
            return out
    ...
    def test_precomp_grad_a_l2_misfit(self):
        ...
        (x, w) = d.quadrature(qtype, qparams)
        (_, params1) = TM.mpi_eval('init_mem_precomp_regression',
                                   dmem_key_out_list=['params1'],
                                   obj=f1, mpi_pool=mpi_pool,
                                   concatenate=False)
        ...
        def L2_misfit(a, params):
            f1.set_coeffs(a)
            out = TM.L2_misfit(f1, f2, x=x, w=w, params1=params1,
                               mpi_pool=mpi_pool)
            return out

        def grad_a_L2_misfit(a, params):
            f1.set_coeffs(a)
            out = TM.grad_a_L2_misfit(f1, f2, x=x, w=w, params1=params1,
                                      mpi_pool=mpi_pool)
            return out
    ...
    def test_precomp_hess_a_l2_misfit(self):
        ...
        def hess_a_L2_misfit(a, params):
            f1.set_coeffs(a)
            out = TM.hess_a_L2_misfit(f1, f2, x=x, w=w, params1=params1,
                                      mpi_pool=mpi_pool)
            return out
...
class LinSpan():
    def setUp_approx(self):
        ...
        self.f2.set_coeffs( np.random.randn( self.f2.get_n_coeffs() ) )
        # Approximating Function
        self.f1 = FUNC.LinearSpanApproximation(
            basis_list, spantype='total', order_list=order_list)
        # Random point on coeff space
        self.coeffs = np.random.randn( self.f1.get_n_coeffs() )
class MonoLinSpan():
    def setUp_approx(self):
        basis_list = [ S1D.HermiteProbabilistsPolynomial() ] * 2
        order_list = [ self.order ]*2
