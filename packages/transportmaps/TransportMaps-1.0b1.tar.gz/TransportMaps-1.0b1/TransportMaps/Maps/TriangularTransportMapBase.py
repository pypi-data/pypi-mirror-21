...
class TriangularTransportMap(TransportMap):
    ...
    def grad_a_partial_xd(self, x, precomp=None, idxs_slice=slice(None)):
        ...
        if x.shape[1] != self.dim_in:
            raise ValueError("dimension mismatch")
        out = np.zeros((x.shape[0], self.dim_out, self.get_n_coeffs()))
        start = 0
        for k,(a,avar,p) in enumerate(zip(self.approx_list,
                                          self.active_vars,
                                          precomp['components'])):
    ...
    def hess_a_log_det_grad_x(self, x, precomp=None, idxs_slice=slice(None)):
        ...
        if x.shape[1] != self.dim:
            raise ValueError("dimension mismatch")
        out = np.zeros((x.shape[0], self.get_n_coeffs(), self.get_n_coeffs()))
        start = 0
        for k,(a,avar,p) in enumerate(zip(self.approx_list,
                                          self.active_vars,
                                          precomp['components'])):
    ...
    def grad_a_hess_x_log_det_grad_x(self, x, precomp=None, idxs_slice=slice(None)):
        ...
        for k,(a,avar,p) in enumerate(zip(self.approx_list, self.active_vars,
            ...
            grad_a_hess_x_sum = (dadx2dxk / dxk[:,nax,nax,nax]) - \
                    ...
                    2*(dadxdxk[:,:,:,nax]*dxdxkT[:,nax,nax,:])/(dxk**2.)[:,nax,nax,nax] + \
                    2*(dxdxkT2*dadxk[:,:,nax,nax])/(dxk**3.)[:,nax,nax,nax]
            
            # print('pt1: ',(dadx2dxk / dxk[:,nax,nax,nax])[0,:,:,:])
            # print('pt2: ',((dx2dxk[:,nax,:,:]*dadxk[:,:,nax,nax])/(dxk**2.)[:,nax,nax,nax])[0,:,:,:])
            # print('pt3: ',(2*(dadxdxk[:,:,:,nax]*dxdxkT[:,nax,nax,:])/(dxk**2.)[:,nax,nax,nax])[0,:,:,:])
            # print('pt4: ',(2*(dxdxkT2*dadxk[:,:,nax,nax])/(dxk**3.)[:,nax,nax,nax])[0,:,:,:])
            
            #print('Component %s', k);
            #print(grad_a_hess_x_sum[0,:,:,:])
            #print('Shape', grad_a_hess_x_sum.shape)
    ...
    def grad_a_inverse(self, x, precomp=None, idxs_slice=slice(None)):
        ...
        out = np.zeros((xinv.shape[0],self.dim,self.get_n_coeffs()))
        rhs = np.zeros((self.dim, self.get_n_coeffs()))
        ...
        if cache_level >= 1:
            for a, p in zip(self.approx_list, precomp['components']):
                p['cached_grad_a'] = ( np.zeros(tot_size,dtype=bool),
                                       np.empty((tot_size, a.get_n_coeffs())) )
    ...
    def from_xml_element(node):
        ...
        for comp_node in node.getchildren():
            ...
            for ncomp in ncomp_list:
                ...
                approx = FUNC.ParametricFunctionApproximation.from_xml_element(
                    approx_node, dcomp)
