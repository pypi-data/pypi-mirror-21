...
def default_command(args):
    sys.exit('Missing command. See --help for details')
...
def login(args):
    ...
    if response.status_code == 201:
        ...
        sys.exit('Error during authentication: %d (%s)' % (response.status_code, response.json()['error']['briefSummary']))
...
def api_handler(args):
    ...
    if op.startswith('get-'):
        ...
        # We shouldn't actually get here, since the arg parser should fail on a non-registered command
        sys.exit('Unknown command: '+args)

    # Determine whether the response was considered successful
    is_error_response = response.status_code not in range(200, 300)

    # Post-process the response for 200-299 (positive HTTP responses)
    if response.status_code == 204:
        # "No content"
        response_data = None
    elif (not is_error_response) and args.query:
    ...
    if response_data is not None:
        if args.output == 'json':
            format_as_json(response_data, out_stream)
        elif args.output == 'text':
            format_as_text(response_data, out_stream)
        else:
            # We shouldn't get here, the argparser should constrain the available values.
            sys.exit('Output format {} not recognised.'.format(args.output))

    # If we're still in the program and it was an error, exit accordingly.
    # This isn't the best place to do this, but will do for now.
    # A better approach would be to exit in the calling code.
    if is_error_response:
        sys.exit(2)
