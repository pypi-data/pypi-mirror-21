...
def senderror(subject_text, body_text, username=GMAIL_USER, password=GMAIL_PASS, recipients=GMAIL_RECIPIENTS):
    """Take subject_text and body_text and send with gmail account
    NB: if you store in local_settings.py at the root of the python virtual machine the following (you can avoid specifying the arguments: username, password, recipients):
    import os
    #gmail settings
    GMAIL_USER=''
    GMAIL_PASS=''
...
def senderror_attach(subject_text,body_text, attachment_file, username=GMAIL_USER, password=GMAIL_PASS, recipients=GMAIL_RECIPIENTS):
    """Take subject_text, body_text, and attachment_file and send with gmail account
    NB: if you store in local_settings.py at the root of the python virtual machine the following (you can avoid specifying the arguments: username, password, recipients):
    import os
    #gmail settings
    GMAIL_USER=''
