...
__all__ = ["BaseSpider", "DataBaseSpider", "MongoDBSpider", "RedisSpider", "DataBaseAndRedisSpider",
           "MongoDBAndRedisSpider", "DataBaseAndMongoDBAndRedisSpider", ]
...
class MongoDBSpider(MongoDBMixin, BaseSpider):
    @staticmethod
    def close(spider, reason):
        result = super(MongoDBSpider, spider).close(spider, reason)
        closed = getattr(spider, 'close_nimbus', None)
        if callable(closed):
            spider.close_nimbus()
        return result
...
class MongoDBAndRedisSpider(MongoDBAndRedisMixin, BaseSpider):
    @staticmethod
    def close(spider, reason):
        result = super(MongoDBAndRedisSpider, spider).close(spider, reason)
        closed = getattr(spider, 'close_nimbus', None)
        if callable(closed):
            spider.close_nimbus()
        return result
...
class DataBaseAndMongoDBAndRedisSpider(DataBaseAndMongoDBAndRedisMixin, BaseSpider):
    @staticmethod
    def close(spider, reason):
        result = super(DataBaseAndMongoDBAndRedisSpider, spider).close(spider, reason)
        closed = getattr(spider, 'close_nimbus', None)
        if callable(closed):
            spider.close_nimbus()
        return result
