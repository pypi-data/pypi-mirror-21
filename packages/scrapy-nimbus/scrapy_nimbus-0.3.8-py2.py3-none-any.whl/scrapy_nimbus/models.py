...
__all__ = ["Base", "BaseModel", "create_table", "create_tables", "ExampleModel", ]
...
def create_tables(engine, *models):
    if not (engine and models):
        return False
    for model in models:
        if model and isinstance(model, Base):
            model.metadata.create_all(bind=engine)
    return True
