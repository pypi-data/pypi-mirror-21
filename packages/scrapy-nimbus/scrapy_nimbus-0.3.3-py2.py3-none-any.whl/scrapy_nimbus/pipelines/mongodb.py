...
from scrapy_nimbus.spiders.mixin import MongoMixin


class MongoAbstractPipeline(object):
    db_name = None

    def __init__(self, *args, **kwargs):
        super(MongoAbstractPipeline, self).__init__()
        self.client = None
        self.db = None
...
class MongoMixinPipeline(MongoAbstractPipeline):
    def __init__(self, *args, **kwargs):
        super(MongoMixinPipeline, self).__init__(*args, **kwargs)

    def get_client(self, spider, *args, **kwargs):
        if isinstance(spider, MongoMixin):
            _name = spider.nimbus_mongo_name
        else:
            _name = spider.name
