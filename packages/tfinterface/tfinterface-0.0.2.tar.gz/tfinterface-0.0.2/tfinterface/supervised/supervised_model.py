...
from tfinterface.base import RequiredModel
from tfinterface.utils import TENSOR
from tfinterface.utils import Required
...
class RequiredSupervisedModel(RequiredModel):
    pass
...
* `[update : Tensor]` -
    ...
    def _post_build(self, *args, **kwargs):
        ...
        if not hasattr(self, "loss"):
            ...
            if loss == "mse":
                self.loss = (tf.reduce_mean)(tf.nn.l2_loss(self.predictions - self.labels))
            elif loss == "sigmoid":
