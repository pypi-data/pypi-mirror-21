...
def load(filename):
    ...
    for item in hdulist:
        if item.header.get(card)==cardres:
            ...
            res = MCres(sampler=sampler, paramstr=paramstr, nwalkers=int(item.header.get(nwalker, 0)), burnInIts=int(item.header.get(nburn, 0)), niters=int(item.header.get(niter, 0)))
            break
    for item in hdulist:
