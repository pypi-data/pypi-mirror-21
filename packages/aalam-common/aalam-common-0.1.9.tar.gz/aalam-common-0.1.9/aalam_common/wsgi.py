...
class BaseHandler(object):
    ...
    def __call__(self, request):
        ...
        self.custom_deserializer = route.get("deserializer", None)
        self.custom_serializer = route.get("serializer", None)
        self.inp_data_qualifier = route.get('dataset', None)
        for h in hardcoded:
            route.pop(h)

        return self.respond(action, request, **route)
    ...
    def _deserializer(self, request):
        call = getattr(self, self.custom_deserializer,
                       None) if self.custom_deserializer else None
        if call:
            return call(request)
    ...
    def _serializer(self, app_data, response):
        call = getattr(self, self.custom_serializer,
                       None) if self.custom_serializer else None
        if call:
    ...
    def respond(self, action, request, **kwargs):
        req_data = self._deserializer(request)
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            logging.error("Invalid input fields- got %s, expected %s dataset"
                          % (str(req_data.keys()), self.inp_data_qualifier))
            msg = "Input data not proper\n"
            if self.inp_data_qualifier:
                msg += "Following are valid, (*) are manadatory\n" + \
                    self.inp_data_qualifier
        ...
        if data is not None:
            self._serializer(data, response)
