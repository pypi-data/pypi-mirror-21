...
class Docstring(Statement):
    ...
    def __init__(self, doc_string="", extended="", extras=None, *args, **kwargs):
        ...
        self.extras = extras
        self.doc_string_body = Statement(doc_string, line_ending="\n")
    ...
    def generate(self, *args, **kwargs):
        ...
        self.code = '"""{}"""'.format(self.doc_string_body.line_ending.join(output))
    ...
    def create_sections(self):
        if self.extras is not None:
            for block, params in self.extras.items():
                ...
                self.doc_string_body.add_children([
                    Statement(),
                    section])
    ...
    def create_doc_section_param(self, id='', param_type='', description=''):
        ...
        if param_type:
            ...
            if description:
                param_type_fmt = "{}: ".format(param_type_fmt)
        if id:
            if description and not param_type:
                id_fmt = "{}: ".format(id_fmt)

        id = id_fmt.format(id)
        param_type = param_type_fmt.format(param_type)
