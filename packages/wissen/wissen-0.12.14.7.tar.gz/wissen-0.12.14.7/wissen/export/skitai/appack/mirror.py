...
def mirror (source, target):
	...
	for col in r.json () ["collections"]:
			...
			for seg, files in segs.items ():
				...
				for sfn, ssize, smtime in files:
					...
					if not os.path.isfile (dfn):
						que.insert (0, (col, 'segment', group, str(seg) + sfn [-4:], dfn))
						new_file = True
					else:	
		...
		if new_file:
			...
			r = requests.post ("%s/%s/collection" % (local_server, col))
			colinfo = r.json ()
			dcolopt = normpath (colinfo ['path'])
			if not os.path.isfile (dcolopt) or colinfo ["mtime"] > os.path.getmtime (dcolopt) or colinfo ["size"] != os.path.getsize (dcolopt):			
				r = requests.post ("%s/%s" % (local_server, col), data = json.dumps (colinfo ['colopt']))
			else:
