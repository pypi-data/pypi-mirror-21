...
def load_data (alias, colopt, numthreads, plock):	
...
def startup (wasc):
	...
	for alias in os.listdir (getdir ("config")):		
		load_data (alias, colopt, app.config.numthreads, wasc.plock)
...
@app.route ("/<alias>/collection/config", methods = ["GET"])
...
@app.route ("/<alias>/segment/<group>/<fn>", methods = ["GET"])
...
@app.route ("/<alias>/records", methods = ["POST", "PUT", "DELETE"])
def add (was, alias):	
	if was.request.command == "delete":
...
@app.route ("/<alias>/records/query", methods = ["GET", "POST"])
...
@app.route ("/<alias>/records/guess", methods = ["GET", "POST"])
...
@app.route ("/<alias>/records/cluster", methods = ["GET", "POST"])
def cluster (was, alias, q, l = 'un'):
	return was.jstream (wissen.cluster (alias, q, l))

@app.route ("/<alias>/records/mguess", methods = ["POST"])
...
@app.route ("/<alias>/records/mquery", methods = ["POST"])
