...
Implementing with XMLRPC
--------------------------

Here's an XMLRPC response example for `Skitai-Saddle`_ app using previous chapter's 'config.py'.

/var/wsgi/wissenapp.py
...
.. code:: python
  ...
  @app.route('/guess')
  def guess (was, alias, qs, lang = "en", top = 0, cond = ""):
    return was.wissen.guess (alias, qs, lang, wissen.NAIVEBAYES, top, cond)
...
Skitai server config:

.. code:: python

  [routes:line]
    
  /wissen/ = /var/wsgi/wissenapp:app
...
.. code:: python
  ...
  s = client.Server ("http://127.0.0.1:5000/wissen/")
  s.query ("data1", "mozart violin sonata")
  
  m = client.MultiCall (s)
  m.query ("data1", "mozart violin sonata") 
  m.guess ("data2", "mozart figaro")
  for result in m ():
    print (result)
...
If you want to response as JSON:

.. code:: python

  @app.route('/query')
  def query (was, alias, qs, offset = 0, fetch = 10, sort = "", summary = 30, lang = "en"):
    was.response.set_header ("Content-Type", "application/json")
    return was.tojson (was.wissen.query (alias, qs, offset, fetch, sort, summary, lang))


Implementing with Websocket + JSON
-----------------------------------------------

The major reason why implementing with Web Socket, it's possible to communicate between servers with HTTP-headerless data.
...
.. code:: python
  ...
  @app.route('/wsquery')
  def wsquery (was, params = ""):
    if "websocket_init" in was.env:
      was.env ["websocket_init"] = (skitai.WEBSOCKET_REQDATA, 60, "params")	
      return ""
    return was.tojson (was.wissen.query (**was.fromjson (params)))

Now, you implementing requests with Javascript for replacing AJAX on web browser.

Or if you want to call within your another WSGI Saddle app on physically seperated machine,
...
.. code:: python
  ...
  def wstest (was):
    s = was.ws (
      "ws://search.myserver.com:5000/wissen/wsquery",
      was.tojson ({"alias": "data1", "qs": "mozart violin sonata"})
    )	
    rs = s.getwait (3)
    ...
    for result in was.fromjson (rs.data)["result"]:
      ...      
    return ...  
