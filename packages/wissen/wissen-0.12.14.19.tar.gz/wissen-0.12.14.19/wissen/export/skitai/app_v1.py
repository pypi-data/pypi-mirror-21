...
def config (was, alias, config = None):
	...
	if was.request.command == "post" and wissen.get (alias):
		return error (was.response, "400 Bad Request", 40004, "resource already exists")		
	...
	for d in colopt ['data_dir']:
		...
		s = b.find (t)
		if s == -1:
			return error (was.response, "400 Bad Request", 40002, "invalid data_dir")
...
def retire (was, alias, command):
	if command == "retire":
		...
		return error (was.response, "400 Bad Request", 40001, "invalid command")
	return ''
