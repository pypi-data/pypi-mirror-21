...
class VolumeEditTest( BaseCliTest ):
    ...
    @patch( "fdscli.services.volume_service.VolumeService.get_volume", side_effect=mock_functions.findVolumeById)
    @patch( "fdscli.services.volume_service.VolumeService.edit_volume", side_effect=mock_functions.editVolume)
    def test_edit_volume_size(self, mockEdit, mockGet):
        '''
        Testing that all the boundary conditions are enforce
        '''

        args = ["volume", "edit", "-volume_id=4" , "-size"]
        self.callMessageFormatter(args)

        print("Testing bad volume size edit - missing size")

        self.cli.run( args )
        assert mockEdit.call_count == 0

        print("Testing bad size edit")
        ...
        print("Testing bad size edit with neg number")
        ...
        print("Testing volume size passed.\n\n")
