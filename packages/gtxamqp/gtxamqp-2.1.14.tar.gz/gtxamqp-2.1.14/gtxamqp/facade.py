...
class AmqpClient(object):
    ...
    def __init__(self,
        ...
        self.bindings = filter(None, bindings)
    ...
    @inlineCallbacks
    def setup_dead_letter(self, channel, dead_letter_exchange):
        exchange_name = get_exchange_name(dead_letter_exchange)
        yield channel.exchange_declare(**dead_letter_exchange)

        queue_name = "{}-dead-letter".format(get_queue_name(self.queue))
        dead_letter_queue = self.queue.copy()
        dead_letter_queue["queue"] = queue_name

        yield channel.queue_declare(**dead_letter_queue)

        parent_exchange_name = get_exchange_name(self.exchange)

        for binding in self.bindings:
            routing_key = get_routing_key(parent_exchange_name,
                                          self.bindings)
            binding = {
                "queue": queue_name,
    ...
    def setup(self):
        ...
        try:
            ...
            if self.queue:
                ...
                dead_letter_exchange = qargs.pop("x-dead-letter-exchange",
                                                 None)

                if dead_letter_exchange:
                    yield self.setup_dead_letter(channel,
                                                 dead_letter_exchange)

                    dead_letter_name = get_exchange_name(dead_letter_exchange)
                    qargs["x-dead-letter-exchange"] = dead_letter_name
