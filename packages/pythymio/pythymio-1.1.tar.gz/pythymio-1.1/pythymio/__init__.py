# -*- coding: utf-8 -*-
"""
	This is the library used to interact with the thymio, LIDAR and raspberry.
	If you are using this library, please make sure all the elements are connected.
"""
import dbus
...
ThymioVariables = ["event.args", "button.backward", "button.left", "button.center", "button.forward", "button.right",
                   "prox.horizontal", "prox.comm.rx", "prox.comm.tx", "prox.ground.ambiant", "prox.ground.reflected",
                   "prox.ground.delta", "motor.left.target", "motor.right.target", "motor.left.speed",
                   "motor.right.speed", "motor.left.pwm", "motor.right.pwm", "acc", "temperature", "rc5.address",
                   "rc5.command", "mic.intensity", "mic.threshold", "timer.period"]
...
ThymioEvents = dict([("button.backward", []),
                     ...
                     ("prox", ["prox.horizontal[0]", "prox.horizontal[1]", "prox.horizontal[2]", "prox.horizontal[3]",
                               "prox.horizontal[4]", "prox.horizontal[5]", "prox.horizontal[6]",
                               "prox.ground.ambiant[0]", "prox.ground.ambiant[1]", "prox.ground.reflected[0]",
                               "prox.ground.reflected[1]", "prox.ground.delta[0]", "prox.ground.delta[1]"]),
...
class thymio(object):
    ...
    def __init__(self, SelectedEvents, CustomEvents=customEvents('colors', 'circle', 'sound', 'music')):
        ...
        print
        "loading : %s into Thymio" % aesl.name
...
class EventQueue:
    ...
    def _parse_sleep_request(self, s):
        """ 
            If s = "sleep 1.5", it returns 1.5.
            If it is not a sleep request, then it returns False.
        """
        try:
            s1, s2 = s.split(' ')
            assert s1 == 'sleep'
    ...
    def queueLoop(self, user_fun):
        """
            This method will decompose the user function.
            It looks if this is a sleep rerquest, in which case it will sleep for the time required.
            If the function is not a sleep request, then the event is sent and the next request is recovered
        """
        userAgent = user_fun()
        answerSentToUser = None  # initially, nothing is sent to user
        try:
            while True:
...
def runThymioControl(userFun, eventlist=None):
    """ 
        Main method which enables the user to run the function he is sending (as an argument) to the Thymio 
    """
    eq = EventQueue()
...
def av(l=500, r=500):
    """ 
        Make the Thymio moving ahead.
        If the user desires to modify the arguments, he must make sure that the left and right arguments are still equal one to eachother
    """
...
def td():
    """ 
        Make the Thymio turning right side.
    """
    av(500, -500)
...
def tg():
    """ 
        Make the Thymio turning left side.
    """
    av(-500, 500)
...
def scan1tour():
    """
		This fonction is used to scan on 360° through LIDAR sensors.
	"""
    res=0
    l=[]
    x=0
    ...
    try:
        res = lidar.iter_measurments()
        for scan in res:

            if x == 0:
                x=scan[2]   

            l.append([scan[2],scan[3]])
            if int(scan[2]- x + 360)%360 < 3 and len(l)>10:
                break
    except:
        print("Erreur")
...
def afficheL(l):
    """
        Print in the shell the members of a list
    """
    for res in l:
        print(res)
...
def takePic(dossier="", nom=""):
    """
        Function which makes the Picamera take a picture.
        The user can :
          - give the files location (absolute path) if he is willing to put it somewhere else than in the default location
          - give a name to the picture
    """
    if nom == "":
        nom = time.strftime("%Y-%m-%d_%H-%M-%S") + ".jpg"
    if dossier == "":
        dossier = "/home/pi/Robotique/Pic/"
    picamera.PiCamera().capture(dossier + nom)
