# -*- coding: utf-8 -*-
"""
	This is the library which is going to  be used for the thymio, LIDAR and raspberry.
...
class EventQueue:
    ...
    def queueLoop(self, user_fun):
        ...
        """
        userAgent = user_fun()
        answerSentToUser = None  # initially, nothing is sent to the user
        try:
            while True:
...
def runThymioControl(userFun, eventlist=None):
    """ 
        Main method which enables the user to run the function he is giving (as an argument) on the Thymio 
    """
    eq = EventQueue()
...
def av(l=500, r=500):
    """ 
        Make the Thymio move ahead.
        If the user desires to modify the arguments, he must make sure that the left and right arguments are still equal one to another
    """
...
def td():
    """ 
        Make the Thymio turn right side.
    """
    av(500, -500)
...
def tg():
    """ 
        Make the Thymio turn left side.
    """
    av(-500, 500)
