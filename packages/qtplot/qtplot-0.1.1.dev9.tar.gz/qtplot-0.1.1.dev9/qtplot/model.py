...
class Model:
    ...
    def init_settings(self):
        ...
        if os.path.exists(self.settings_file):
            ...
            msg = 'Could not find settings {}, loading default settings'
            logger.warning(msg.format(self.settings_file))
        ...
        if os.path.isfile(profile_file):
            ...
            path = os.path.join(self.dir, 'default_profile.json')
            self.profile = load_json(path)
            save_json(self.profile, profile_file)

            msg = 'Could not find profile {}, loading default profile'
            logger.warning(msg.format(profile_file))
    ...
    def load_operations(self, filename):
        if not os.path.isfile(filename):
            msg = 'Could not find operations file {}'
            logger.warning(msg.format(filename))
    ...
    def apply_operations(self):
        if self.data2d is None:
            logger.warning('There is no Data2D to apply operations to')
