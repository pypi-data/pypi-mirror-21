...
from ujson import (
    dumps as ujson_dumps,
    loads as ujson_loads
)
...
class ForceTLSHTTPAdapter(HTTPAdapter):

    def __init__(self, max_retries):
        HTTPAdapter.__init__(self, max_retries=max_retries)
...
class HTTPClient(object):
    ...
    def __init__(self, default_endpoint, timeout=60.0):
        ...
        self.__session.mount('https://', ForceTLSHTTPAdapter(max_retries=2))
        self.__session.mount('https://', HTTPAdapter(max_retries=2))
    ...
    def __set_auth(self, request, endpoint):
        ...
        if auth_method == 'NONE':
            ...
            uid = str(auth_options['user'])
            request.headers['X-API-KEY'] = '%s:%s' % (
                uid, hmac.new(str(auth_options['key']), uid, sha1).hexdigest()
            )
