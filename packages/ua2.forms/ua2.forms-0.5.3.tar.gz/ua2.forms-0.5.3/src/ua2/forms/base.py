...
class BaseForm(object):
    ...
    def iter_column_key(self):
        """
        Iterate over column keys
        Used to override fields orders
        """
    ...
    def output_via_template(self):
        ...
        """
        bound_fields = [
            forms.forms.BoundField(self, self.fields[field_key], field_key) for field_key in self.iter_column_key()]

        c = Context(dict(form=self,
                         bound_fields=bound_fields))
        t = loader.get_template(self.get_template_name())
        return t.render(c)

    def as_template(self):
        """ Use in table as {{ form.as_template }}
        """
        for field in self.iter_column_key():
