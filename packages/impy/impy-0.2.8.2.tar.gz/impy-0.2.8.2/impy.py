...
@click.option('-b', '--binning-method', help="Binning method to use.",  type=click.Choice(['maxbin', 'no']), default='maxbin')
@click.option('-c', '--config-file-path', help='Set different config file path.')
@click.option('-d', '--database-path', help='Set different database path.', default=IMP_DEFAULT_DB_DIR)
@click.option('-e', '--environment', help='Add an environment variable (-e <key>=<value>).', multiple=True)
...
@click.pass_context
def cli(ctx, image_name, image_tag, image_repo, threads, memtotal, assembler, binning_method, memcore, database_path, config_file_path, source_code, enter, environment):
    ...
    ctx.obj['config-file-path'] = None
    if config_file_path is not None:
        config_file_path = Path(config_file_path).abspath()
        if not config_file_path.exists():
            click.secho("`config-file-path` must exists.", fg='red', bold=True)
            ctx.abort()
        if not config_file_path.isfile():
            click.secho("`config-file-path` must be a file.", fg='red', bold=True)
            ctx.abort()
        try:
            with open(config_file_path, 'r') as cfile:
                config = json.load(cfile)
        except Exception as e:
            click.secho("Can't load configuration from `config-file-path`: %s. reason: %s." % (config_file_path, str(e)), fg='red', bold=True)
            ctx.abort()
...
@click.option('-n', '--name', help='Name of the archive', default='imp-report')
@click.option('-m', '--method', help='Method to use', type=click.Choice(['zip']), default='zip')
@click.pass_context
def archive_report(ctx, report, name, method):
    """
    Zip/Tar the report with only required file to make the HTML works.
    """
    report = Path(report)
    if report.isdir():
        report = report / 'REPORT.html'
    if not report.exists():
        click.secho("'%s' no found." % report, fg='red', bold=True)
        ctx.abort()
    root = report.parent
    ...
    name = name + "." + method
    files_patterns = (
        '*.js', '*.css', '*.png', '*.pdf', '*.json', '*.html'
    )
    files_to_archive = []
    for p in files_patterns:
        files_to_archive.extend(root.walkfiles(p))
...
def install_imp_container(ctx):
    ...
    if  ctx.obj['image-repo'][:4].startswith('http'):  # download
        ...
        shutil.copy(ctx.obj['image-repo'], fname)
    try:
        # load image
...
def generate_docker_cmd(container_name, database_path, configuration_file_path,
    ...
    # add db volume
    volumes = " -v {database_path}:{container_database_dir}".format(
    ...
    if configuration_file_path is not None:
        ...
        volumes += " -v {configuration_file_dir}:{container_configuration_file_dir}".format(
    ...
    environments = """ -e "LOCAL_USER_ID=`id -u $USER`" -e "LOCAL_GROUP_ID=`id -g $USER`" """

    # add conf file env
    if configuration_file_path is not None:
        environments += " -e CONFIGFILE={container_configuration_file_dir}/{configuration_file_name}".format(
            container_configuration_file_dir=CONTAINER_CONF_DIR,
...
@click.option('--index', help='Fasta file path to use for filtering.')
@click.pass_context
def init(ctx, generate, index):
    """
    Initialise databases.
    The initialisation is in two steps:
    Without the `generate` flag, it download the default filtering database: `hg38`.
    You may omit this step as you can skip the filtering process later on.
    Then you need to add the `generate` flag to initialize tools databases.
    """
    # check if container is installed first
    if not is_imp_container_installed(ctx.obj['image-name'], ctx.obj['image-tag']):
        ...
        ctx.abort()
    click.secho('IMP image found', fg='green')

    ev = ctx.obj['environment']

    # check if fasta file is provided
    if index:
        screen_path = Path(index).abspath()
        if not screen_path.exists():
            click.secho('Path provided does not exist: `%s`.' % screen_path, fg='red', bold=True)
            ctx.abort()
        screen_path_name = screen_path.name.splitext()[0]
        database_path = Path(ctx.obj['database-path']).abspath() / 'filtering'
        shutil.copy(screen_path, database_path / screen_path_name + '.fa')
        ev['FILTER'] = screen_path_name
    # if not generate:
    #     # check if filter db is downloaded and extracted, by default, filtering is set to hg38.
    #     filtering = 'hg38'
    #     if ctx.obj['config-file-path'] is not None and ctx.obj['config-file-path'].exists():
    #         with open(ctx.obj['config-file-path']) as cfile:
    #             config = json.load(cfile)
    #             filtering = config['filtering']['filter']
    #     filter_db = ctx.obj['database-path'] / '%s.fa' % filtering
    #     if not Path(filter_db).exists():
    #         # download default db or abort
    #         if filtering == 'hg38':
    #             url = "https://webdav-r3lab.uni.lu/public/R3lab/IMP/db/hg38.tgz"
    #             tmp = ctx.obj['database-path'] / 'db.tgz'
    ...
    #         # abort as we don't know where the filtering db could be
    #         else:
    #             click.secho('Filter database %s not found. Please download and extract the corresponding file' % filter_db,
    #                         fg='red')
    #             ctx.abort()
    #     else:
    #         click.secho('Filter database found', fg='green')
    #     click.secho('If you want to initialise third party databases for all included tools, please run `impy init` with the `--generate` flag.', blod=True)
    # else:
...
def preprocessing(ctx, metagenomic, metranscriptomic,
    ...
    if ctx.obj['binning-method'] == 'no':
        steps =  ['preprocessing', 'assembly', 'analysis', 'report']
    if single_step:
        steps = ['preprocessing']
    ...
    if screen:
        ...
        if not screen_path.exists():
            ...
            ctx.abort()
        screen_path_name = screen_path.name.splitext()[0]
        shutil.copy(screen_path, database_path / 'filtering' / screen_path_name + '.fa')
    ...
    ev =  ctx.obj['environment']
    ev['IMP_BINNING_METHOD'] = ctx.obj['binning-method']
    ev['MEMTOTAL'] = ctx.obj['memtotal']
    ev['MEMCORE'] = ctx.obj['memcore']
    ev['THREADS'] = ctx.obj['threads']
    ev['MG'] = ' '.join(mg_data)
    ev['MT'] = ' '.join(mt_data)
    ev['IMP_ASSEMBLER'] = ctx.obj['assembler']
    ev['IMP_STEPS'] = ' '.join(steps)
    if no_filtering:
...
def assembly(ctx, metagenomic, metranscriptomic,
    ...
    if ctx.obj['binning-method'] == 'no':
        steps =  ['assembly', 'analysis', 'report']
    if single_step:
        steps = ['assembly']
...
def binning(ctx, data_dir, single_omics,
    ...
    if ctx.obj['binning-method'] == 'no':
        click.secho("Can't run binning with no method'.", fg='red', bold=True)
        ctx.abort()
    if single_step:
        steps = ['binning']
    ...
    data_dir = Path(data_dir).abspath()
    if not data_dir.isdir():
        click.secho('--data-dir must be a directory.', fg='red', bold=True)
        ctx.abort()
