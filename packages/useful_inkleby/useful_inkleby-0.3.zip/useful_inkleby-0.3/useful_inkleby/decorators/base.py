...
class GenericDecorator(six.with_metaclass(DecortorMeta)):
    ...
    def __call__(self,*args,**kwargs):
        ...
        if self.name: #this is a bare class that's already been used, create new child
    ...
    def allow_bare(cls):
        ...
        only if no args_map - or all values in args_map are already provided by default_kwargs
        
        """
        if cls.args_map:
            non_default = [x for x in cls.args_map if x not in cls.default_kwargs]
            if non_default:
                return False
            else:
                return True
    ...
    def raw_decorator(self):
        ...
        """
        obj = self.arg_decorator(self.function,*self.function_args,**self.function_kwargs)
