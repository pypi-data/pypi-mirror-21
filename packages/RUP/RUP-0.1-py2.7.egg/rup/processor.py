...
class Processor:
    ...
    def __process(self, rule, run):
        processor = SubProcessor(rule, run, self.rundb, self.hostname)
...
class SubProcessor:
    def __init__(self, rule, run, rundb, hostname):
        """
        ...
        for f in self.files:
            try:
                ...
                if not os.path.exists(dst):
                    ...
                    overwrite = self.__handle_overwrite(f, dst)
                    publish = overwrite
                    self.logging.info("{} already in correct dir. Skipped.".format(base))

                if publish:
                    self.rundb_files.append(construct_rundb_file(dst, 'raw', self.hostname))
    ...
    def __handle_unpacking(self):
        ...
        self.logging.info("Tmp output dir: {}".format(tmp_output_dir))

        for f in self.tmp_raw_files:
            try:
                ...
                self.logging.info("Log from image:")
                ...
                for output in os.listdir(tmp_output_dir):
                    ...
                    if output.endswith('.root'):
                        ...
                        os.rename(src, dst)
                        self.rundb_files.append(construct_rundb_file(dst, 'unpacked', self.hostname))
                        n_output += 1
                    else:
