...
from anna.exceptions import IncompleteConfigurationError, InvalidPathError, \
    InvalidRepresentationError, InvalidUnitError, ParameterError
...
class Parameter(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def field_name(self):
        ...
        return self._field_name or \
            '_%s' % convert_camel_case_to_snake_case(self.name.replace(' ', '_').replace('-', ''))
    ...
    def is_optional(self):
        ...
        True
        """
        return 'optional' in self._specifications
    ...
    def load_from_representation(self, text, meta):
        ...
        try:
            ...
            raise err.__class__(six.text_type(err), self)
    ...
    def validate_representation(cls, text, meta):
        ...
        if not text:
            raise InvalidRepresentationError('Is empty')
...
class BoolParameter(Parameter):
    ...
    def validate_representation(cls, text, meta=None):
        ...
        [...]
        InvalidRepresentationError: Must be either "true" or "false"
        """
...
class IntegerParameter(Parameter):
    ...
    def validate_representation(cls, text, meta=None):
        ...
        [...]
        InvalidRepresentationError: Division is not supported for IntegerParameters in order to
        prevent misunderstandings arising from integer division
        >>> IntegerParameter.validate_representation('1.1 * 5')
        [...]
        InvalidRepresentationError: Floating point numbers are not allowed for integer values
        """
        ...
        if re.match(r'^[0-9\s+*\-()]+$', text) is None:
            raise InvalidRepresentationError('"%s" is not a valid integer' % text)
    ...
    def _convert_representation(cls, text, meta=None):
        ...
        return datatypes.convert_to(datatypes.integer)(
            safe_math_eval(text, InvalidRepresentationError)
        )
...
class NumberParameter(Parameter):
    ...
    def validate_representation(cls, text, meta=None):
        ...
        try:
            ...
            raise InvalidRepresentationError('"%s" is not a valid number' % text)
    ...
    def _convert_representation(cls, text, meta=None):
        ...
        return datatypes.convert_to(datatypes.number)(
            safe_math_eval(text, InvalidRepresentationError, {'np': numpy})
        )
...
class PhysicalQuantityParameterBase(Parameter):
    ...
    def validate_representation(cls, text, meta):
        ...
        if 'unit' not in meta:
            raise InvalidRepresentationError('Does not declare a unit')
...
class PhysicalQuantityParameter(PhysicalQuantityParameterBase, NumberParameter):
    ...
    def validate_representation(cls, text, meta):
        ...
        try:
            ...
            raise InvalidRepresentationError('"%s" contains invalid keys for constants')
...
class ChoiceParameter(object):
    ...
    def validate_representation(self, text, meta):
        ...
        if (text not in self._options and
            ...
            raise InvalidRepresentationError('%s is not a valid choice' % text)
...
class ParameterGroup(object):
    ...
    >>> @parametrize(
    ... ParameterGroup(Number('StandardDeviation'))
    ...
    def add_option(self, parameter, conversion):
        ...
        Examples
        --------
        >>> standard_deviation = ParameterGroup(NumberParameter('StandardDeviation'))
    ...
    def load_from_representation(self, text, meta, *args):
        ...
        raise NotImplementedError(
            'Loading from representation is not supported for ParameterGroups '
...
class _TupleParameterTemplate(_VectorParameterTemplate):
    ...
    def validate_representation(cls, text, meta):
        ...
        if len(cls._extract_elements(text)) != cls.NUMBER_OF_ELEMENTS:
            raise InvalidRepresentationError(
                'Number of elements does not match: %d != %d'
                % (len(cls._extract_elements(text)), cls.NUMBER_OF_ELEMENTS)
            )
