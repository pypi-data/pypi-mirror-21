...
class Configurable(six.with_metaclass(_ConfigMeta, object)):
    ...
    >>> from anna import PhysicalQuantity, Vector
    >>> @parametrize(
    ... Vector('Limits'),
    ...
    def __init__(self, configuration=None):
        ...
        if configuration is None and filter(lambda p: not p.is_expert, parameters):
    ...
    *components
    ...
    def add_dependency(cls):
        for component in components:
            setattr(
                cls,
                '_dependency_for_%s_%x' % (component.__name__, hash(component)),
