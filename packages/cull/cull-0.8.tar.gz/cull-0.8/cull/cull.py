...
class Cull(object):
    def ec2(self, cloud, service):
        ...
        instance_ids = [instance.instance_id for instance in ec2.instances.filter(Filters=filters)]
        ...
        for index, name in enumerate(instance_names):
            print "Name:" + name + " | ID:" + instance_ids[index] + " | State:" + instance_lifecycle_states.get(
                instance_ids[index], {}) + " | ssh://ubuntu@" + \
                  instance_ips[index]
    ...
    def s3list(self, cloud):
        ...
        cloud_buckets = [bucket.name for bucket in S3.buckets.all() if cloud in bucket.name]
        print "\n".join(cloud_buckets)
    ...
    def s3delete(self):
        ...
        """
        try:
            buckets_to_delete = raw_input('Bucket names (use spaces to pass multiple names)?:').split()
