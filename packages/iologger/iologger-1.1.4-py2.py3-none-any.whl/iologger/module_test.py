...
def test_iologger_debug_level(t_str, t_int):
    with StringIO() as logfile:
        ...
        assert "passed_args" in logfile.getvalue()
        assert "passed_kwargs" in logfile.getvalue()
        assert "returned" in logfile.getvalue()
...
def test_iologger_notice_level(t_str, t_int):
    with StringIO() as logfile:
        ...
        assert "passed_args" not in logfile.getvalue()
        assert "passed_kwargs" not in logfile.getvalue()
        assert "returned" not in logfile.getvalue()
        assert logfile.getvalue() == ""
