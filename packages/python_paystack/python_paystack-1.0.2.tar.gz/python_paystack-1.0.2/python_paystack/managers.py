...
class TransactionsManager(Manager):
    ...
    def initialize_transaction(self, method, transaction: Transaction,
        ...
        if self.PASS_ON_TRANSACTION_COST:
            transaction.amount = transaction.full_transaction_cost(transaction.card_locale, self.LOCAL_COST, self.INTL_COST)
    ...
    def charge_authorization(self, transaction: Transaction):
...
class CustomersManager(Manager):
    ...
    def get_customers(self):
        ...
        if status:
            ...
            customers = []
            for item in data:
                customer = Customer.from_json(item, pickled=False)
    ...
    def get_customer(self, customer_id):
        ...
        if status:
            data = content['data']
            customer = Customer.from_json(data, pickled='False')
            return customer
        else:
            raise APIConnectionFailedError(message)
