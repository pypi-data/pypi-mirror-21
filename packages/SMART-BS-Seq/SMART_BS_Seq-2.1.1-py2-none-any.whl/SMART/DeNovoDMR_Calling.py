...
class DeNovoDMR_Calling():
    ...
    def MissReplace_DMC_callling(self,MethyData,MissReplace,AvailableGroup,MSthreshold,OutFolder,DMCmethyfile,statisticout):
        '''
        ...
        OutFolderName=OutFolder+'tmp/DifferMethlCpG/'
        ...
        while filecontent:
            ...
            if chrome != CurrentStr:  #If search the new chrome,
                ...
                Starttime=time.strftime('%Y-%m-%d %X',time.localtime(time.time()))
                print Starttime+" Identifying DMC: "+chrome
    ...
    def GenomeSegmentMeanMethy(self,ChromeArray,SampleNames,Group_Key,Column_Key_Group,MethySpeInputFolder,MethySegOutputFolder,SegmentBedOutFile,MSthreshold,EDthreshold,SMthreshold,CDthreshold,p_DMR,statisticout):
        '''
...
#                                 print aov_table
                        ...
                        #print NAOVA_pvalue
                        #Output
                        SegmentInfor=RegionChrome+":"+SegmentCpGlist[0]+"-"+str(int(SegmentCpGlist[-1])+2)+"\t"+str(MethySpecificity)+"\t"+str(SegmentCpGNum)
...
#             PreviousSpecificity=1.0-Methyentropy.EntropyCalculate(PreviousMethydataSet_A)
    ...
    def SegmentMerge(self,ChromeArray,SampleNames,Groups,MethySegOutputFolder,MSthreshold,EDthreshold,SMthreshold,CDthreshold,statisticout):
        '''
        ...
        for RegionChrome in ChromeArray:
            ...
            while j<=(len(list0)-1):
                ...
                if SimilarityEntropyvalue< SMthreshold and EucDistance< EDthreshold and (int(list0[j][1])-int(list0[0][2]))<= CDthreshold and IntraCpGNum<=10:
    ...
    def FoldertxttoBed9withMethyandSpecificity(self, MethySpeInputFolder, InputFolder,BedOutFile,MergedSegmentOutFile,MergedSegmentwithmethylationOutFile,MergedHighLowSpeSegmentwithspecificityOutFile,SegmentCellTypeMethymarkPvalue,ChromeArray,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,statisticout):
        ...
        for chrome in ChromeArray:
            ...
            EntropyDEEDFile =gzip.open(MethySpeInputFolder+chrome+".txt.gz", 'r')  #Open the corresbondign methylation file
            CurrentCline=EntropyDEEDFile.readline()     #Read the first line (Colomn Name) of the methylation file
    ...
    def DM_ROI(self, MethySpeInputFolder,GenomeRegions,GenomeRegionFolder,BedOutFile,MergedSegmentOutFile,MergedSegmentwithmethylationOutFile,MergedHighLowSpeSegmentwithspecificityOutFile,SegmentCellTypeMethymarkPvalue,ChromeArray,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,statisticout):
        '''
        ...
        while filecontent:
            ...
            if chrome != CurrentStr:  #If search the new chrome,
                ...
                CurrentStr=chrome
                Starttime=time.strftime('%Y-%m-%d %X',time.localtime(time.time()))
                print Starttime+" Checking regions in: "+chrome
                statisticout.write(Starttime+" Checking regions in: "+chrome+"\n")
        ...
        for chrome in ChromeArray:
            Starttime=time.strftime('%Y-%m-%d %X',time.localtime(time.time()))
            print Starttime+" Identifying DM ROI for: "+chrome
            statisticout.write(Starttime+" Identifying DM ROI for: "+chrome+"\n")
            ...
            EntropyDEEDFile =gzip.open(MethySpeInputFolder+chrome+".txt.gz", 'r')  
            CurrentCline=EntropyDEEDFile.readline()     #Read the first line (Colomn Name) of the methylation file
            CurrentCline=EntropyDEEDFile.readline()     # First data line
    ...
    def runDeNovo(self,MethyData,ProjectName,MissReplace,MSthreshold,EDthreshold,SMthreshold,CDthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,OutFolder):
#         MissReplace = 0.5
#         AvailableGroup = 1.0
#         MSthreshold = 0.5
#         EDthreshold = 0.2
...
#         p_MethylMark = 0.05
        ...
        MethySpeInputFolder=OutFolder+'tmp/DifferMethlCpG/'
        MethySegOutputFolder=OutFolder+'tmp/'
        ...
        ChromeArray,SampleNames,Group_Key,Column_Key_Group,TotalCpGNum,AvailableCpGNum,DMC_Num = Searchregion.MissReplace_DMC_callling(MethyDataFolderName,MissReplace,AvailableGroup,MSthreshold,OutFolder,DMCmethyfile,statisticout)
        ...
        SmallSegNum,Groups=Searchregion.GenomeSegmentMeanMethy(ChromeArray,SampleNames,Group_Key,Column_Key_Group,MethySpeInputFolder,MethySegOutputFolder,SegmentBedOutFile,MSthreshold,EDthreshold,SMthreshold,CDthreshold,p_DMR,statisticout)
        ...
        Searchregion.SegmentMerge(ChromeArray,SampleNames,Groups,MethySegOutputFolder,MSthreshold,EDthreshold,SMthreshold,CDthreshold,statisticout)
        ...
        BedOutFile=gzip.open(FinalResultFolder+"3_MergedSegment.bed.gz",'wb') #Build  a bed file
        ...
        MergedSegmentOutFile=gzip.open(FinalResultFolder+"4_MergedSegment.txt.gz",'wb') #Build a bed file    
        ...
        MergedSegmentwithmethylationOutFile=gzip.open(FinalResultFolder+"5_MergedSegmentwithmethylation.txt.gz",'wb') #Build  a bed file 
        ...
        MergedHighLowSpeSegmentwithspecificityOutFile=gzip.open(FinalResultFolder+"6_MergedSegment_GroupSpecificity.txt.gz",'wb') #Build a bed file    
        ...
        SegmentCellTypeMethymarkPvalue=gzip.open(FinalResultFolder+"7_GroupSpecific_Methylmark.txt.gz",'wb') #Build a bed file  
        ...
        Numdec,CellTypeHypoNum,CellTypeHyperNum=Searchregion.FoldertxttoBed9withMethyandSpecificity(MethySpeInputFolder,InputFolder,BedOutFile,MergedSegmentOutFile,MergedSegmentwithmethylationOutFile,MergedHighLowSpeSegmentwithspecificityOutFile,SegmentCellTypeMethymarkPvalue,ChromeArray,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,statisticout)
        #print Numdec
        shutil.rmtree(MethySegOutputFolder) #Delete tmp files
        ...
        print Endtime+" Detailed results in "+FinalResultFolder
        ...
        statisticout.write(Endtime+" The final Results can be found in "+FinalResultFolder+"\n")
        statisticout.write(Endtime+" For any questions, contact Hongbo Liu (hongbo919@gmail.com)\n")
        statisticout.close()


    def runDMROI(self,MethyData,ProjectName,MissReplace,GenomeRegions,MSthreshold,EDthreshold,SMthreshold,CDthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,OutFolder):
#         MissReplace = 0.5
#         AvailableGroup = 1.0
#         MSthreshold = 0.5
#         EDthreshold = 0.2
...
#         p_MethylMark = 0.05
        ...
        RecordFile=FinalResultFolder+"Summary.txt"
        GenomeRegionFolder=MethySegOutputFolder+"GenomeRegions/"
        ...
        SegmentCellTypeMethymarkPvalue.write("#Methylation marks of each group.\nChrome\tRegionStart\tRegionEnd\tSpecificityState\tCellTypeName\tMethyMarkType\tMethylMarkPvalue\n")    
        TotalRegionNum,Numdec,CellTypeHypoNum,CellTypeHyperNum=Searchregion.DM_ROI(MethySpeInputFolder,GenomeRegions,GenomeRegionFolder,BedOutFile,MergedSegmentOutFile,MergedSegmentwithmethylationOutFile,MergedHighLowSpeSegmentwithspecificityOutFile,SegmentCellTypeMethymarkPvalue,ChromeArray,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,statisticout)
        #print Numdec
        shutil.rmtree(MethySegOutputFolder) #Delete tmp files
    ...
    def runDMC(self,MethyData,ProjectName,MissReplace,MSthreshold,EDthreshold,SMthreshold,CDthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,OutFolder):
#         MissReplace = 0.5
#         AvailableGroup = 1.0
#         MSthreshold = 0.5
#         EDthreshold = 0.2
