...
class DeNovoDMR_Calling():
    ...
    def MissReplace_DMC_callling(self,MethyData,MissReplace,AvailableGroup,MSthreshold,tmpDMCfolder,DMCmethyfile,statisticout):
        '''
        ...
        while filecontent:
            ...
            if chrome != CurrentStr:  #If search the new chrome,
                ...
                chromemethyfile=tmpDMCfolder+chrome+".txt.gz"
            ...
            if AvailableGroupNum/GroupNum >= AvailableGroup: #Check whether there are enough values for entropy calculate
                ...
                if MethySpecificity >= 0:
                    if MethySpecificity >= MSthreshold:
                        DMCState='DMC'
                        DMC_Num=DMC_Num+1
                    else:
                        DMCState='NonDMC'    
        ...
        return ChromeArray,SampleNames,Group_Key,Column_Key_Group,TotalCpGNum,AvailableCpGNum,DMC_Num
    ...
    def GenomeSegmentMeanMethy(self,ChromeArray,SampleNames,Group_Key,Column_Key_Group,tmpDMCfolder,tmpFolder,SegmentBedOutFile,MSthreshold,EDthreshold,SMthreshold,CDthreshold,p_DMR,statisticout):
        '''
        ...
        Methyentropy=NewEntropy.Entropy()
        SegmentBedOutFile.write("#Small segments obtained after first segmentation of genome based on CpG methylation\n")
        OutputFirstline="Chrome"+"\t"+"Start"+"\t"+"End"+"\t"+"CpG Num"+"\t"+"DMRState"+"\t"+"MethySpecificity"+"\t"+"NAOVA_pvalue"+"\t"+"Methydata:"+','.join(Groups)+"\n"
        SegmentBedOutFile.write(OutputFirstline)
        ...
        for RegionChrome in ChromeArray:
            ...
            EntropyDEEDFile =gzip.open(tmpDMCfolder+RegionChrome+".txt.gz", 'r')  #Open the corresbondign methylation file
...
#         Starttime=time.strftime('%Y-%m-%d %X',time.localtime(time.time()))
#         print Starttime+" Summary of genome segmentation (Small segment): "
#         print "\t\t    Number of small segments: "+str(SmallSegNum)
#         print "\t\t    Number of small DMRs identified based threshold MethylSpecificity >= "+str(MSthreshold)+" and NAOVA p value < "+str(p_DMR)+": "+str(SmallDMRNum)
#         statisticout.write(Starttime+" Summary of genome segmentation (Small segment): "+"\n")
#         statisticout.write("\t\t    Number of small segments: "+str(SmallSegNum)+"\n")
#         statisticout.write("\t\t    Number of small DMRs identified based threshold MethylSpecificity >= "+str(MSthreshold)+" and NAOVA p value < "+str(p_DMR)+": "+str(SmallDMRNum)+"\n")
        return SmallSegNum,Groups
...
#                     SegmentBedOutFile.write(RegionChrome+"\t"+SegmentCpGlist[0]+"\t"+str(int(SegmentCpGlist[-1])+2)+"\t"+str(SegmentCpGNum)+"\t"+SegmentState+"\t"+str(MethySpecificity)+"\t"+str(NAOVA_pvalue)+"\t"+Methydata+"\n")
            ...
            CurrentCline=EntropyDEEDFile.readline()
        Starttime=time.strftime('%Y-%m-%d %X',time.localtime(time.time()))
        print Starttime+" Finish segmenting for "+RegionChrome  #Report the region are segmented
        #statisticout.write(Starttime+" Finish segmenting for "+RegionChrome+"\n")    
        return SmallSegNum
    ...
    def MethySpecificity_Calculator_Merge(self,PreviousMethydataSet,CurrentMethydataSet,Groups):
        '''
        Constructor
        ...
        for GroupOrder in range(GroupNum):
            if (PreviousMethydataSet[GroupOrder] != "'-'" and CurrentMethydataSet[GroupOrder] != "'-'"):
                ...
                AvailableCommonGroupNum=AvailableCommonGroupNum+1
        if AvailableCommonGroupNum>0: #Check whether there are enough values for entropy calculate
            EucDistance=scipy.spatial.distance.euclidean( PreviousMethydataSet_A, CurrentMethydataSet_A)/sqrt(AvailableCommonGroupNum)  #calculate the euclidean distance between the methylation leveles of current C and previous C
            DifferMethydataSet=list(map(lambda x: abs(x[0]-x[1]), zip( PreviousMethydataSet_A, CurrentMethydataSet_A)))
            DifferMethydataSet=tuple(DifferMethydataSet)
            SimilarityEntropyvalue=1.0-Methyentropy.EntropyCalculate(DifferMethydataSet)
...
#             PreviousSpecificity=1.0-Methyentropy.EntropyCalculate(PreviousMethydataSet_A)
    ...
    def SegmentMerge(self,ChromeArray,SampleNames,Groups,tmpFolder,MSthreshold,EDthreshold,SMthreshold,CDthreshold,statisticout):
        '''
        ...
        for RegionChrome in ChromeArray:
            ...
            file=gzip.open(tmpFolder+"GenomeSegmentMethy/"+RegionChrome+".txt.gz", 'r')  #Open the file for segment and methylation
...
#                 print list0[j]
                ...
                SimilarityEntropyvalue,EucDistance = self.MethySpecificity_Calculator_Merge(PreviousMethydataSet,MethydataSet,Groups)
...
#                 print str(math.fabs(PreviousSpecificity-CurrentSpecificity))+":"+CurrentState+PreviousState+":"+str(SimilarityEntropyvalue)+":"+str(EucDistance)+":"+str((int(list0[j][1])-int(list0[0][2])))+":"+str(IntraCpGNum)
    ...
    def FoldertxttoBed9withMethyandSpecificity(self, tmpDMCfolder, InputFolder,BedOutFile,MergedSegmentOutFile,MergedSegmentwithmethylationOutFile,MergedHighLowSpeSegmentwithspecificityOutFile,SegmentCellTypeMethymarkPvalue,ChromeArray,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,statisticout):
        '''
        ...
        for chrome in ChromeArray:
            ...
            EntropyDEEDFile =gzip.open(tmpDMCfolder+chrome+".txt.gz", 'r')  #Open the corresbondign methylation file
            CurrentCline=EntropyDEEDFile.readline()     #Read the first line (Colomn Name) of the methylation file
            ...
            while RegionFileLine:
                ...
                if len(MethylationRawList) > 0: # Avoid the errors induced by the short segment
                    ...
                    if (int(RegionCNum) >= SCthreshold and int(SegmentLength) >= SLthreshold):   #Only output the segment with more than 5 CpGs and 20bp
                        ...
                        MergedSegmentwithmethylationOutFile.write(RegionChrome+"\t"+str(RegionStart)+"\t"+str(RegionEnd)+"\t"+SpecificityState+"\t"+MethyState+"\t"+str(MethySpecificity)+"\t"+str(NAOVA_pvalue)+"\t"+str(MeanMethy)+"\t"+"\t"+str(RegionCNum)+"\t"+str(SegmentLength)+"\t"+Methydata+"\n")
                        ...
                        if (float(MethySpecificity)>=MSthreshold and float(NAOVA_pvalue) < p_DMR):
                            ...
                            MergedHighLowSpeSegmentwithspecificityOutFile.write(RegionChrome+"\t"+str(RegionStart)+"\t"+str(RegionEnd)+"\t"+SpecificityState+"\t"+MethyState+"\t"+str(MethySpecificity)+"\t"+str(NAOVA_pvalue)+"\t"+str(MeanMethy)+"\t"+str(RegionCNum)+"\t"+str(SegmentLength)+"\t"+CellTypeSpecificityPvalue+"\n")
                
                RegionFileLine=InputFile.readline()
            Starttime=time.strftime('%Y-%m-%d %X',time.localtime(time.time()))
            print Starttime+" Finish generating Results for "+chrome
        #print Numdec
    ...
    def DeNovoFinalResult_MultiProgress_Chrome(self,chrome,tmpFolder,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark):
        ...
        BedOutFile=gzip.open(tmpFinalResultsFolder+"2_MergedSegment_"+chrome+".bed.gz",'wb') #Build  a bed file
        MergedSegmentOutFile=gzip.open(tmpFinalResultsFolder+"3_MergedSegment_"+chrome+".txt.gz",'wb') #Build a bed file    
        MergedSegmentwithmethylationOutFile=gzip.open(tmpFinalResultsFolder+"4_MergedSegmentwithmethylation_"+chrome+".txt.gz",'wb') #Build  a bed file 
        MergedHighLowSpeSegmentwithspecificityOutFile=gzip.open(tmpFinalResultsFolder+"5_MergedSegment_GroupSpecificity_"+chrome+".txt.gz",'wb') #Build a bed file    
        SegmentCellTypeMethymarkPvalue=gzip.open(tmpFinalResultsFolder+"6_GroupSpecific_Methylmark_"+chrome+".txt.gz",'wb') #Build a bed file  
...
#         Methyentropy=NewEntropy.Entropy()
        ...
        for chrome in ChromeArray:
            BedOutFile_chrome=gzip.open(tmpFinalResultsFolder+"2_MergedSegment_"+chrome+".bed.gz",'r') #Build  a bed file
            MergedSegmentOutFile_chrome=gzip.open(tmpFinalResultsFolder+"3_MergedSegment_"+chrome+".txt.gz",'r') #Build a bed file    
            MergedSegmentwithmethylationOutFile_chrome=gzip.open(tmpFinalResultsFolder+"4_MergedSegmentwithmethylation_"+chrome+".txt.gz",'r') #Build  a bed file 
            MergedHighLowSpeSegmentwithspecificityOutFile_chrome=gzip.open(tmpFinalResultsFolder+"5_MergedSegment_GroupSpecificity_"+chrome+".txt.gz",'r') #Build a bed file    
            SegmentCellTypeMethymarkPvalue_chrome=gzip.open(tmpFinalResultsFolder+"6_GroupSpecific_Methylmark_"+chrome+".txt.gz",'r') #Build a bed file  
            ...
            CurrentCline=MergedSegmentwithmethylationOutFile_chrome.readline()
            while CurrentCline:
                MergedSegmentwithmethylationOutFile.write(CurrentCline)
                CurrentCline=MergedSegmentwithmethylationOutFile_chrome.readline()
            
            CurrentCline=MergedHighLowSpeSegmentwithspecificityOutFile_chrome.readline()
            while CurrentCline:
                MergedHighLowSpeSegmentwithspecificityOutFile.write(CurrentCline)   
                CurrentCline=MergedHighLowSpeSegmentwithspecificityOutFile_chrome.readline()
        ...
        Starttime=time.strftime('%Y-%m-%d %X',time.localtime(time.time()))
        print Starttime+" Finish generating results for all chromesomes"
        return Total_Numdec,CellTypeHypoNum,CellTypeHyperNum


    def DM_ROI(self, tmpDMCfolder,GenomeRegions,GenomeRegionFolder,BedOutFile,MergedSegmentOutFile,MergedSegmentwithmethylationOutFile,MergedHighLowSpeSegmentwithspecificityOutFile,SegmentCellTypeMethymarkPvalue,ChromeArray,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,statisticout):
        '''
        ...
        #RawMethyDataFolderName=MethyDataFolder+'*.wig.gz'
        GenomeRegionsInput=open(GenomeRegions,'rb') #open current methylation file 
        TotalRegionNum=0
        CurrentStr='chr'
        ROI_ChromeArray=[]
        ...
        while filecontent:
            ...
            if chrome != CurrentStr:  #If search the new chrome,
                ...
                chromeregionfile=GenomeRegionFolder+chrome+".txt.gz"
                #print chromemethyfile
                chromeregionfile_out=gzip.open(chromeregionfile,'wb')
            # Data output
            chromeregionfile_out.write(currentRegion[0]+"\t"+currentRegion[1]+"\t"+currentRegion[2]+"\n")
            # Next
            filecontent=GenomeRegionsInput.readline()
        ...
        if len(commonChrome)==0:
            ...
            Starttime=time.strftime('%Y-%m-%d %X',time.localtime(time.time()))
            print Starttime+" Start identifying DM ROIs ..."
            statisticout.write(Starttime+" Start to identifying DM ROIs...\n")
            MethyTB=NewEntropyNormal.Entropy()
            Methyentropy=NewEntropy.Entropy()
            ...
            for chrome in ChromeArray:              
                ...
                chromeregionfile_input=gzip.open(GenomeRegionFolder+chrome+".txt.gz",'r')
                #Open the corresbondign methylation file
                EntropyDEEDFile =gzip.open(tmpDMCfolder+chrome+".txt.gz", 'r')  
                CurrentCline=EntropyDEEDFile.readline()     #Read the first line (Colomn Name) of the methylation file
                CurrentCline=EntropyDEEDFile.readline()     # First data line
        ...
        #print Numdec
        return TotalRegionNum,Numdec,CellTypeHypoNum,CellTypeHyperNum
    ...
    def runDeNovo(self,UserArgslist,MethyData,ProjectName,MissReplace,MSthreshold,EDthreshold,SMthreshold,CDthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,OutFolder):
        ...
        Starttime=time.strftime('%Y-%m-%d %X',time.localtime(time.time()))
        print Starttime+" Start to fill missing value and identify Differentially methylated CpG sites (DMC)..."
        statisticout.write(Starttime+" Start to fill missing value and identify Differentially methylated CpG sites (DMC)...\n")
        
        DMCmethyfile=gzip.open(FinalResultFolder+"1_DifferMethlCpGs.txt.gz",'wb') #Built a txt file 
        ChromeArray,SampleNames,Group_Key,Column_Key_Group,TotalCpGNum,AvailableCpGNum,DMC_Num = Searchregion.MissReplace_DMC_callling(MethyDataFolderName,MissReplace,AvailableGroup,MSthreshold,tmpDMCfolder,DMCmethyfile,statisticout)
        Groups=Group_Key.keys()
        ...
        #SegmentBedOutFile=gzip.open(FinalResultFolder+"2_SmallSegment.bed.gz",'wb') #Built a bed file  
        ...
        Searchregion.SegmentMerge(ChromeArray,SampleNames,Groups,tmpFolder,MSthreshold,EDthreshold,SMthreshold,CDthreshold,statisticout)
        ...
        BedOutFile=gzip.open(FinalResultFolder+"2_MergedSegment.bed.gz",'wb') #Build  a bed file
        BedOutFile.write("track name=\"MergedMethySegment\" description=\"Merged Methylation Segment by SMART2\" visibility=2 itemRgb=\"On\"\n")
        ...
        MergedSegmentOutFile=gzip.open(FinalResultFolder+"3_MergedSegment.txt.gz",'wb') #Build a bed file    
        MergedSegmentOutFile.write("#Merged segments based small segments.\nChrome\tStart\tEnd\tSpecificityState\tMethyState\tMethySpecificity\tNAOVA_pvalue\tMeanMethy\tCNum\tLength\n")    
        ######SegmentwithMethylation data
        MergedSegmentwithmethylationOutFile=gzip.open(FinalResultFolder+"4_MergedSegmentwithmethylation.txt.gz",'wb') #Build  a bed file 
        MergedSegmentwithmethylationOutFile.write("#Mean methylation level of merged segments in each group.\nChrome\tStart\tEnd\tSpecificityState\tMethyState\tMethySpecificity\tNAOVA_pvalue\tMeanMethy\tCNum\tLength\tMethylData:"+','.join(Groups)+"\n")    
        ######Cell-type-specificity
        MergedHighLowSpeSegmentwithspecificityOutFile=gzip.open(FinalResultFolder+"5_MergedSegment_GroupSpecificity.txt.gz",'wb') #Build a bed file    
        MergedHighLowSpeSegmentwithspecificityOutFile.write("#Methylation specificity of merged segments in each group.\nChrome\tStart\tEnd\tSpecificityState\tMethyState\tMethySpecificity\tNAOVA_pvalue\tMeanMethy\tCNum\tLength\tMethylMarkPvalue:"+','.join(Groups)+"\n")    
        ######Cell-type-specificity2
        SegmentCellTypeMethymarkPvalue=gzip.open(FinalResultFolder+"6_GroupSpecific_Methylmark.txt.gz",'wb') #Build a bed file  
        SegmentCellTypeMethymarkPvalue.write("#Methylation marks of each group.\nChrome\tRegionStart\tRegionEnd\tSpecificityState\tCellTypeName\tMethyMarkType\tMethylMarkPvalue\n")    
        Numdec,CellTypeHypoNum,CellTypeHyperNum=Searchregion.DeNovoFinalResult_MultiProgress(tmpFolder,BedOutFile,MergedSegmentOutFile,MergedSegmentwithmethylationOutFile,MergedHighLowSpeSegmentwithspecificityOutFile,SegmentCellTypeMethymarkPvalue,ChromeArray,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,statisticout)
        ...
        MergedSegmentwithmethylationOutFile.close()
        MergedHighLowSpeSegmentwithspecificityOutFile.close()
        ...
        print Endtime+" Number of identified DMC based threshold "+str(MSthreshold)+"       :"+str(DMC_Num)
        ...
        statisticout.write(Endtime+" Number of identified DMC based threshold "+str(MSthreshold)+"       :"+str(DMC_Num)+"\n")
    ...
    def runDMROI(self,UserArgslist,MethyData,ProjectName,MissReplace,GenomeRegions,MSthreshold,EDthreshold,SMthreshold,CDthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,OutFolder):
        ...
        Groups=Group_Key.keys()
        ###### Bed file
        BedOutFile=gzip.open(FinalResultFolder+"2_DifferMethlROIs.bed.gz",'wb') #Build  a bed file 
        BedOutFile.write("track name=\"GenomeRegions\" description=\"Genome regions of interest by SMART2\" visibility=2 itemRgb=\"On\"\n")
        ######Bed9 for Visualization in Hub of UCSC genome browser
        MergedSegmentOutFile=gzip.open(FinalResultFolder+"3_DifferMethlROIs.txt.gz",'wb') #Build a bed file    
        MergedSegmentOutFile.write("#Genome regions of interest with methylation specificity.\nChrome\tStart\tEnd\tSpecificityState\tMethyState\tMethySpecificity\tNAOVA_pvalue\tMeanMethy\tCNum\tLength\n")    
        ######SegmentwithMethylation data
        MergedSegmentwithmethylationOutFile=gzip.open(FinalResultFolder+"4_DifferMethlROIs_withmethylation.txt.gz",'wb') #Build  a bed file 
        MergedSegmentwithmethylationOutFile.write("#Mean methylation level of genome regions of interest in each group.\nChrome\tStart\tEnd\tSpecificityState\tMethyState\tMethySpecificity\tNAOVA_pvalue\tMeanMethy\tCNum\tLength\tMethylData:"+','.join(Groups)+"\n")    
        ######Cell-type-specificity1
        MergedHighLowSpeSegmentwithspecificityOutFile=gzip.open(FinalResultFolder+"5_DifferMethlROIs_GroupSpecificity.txt.gz",'wb') #Build a bed file    
        MergedHighLowSpeSegmentwithspecificityOutFile.write("#Methylation specificity of genome regions of interest in each group.\nChrome\tStart\tEnd\tSpecificityState\tMethyState\tMethySpecificity\tNAOVA_pvalue\tMeanMethy\tCNum\tLength\tMethylMarkPvalue:"+','.join(Groups)+"\n")    
        ######Cell-type-specificity2
        SegmentCellTypeMethymarkPvalue=gzip.open(FinalResultFolder+"6_DifferMethlROIs_Methylmark.txt.gz",'wb') #Build a bed file  
        SegmentCellTypeMethymarkPvalue.write("#Methylation marks of each group.\nChrome\tRegionStart\tRegionEnd\tSpecificityState\tCellTypeName\tMethyMarkType\tMethylMarkPvalue\n")    
        TotalRegionNum,Numdec,CellTypeHypoNum,CellTypeHyperNum=Searchregion.DM_ROI(tmpDMCfolder,GenomeRegions,GenomeRegionFolder,BedOutFile,MergedSegmentOutFile,MergedSegmentwithmethylationOutFile,MergedHighLowSpeSegmentwithspecificityOutFile,SegmentCellTypeMethymarkPvalue,ChromeArray,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,statisticout)
        #print Numdec
        shutil.rmtree(tmpFolder) #Delete tmp files
        ...
        print Endtime+" Number of CpG sites with methylation in all groups      :"+str(AvailableCpGNum)
        print Endtime+" Number of identified DMC based threshold "+str(MSthreshold)+"            :"+str(DMC_Num)
        print Endtime+" Number of total genome regions                          :"+str(TotalRegionNum)
        print Endtime+" Number of genome regions with methylation in all groups :"+str(Numdec['AvilableGenomeRegion'])
        print Endtime+" DMR Number                                              :"+str(Numdec['DMR'])
        print Endtime+" NonDMR Number                                           :"+str(Numdec['NonDMR'])
        print Endtime+" NonDMR-UniHypo Segment Number                           :"+str(Numdec['UniHypo'])
        print Endtime+" NonDMR-UnipLow Segment Number                           :"+str(Numdec['UnipLow'])
        ...
        statisticout.write(Endtime+" Number of CpG sites with methylation in all groups      :"+str(AvailableCpGNum)+"\n")
        statisticout.write(Endtime+" Number of identified DMC based threshold "+str(MSthreshold)+"            :"+str(DMC_Num)+"\n")
        statisticout.write(Endtime+" Number of total genome regions                          :"+str(TotalRegionNum)+"\n")
        statisticout.write(Endtime+" Number of genome regions with methylation in all groups :"+str(Numdec['AvilableGenomeRegion'])+"\n")
        statisticout.write(Endtime+" DMR Number                                              :"+str(Numdec['DMR'])+"\n")
        statisticout.write(Endtime+" NonDMR Number                                           :"+str(Numdec['NonDMR'])+"\n")
        statisticout.write(Endtime+" NonDMR-UniHypo Segment Number                           :"+str(Numdec['UniHypo'])+"\n")
        statisticout.write(Endtime+" NonDMR-UnipLow Segment Number                           :"+str(Numdec['UnipLow'])+"\n")
    ...
    def runDMC(self,UserArgslist,MethyData,ProjectName,MissReplace,MSthreshold,EDthreshold,SMthreshold,CDthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,OutFolder):
        ...
        DMCmethyfile=gzip.open(FinalResultFolder+"DifferMethlCpGs.txt.gz",'wb') #Built a txt file 
        ChromeArray,SampleNames,Group_Key,Column_Key_Group,TotalCpGNum,AvailableCpGNum,DMC_Num = Searchregion.MissReplace_DMC_callling(MethyDataFolderName,MissReplace,AvailableGroup,MSthreshold,tmpDMCfolder,DMCmethyfile,statisticout)
        DMCmethyfile.close()
        shutil.rmtree(tmpFolder) #Delete tmp files
...
if __name__ == '__main__':
    ...
    OutFolder="/Users/hongbo.liu/Documents/HongboData/Projects/00SMART2/Testdata/Results/"
