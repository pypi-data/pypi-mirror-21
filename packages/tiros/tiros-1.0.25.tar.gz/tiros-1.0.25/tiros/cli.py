...
viz_parser = subparsers.add_parser(
    'viz',
    parents=[general_args, server_args],
    help=textwrap.dedent('''Tiros Snapshot Visualization.\
    (i) generate a snapshot with 'tiros snapshot > mysnapshot.json'
    (ii) \'tiros viz\' and go to \'localhost:5000\' in your browser
    (iii) upload \'mysnapshot.json\'
    ''')
)
...
def viz(args):
    ...
    if args.region:
        session = util.change_session_region(session, args.region)
    tiros_viz_app.main(session, args)
