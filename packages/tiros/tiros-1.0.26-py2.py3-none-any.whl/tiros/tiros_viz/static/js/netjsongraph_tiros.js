...
/*
* D3js module from https://github.com/netjson/netjsongraph.js
* Modified to adopt to Tiros Vizualization
*/
...
(function () {
    ...
    d3.netJsonGraph = function(url, opts) {
        ...
        var el = d3.select(opts.el).style("position", "relative"),
            ...
            snapshot_view = d3.select(opts.el).append("div").attr("class", "njg-metadata2"),
            metadataInner = metadata.append("div").attr("class", "njg-inner"),
            queryInner = metadata.append("div").attr("class", "njg-inner"),
            tiros_query_answer = d3.select(opts.el).append("div").attr("class", "tiros-metadata"),
            ...
            processJson = function(graph) {
                    ...
                    node = groups.append("circle")
                                 .attr("class", function (node) {
                                     var baseClass = "njg-node",
                                         addClass = null;
                                         value = node.properties && node.properties['color'];
                                 ...
                                 .attr("r", function(node) {
                                             switch(node.properties['color']){
                                               case 'vpc': return 20;
                                               case 'subnet':return 15;
                                               default: return opts.circleRadius;
                                             }})
                ...
                html += "<p><b>Tiros Service</b>: <span>" + opts.tiros_status + "</span></p>";
                html += "<form action=\"snap_view\" method=\"post\" enctype=\"multipart/form-data\">"
                html += "<select name=\"selection\" onchange=\"this.form.submit()\">"
                html+=  "<option value=\"\">Query View</option>"
                html+= "<option value=\"vsi\">VPC-Subnet-Instance</option>"
                html+= "<option value=\"acl\">ACL</option>"
                html+= "</select>"
                html+="</form>"
                html += "<form action=\"collapse_view\" method=\"post\" enctype=\"multipart/form-data\">"
                html += "<select name=\"selection\" onchange=\"this.form.submit()\">"
                html+=  "<option value=\"\">Search View</option>"
                html+= "<option value=\"vsi_collapse\">VPC-Subnet-Instance</option>"
                html+= "</select>"
                html+="</form>"
                if (opts.cached_status!="0"){
                  html += "<form action=\"cached_result\" method=\"post\" enctype=\"multipart/form-data\">"
                  html += "<select name=\"selection\" onchange=\"this.form.submit()\">"
                  html += "<option value=\"\">Filter Instances</option>"
                  html +=  "<option value=\"ok-instance\">OK-Instances</option>"
                  html +=  "<option value=\"ssh-instance\">Instances-SSH-Internet</option>"
                  html +=  "</select>"
                  html +=  "</form>"
              }
                ...
                /*Tiros Input*/
                if (opts.query_answer!=""){
                  console.log(opts.query_answer);
                  tiros_query_answer.attr("class", "tiros-metadata").style("display", "block");
                  var ans_html = "<h3> Tiros Response</h3>";
                  ans_html += "<p>" +opts.query_answer+"</p>";
                  tiros_query_answer.html(ans_html);
                  query_close = tiros_query_answer.append("a").attr("class", "njg-close");
                  tiros_query_answer.classed("njg-hidden", false);
                  ...
                  query_close.on("click", function() {
                          removeOpenClass();
                          tiros_query_answer.classed("njg-hidden", true);
                  });
              }
