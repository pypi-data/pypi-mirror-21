...
class Api_table_name:
	def get(self, primary_key):
		try:
			# http://api_table_name?user_hash=12345&action=get
			if primary_key == None:
				...
				# http://api_table_name?user_hash=12345&action=get&primary_key=1
				result = config.model.get_table_name(int(primary_key))
				table_name_json = []
				table_name_json.append(str(dict(result)))
				web.header('Content-Type', 'application/json')
				return json.dumps(table_name_json)
...
# https://0.0.0.0:8080/api_table_name?user_hash=12345&action=update&primary_key=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=default.jpg
