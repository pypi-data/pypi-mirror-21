...
class Deployments(Command):
    ...
    @WaitCompletion(logger=logger)
    def update(self, name, code_file, requirements_file, new_name, wait):
        logger.debug('Going to update deployment: {0}'.format(name))
        if not (code_file or requirements_file or new_name):
            return 'No action requested.'

        uri = 'deployments/{0}'.format(name)
        query_params = None
        code_file = self._encode_content(code_file)
    ...
    def _encode_content(data):
        return urllib.quote(data.read().encode("utf-8")) if data else ""
