...
from .models import Customer, User, ProductPageVisit
...
def clean_visits_history():
    ...
    visits_counts = \
        ProductPageVisit.objects.order_by('customer').values('customer') \
        .annotate(history_length=Count('customer')).filter(history_length__gt=_limit)

    for v in visits_counts:
        ProductPageVisit.objects.filter(customer=v['customer']).order_by('-changed')[_limit:].delete()
