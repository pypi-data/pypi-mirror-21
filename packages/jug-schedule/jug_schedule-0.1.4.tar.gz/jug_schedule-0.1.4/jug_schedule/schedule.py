...
class Job(object):
    ...
    def reap(self):
        ...
        Calling this function is necessary to obtain job statistics and avoid a memory leak
        """
...
class SessionHandler(object):
    ...
    def __init__(self, options, store):
        ...
        self.exit_on_error = options.schedule_exit_on_error
    ...
    def run(self, target, jobs=1, wait_cycles=8, wait_cycle_time=15):
        ...
        # Use only the function name as name of the job
        jt.jobName = target.split(' ', 1)[0]
        jt.workingDirectory = os.getcwd()

        # Construct jt.nativeSpecification with specified resources
        res = self._system.native_resources(target)
        ...
        if self._system.is_slurm():
            ...
            jt.outputPath = ":" + os.path.join(self.logs, "jug-%j.out")
            jt.errorPath = ":" + os.path.join(self.logs, "jug-%j.err")
            # Would have been "jug-%A_%a.err" if we could submit arrays
        else:
        ...
        if jobs == 1:
            jobid = self.s.runJob(jt)
            self._add_job(target, jobid)
        else:
            if self._system.is_slurm():
                logging.warning(
                    "Current DRMAA implementation cannot submit bulk jobs. "
                    "Falling back to single jobs. This may impact cluster performance. "
                    "More information at: https://git.io/vyld6"
                )
                # FIXME Batch arrays segfault on SLURM. Submitting single jobs works though.
                for i in range(jobs):
                    jobid = self.s.runJob(jt)
                    self._add_job(target, jobid)
            else:
                jobids = self.s.runBulkJobs(jt, beginIndex=1, endIndex=jobs, step=1)
    ...
    def collect_jobs(self):
        ...
        for target in self.assigned:
            for job in self.assigned[target].copy():
                if job.is_done():
                    jobinfo = job.reap()
                    logging.info("Reaped a jug instance from target %s", target)

                    self._rm_job(target, job)

                    if jobinfo.exitStatus or jobinfo.wasAborted:
                        logging.error("One jug instance failed with exit code %s and abort status %s. "
                                      "Check %s for more information",
                                      jobinfo.exitStatus, jobinfo.wasAborted, self.logs)
    ...
    def assign_jobs(self):
        ...
        if available_jobs == 0:
            logging.debug("All job slots are in use or we are out of slots - No new jobs queued")
            return
        ...
        if total_pending == 0:
            logging.debug("All work is taken or complete - No new jobs queued")
            return
...
class ScheduleCommand(SubCommand):
    ...
    def parse(self, parser):
        ...
        parser.add_argument("--exit-on-error", action="store_const", const="True",
                            dest="schedule_exit_on_error",
                            help="Abort as soon as one queued job fails")
