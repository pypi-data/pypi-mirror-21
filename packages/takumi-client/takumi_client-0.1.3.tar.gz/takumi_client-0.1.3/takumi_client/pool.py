...
class _PoolWrapper(object):
    """Wraps the actual pool to support creating pool lazily.
    ...
    def set_pool(self, pool):
        ...
        :param pool: an instance of :class:`.Pool`
        """
        self.__pool = pool

    def _assert_pool(self):
        if not self.__pool:
            raise RuntimeError('Pool {!r} not created', self.name)

    def client_ctx(self):
        """Context manager to get a client.
        """
        self._assert_pool()
        return self.__pool.client_ctx()

    @property
    def thrift_module(self):
        """Thrift module of this pool.
        """
        self._assert_pool()
        return self.__pool.thrift_module
    ...
    def service_name(self):
        ...
        """
        self._assert_pool()
        return self.__pool.service_name
...
class _PoolDict(dict):
    ...
    def __init__(self, pool_size=30, timeout=30, check_time=10):
        ...
        self.check_time = check_time
        self.__settings = config.settings['CLIENT_SETTINGS']
        # Validate settings
        try:
            self.settings_schema.validate(self.__settings)
    ...
    def __getitem__(self, key):
        if key not in self:
            self[key] = _PoolWrapper(key)
            try:
                self[key].set_pool(self._get_pool(key))
            except Exception:
                self.pop(key)
                raise
        return self.get(key)

# This object should be coroutine safe
clients = _PoolDict()
