#!/usr/bin/env python
# gython to JavaScript Translator
...
class JSGenerator(NodeVisitor):  # , inline_function.Inliner):
    def __init__(self, requirejs=True, insert_runtime=True, webworker=False, function_expressions=True, fast_javascript=False, fast_loops=False):
        ...
        self.special_decorators = set(
            ['__typedef__', '__glsl__', '__pyfunction__', 'expression'])
        self._glsl = False
    ...
    def visit_Assign(self, node):
        # XXX: I'm not sure why it is a list since, mutiple targets are inside
        # a tuple
        target = node.targets[0]
        if isinstance(target, Tuple):
            raise NotImplementedError(
                'target tuple assignment should have been transformed to flat assignment by python_to_gython.py')
        else:
            target = self.visit(target)
            value = self.visit(node.value)
            # visit_BinOp checks for `numpy.float32` and changes the operands
            # from `a*a` to `a[id]*a[id]`
    ...
    def visit_With(self, node):
        ...
        if isinstance(node.context_expr, Name) and node.context_expr.id == '__default__':
            ...
            if len(node.context_expr.args):
                ...
                a = '%s = %s' % (node.context_expr.keywords[0].arg, self.visit(
    ...
    def visit_Module(self, node):
        ...
        if self._insert_runtime:
            ...
            runtime = open(os.path.join(dirname, 'gython.js')).read()
        ...
        if len(modules) == 1:
            ...
            for mod in modules:
                ...
                d[mod['name']] = '\n'.join(l if isinstance(
                    l, str) else l.encode("utf-8") for l in lines)
            return d
    ...
    def visit_Lambda(self, node):
        ...
        if args and args[0] == '__INLINE_FUNCTION__':
            ...
            # return '<LambdaError>'   ## skip node, the next function contains
            # the real def
    ...
    def visit_FunctionDef(self, node):
        ...
        # could do something special here with global function
        if node == self._function_stack[0]:
            # buffer += 'gython.%s = %s' %(node.name, node.name)  ## this is
            # no longer needed
    ...
    def _visit_call_helper_var_glsl(self, node):
        ...
        for key in node.keywords:
            ...
            if ptrs:
                ...
                # __glsl_dynamic_typedef() is inserted just before the
                # assignment.
                pass
            else:
    ...
    def _visit_function(self, node):
        ...
        if glsl:
            ...
            for i, arg in enumerate(args):
                if gpu_vectorize and arg not in args_typedefs:
                    ...
                    if arg in args_typedefs:
                        x.append('%s %s' % (
                            args_typedefs[arg].replace('POINTER', '*'), arg))
                    elif gpu_method and i == 0:
                        x.append('%s self' % arg)
                    else:
                        # x.append( 'float* %s' %arg )  ## this could be a way
                        # to default to the struct.
                        raise SyntaxError(
                            'GLSL functions require a typedef: %s' % arg)

            if is_main:
                # each call to the wrapper function recompiles the shader
                lines.append(
                    'var glsljit = glsljit_runtime(__shader_header__);')
                if x:
                    lines.append(
                        'glsljit.push("void main(%s) {");' % ','.join(x))
                else:
                    # WebCLGL parser requires the space in `main( )`
                    lines.append('glsljit.push("void main( ) {");')

            elif return_type:
                # if gpu_method:
                #	lines.append( '__shader_header__.push("%s %s(struct this, %s ) {");' %(return_type, node.name, ', '.join(x)) )
                # else:
                lines.append('__shader_header__.push("%s %s( %s ) {");' % (
                    return_type, node.name, ', '.join(x)))
            else:
                lines.append('__shader_header__.push("void %s( %s ) {");' % (
                    node.name, ', '.join(x)))
            ...
            if is_main:
                ...
                lines.append(
                    '__shader_header__.push("vec2 _id_ = get_global_id();");')
            ...
            self.pull()
            if is_main:
                # fixes WebCLGL 2.0 parser
                lines.append('glsljit.push(";(1+1);");')
            ...
            if is_main:
                ...
                if args:
                    lines.append('function %s( %s, __offset ) {' % (
                        glsl_wrapper_name, ','.join(args)))
                else:
                    lines.append(
                        'function %s( __offset ) {' % glsl_wrapper_name)

                # note by default: 0 allows 0-1.0 ## TODO this needs to be set
                # per-buffer
                lines.append('	__offset =  __offset || 1024')
                ...
                lines.append(
                    '  var __kernel = __webclgl.createKernel( shader, header );')
                ...
                if gpu_return_types:
                    if 'array' in gpu_return_types:
                        if ',' in gpu_return_types['array']:
                            ...
                            lines.append(
                                '  var __return_length = %s * %s' % (w, h))
                        else:
                            lines.append('  var __return_length = %s' %
                                         gpu_return_types['array'])
                    elif 'vec4' in gpu_return_types:
                        if ',' in gpu_return_types['vec4']:
                            w, h = gpu_return_types['vec4'][1:-1].split(',')
                            lines.append(
                                '  var __return_length_vec4 = %s * %s' % (w, h))
                        else:
                            lines.append('  var __return_length_vec4 = %s' %
                                         gpu_return_types['vec4'])

                    elif 'mat4' in gpu_return_types:
                        # minimum size is 64
                        lines.append('  var __return_length = 64')
                    else:
                        raise NotImplementedError
                else:
                    # minimum size is 64
                ...
                for i, arg in enumerate(args):
                    ...
                    lines.append('    var %s_buffer = __webclgl.createBuffer(%s.dims || %s.length, "FLOAT", %s.scale || __offset)' % (
                        arg, arg, arg, arg))
                    lines.append(
                        '    __webclgl.enqueueWriteBuffer(%s_buffer, %s)' % (arg, arg))
                    lines.append(
                        '  __kernel.setKernelArg(%s, %s_buffer)' % (i, arg))
                    lines.append(
                        '  } else { __kernel.setKernelArg(%s, %s) }' % (i, arg))
                ...
                if gpu_return_types:
                    if 'vec4' in gpu_return_types:
                        ...
                        lines.append(
                            '	var rbuffer_vec4 = __webclgl.createBuffer(%s, "FLOAT4", __offset)' % dim)
                        lines.append(
                            '	__webclgl.enqueueNDRangeKernel(__kernel, rbuffer_vec4)')
                        lines.append(
                            '  var __res = __webclgl.enqueueReadBuffer_Float4( rbuffer_vec4 )')
                        lines.append(
                            '	return glsljit.unpack_vec4(__res, %s)' % gpu_return_types['vec4'])
                    elif 'array' in gpu_return_types:
                        dim = gpu_return_types['array']
                        lines.append(
                            '	var rbuffer_array = __webclgl.createBuffer(%s, "FLOAT", __offset)' % dim)
                        lines.append(
                            '	__webclgl.enqueueNDRangeKernel(__kernel, rbuffer_array)')
                        lines.append(
                            '  var __res = __webclgl.enqueueReadBuffer_Float( rbuffer_array )')
                        lines.append(
                            '	return glsljit.unpack_array2d(__res, %s)' % gpu_return_types['array'])

                    elif 'mat4' in gpu_return_types:
                        lines.append(
                            '	var rbuffer = __webclgl.createBuffer([4,glsljit.matrices.length], "FLOAT4", __offset)')
                        lines.append(
                            '	__webclgl.enqueueNDRangeKernel(__kernel, rbuffer)')
                        lines.append(
                            '  var __res = __webclgl.enqueueReadBuffer_Float4( rbuffer )')  # slow
                        lines.append('	return glsljit.unpack_mat4(__res)')
                    else:
                        raise SyntaxError(
                            'invalid GPU return type: %s' % gpu_return_types)

                else:
                    raise SyntaxError('GPU return type must be given')
                    lines.append(
                        '  var __return = __webclgl.createBuffer(__return_length, "FLOAT", __offset)')
                    lines.append(
                        '	__webclgl.enqueueNDRangeKernel(__kernel, __return)')
                    lines.append(
                        '	return __webclgl.enqueueReadBuffer_Float( __return )')

                lines.append('} // end of wrapper')
                lines.append('%s.return_matrices = glsljit.matrices' %
                             glsl_wrapper_name)
    ...
    def visit_Call(self, node):
        ...
        if name in list(typedpython.GO_SPECIAL_CALLS.values()):
            ...
            # in the if-assignment block, the typedef is not given because
            # `Iter_n` already has been typed beforehand.
            sname = node.args[0].s
            ...
            lines = [
                ...
                # capture first item with target name so that for loops can
                # get the length of member arrays
                '`@var %s = %s[0];`' % (target, iter)
            ]
            ...
            # at runtime loop over subarray, for each index inline into the
            # shader's for-loop an if test,
            lines.append('`@for (var __j=0; __j<__length__; __j++) {`')
            #lines.append(     '`@glsljit.push("if (OUTPUT_INDEX==" +__j+ ") { %s %s=%s_" +__j+ ";}");`' %(sname, target, iter))
            lines.append(
                '`@glsljit.push("if (matrix_index()==" +__j+ ") { %s=%s_" +__j+ ";}");`' % (target, iter))
            lines.append('`@}`')
            ...
            n = node.args[0].s
            # special case bypass visit_Attribute
            if isinstance(node.args[1], ast.Attribute):
                v = '%s.%s' % (node.args[1].value.id, node.args[1].attr)
            else:
                v = self.visit(node.args[1])

            # this is known this entire expression is an external call.
            v = v.replace('`', '')

            # check if number is required because literal floats like `1.0`
            # will get transformed to `1` by javascript toString
            ...
            if v.isdigit() or (v.count('.') == 1 and v.split('.')[0].isdigit() and v.split('.')[1].isdigit()):
                ...
                # return '`@%s=%s; %s if (%s instanceof Array)
                # {glsljit.array(%s, "%s")} else {%s}};`' %(n,v, if_number, n,
                # n,n, orelse)
        ...
        #	raise NotImplementedError
        # return '`__glsl_inline_array(%s, "%s")`' %(self.visit(node.args[0]),
        # node.args[1].s)
    ...
    def inline_helper_remap_names(self, remap):
        return "var %s;" % ','.join(list(remap.values()))
    ...
    def _inline_code_helper(self, s):
        ...
        # AttributeError: 'BinOp' object has no attribute 's' - this is caused
        # by bad quotes
        s = s.replace('\n', '\\n').replace('\0', '\\0')
    ...
    def visit_BinOp(self, node):
        ...
        if op == '>>' and left == '__new__':
            ...
            # this can happen because python_to_gython.py will catch when a new class instance is created
    ...
    def _visit_for_prep_iter_helper(self, node, out, iter_name):
        ...
        # new style - Object.keys only works for normal JS-objects, not ones
        # created with `Object.create(null)`
        if not self._fast_loops:
            out.append(
                self.indent() + 'if (! (%s instanceof Array || typeof %s == "string" || __is_typed_array(%s) || __is_some_array(%s) )) { %s = __object_keys__(%s) }' % (
                    iter_name, iter_name, iter_name, iter_name, iter_name, iter_name)
            )
    ...
    def _visit_for_glsl(self, node):
        ...
        if isinstance(node.iter, ast.Call) and isinstance(node.iter.func, ast.Name) and node.iter.func.id == 'iter':  # `for i in iter(n):`
            ...
            # this fixes attributes on structs
            self._typed_vars[target] = 'struct*'
            ...
            lines.append('`@for (var __j=0; __j<__length__; __j++) {`')
            lines.append(
                '`@glsljit.push("if (_iter==" +__j+ ") { %s=%s_" +__j+ ";}");`' % (target, iter))
            lines.append('`@}`')
            ...
            lines = [
                'for (int %s=0; %s < %s; %s++) {' % (target, target, iter, target)]
            ...
            # this is required to inline the lengths as constants into the GLSL
            # for loops
            ...
            # shader's for-loop an if test,
            lines.append('`@for (var __j=0; __j<__length__; __j++) {`')
            # below checks if the top-level iterator is the same index, and if
            # so copy its contents into the local subarray,
            lines.append(
                '`@glsljit.push("if (_iter==" +__j+ ") { for (int _J=0; _J<" +__length__+ "; _J++) {%s[_J] = %s_" +__j+ "[_J];} }");`' % (target, node.iter.id))
            lines.append('`@}`')
            # this works because the function glsljit.array will unpack an array of arrays using the variable name with postfix "_n"
            # note the extra for loop `_J` is required because the local
            # subarray can not be assigned to `A_n`
    ...
    def visit_For(self, node):
        ...
        if your are trying to opitmize looping over a gython list, you can do this:
                for v in mylist[...]:
                        print v
        ...
        if self._fast_loops:
            out.append('for (var %s=0; %s < %s.length; %s++)' %
                       (index, index, iname, index))
            out.append(self.indent() + '{')

        else:
            out.append(self.indent(
            ) + 'for (var %s=0; %s < %s.length; %s++) {' % (index, index, iname, index))
        self.push()

        body.append(self.indent() +
                    'var %s = %s[ %s ];' % (target, iname, index))
...
def generate_minimal_runtime():
    from .python_to_gython import main as py2pyjs
    a = py2pyjs(
...
def generate_runtime():
    ...
    lines = [
        main(open('runtime/pythongython.py', 'rb').read(), requirejs=False, insert_runtime=False,
             function_expressions=True, fast_javascript=True),  # lowlevel gython
        main(builtins, requirejs=False, insert_runtime=False,
             function_expressions=True, fast_javascript=True)
    ]
    return '\n'.join(lines)
...
if __name__ == '__main__':
    if '--runtime' in sys.argv:
        print('creating new runtime: gython.js')
        open('gython.js', 'wb').write(generate_runtime())
    elif '--miniruntime' in sys.argv:
        print('creating new runtime: gython-minimal.js')
        open('gython-minimal.js', 'wb').write(generate_minimal_runtime())
