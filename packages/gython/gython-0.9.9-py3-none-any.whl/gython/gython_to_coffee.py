#!/usr/bin/env python
# gython to CoffeeScript Translator
...
class TransformSuperCalls(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        if isinstance(node.func, ast.Attribute) and isinstance(
                node.func.value,
                ast.Name) and node.func.value.id in self._class_names:
...
class CoffeeGenerator(gython.JSGenerator):
    _classes = dict()
    _class_props = dict()
    ...
    def visit_ClassDef(self, node):
        ...
        for decor in node.decorator_list:  # class decorators
            ...
            elif isinstance(decor, ast.Attribute) and isinstance(
                    decor.value, ast.Name) and decor.value.id == 'dart':
        ...
        if bases:
            if extends:
                assert len(bases) == 1
                out.append('class %s extends %s {' % (node.name,
                                                      ','.join(bases)))
            else:
                # if bases[0] == 'object':
                #	out.append('class %s {' %node.name)
                # else:
                out.append('class %s implements %s {' % (node.name,
                                                         ', '.join(bases)))
        ...
        for b in node.body:
            ...
            if isinstance(b, ast.FunctionDef) and len(
                    b.decorator_list):  # getter/setters
                ...
                out.append(self.visit(b))
                if args:
                    out.append(self.indent() + '%s(%s) {%s.__init__(this,%s);}'
                               % (node.name, args, node.name, args))
                else:
                    out.append(self.indent() + '%s() {%s.__init__(this);}' %
                               (node.name, node.name))
                ...
                if operator and args:
                    out.append(self.indent() +
                               '%s(%s) { return %s.__%s(this,%s); }' %
                               (operator, args, node.name, b.name, args))

                elif operator:
                    out.append(self.indent() + '%s() { return %s.__%s(this); }'
                               % (operator, node.name, b.name))

                elif args:
                    out.append(self.indent() +
                               '%s(%s) { return %s.__%s(this,%s); }' %
                               (b.name, args, node.name, b.name, args))
                else:
                    out.append(self.indent() + '%s() { return %s.__%s(this); }'
                               % (b.name, node.name, b.name))
                ...
                for b in bnode.body:
                    if isinstance(b, ast.FunctionDef):
                        ...
                        if args:
                            out.append(self.indent() +
                                       '%s(%s) { return %s.__%s(this,%s); }' %
                                       (b.name, args, bnode.name, b.name,
                                        args))
                        else:
                            out.append(self.indent() +
                                       '%s() { return %s.__%s(this); }' %
                                       (b.name, bnode.name, b.name))
    ...
    def _visit_for_prep_iter_helper(self, node, out, iter_name):
        out.append(
            ...
            self.indent() + 'if (%s is dict) %s = %s.keys();' %
        ...
        for decor in node.decorator_list:
            if isinstance(decor, ast.Name) and decor.id == 'property':
                getter = True
                    ...
                    decor.value, ast.Name) and decor.attr == 'setter':
                setter = True
            elif isinstance(decor, ast.Attribute) and isinstance(
                    decor.value, ast.Name) and decor.attr == 'prototype':
    ...
    def visit_ExceptHandler(self, node):
        ...
        if node.type:
            out.append(self.indent() +
                       '###exception: %s' % self.visit(node.type))
        for n in node.body:
            out.append(self.indent() + self.visit(n))
