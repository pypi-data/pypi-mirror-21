...
glsl_types = ['struct*', 'int*', 'float*', 'vec2',
...
native_number_types = ['int', 'float', 'double']
    ...
    for line in source.splitlines():
        ...
        for i, char in enumerate(line):
            ...
            ## go array and map syntax ##
                ...
                while b != '[':
                    ...
                    b = a.pop()
                gotype.reverse()
                # fixes spaces inside brackets `[ 1 ]string()`
                gotype = ''.join(gotype).strip()
                if not gotype:
                    if nextchar == '[':
                ...
                # a.append(', type=True),')  ## this breaks function
                # annotations that splits on ','
                a.append('<<typedef),')
                hit_go_typedef = False
                ...
                b = ''.join(a)
                b = b.strip()
                is_class_type = b.startswith(
                    'class:') and len(b.split(':')) == 2
        ...
        if ' except ' in c and ':' in c:  # PEP 463 - exception expressions
            ...
            if len(s) == 2 and '=' in s[0] and ':' in s[1]:
                ...
                output.append('%sexcept %s: %s=%s' %
                              (indent, exception, s0.split('=')[0], default))
                c = ''
        ...
        if '<-' in c:
            if '=' in c:
                ...
                # this is not used now because it is not required by the Go
                # backend.
        ...
        if '->' in c:
            ...
            c = c.replace('->' + method_name, '.' +
                          method_name + '.bind(%s)' % this_name)
        ...
        if '=def ' in c or '= def ' in c or ': def ' in c or ':def ' in c:
            ...
            if d.startswith('='):
                c += '=lambda __INLINE_FUNCTION__: %s' % tail.strip().split(':')[
                    0]
        ...
        if 'def ' in c and c.count(':') > 1:
            ...
            for x in tail.split(','):
                ...
                if ':' in y:
                    ...
                    if chan:
                        output.append('%s@typedef_chan(%s=%s)' %
                                      (indent, arg_name, typedef))
                    else:
                        output.append('%s@typedef(%s=%s)' %
                                      (indent, arg_name, typedef))
                    if kw:
                        arg += '=' + kw
                    args.append(arg)
