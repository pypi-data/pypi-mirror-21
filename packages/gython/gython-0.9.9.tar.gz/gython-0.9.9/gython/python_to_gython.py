...
from . import typedpython
from . import ministdlib
from . import inline_function
...
class Typedef(object):
    ...
    def __init__(self, **kwargs):
        for name in list(kwargs.keys()):  # name, methods, properties, attributes, class_attributes, parents
        ...
        for name in self.methods:
            if name in self._opmap:
                op = self._opmap[name]
                self.operators[op] = self.get_gython_function_name(name)

    def get_gython_function_name(self, name):
    ...
    def format_error(self, node):
        ...
        for l, n in enumerate(self._stack):
            ...
            msg += '%s%s line:%s col:%s\n' % (' ' * (l + 1) * 2,
                                              n.__class__.__name__, n.lineno, n.col_offset)
        return msg


class PythonTogython(NodeVisitorBase, inline_function.Inliner):
    ...
    def __init__(self, source=None, modules=False, module_path=None, dart=False, coffee=False, lua=False, go=False, fast_javascript=False, pure_javascript=False):
        #super(PythonTogython, self).__init__()
        NodeVisitorBase.__init__(self)
        self._modules = modules  # split into mutiple files by class
        # used for user `from xxx import *` to load .py files in the same
        # directory.
                    ...
                    if 'type="text/python"' in line:
                        ...
                        if os.path.isfile(os.path.expanduser(x)):
                            ...
                            if x.lower().endswith('.coffee'):
                                ...
                                o.append(
                                    open(os.path.expanduser(x), 'rb').read())
                            o.append('</script>')
                            if script is True:
        ...
        # 'array' or float32, or array of 'vec4' float32's.
        ...
        # inferred class instances, TODO regtests to confirm that this never
        # breaks ##
        ...
        # class name : typedef  (deprecated - part of the old static type
        # finder)
        self._typedefs = dict()
        ...
        # global name : set  (if len(set)==1 then we know it is a typed list)
        self._global_typed_lists = dict()
        ...
        # this is only used in full python mode.
        ...
        for line in source.splitlines():
            if line.strip().startswith('import tornado'):
                ...
                header = open(os.path.join(dirname, os.path.join(
                    'fakelibs', 'tornado.py'))).read()
                source = header + '\n' + source
                self._source = source.splitlines()
            elif line.strip().startswith('import os'):
                dirname = os.path.dirname(os.path.abspath(__file__))
                header = open(os.path.join(
                    dirname, os.path.join('fakelibs', 'os.py'))).read()
                source = header + '\n' + source
                self._source = source.splitlines()
            elif line.strip().startswith('import tempfile'):
                dirname = os.path.dirname(os.path.abspath(__file__))
                header = open(os.path.join(dirname, os.path.join(
                    'fakelibs', 'tempfile.py'))).read()
                source = header + '\n' + source
                self._source = source.splitlines()
            elif line.strip().startswith('import sys'):
                dirname = os.path.dirname(os.path.abspath(__file__))
                header = open(os.path.join(
                    dirname, os.path.join('fakelibs', 'sys.py'))).read()
                source = header + '\n' + source
                self._source = source.splitlines()
            elif line.strip().startswith('import subprocess'):
                dirname = os.path.dirname(os.path.abspath(__file__))
                header = open(os.path.join(dirname, os.path.join(
                    'fakelibs', 'subprocess.py'))).read()
                source = header + '\n' + source
                self._source = source.splitlines()
    ...
    def has_webworkers(self):
        return len(list(self._webworker_functions.keys()))
        ...
        if class_name:
            ...
            if class_name not in self._typedefs:
                self._typedefs[class_name] = Typedef(
                    ...
                    properties=self._decorator_class_props.get(
                        class_name, set()),
                    attributes=self._instance_attributes.get(
                        class_name, set()),
                    class_attributes=self._class_attributes.get(
                        class_name, set()),
    ...
    def visit_Import(self, node):
        ...
        for alias in node.names:
            if self._with_go:
                ...
                pass  # gython/fakelibs/tornado.py
            elif alias.name == 'tempfile':
                pass  # gython/fakelibs/tempfile.py
            elif alias.name == 'sys':
                pass  # gython/fakelibs/sys.py
            elif alias.name == 'subprocess':
                pass  # gython/fakelibs/subprocess.py
                ...
                # allow "workerjs" to be loaded as a fallback, however this
                # appears to not work in nodewebkit.
                writer.write(
                    'if __NODEJS__==True and typeof(Worker)=="undefined": Worker = require("workerjs")')

            elif alias.asname:
                #writer.write( '''inline("var %s = requirejs('%s')")''' %(alias.asname, alias.name) )
                writer.write('''inline("var %s = require('%s')")''' % (
                    alias.asname, alias.name.replace('__DASH__', '-')))

            elif '.' in alias.name:
                raise NotImplementedError(
                    'import with dot not yet supported: line %s' % node.lineno)
            else:
    ...
    def visit_ImportFrom(self, node):
        ...
        if node.module == 'time' and node.names[0].name == 'sleep':
            ...
            # in gython it is a builtin function defined in builtins.py
            pass
                ...
                if n.name in lib[node.module]:
                    if not imported:
                        ...
                        if ministdlib.REQUIRES in lib[node.module]:
                            writer.write('import %s' % ','.join(
                                lib[node.module][ministdlib.REQUIRES]))
            ...
            # note: the current gython.configure mode here maybe different
            # from the subcontext.
            data = open(path, 'rb').read()
            subtrans = PythonTogython(
                data,
            ...
            # TODO - what other typedef info needs to be copied here?
            self._js_classes.update(subtrans._js_classes)
    ...
    def visit_Dict(self, node):
        ...
        for i in range(len(node.keys)):
            ...
            if self._with_dart or self._with_ll or self._with_go or self._fast_js:
                ...
                a.append('JSObject(key=%s, value=%s)' %
                         (k, v))  # this allows non-string keys
    ...
    def visit_List(self, node):
        ...
        if self._with_ll:
            ...
            a = '__get__(list, "__call__")({}, {pointer:%s, length:%s})' % (
                a, len(node.elts))
        return a
    ...
    def _gen_comp(self, generators, node):
        ...
        if isinstance(gen.iter, ast.Call) and isinstance(gen.iter.func, ast.Name) and gen.iter.func.id in ('range', 'xrange'):
            ...
            writer.write('while idx%s < __get__(len, "__call__")([iter%s], JSObject()):' % (
                id, id))  # TODO optimize
            writer.push()

            writer.write('var(%s)' % gen.target.id)
            writer.write('%s=get%s( [idx%s], JSObject() )' %
                         (gen.target.id, id, id))
        ...
        if self._with_lua:  # convert to list
            writer.write(
                '%s = list.__call__({},{pointer:%s, length:idx%s})' % (cname, cname, id))
    ...
    def _gen_comp_helper(self, cname, node):
        if isinstance(node, ast.DictComp):
            ...
            if self._with_dart:
                ...
                writer.write('table.insert(%s, %s )' %
                             (cname, self.visit(node.elt)))
            elif self._with_go:
                writer.write('%s = append(%s, %s )' %
                             (cname, cname, self.visit(node.elt)))
            else:
    ...
    def visit_NotIn(self, node):
        # return ' not in '
        raise RuntimeError(
            '"not in" is only allowed in if-test: see method - visit_Compare')
    ...
    def visit_AugAssign(self, node):
        ...
        if self._with_lua:
            ...
            if self._with_go:
                ...
                a = '%s = (%s/%s).floor()' % (target,
                                              target, self.visit(node.value))
            else:
                a = '%s = Math.floor(%s/%s)' % (target,
                                                target, self.visit(node.value))
            writer.write(a)
            ...
            if isinstance(node.target, ast.Name) and node.target.id in self._typedef_vars and self._typedef_vars[node.target.id] in typedpython.native_number_types + typedpython.vector_types:
                ...
                writer.write('if instanceof(%s, Number) or instanceof(%s, String): %s' % (
                    target, target, b))
            ...
            #a = '__get__(%s, "__setitem__")( [%s, __get__(%s, "__getitem__")([%s], {}) %s (%s)], {} )'
            fallback = '__get__(%s, "__setitem__")( [%s, __get__(%s, "__getitem__")([%s], {}) %s (%s)], {} )' % (
                name, slice, name, slice, op, value)
            if isinstance(node.target.value, ast.Name):
                # TODO also check for arr.remote (RPC) if defined then
                # __setitem__ can not be bypassed
                ...
                writer.write('if instanceof(%s, Array): %s[%s] %s= %s' % (
                    name, name, slice, op, value))
                writer.write('else: %s' % fallback)
            else:
                writer.write(fallback)

        else:
            # TODO extra checks to make sure the operator type is valid in this
            # context
            a = '%s %s %s' % (target, op, self.visit(node.value))
            writer.write(a)
    ...
    def _visit_js_classdef(self, node):
        ...
        for item in node.body:
            if isinstance(item, FunctionDef):
                ...
                if self.is_gpu_method(item):
                    # change self to the class name, gython.py changes it to
                    # 'ClassName self'
                    item.args.args[0].id = name
                else:
                    item.args.args = item.args.args[1:]  # remove self
        ...
        if not self._fast_js:
            ...
            # isinstance runtime builtin requires this
            writer.write('this.__class__ = %s' % name)

            ## instance UID ##
            writer.write('this.__uid__ = "￼" + _gython_UID')
            writer.write('_gython_UID += 1')

        writer.pull()

        if not self._fast_js:
            ## class UID ##
            writer.write('%s.__uid__ = "￼" + _gython_UID' % name)
            writer.write('_gython_UID += 1')
        ...
        for mname in method_names:
            ...
            if gpu_method:
                ...
                if not self._fast_js:
                    ...
                    f = 'function () { return %s.prototype.%s.apply(arguments[0], Array.prototype.slice.call(arguments,1)) }' % (
                        name, mname)
        ...
        for item in class_vars:
            if isinstance(item, Assign) and isinstance(item.targets[0], Name):
                ...
                writer.write('%s.prototype.%s = %s' %
                             (name, item_name, item.targets[0].id))
    ...
    def visit_ClassDef(self, node):
        ...
        if self._modules:
            # triggers a new file in final stage of translation.
        ...
        self._catch_attributes = None
        # property names :  {'get':func, 'set':func}
        self._decorator_properties = dict()
        ...
        if not self._with_coffee:
            writer.write('var(%s, __%s_attrs, __%s_parents)' %
                         (name, name, name))
        ...
        for item in node.body:
            if isinstance(item, FunctionDef):
                ...
                writer.write('__%s_attrs.%s = %s' %
                             (name, item_name, item.targets[0].id))
                # should this come before self.visit(item) ??
                self._class_attributes[name].add(item_name)
                ...
                for sub in item.body:
                    if isinstance(sub, Assign) and isinstance(sub.targets[0], Name):
                        ...
                        # this will output the code for the assign
                        self.visit(sub)
                        writer.write('__%s_attrs.%s = %s' %
                                     (name, item_name, sub.targets[0].id))
                        # should this come before self.visit(item) ??
                        self._class_attributes[name].add(item_name)
                    else:
        ...
        for prop_name in self._decorator_properties:
            ...
            writer.write(
                '__%s_properties["%s"] = JSObject()' % (name, prop_name))
            writer.write('__%s_properties["%s"]["get"] = %s' % (
                name, prop_name, getter))
            if self._decorator_properties[prop_name]['set']:
                setter = self._decorator_properties[prop_name]['set']
                writer.write('__%s_properties["%s"]["set"] = %s' % (
                    name, prop_name, setter))
        ...
        writer.write('%s = __create_class__("%s", __%s_parents, __%s_attrs, __%s_properties)' % (
        ...
        for dec in class_decorators:
            writer.write('%s = __get__(%s,"__call__")( [%s], JSObject() )' % (
                name, self.visit(dec), name))
    ...
    def visit_If(self, node):
        if self._with_dart and writer.is_at_global_level():
            raise SyntaxError(self.format_error(
                'if statements can not be used at module level in dart'))
        elif self._with_lua:
            writer.write('if __test_if_true__(%s):' % self.visit(node.test))

        elif isinstance(node.test, ast.Dict):
            if self._with_js:
                writer.write('if Object.keys(%s).length:' %
    ...
    def visit_Raise(self, node):
        ...
        if isinstance(node.type, ast.Name):
            ...
            if node.type.args:
                writer.write('raise %s(%s)' % (self.visit(
                    node.type.func), self.visit(node.type.args[0])))
            else:
                writer.write('raise %s()' % self.visit(node.type.func))

    def visit_ExceptHandler(self, node):
        if node.type and node.name:
            writer.write('except %s, %s:' %
                         (self.visit(node.type), self.visit(node.name)))
    ...
    def visit_Return(self, node):
        if node.value:
            ...
            if self._with_glsl and self._in_gpu_main:
                # _id_ is inserted into all function headers by gython.py for
                # glsl functions.
                if not self._gpu_return_types:
                    raise SyntaxError(self.format_error(
                        'function return type unknown - required decorator `@returns(array/vec4=[w,h])`'))
                ...
                if 'array' in self._gpu_return_types:
                    ...
                    writer.write(
                        'inline("mat4 _res_ = %s; int _row = matrix_row();")' % nv)
                    ...
                    raise SyntaxError(self.format_error(
                        'invalid GPU return type: %s' % self._gpu_return_types))

            elif self._inline:
                writer.write('__returns__%s = %s' %
                             (self._inline[-1], self.visit(node.value)))
                ...
                writer.write('return %s;' % ','.join(
                    [self.visit(e) for e in node.value.elts]))
    ...
    def visit_BinOp(self, node):
                ...
                if isinstance(node.left, ast.Call) and isinstance(node.left.func, ast.Name):
                    if node.left.func.id == '__go__array__' and isinstance(node.right, ast.GeneratorExp):
                        ...
                        if isinstance(node.left.args[1], ast.Call):
                            is_go_listcomp = True
                            node.right.go_dictcomp_type = (
                                node.left.args[0].id, self.visit(node.left.args[1]))
                        else:
                            node.right.go_dictcomp_type = (
                                node.left.args[0].id, node.left.args[1].id)
        ...
        if self._with_glsl:
            ...
            if isinstance(node.right, Str):
                ...
                if op.decode('utf-8') in self._custom_operators:
            ...
            if self._with_js:
                # assumes that right is a tuple, or list.
                return '__sprintf( %s, %s )' % (left, right)
            else:
                # assumes that right is a tuple, or list.
                return '__sprintf( %s, %s )' % (left, right)
            ...
            if '+' in self._direct_operators:
                ...
                # this is also required when in an inlined lambda like "(lambda
                # a,b: a+b)(1,2)"
                ...
                # else __add_op will call the "__add__" method of the left
                # operand, passing right as the first argument.
        ...
        for i in range(len(node.ops)):
            if i == 0 and isinstance(node.left, ast.Name) and node.left.id in self._typedef_vars and self._typedef_vars[node.left.id] == 'long':
                if isinstance(node.ops[i], ast.Eq):
                    ...
                    comp = ['%s.greaterThan(%s)' % (
                        left, self.visit(node.comparators[i]))]

                elif isinstance(node.ops[i], ast.LtE):
                    comp = ['%s.lessThanOrEqual(%s)' % (
                        left, self.visit(node.comparators[i]))]
                elif isinstance(node.ops[i], ast.GtE):
                    comp = ['%s.greaterThanOrEqual(%s)' % (
                        left, self.visit(node.comparators[i]))]
                ...
                if self._with_dart:
                    ...
                    comp.append(
                        "__get__(__get__(%s, '__contains__'), '__call__')([%s], JSObject())" % a)
    ...
    def visit_UnaryOp(self, node):
        ...
        if operand is None:
            raise RuntimeError(node.operand)
        return op + operand
    ...
    def visit_Attribute(self, node):
        ...
        # this is required because lua has no support for inplace assignment
        # ops like "+="
        elif self._with_lua and self._in_assign_target:
            return '%s.%s' % (node_value, node.attr)
            ...
            src = src.replace('"', '\\"')
            err = 'missing attribute `%s` - line %s: %s' % (
                node.attr, node.lineno, src.strip())
    ...
    def visit_Subscript(self, node):
        ...
        if isinstance(node.slice, ast.Ellipsis):
            ...
            if isinstance(node.slice, ast.Slice):  # allow slice on Array
                if self._with_dart:
                    # this is required because we need to support slices on
                    # String ##
                ...
                if node.slice.value.n < 0:
                    # the problem with this is it could be a dict with negative
                    # numbered keys
                ...
                if self._in_catch_exception == 'KeyError':
                    ...
                    # in that case it is safe to assume that this is a direct
                    # key.
                    return '%s[ %s ]' % (name, self.visit(node.slice))
                    ...
                    # return '%s[ __ternary_operator__(%s.__uid__, %s) ]'
                    # %(name, s, s)
                    check_array = '__ternary_operator__( instanceof(%s,Array), JSON.stringify(%s), %s )' % (
                        s, s, s)
            ...
            value = self.visit(node.value)
            slice = self.visit(node.slice)
            fallback = '__get__(%s, "__getitem__", "%s")([%s], __NULL_OBJECT__)' % (
                value, err, slice)
    ...
    def visit_Assign(self, node):
        use_runtime_errors = not (
            self._with_js or self._with_ll or self._with_dart or self._with_coffee or self._with_lua or self._with_go)
        ...
        if isinstance(target, ast.Name) and target.id in typedpython.types:
            if len(targets) == 2 and isinstance(targets[1], ast.Name):
                ...
                if target.id == 'long' and isinstance(node.value, ast.Num):
                    ...
                    writer.write('%s = long.fromString("%s")' %
                                 (targets[1].id, self.visit(node.value)))
                    return None
                else:
                    targets = targets[1:]
                ...
                if target.id == 'long' and isinstance(node.value, ast.Num):
                    ## requires long library ##
                    writer.write('%s = long.fromString("%s")' %
                                 (targets[1].value.id, self.visit(node.value)))
                    return None
                else:
                    targets = targets[1:]
            ...
            if len(targets) == 2 and isinstance(targets[1], ast.Attribute) and isinstance(targets[1].value, ast.Name) and targets[1].value.id == 'self' and len(self._class_stack):
                if target.value.id == '__go__array__':
                    self._class_stack[-1]._struct_vars[targets[1]
                                                       .attr] = '__go__array__(%s<<typedef)' % self.visit(target.slice)
                elif target.value.id == '__go__class__':
                    self._class_stack[-1]._struct_vars[targets[1]
                                                       .attr] = self.visit(target.slice)
                elif target.value.id == '__go__pointer__':
                    self._class_stack[-1]._struct_vars[targets[1]
                                                       .attr] = '"*%s"' % self.visit(target.slice)
                ...
                if len(targets) == 2:
                    writer.write(
                        'inline("var %s *%s;")' % (self.visit(
                            targets[1]), self.visit(target.slice))
                    )
                else:
                    writer.write(
                        'inline("var %s *%s;")' % (self.visit(node.value),
                ...
                if isinstance(node.value, ast.Call) and len(node.value.args) and isinstance(node.value.args[0], ast.GeneratorExp):
                    node.value.args[0].go_listcomp_type = self.visit(
                        target.slice)
                else:
                    raise SyntaxError(
                        'only a variable created by a generator expressions needs an array typedef')
            else:
                raise SyntaxError(self.visit(target))
            ...
            writer.write('__rpc_set__(%s, "%s", %s)' %
                         (self._with_rpc, target.attr, self.visit(node.value)))
            return None
            ...
            writer.write('%s = __rpc_get__(%s, "%s")' %
    ...
    def _visit_assign_helper(self, node, target):
        if isinstance(node.value, ast.Lambda):
            ...
            if not key_type:
                raise SyntaxError(self.format_error(
                    'can not determine dict key type'))
            if not val_type:
                raise SyntaxError(self.format_error(
                    'can not determine dict value type'))

            t = self.visit(target)
            v = self.visit(node.value)
            writer.write('%s = __go__map__(%s, %s) << %s' %
                         (t, key_type, val_type, v))
            ...
            if guess_type:
                t = self.visit(target)
                v = self.visit(node.value)
                writer.write('%s = __go__array__(%s) << %s' %
                             (t, guess_type, v))
            else:
                raise SyntaxError(self.format_error(
                    'can not determine type of array'))

        elif isinstance(target, Subscript):
            # target.value may have "returns_type" after being visited
            ...
            if isinstance(target.slice, ast.Ellipsis):
                ...
                code = '%s[...] = %s' % (self.visit(
                    target.value), self.visit(node.value))

            elif isinstance(target.slice, ast.Slice):
                code = '%s.__setslice__(%s, %s)' % (self.visit(
                    target.value), self.visit(target.slice), self.visit(node.value))

            elif self._with_dart or self._with_ll or self._with_glsl or self._with_go:
                code = '%s[ %s ] = %s'
                code = code % (self.visit(target.value), self.visit(
                    target.slice.value), self.visit(node.value))

            elif self._with_js:
                s = self.visit(target.slice.value)
                if isinstance(target.slice.value, ast.Num) or isinstance(target.slice.value, ast.BinOp):
                    code = '%s[ %s ] = %s' % (self.visit(
                        target.value), s, self.visit(node.value))
                elif self._with_direct_keys:
                    code = '%s[ %s ] = %s' % (self.visit(
                        target.value), s, self.visit(node.value))
                else:
                    check_array = '__ternary_operator__( instanceof(%s,Array), JSON.stringify(%s), %s )' % (
                        s, s, s)
                    code = '%s[ __ternary_operator__(%s.__uid__, %s) ] = %s' % (
                        self.visit(target.value), s, check_array, self.visit(node.value))

            elif name in self._func_typedefs and self._func_typedefs[name] == 'list':
                code = '%s[%s] = %s' % (name, self.visit(
                    target.slice.value), self.visit(node.value))
            ...
            if isinstance(target.value, Name):
                ...
                typedef = self.get_typedef(
                    class_name=target.value.returns_type)

            #####################################

            if self._with_js or self._with_dart or self._with_go:
                writer.write('%s.%s=%s' %
                             (target_value, target.attr, self.visit(node.value)))
                ...
                writer.write('%s( [%s, %s], JSObject() )' % (
                    setter, target_value, self.visit(node.value)))
            ...
            #	writer.write( '''%s['__class__']['%s'] = %s''' %(target_value, target.attr, self.visit(node.value)))
                ...
                parent_prop = typedef.check_for_parent_with(
                    property=target.attr)
                #parent_classattr = typedef.check_for_parent_with( class_attribute=target.attr )
                parent_setattr = typedef.check_for_parent_with(
                    method='__setattr__')
                ...
                #	writer.write( "__%s_attrs.%s = %s" %(parent_classattr.name, target.attr, self.visit(node.value)) )
                    ...
                    func = parent_setattr.get_gython_function_name(
                        '__setattr__')
                    writer.write('%s([%s, "%s", %s], JSObject() )' % (
                        func, target_value, target.attr, self.visit(node.value)))

                elif '__setattr__' in typedef.methods:
                    func = typedef.get_gython_function_name('__setattr__')
                    writer.write('%s([%s, "%s", %s], JSObject() )' % (
                        func, target_value, target.attr, self.visit(node.value)))
                ...
                func = typedef.get_gython_function_name('__setattr__')
                log('__setattr__ in instance typedef.methods - func:%s target_value:%s target_attr:%s' %
                    (func, target_value, target_attr))
                writer.write('%s([%s, "%s", %s], JSObject() )' % (
                    func, target_value, target.attr, self.visit(node.value)))
            ...
            # node.value may have extra attributes after being visited
            node_value = self.visit(node.value)

            if node_value in self._typedef_vars:
                writer.write('%s = %s' %
                             (self.visit(target), self.visit(node.value)))

            elif isinstance(node.value, ast.Subscript) and isinstance(node.value.slice, ast.Ellipsis):
                writer.write('glsl_inline_assign_from_iterable("%s", "%s", %s)' % (
                    self._typedef_vars[target.id], target.id, self.visit(node.value.value)))
                ...
                if not isinstance(node.value, (ast.BinOp, ast.Call)):
                    if isinstance(node.value, ast.Subscript) and isinstance(node.value.slice, ast.Slice):
                        x = node_value.split(
                            '(')[-1].split(')')[0].split('[')[0]
                        writer.write('glsl_inline_push_js_assign("%s", %s.__getslice__(%s))' % (
                            target.id, x, self.visit(node.value.slice)))
                    else:
                        writer.write('glsl_inline_push_js_assign("%s", %s)' % (
                            target.id, self.visit(node.value)))
                else:
                    writer.write('%s = %s' %
                                 (target.id, self.visit(node.value)))
            ...
            if isinstance(node.value, Call) and hasattr(node.value.func, 'id') and node.value.func.id in self._classes:
                ...
                self._instances[target.id] = node.value.func.id
                ...
                if method in typedef.methods:
                    func = typedef.get_gython_function_name(method)
                    if func in self._function_return_types:
                        self._instances[target.id] = self._function_return_types[func]
                    else:
                        writer.write(
                            '## %s - unknown return type for: %s' % (typedef.name, func))
                else:
                    writer.write('## %s - not a method: %s' %
                                 (typedef.name, method))

            # if this is a simple copy: "a = b" and "b" is known to be of some
            # class
            elif isinstance(node.value, Name) and node_value in self._instances:
            ...
            for i, target in enumerate(target.elts):
                if isinstance(target, Attribute):
                    ...
                    fallback = "__get__(__get__(%s, '__getitem__'), '__call__')([%s], __NULL_OBJECT__)" % (
                        r, i)
                    writer.write("%s = __ternary_operator__(instanceof(%s,Array), %s[%s], %s)" % (
                        self.visit(target), r, r, i, fallback))
    ...
    def visit_Call(self, node):
        ...
        if not self._with_dart and isinstance(node.func, ast.Attribute) and isinstance(node.func.value, Name) and node.func.value.id in self._typedef_vars and self._typedef_vars[node.func.value.id] == 'list':
            if node.func.attr == 'append':
                # return '%s.append( [%s], __NULL_OBJECT__)'
                # %(node.func.value.id, self.visit(node.args[0]) )
        ...
        if isinstance(node.func, ast.Attribute) and isinstance(node.func.value, Name) and node.func.value.id == 'numpy' and node.func.attr == 'array':
            args = [self.visit(arg) for arg in node.args]
            if node.keywords:
                kwargs = ['%s=%s' % (x.arg, self.visit(x.value))
                          for x in node.keywords]
                return 'numpy.array(%s, %s)' % (','.join(args), ','.join(kwargs))
            else:
                return 'numpy.array(%s)' % ','.join(args)

        elif isinstance(node.func, ast.Attribute) and isinstance(node.func.value, Name) and node.func.value.id == 'gython' and node.func.attr == 'configure':
            for kw in node.keywords:
            ...
            if node.keywords:
                args.extend(['%s=%s' % (x.arg, self.visit(x.value))
                             for x in node.keywords])
            if node.starargs:
                args.append('*%s' % self.visit(node.starargs))

            # if isinstance(node.func, Name) and node.func.id in self._js_classes:  ## TODO moving to gython_to_go
            ...
            # inlined js
            elif name in self._builtin_functions and self._builtin_functions[name]:
                if args:
                ...
                assert len(args) == 2
                if args[1] == 'dict':
                    # this fails when testing "isinstance(a, dict)==False" when
                    # a is an instance of some class.
                    args[1] = 'Object'
                elif args[1] == 'list':
                    args[1] = 'Array'
            ...
            # special method calls
            elif isinstance(node.func, ast.Attribute) and not self._with_dart:
                anode = node.func
                self._in_assign_target = True
                ...
                if anode.attr == 'get' and len(args) > 0 and len(args) <= 2:
                    ...
                    if self._strict_mode:
                        raise SyntaxError(self.format_error(
                            'method `keys` is not allowed without arguments'))
                    ...
                    if self._strict_mode:
                        raise SyntaxError(self.format_error(
                            'method `values` is not allowed without arguments'))
                    return '__jsdict_values(%s)' % self.visit(anode.value)

                elif anode.attr == 'items' and not args:
                    if self._strict_mode:
                        raise SyntaxError(self.format_error(
                            'method `items` is not allowed without arguments'))
                    ...
                    if self._strict_mode:
                        raise SyntaxError(self.format_error(
                            'method `split` is not allowed without arguments'))
                    ...
                    if self._strict_mode:
                        raise SyntaxError(self.format_error(
                            'method `sort` is not allowed without arguments'))
                    ...
                    if node.keywords:
                        ...
                        if args:
                            if node.starargs:
                                a = (method, ctx, ','.join(args), self.visit(
                                    node.starargs), ','.join(kwargs))
                                # note: this depends on the fact that [].extend in gython returns self (this),
                                # which is different from regular python where
                            ...
                            if node.starargs:
                                a = (self.visit(node.func), ctx, self.visit(
                                    node.starargs), ','.join(kwargs))
                        ...
                        if node.starargs:
                            a = (self.visit(node.func), ctx, ','.join(
                ...
                if node.keywords:
                    kwargs = ','.join(
                        ['%s=%s' % (x.arg, self.visit(x.value)) for x in node.keywords])
                    if args:
                ...
                if node.keywords:
                    ...
                    if args:
                        if node.starargs:
                            a = (self.visit(node.func), self.visit(node.func), ','.join(
                                args), self.visit(node.starargs), ','.join(kwargs))
                            return '%s.apply( %s, [].extend([%s]).extend(%s).append({%s}) )' % a
                        else:
                            return '%s(%s, {%s})' % (self.visit(node.func), ','.join(args), ','.join(kwargs))
                    else:
                        if node.starargs:
                            a = (self.visit(node.func), self.visit(node.func),
                                 self.visit(node.starargs), ','.join(kwargs))
                            return '%s.apply(%s, [].extend(%s).append({%s}) )' % a
                        else:
                    ...
                    if node.starargs:
                        a = (self.visit(node.func), self.visit(node.func),
                             ','.join(args), self.visit(node.starargs))
                        return '%s.apply(%s, [].extend([%s]).extend(%s))' % a
                    else:
                        return '%s(%s)' % (self.visit(node.func), ','.join(args))
            ...
            # map in py3 returns an iterator not a list
            args = list(map(self.visit, node.args))
            if node.func.id == 'var':
                ...
                kwargs = ['%s=%s' %
                             (x.arg, self.visit(x.value)) for x in node.keywords]
                args.extend(kwargs)
            args = ', '.join(args)
                ...
                if gtype == 'list' and node.func.attr == 'append':
                    if isinstance(node.args[0], Name):
                        if node.args[0].id in self._instances:
                            ...
                            if len(gset) != 1:
                                raise SyntaxError(
                                    'global lists can only contain one type: instance "%s" is different' % node.args[0].id)
                        else:
                            raise SyntaxError(
                                'global lists can only contain one type: instance "%s" is unknown' % node.args[0].id)

            call_has_args_only = len(node.args) and not (
                len(node.keywords) or node.starargs or node.kwargs)
            call_has_args_kwargs_only = len(node.args) and len(
                node.keywords) and not (node.starargs or node.kwargs)
            call_has_args = len(node.args) or len(
            ...
            if call_has_args_only:  # lambda only supports simple args for now.
                ...
                if node.starargs:
                    writer.append('%s.push.apply(%s, %s)' % (
                        args_name, args_name, self.visit(node.starargs)))
            ...
            if isinstance(node.func, ast.Attribute) and node.func.attr in ('get', 'keys', 'values', 'pop', 'items', 'split', 'replace', 'sort') and not self._with_lua:
                ...
                # return '__get__(%s, "__call__")( JSArray(), JSObject() )'
                # %name  ## SLOW ##
                ...
                # TODO - remove this optimization, or provide the user with a
    ...
    def visit_FunctionDef(self, node):
        ...
        # clear typed variables: filled in below by @typedef or in visit_Assign
        self._typedef_vars = dict()
        self._gpu_return_types = set()
        gpu = False
        gpu_main = False
        ...
        for decorator in reversed(node.decorator_list):
            ...
            # js function expressions are now the default
                ...
                for kw in c.keywords:
                    ...
                    if decorator.func.id == 'typedef_chan':
                        ...
                        writer.write('@__typedef_chan__(%s=%s)' %
                                     (kw.arg, kwval))
                    else:
                ...
                if decorator.keywords:
                    for k in decorator.keywords:
                        ...
                        # used in visit_Return ##
                        self._gpu_return_types.add(key)
                ...
                self._decorator_properties[node.original_name] = dict(
                    get=n, set=None)
                node.name = n
        ...
        if threaded:
            ...
            if len(writer.functions) <= 1:
                ...
                writer.push()
                # need a better way to quit the worker after work is done,
                # check if threading._blocking_callback is waiting, else
                # terminate
                writer.write('if e.data.type=="execute":')
                writer.push()
                writer.write(		'%s.apply(self, e.data.args)' % node.name)
                writer.write(
                    'if not threading._blocking_callback: self.postMessage({"type":"terminate"})')
                writer.pull()
                writer.write(
                    'elif e.data.type=="append": __wargs__[ e.data.argindex ].push( e.data.value )')
                writer.write(
                    'elif e.data.type=="__setitem__": __wargs__[ e.data.argindex ][e.data.key] = e.data.value')
                writer.write(
                    'elif e.data.type=="return_to_blocking_callback": threading._blocking_callback( e.data.result )')
                # writer.push()
                ...
                for v in local_vars:
                    ...
                    if '=' in v:
                        ...
                        if t == 'long' and inlined_long == False:
                            inlined_long = True
                            writer.write(
                                '''inline("if (__NODEJS__==true) var long = require('long')")''')  # this is ugly

                        if n in args:
                            args_typedefs.append(v)
                        else:
                            local_typedefs.append(v)
                    elif v in args or v in local_typedef_names:
                        pass
                    else:
                        vars.append(v)
        ...
        if self._with_dart:
            ...
            if node.args.kwarg:
                raise SyntaxError(self.format_error(
                    'dart functions can not take variable keyword arguments (**kwargs)'))
            ...
            if node.args.vararg:
                if node.args.defaults:
                    raise SyntaxError(self.format_error(
                        'dart functions can not use variable arguments (*args) and have keyword arguments'))
            ...
            if node.args.vararg:
                ...
                writer.write(
                    '#WARNING - NOT IMPLEMENTED: javascript-mode functions with (*args)')
            ...
            if len(node.args.defaults) or node.args.kwarg:
                if args:
                    writer.write('def %s( %s, %s ):' %
                                 (node.name, ','.join(args), kwargs_name))
                else:
        ...
        if self._with_dart or self._with_glsl or self._with_go:
            ...
            if node.args.defaults:
                ...
                for i, arg in enumerate(node.args.args):
                    ...
                    if dindex >= 0:
                        ...
                        a = (arg.id, spaces, kwargs_name, kwargs_name, arg.id,
                             spaces, default_value, spaces2, kwargs_name, arg.id)
            ...
            for i, arg in enumerate(node.args.args):
                ...
                if dindex >= 0 and node.args.defaults:
                    default_value = self.visit(node.args.defaults[dindex])
                    writer.write(
                        '''JS("var %s = kwargs[ '%s' ]")''' % (arg.id, arg.id))
                    writer.write('''JS("if (%s == undefined) %s = %s")''' % (
                        arg.id, arg.id, default_value))
            ...
            writer.write('var(%s)' % ','.join(
                [arg.id for arg in node.args.args]))
            offset = len(node.args.args) - len(node.args.defaults)
            for i, arg in enumerate(node.args.args):
                dindex = i - offset
                if dindex >= 0 and node.args.defaults:
                    default_value = self.visit(node.args.defaults[dindex])
                    writer.write("%s = kwargs.%s or %s" %
                                 (arg.id, arg.id, default_value))
                else:
            ...
            # new gython' python function arguments handling
            ...
            kwargsdefault = [keyword(self.visit(x[0]), x[1]) for x in zip(
                node.args.args[-L:], node.args.defaults)]
            kwargsdefault = Call(
                Name('JSObject', None),
                [],
                kwargsdefault,
            ...
            args = Call(
                Name('JSArray', None),
                [Str(x.id) for x in node.args.args],
                [],
                None,
                None
            )
            ...
            signature = ', '.join(['%s:%s' % (
                self.visit(x.arg), self.visit(x.value)) for x in keywords])
            ...
            if not self._with_go:
                writer.write(
                    """if instanceof(args,Array) and Object.prototype.toString.call(kwargs) == '[object Object]' and arguments.length==2:""")
                writer.push()
                # do nothing if it's not called from javascript
                writer.write('pass')
                writer.pull()
                ...
                # This means you can't pass keyword argument from javascript
                # but we already knew that
                writer.write('kwargs = JSObject()')
                writer.pull()

            writer.write(
                '__args__ = __getargs__("%s", __sig__, args, kwargs)' % node.name)
            # # then for each argument assign its value
            for arg in node.args.args:
                writer.write(
                    """JS("var %s = __args__['%s']")""" % (arg.id, arg.id))
            if node.args.vararg:
                writer.write("""JS("var %s = __args__['%s']")""" % (
        ...
        if threaded and is_worker_entry:
            for i, arg in enumerate(node.args.args):
                writer.write('%s = __webworker_wrap(%s, %s)' %
                             (arg.id, arg.id, i))
            ...
            if self._use_threading and isinstance(b, ast.Assign) and isinstance(b.value, ast.Call):
                if isinstance(b.value.func, ast.Attribute) and isinstance(b.value.func.value, Name) and b.value.func.value.id == 'threading':
                    if b.value.func.attr == 'start_new_thread':
                        ...
                        writer.push()
                        # workerjs for nodejs requires at least 100ms to
                        # initalize onmessage/postMessage
                        timeouts.append(0.2)
                        continue
                    ...
                    #raise SyntaxError(b)
                    self.visit(b)
                    writer.write('def __blocking( %s ):' %
                                 self.visit(b.targets[0]))
                    writer.push()
                ...
                if isinstance(b, ast.Call) and isinstance(b.func, ast.Name) and b.func.id == 'sleep':
                    ...
                    if has_sleep > 0.0:
                        ...
                        for bb in b.body:
                            ...
                            if isinstance(bb, ast.Call) and isinstance(bb.func, ast.Name) and bb.func.id == 'sleep':
                                continue
                                # TODO - split body and generate new callback -
                                # now sleep is only valid at the end of the
                                # while loop
                        ...
                        writer.write('if %s: __run_while__ = True' %
                                     self.visit(b.test))
                        writer.write('else: __run_while__ = False')

                        writer.write(
                            'if __run_while__: setTimeout(__while, %s)' % (has_sleep))
                        writer.write(
                            'elif __continue__: setTimeout(__callback%s, 0)' % len(timeouts))
        ...
        while timeouts:
            ...
            if ms == 'BLOCKING':
                ...
                ms *= 1000
                writer.write(
                    'if __run__: setTimeout(__callback%s, %s)' % (i, ms))
                writer.write(
                    'elif __continue__: setTimeout(__callback%s, %s)' % (i + 1, ms))
            i -= 1
        ...
        if setter and 'set' in self._injector:  # inject extra code
            ...
            inject = [
                ...
                'self.property_callbacks["%s"](["%s", %s, self], JSObject())' % (
                    prop_name, prop_name, value_name)
            ]
        ...
        if not self._with_dart and not self._with_lua:  # Dart functions can not have extra attributes?
            if self._introspective_functions:
                ...
                # the compiler creates anonymous functions with name set to an
                # empty string.
                writer.write('%s.NAME = "%s"' % (node.name, node.name))

                writer.write('%s.args_signature = [%s]' % (
                    node.name, ','.join(['"%s"' % n.id for n in node.args.args])))
                defaults = ['%s:%s' % (self.visit(x[0]), self.visit(x[1])) for x in zip(
                    node.args.args[-len(node.args.defaults):], node.args.defaults)]
                writer.write('%s.kwargs_signature = {%s}' % (
                    node.name, ','.join(defaults)))
                if self._with_fastdef or fastdef:
                    writer.write('%s.fastdef = True' % node.name)

                writer.write('%s.types_signature = {%s}' % (
                    node.name, ','.join(types)))

                if return_type:
                    writer.write('%s.return_type = "%s"' %
        ...
        if self._with_js and with_js_decorators:
            for dec in with_js_decorators:
                if '.prototype.' in dec:
                    ...
                    # this fixes external javascript that is using `for (var i
                    # in anArray)`
                    ...
                    # note to overwrite one of these props
                    # Object.defineProperty needs to be called again
                    # (ob.xxx=yyy will not work)
                    writer.write(
                        'Object.defineProperty(%s.prototype, "%s", {enumerable:False, value:%s, writeable:True, configurable:True})' % a)
                    ...
                    # TODO: check ifdecorators in javascript mode are working
                    # properly
                    writer.write('%s = __get__(%s,"__call__")( [%s], {} )' % (
                        node.name, dec, node.name))
    ...
    # Python a list is allowed to grow or string while looping over it.
    # FAST_FOR only deals with a fixed size thing to loop over.
    FAST_FOR = True
    ...
    def visit_For(self, node):
        if node.orelse:
            raise SyntaxError(self.format_error(
                'the syntax for/else is deprecated'))
            ...
            for n in node.body:
                ...
                for c in calls:
                    if isinstance(c.func, ast.Name):  # these are constant for sure
                        i = self._call_ids
                        writer.write('''JS('var __call__%s = __get__(%s,"__call__")')''' % (
                            i, self.visit(c.func)))
        ...
        if self._with_rpc_name and isinstance(node.iter, ast.Attribute) and isinstance(node.iter.value, ast.Name) and node.iter.value.id == self._with_rpc_name:
            ...
            writer.push()
            writer.write(	'%s = __rpc_iter__(%s, "%s")' %
                          (target, self._with_rpc, node.iter.attr))
            writer.write(
                'if %s == "__STOP_ITERATION__": __continue__()' % target)
            writer.write(	'else:')
            writer.push()
            list(map(self.visit, node.body))
            ...
            # because this def comes after, it needs to be `hoisted` up by the
            # javascript VM
            writer.write('def __continue__():')
            writer.push()
            return None
        ...
        if isinstance(target, ast.Tuple):
            ...
            for elt in target.elts:
                if isinstance(elt, ast.Name):
                    ...
                    raise NotImplementedError(
                        'unknown iterator sub-target type: %s' % target)
        elif isinstance(target, ast.Name):
            vars.append(target.id)
        else:
            raise NotImplementedError(
                'unknown iterator target type: %s' % target)

        if self._with_ll:
            writer.write('for %s in %s:' %
                         (self.visit(target), self.visit(iter)))
            writer.push()
            list(map(self.visit, node.body))
            writer.pull()
            ...
            if isinstance(iter, ast.Call) and isinstance(iter.func, Name) and iter.func.id in ('range', 'xrange'):
                ...
                writer.write('__generator__%s = %s' %
                             (iterid, self.visit(iter)))
                writer.write('while __generator__%s.__done__ != 1:' % iterid)
                writer.push()
                writer.write('%s = __generator__%s.next()' %
                             (iter_name, iterid))
                list(map(self.visit, node.body))
                writer.pull()
                ...
                if multi_target:
                    ...
                    writer.write('for __mtarget__%s in %s:' %
                                 (iterid, self.visit(iter)))
                    writer.push()
                    for i, elt in enumerate(multi_target):
                        writer.write(
                            '%s = __mtarget__%s[%s]' % (elt, iterid, i))
            ...
            if isinstance(iter, Name) and iter.id in self._global_typed_lists:
                self._instances[target.id] = list(
                    self._global_typed_lists[iter.id])[0]
            ...
            if self.FAST_FOR and isinstance(iter, ast.Call) and isinstance(iter.func, Name) and iter.func.id in ('range', 'xrange'):
                ...
                if hasattr(node, 'lineno'):
                    src = self._source[node.lineno - 1]
                    src = src.replace('"', '\\"')
                    err = 'no iterator - line %s: %s' % (
                        node.lineno, src.strip())
                    writer.write('__iterator__%s = __get__(__get__(%s, "__iter__", "%s"), "__call__")([], __NULL_OBJECT__)' % (
                        iterid, self.visit(iter), err))

                else:
                    writer.write('__iterator__%s = __get__(__get__(%s, "__iter__"), "__call__")([], __NULL_OBJECT__)' % (
                        iterid, self.visit(iter)))

            if is_generator:
                iter_name = self.visit(target)
                if not self._with_coffee:
                    writer.write('var(%s, __generator__%s)' %
                                 (iter_name, iterid))
                writer.write('__generator__%s = %s' %
                             (iterid, self.visit(iter)))
                writer.write('while __generator__%s.__done__ != 1:' % iterid)
                ...
                # this fails with the ternary __add_op
                writer.write('while %s < %s__end__:' % (iter_name, iter_name))
                ...
                writer.write(
                    '__next__%s = __get__(__iterator__%s, "next")' % (iterid, iterid))
                writer.write(
                    'while __iterator__%s.index < __iterator__%s.length:' % (iterid, iterid))

                writer.push()

                if multi_target:
                    writer.write('__mtarget__%s = __next__%s()' %
                                 (iterid, iterid))
                    for i, elt in enumerate(multi_target):
                        if self._with_lua:
                            writer.write('%s = __mtarget__%s[...][%s]' % (
                                elt, iterid, i + 1))
                        else:
                            writer.write(
                                '%s = __mtarget__%s[%s]' % (elt, iterid, i))
    ...
    def visit_While(self, node):
        if self._cache_while_body_calls:  # TODO add option for this
            for n in node.body:
                ...
                for c in calls:
                    if isinstance(c.func, ast.Name):  # these are constant for sure
                        i = self._call_ids
                        writer.write('__call__%s = __get__(%s,"__call__")' % (
                            i, self.visit(c.func)))
                        c.func.id = '__call__%s' % i
                        c.constant = True
                        self._call_ids += 1
        ...
        if node.orelse:
            raise SyntaxError(self.format_error(
                'the syntax while/else is deprecated'))
    ...
    def visit_With(self, node):
        ...
        if isinstance(node.context_expr, Name) and node.context_expr.id == 'glsl':
            if not isinstance(node.optional_vars, ast.Name):
                raise SyntaxError(self.format_error(
                    'wrapper function name must be given: `with glsl as myfunc:`'))
            main_func = None
            ...
            if not main_func:
                raise SyntaxError(self.format_error(
                    'a function named `main` must be defined as the entry point for the shader program'))
            ...
            # compatible with nodewebkit
            writer.write(
                'if typeof(require) != "undefined": requirejs = require')
            ...
            if not self._with_js:
                raise SyntaxError(
                    '"with python:" is only used inside of a "with javascript:" block')
            self._with_js = False
            list(map(self.visit, node.body))
            self._with_js = True
            ...
            if node.context_expr.keywords:
                ...
                a = 'with %s(%s=%s):' % (
                    self.visit(node.context_expr.func), k, v)
                writer.write(a)
            else:
                writer.write('with %s:' % self.visit(node.context_expr))
...
EXTRA_WITH_TYPES = ('__switch__', '__default__',
                    '__case__', '__select__', 'gojs')


class GeneratorFunctionTransformer(PythonTogython):
    '''
    ...
    def _visit_genfunc_go(self, node):
        ...
        for b in node.body:
            if loop_node:
                ...
                # typed local
                if isinstance(b, ast.Assign) and isinstance(b.targets[0], ast.Name) and len(b.targets) == 2:
        ...
        if not tail_yield:
            writer.write(
                'if self.__iter_index == self.__iter_end: self.__done__ = 1')
    ...
    def _visit_genfunc_js(self, node):
        ...
        if not tail_yield:
            writer.write(
                'if this.__iter_index == this.__iter_end: this.__done__ = 1')
...
def main(script, **kwargs):
    ...
    if translator.has_webworkers():
        ...
        if '--debug' in sys.argv:
            try:
                open('/tmp/python-to-gython.debug.py', 'wb').write(code)
            except:
                pass
        return code
...
if __name__ == '__main__':
    # if run directly prints source transformed to python-js-subset, this is
    # just for debugging ##
    scripts = []
    if len(sys.argv) > 1:
        argv = sys.argv[1:]
