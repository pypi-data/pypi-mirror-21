        ...
        if s.startswith('class '):
            ...
            for pname in class_parents:
                if pname in self.classes:
                    ...
                    self.output.write(
                        'edges.push({from:%s, to:%s, length:200, style:"arrow"})\n' % (pid, id))
        ...
        if self.nodes:
            if self.level <= 1 and s.startswith('def') or class_name:
                ...
                if self.title_blocks:
                    self.output.write(
                        'stack[stack.length-1].label += "--block%s-->\\n"\n' % id)
                else:
                ...
                if prev.startswith('if '):
                    ...
                    length = 150
                    self.output.write(
                        'edges.push({from:%s, to:stack[stack.length-1].id, style: "%s", width: 1, length: %s})\n' % (id, style, length))
                elif prev.startswith('class '):
                    style = 'line'
                    width = 3
                    length = 200
                self.output.write(
                    'edges.push({to:%s, from:stack[stack.length-1].id, style: "%s", width: %s, length: %s})\n' % (id, style, width, length))
...
class PythonToVisJS(ast.NodeVisitor):
    def __init__(self, source=None):
        ...
        writer.push_block('#module#', shape='circle',
    ...
    def visit_If(self, node):
        ...
        writer.push_block('if \\n%s' % self.visit(node.test), color=(
            'lightgreen', 'green'), size=5, font_size=16)
        writer.push()
        ...
        writer.pull()
        if node.orelse:
            writer.push_block('else:', color=(
                'pink', 'red'), size=5, font_size=16)
            writer.push()
    ...
    def visit_For(self, node):
        a = 'for \\n%s in %s' % (self.visit(
            node.target), self.visit(node.iter))
    ...
    def visit_ClassDef(self, node):
        ...
        writer.push_block(a, color=('orange', 'black'),
                          font_size=16, shape='database')
        for n in node.body:
