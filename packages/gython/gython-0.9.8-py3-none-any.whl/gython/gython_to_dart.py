...
class DartGenerator(gython.JSGenerator):
    ...
    def visit_ClassDef(self, node):
        ...
        if bases:
            if extends:
                assert len(bases) == 1
                out.append('class %s extends %s {' % (
                    node.name, ','.join(bases)))
            else:
                # if bases[0] == 'object':
                #	out.append('class %s {' %node.name)
                # else:
                out.append('class %s implements %s {' % (
                    node.name, ', '.join(bases)))
        ...
        for b in node.body:
            ...
            if isinstance(b, ast.With):
                ...
                args, kwargs = self.get_args_kwargs_from_funcdef(
                    b, skip_self=True)
                kwargs_init = ['%s:%s' %
                               (x.split(':')[0], x.split(':')[0]) for x in kwargs]
                ...
                if args:
                    ...
                    if kwargs:
                        out.append(
                            self.indent() + '%s(%s, {%s}) {%s.__init__(this,%s,%s);}' % (
                                node.name, args, ','.join(kwargs), node.name, args, ','.join(kwargs_init))
                        )

                    else:
                        out.append(
                            self.indent() +
                            '%s(%s) {%s.__init__(this,%s);}' % (
                                node.name, args, node.name, args)
                        )
                elif kwargs:
                    out.append(
                        self.indent() + '%s( {%s} ) {%s.__init__(this,%s);}' % (
                            node.name, ','.join(kwargs), node.name, ','.join(kwargs_init))
                    )

                else:
                    out.append(
                        self.indent() +
                        '%s() {%s.__init__(this);}' % (node.name, node.name)
                    )
                ...
                if operator and args:
                    ...
                    ) + '%s(%s) { return %s.__%s(this,%s); }' % (operator, args, node.name, b.name, args))

                elif operator:
                    out.append(
                        self.indent() + '%s() { return %s.__%s(this); }' % (operator, node.name, b.name))

                elif args:
                    out.append(self.indent(
                    ) + '%s(%s) { return %s.__%s(this,%s); }' % (b.name, args, node.name, b.name, args))
                else:
                    out.append(
                        self.indent() + '%s() { return %s.__%s(this); }' % (b.name, node.name, b.name))
            ...
            for bnode in base_classes:
                for b in bnode.body:
                    if isinstance(b, ast.FunctionDef):
                        ...
                        if args:
                            out.append(self.indent(
                            ) + '%s(%s) { return %s.__%s(this,%s); }' % (b.name, args, bnode.name, b.name, args))
                        else:
                            out.append(
                                self.indent() + '%s() { return %s.__%s(this); }' % (b.name, bnode.name, b.name))
    ...
    def _visit_call_helper(self, node):
        ...
        if node.keywords:
            kwargs = ','.join(['%s:%s' % (x.arg, self.visit(x.value))
    ...
    def _visit_call_helper_numpy_array(self, node):
        ...
        if isinstance(node.args[0], ast.Name):
            ...
            # simple rule: if there are 4 items, its a direct SIMD type
            if len(node.args[0].elts) == 4:
                direct = True
