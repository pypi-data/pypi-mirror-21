...
class SafeIDownloadablePrimaryView(idownloadable.IDownloadablePrimaryView):
    """Safe version of IDownloadablePrimaryView that does not crash when no
    path between source and target MIME types can be found.
    """

    def render_data(self, entity, sourcemt, targetmt):
        try:
            return super(SafeIDownloadablePrimaryView, self).render_data(
                entity, sourcemt, targetmt)
...
def registration_callback(vreg):
    vreg.register_all(globals().values(), __name__,
                      [SafeIDownloadablePrimaryView])
    vreg.register_and_replace(SafeIDownloadablePrimaryView,
