...
logger = logging.getLogger(u"kudubot.services.anime_reminder.database")
u"""
...
def subscription_exists(user, show_name, db):
    ...
    """
    subscriptions = ifilter(lambda x: x[u"show_name"].lower() == show_name.lower(), get_subscriptions(db, user))
...
def store_subscription(user, show_name, db):
    ...
    if not exists:
        logger.debug(u"Creating new subscription for user " + unicode(user) + u" and show " + show_name)
        db.execute(u"INSERT INTO anime_reminder_subscriptions (user_id, show_name) VALUES (?, ?)", (user, show_name))
        db.commit()
    else:
        logger.debug(u"Subscription for user " + unicode(user) + u" and show " + show_name + u" already exists")
...
def delete_subscription(user, show_name, db):
    ...
    if exists:
        logger.debug(u"Deleting subscription for user " + unicode(user) + u" and show " + show_name)
        db.execute(u"DELETE FROM anime_reminder_subscriptions WHERE user_id=? AND show_name LIKE ?",
                   (user, show_name))
        db.commit()
    else:
        logger.debug(u"Subscription for user " + unicode(user) + u" and show " + show_name + u" did not exist. "
                                                                                       u"Not deleting anything.")
...
def get_subscriptions(db, user=-1):
    ...
    if user == -1:
        subs = db.execute(u"SELECT id, user_id, show_name FROM anime_reminder_subscriptions").fetchall()

    else:
        subs = db.execute(u"SELECT id, user_id, show_name FROM anime_reminder_subscriptions WHERE user_id=?", (user,))\
            .fetchall()

    formatted_subs = []
    for sub in subs:
        formatted_subs.append({u"id": sub[0], u"user_id": sub[1], u"show_name": sub[2]})
...
def thread_exists(thread, db):
    ...
    """
    result = db.execute(u"SELECT * FROM anime_reminder_threads WHERE show_name=? AND episode=? AND thread=?",
...
def store_thread(thread, db):
    ...
    db.execute(u"INSERT INTO anime_reminder_threads (show_name, episode, thread) VALUES (?, ?, ?)",
               (thread[u"show_name"], thread[u"episode"], thread[u"url"]))
    db.commit()
