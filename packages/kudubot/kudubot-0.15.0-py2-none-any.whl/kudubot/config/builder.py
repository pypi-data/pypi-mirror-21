...
def build(service_name, service_dir, source_file, destination_file):
    u"""
    ...
    if source_file.endswith(u".rs"):  # Rust
        os.chdir(service_dir)
        run_safe_popen([u"cargo", u"build", u"--release"])
        safe_move(os.path.join(u"target", u"release", service_name), os.path.join(u"build", service_name))
...
def build_external(move_to = u""):
    ...
    for service in os.listdir(external_dir):
        ...
        src = os.path.join(service_dir, u"src")
        build_dir = os.path.join(service_dir, u"build")
        ...
        for source_file in os.listdir(src):
            ...
            if os.path.isfile(source_path) and source_file.lower().startswith(u"main"):
