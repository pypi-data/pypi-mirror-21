...
class GlobalConfigHandler(object):
    ...
    def __init__(self, config_location = os.path.join(os.path.expanduser(u"~"), u".kudubot")):
        u"""
        ...
        self.global_connection_config_location = os.path.join(self.config_location, u"connections.conf")
        self.services_config_location = os.path.join(self.config_location, u"services.conf")
        self.data_location = os.path.join(self.config_location, u"data")
        self.specific_connection_config_location = os.path.join(self.config_location, u"connection_config")
        self.external_services_directory = os.path.join(self.config_location, u"external")
        self.external_services_executables_directory = os.path.join(self.external_services_directory, u"bin")
    ...
    def validate_config_directory(self):
        ...
        try:
            if not os.path.isdir(self.config_location):
                ...
                raise InvalidConfigException(u"External Service directory does not exist")
            elif not os.path.isdir(self.external_services_executables_directory):
                raise InvalidConfigException(u"External Service executable directory does not exist")
            else:
    ...
    def generate_configuration(self, delete_old=False):
        ...
        if not os.path.isfile(self.global_connection_config_location):
            self.logger.info(u"Creating file " + self.global_connection_config_location)
        ...
        if not os.path.isdir(self.specific_connection_config_location):
            self.logger.info(u"Creating directory " + self.specific_connection_config_location)
            os.makedirs(self.specific_connection_config_location)

        if not os.path.isdir(self.external_services_directory):
            self.logger.info(u"Creating directory " + self.external_services_directory)
            os.makedirs(self.external_services_directory)

        if not os.path.isdir(self.external_services_executables_directory):
            self.logger.info(u"Creating directory " + self.external_services_executables_directory)
