...
class ReminderService(HelperService):
    ...
    logger = logging.getLogger(u"kudubot.services.reminder.ReminderService")
    u"""
    ...
    def init(self):
        ...
        """
        self.logger.info(u"Initializing Reminder Database Table")
        initialize_database(self.connection.db)

        self.logger.info(u"Starting Reminder background thread")
    ...
    def define_language_text(self):
        ...
        return {
            ...
            u"@stored_reply_title": {u"en": u"Message Stored", u"de": u"Nachricht gespeichert"},
            u"@stored_confirmation_message": {u"en": u"The reminder message has successfully been stored.",
                                             u"de": u"Die Erinnerungsnachricht wurde erfolgreich gespeichert"},
            u"@list_stored_message_start": {u"en": u"The following reminders are pending",
                                           u"de": u"Die folgenden Erinnerungen stehen noch aus"},
            u"@list_response_title": {u"en": u"List of Reminders", u"de": u"Liste der Erinnerungen"},
            u"@invalid_title": {u"en": u"Invalid Reminder", u"de": u"Ungültige Erinnerung"},
            u"@invalid_message": {u"en": u"The reminder is invalid. Please consult /remind syntax for more information.",
                                 u"de": u"Die Erinnerung ist fehlerhaft. Sehe dir /erinner syntax an, und versuch es "
                                       u"dann noch einmal."}
        }
    ...
    def handle_message(self, message):
        ...
        :return: None
        """
        super(ReminderService, self).handle_message(message)
        ...
        if command[u"mode"] == u"store":
            store_reminder(self.connection.db,
                           ...
                           target.database_id)
            self.reply_translated(u"@stored_reply_title", u"@stored_confirmation_message", message)

        elif command[u"mode"] == u"list":
            reminders = list(ifilter(lambda x: x[u"receiver"].database_id == target.database_id,
                                    get_unsent_reminders(self.connection.db)))
            text = u"@list_stored_message_start:\n\n"
            for reminder in reminders:
                text += convert_datetime_to_string(reminder[u"due_time"]) + u":" + reminder[u"message"] + u"\n"
            self.reply_translated(u"@list_response_title", text, message)

        elif command[u"mode"] == u"invalid":
            self.reply_translated(u"@invalid_title", u"@invalid_message", message)
    ...
    def is_applicable_to(self, message):
        ...
        """
        if super(ReminderService, self).is_applicable_to(message):
            return True
        ...
        regex = u"^@remind_command ([0-9]+ (" \
                u"@second_singular|@second_plural|@minute_singular|@minute_plural|@hour_singular|@hour_plural|" \
                u"@day_singular|@day_plural|@week_singular|@week_plural|@year_singular|@year_plural) )+" \
                u"\"[^\"]+\"$"
        lang_regex = re.compile(self.translate(regex, language))

        if re.match(lang_regex, body) or body == self.translate(u"@remind_command @list_argument", language):
            self.logger.info(u"Message is applicable")
            return True

        else:
            self.logger.info(u"Message is not applicable")
            return False
    ...
    def background_loop(self):
        ...
        while True:
            ...
            for reminder in unsent:
                if reminder[u"due_time"].timestamp() < datetime.datetime.utcnow().timestamp():
                    self.connection.send_message(Message(u"Reminder", reminder[u"message"],
                                                         reminder[u"receiver"], self.connection.user_contact))
    ...
    def define_syntax_description(self, language):
        ...
        """
        return {
            u"en": u"/remind X second(s) \"Message\"\n"
            ...
            u"de": u"/erinner X sekunde(n) \"Message\"\n"
    ...
    def define_help_message(self, language):
        ...
        """
        return {
            u"en": u"The reminder service allows you to store a reminder on the server to be "
                  u"sent back to you at a specified time. See /remind syntax for possible ways "
                  u"to use the reminder service.",
            u"de": u"Der Erinnerungsdienst erlaubt es einem Nutzer eine Erinnerung auf dem Server "
                  u"zu speicher, welcher dann zu einem spezifizierten Zeitpunkt zurückgesendet wird. "
                  u"Sehe dir /erinner syntax an, um dir ein Bild davon zu machen wie man den Erinnerungsdienst "
                  u"verwendet."
        }[language]
    ...
    def parse_message(self, text, language):
        ...
        if self.translate(u"@remind_command @list_argument", language) == text:
            return {u"mode": u"list"}
        else:

            time_string = text.split(u" \"")[0].split(self.translate(u"@remind_command ", language))[1]
            ...
            if usertime is None:
                ...
                return {u"mode": u"store", u"data": {u"message": reminder_message, u"due_time": usertime}}
    ...
    def parse_time_string(self, time_string, language):
        ...
        for unit in [u"second", u"minute", u"hour", u"day", u"week", u"year"]:
            for mode in [u"plural", u"singular"]:  # The order is very important here!
        ...
        try:
            for i in xrange(0, len(parsed), 2):
                ...
                if key == u"year":
                    ...
                    self.logger.debug(u"Invalid time keyword used")
                    return None
