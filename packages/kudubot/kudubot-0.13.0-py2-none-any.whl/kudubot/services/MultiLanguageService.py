...
class MultiLanguageService(Service):
    ...
    def __init__(self, connection):
        ...
        """
        super(MultiLanguageService, self).__init__(connection)
        self.connection.db.execute(u"CREATE TABLE IF NOT EXISTS language_preferences ("
                                   u"user_id INTEGER CONSTRAINT constraint_name PRIMARY KEY, "
                                   u"lang_pref VARCHAR(255) NOT NULL,"
                                   u"user_initiated INT NOT NULL"
                                   u")")
        self.connection.db.commit()

    def store_language_preference(self, user, language, user_initiated = False):
        u"""
        ...
        :return: None
        """
        fetch = self.connection.db.execute(u"SELECT user_initiated FROM language_preferences WHERE user_id=?",
                                           (user,)).fetchall()
        ...
        if was_user_initiated and not user_initiated:
            ...
            self.connection.db.execute(u"INSERT OR REPLACE INTO language_preferences "
                                       u"(user_id, lang_pref, user_initiated) VALUES (?,?,?)",
                                       (user, language, user_initiated))
            self.connection.db.commit()

    def get_language_preference(self, user, default = u"en", db=None):
        u"""
        ...
        result = db.execute(u"SELECT lang_pref FROM language_preferences WHERE user_id=?", (user,)).fetchall()
    ...
    def define_fallback_language(self):
        u"""
    ...
    def translate(self, text, language, translation_dict=None):
        u"""
    ...
    def reply_translated(self, title, body, message):
        u"""
    ...
    def is_applicable_to(self, message):
        ...
        return body.startswith(u"/language ") or body.startswith(u"/sprache ") or body in [u"/language", u"/sprache"]
    ...
    def handle_message(self, message):
        ...
        dictionary = {u"@title": {u"en": u"Language Change",
                                 u"de": u"Sprachwechsel"},
                      u"@success_message": {u"en": u"Successfully changed language to",
                                           u"de": u"Sprache erfolgreich geändert zu"},
                      u"@fail_message": {u"en": u"Failed to switch to language",
                                        u"de": u"Konnte nicht Sprache wechseln zu"}}

        command_keywords = [u"/language", u"/sprache"]
        aliases = {u"en": [u"en", u"english", u"englisch"],
                   u"de": [u"de", u"deutsch", u"german"]}

        params = message.message_body.lower().split(u" ")
        user_id = message.get_direct_response_contact().database_id

        if len(params) == 1 and params[0] in command_keywords:
            language = self.get_language_preference(user_id, u"en")
            self.reply(self.translate(u"@title", language, dictionary),
                       language, message)
        ...
        if len(params) == 2 and params[0] in command_keywords:
            ...
            language = self.get_language_preference(user_id, u"en")  # the new language will already be stored
            title = self.translate(u"@title", language, dictionary)

            if found_language:
                self.reply(title, self.translate(u"@success_message: " + language, language, dictionary), message)
            else:
                self.reply(title, self.translate(u"@fail_message: " + params[1], language, dictionary), message)
