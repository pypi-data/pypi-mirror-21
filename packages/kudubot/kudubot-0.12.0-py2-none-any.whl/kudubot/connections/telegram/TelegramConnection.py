...
class TelegramConnection(Connection):
    ...
    logger = logging.getLogger(u"kudubot_telegram.TelegramConnection")
    u"""
    ...
    def __init__(self, services):
        ...
        """
        super(TelegramConnection, self).__init__(services)
    ...
    def load_config(self):
        ...
        try:
            ...
            if parsed_config[u"api_key"] == u"":
                self.logger.warning(u"Config Parsing Failed. Empty API Key supplied.")
                raise InvalidConfigException(u"Invalid Telegram Configuration File - Empty API key detected")
            ...
            self.logger.warning(u"Config Parsing Failed. No credentials section in config file.")
            raise InvalidConfigException(u"Invalid Telegram Configuration File - No credentials section")
        except KeyError, e:
            self.logger.warning(u"Config Parsing Failed. No attribute " + unicode(e) + u" found.")
            raise InvalidConfigException(u"Invalid Telegram Configuration File - No '" + unicode(e) + u"' attribute found")
    ...
    def generate_configuration(self):
        ...
        with open(self.config_file_location, u'w') as config:
            ...
            self.logger.info(u"Wrote new Configuration file at " + self.config_file_location)
    ...
    def send_message(self, message):
        ...
        :return: None
        """
        self.logger.info(u"Sending text Message to " + message.receiver.display_name)
        ...
        while True:
            ...
            try:
                ...
                for update in self.bot.get_updates(offset=update_id, timeout=10):
                    ...
                    name = _from[u"username"] if _from[u"username"] != u"" else _from[u"first_name"] + _from[u"last_name"]

                    if chat[u'type'] == u"group":
                        sender = self.address_book.add_or_update_contact(Contact(-1, name, _from[u'id']))
                        group = self.address_book.add_or_update_contact(Contact(-1, chat[u'title'], unicode(chat[u'id'])))
                    else:
                        sender = self.address_book.add_or_update_contact(Contact(-1, name, unicode(chat[u'id'])))
                        group = None

                    message = Message(u"", telegram_message[u'text'], self.user_contact, sender, group,
                                      telegram_message[u'date'])
                    self.logger.info(u"received message '" + message.message_body +
