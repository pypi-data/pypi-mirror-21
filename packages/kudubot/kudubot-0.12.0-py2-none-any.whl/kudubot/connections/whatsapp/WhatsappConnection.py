...
class WhatsappConnection(Connection):
    ...
    logger = logging.getLogger(u"kudubot_whatsapp.WhatsappConnection")
    u"""
    ...
    def __init__(self, services):
        ...
        """
        super(WhatsappConnection, self).__init__(services)
    ...
    def load_config(self):
        ...
        try:
            ...
            if parsed_config[u"number"] == u"":
                self.logger.warning(u"Config Parsing Failed. No number supplied.")
                raise InvalidConfigException(u"Invalid Whatsapp Configuration File - Missing number detected")
            elif parsed_config[u"pass"] == u"":
                self.logger.warning(u"Config Parsing Failed. No password supplied.")
                raise InvalidConfigException(u"Invalid Whatsapp Configuration File - Missing password detected")
            ...
            raise InvalidConfigException(u"Invalid Whatsapp Configuration File - No credentials section")
        except KeyError, e:
            self.logger.warning(u"Config Parsing Failed. No attribute " + unicode(e) + u" found.")
            raise InvalidConfigException(u"Invalid Whatsapp Configuration File - No '" + unicode(e) + u"' attribute found")
