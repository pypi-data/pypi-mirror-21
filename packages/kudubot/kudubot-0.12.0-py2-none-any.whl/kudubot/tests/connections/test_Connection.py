...
class UnitTests(unittest.TestCase):
    ...
    def test_processing_message(self):
        ...
        :return: None
        """
        user = Contact(1, u"1", u"1")

        connection = DummyConnection([DummyService])
        connection.apply_services(Message(u"Test", u"Body", user, user))

        old = DummyService.is_applicable_to
        DummyService.is_applicable_to = lambda x, y: True

        connection = DummyConnection([DummyService])
        connection.apply_services(Message(u"Test", u"Body", user, user), True)
        connection.apply_services(Message(u"Test", u"Body", user, user), False)
