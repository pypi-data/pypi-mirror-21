...
class AnimeReminderService(HelperService):
    ...
    logger = logging.getLogger(u"kudubot_anime_reminder.AnimeReminderService")
    u"""
    ...
    def __init__(self, connection):
        ...
        """
        super(AnimeReminderService, self).__init__(connection)
        self.connection = connection

        initialize_database(self.connection.db)

        self.logger.info(u"Starting Anime Reminder Background Thread")
    ...
    def handle_message(self, message):
        ...
        :return: None
        """
        super(AnimeReminderService, self).handle_message(message)
        ...
        if mode in [self.translate(u"@subscribe_command", x) for x in self.supported_languages()]:

            result = u"@successful_add_message" if store_subscription(user_id, show, self.connection.db) \
                else u"@exists_message"
            self.reply_translated(u"@reply_title", u"@subscribe_message_start " + show + u" " + result + u".", message)

        elif mode in [self.translate(u"@unsubscribe_command", x) for x in self.supported_languages()]:

            result = u"@successful_remove_message" if delete_subscription(user_id, show, self.connection.db) \
                else u"@does_not_exist_message"
            self.reply_translated(u"@reply_title", u"@unsubscribe_message_start " + show + u" " + result + u".", message)

        elif mode in [self.translate(u"@list_command", x) for x in self.supported_languages()]:
    ...
    def is_applicable_to(self, message):
        ...
        if super(AnimeReminderService, self).is_applicable_to(message):
            return True

        applicable = False
        for language in self.supported_languages():

            regex = u"^@command_name (@list_command|@sub_unsub_command \"[^\"]+\")$"
            regex = re.compile(self.translate(regex, language))
            applicable = bool(re.search(regex, message.message_body)) or applicable

        self.logger.debug(u"Message " + message.message_body + u" is" + (u"" if applicable else u"not") +
                          u" applicable to " + self.define_identifier())
    ...
    def define_syntax_description(self, language):
        ...
        skeleton += u"@command_name @subscribe_command \"@show_name_parameter\"\n"
        skeleton += u"@command_name @unsubscribe_command \"@show_name_parameter\""
    ...
    def define_language_text(self):
        ...
        return {
            ...
            u'@successful_remove_message': {u"en": u"successfully removed",
                                           u"de": u"wurde erfolgreich abbestellt"},
            ...
            u"@remind_title": {u"en": u"Anime Reminder",
                              u"de": u"Anime Erinnerung"},
            u"@subscription_list_message": {u"en": u"You are subscribed to the following shows",
                                           u"de": u"Du hast folgende Serien abonniert"},
            u"@episode": {u"en": u"episode",
                         u"de": u"Episode"},
            u"@has_been_released": {u"en": u"has been released! Discuss it on reddit",
                                   u"de": u"wurde veröffentlicht! Diskuttier sie auf reddit"},
            u"@help_message": {
                u"en": u"The Anime Reminder Service periodically checks for new anime discussion threads on reddit's "
                      u"r/anime board. A user can subscribe to specific shows and will then be notified whenever a "
                      u"new thread appears, which usually coincides with the point in time that the episode is "
                      u"available on streaming services like Crunchyroll, Amazon or Funimation.\n"
                      u"Users can also unsubscribe from shows and list all their subscriptions. Subscription names "
                      u"are case-insensitive.",
                u"de": u"Der Anime Erinnerungs-Service schaut in periodischen Abständen nach, ob neue Anime Diskussionen "
                      u"auf reddit.com/r/anime entstanden sind, welche neue Anime Episode diskuttieren. Ein Nutzer kann "
                      u"einzelne Serien abonnieren und wieder abbestellen. Für abonnierte Serien erhält der Nutzer "
                      u"eine Nachricht sobal eine neue Episode verfügbar ist."
                      u"Die Namen für die Abonnements achten nicht auf Groß- und Kleinschreibung. Man kann auch alle"
    ...
    def background_loop(self):
        ...
        while True:
            ...
            self.logger.info(u"Checking for due subscriptions")
            ...
            for thread in new_threads:
                ...
                if not thread_exists(thread, db):
                    ...
                    for subscription in get_subscriptions(db):
                        if subscription[u"show_name"].lower() == thread[u"show_name"].lower():

                            receiver = self.connection.address_book.get_contact_for_id(subscription[u"user_id"], db)

                            message_text = thread[u"show_name"] + u" @episode " + unicode(thread[u"episode"])
                            message_text += u" @has_been_released: " + thread[u"url"]
