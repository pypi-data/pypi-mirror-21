...
class ReminderService(Service):
    ...
    logger = logging.getLogger(u"kudubot_reminder.ReminderService")
    u"""
    ...
    def __init__(self, connection):
        ...
        """
        super(ReminderService, self).__init__(connection)
    ...
    def handle_message(self, message):
        ...
        help_message = u"/remind help\n/remind <time> \"message\"\n\n<time> can be either:\n\n" \
                       u"x seconds/minutes/hours\nx days/weeks/years\nYYYY-MM-DD\nYYYY-MM-DD:hh-mm-ss\nhh-mm-ss>"
        ...
        if command[u"status"] == u"help":
            self.connection.send_message(Message(u"Help", help_message, target, self.connection.user_contact))

        elif command[u"status"] == u"store":
            store_reminder(self.connection.db, command[u"data"][u"message"],
                           command[u"data"][u"due_time"], target.database_id)
            self.connection.send_message(Message(u"Stored", u"Reminder Stored", target, self.connection.user_contact))
    ...
    def is_applicable_to(self, message):
        ...
        applicable = parse_message(message.message_body)[u"status"] != u"no-match"
        if applicable:
            self.logger.info(u"Message is applicable")
        else:
            self.logger.debug(u"Message is not applicable")
        return applicable
    ...
    def background_loop(self):
        ...
        while True:
            self.logger.debug(u"Checking reminder status")
