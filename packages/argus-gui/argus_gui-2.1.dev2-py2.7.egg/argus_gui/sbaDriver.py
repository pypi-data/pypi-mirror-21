...
class sbaArgusDriver():
    ...
    def rearrange(self): 
        ...
        if best != self.ncams - 1:
            if self.ppts is not None:
                _ = copy.copy(self.ppts[:,2*(self.ncams-1):2*self.ncams])
                self.ppts[:,2*(self.ncams-1):2*self.ncams] = self.ppts[:,best*2:best*2 + 2]
                self.ppts[:,best*2:best*2 + 2] = _
