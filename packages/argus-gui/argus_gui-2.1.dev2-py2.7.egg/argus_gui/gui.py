...
class pygletDriver():
    ...
    def run(self):
        ...
        args = [movieString, str(self.end), offsetString]
        cmd = cmd + args
...
class clickerGUI(GUI):
    def __init__(self):
        ...
        Label(self.root, text = "Argus-Clicker", font = ("Helvetica", 30), fg = '#56A0D3').grid(row = 0, column = 0, padx = 15, pady = 15, sticky = S)
        ...
        for movie in movies:
            ...
            for m in _:
                if movie == m:
                    ...
                    return
        if len(movies) != 0:
            driver = pygletDriver(movies, self.offsets)
            driver.run()
...
class dwarpGUI(GUI):
    def __init__(self):
        ...
        specButton = Button(self.root, text = 'Specify', command =lambda: self.set_out_filename(self.ofnam), padx = 10, pady = 10)
    ...
    def getCoefficients(self):
        try:
            if not '(CMei)' in self.sModeStr.get():
                ...
                co = [self.f.get(), str(self.width), str(self.height), self.cx.get(), self.cy.get(), self.k1.get(), self.k2.get(), self.t1.get(), self.t2.get(), self.xi.get()]
                ret = ','.join(co)
                return ret
        except:
            six.moves.tkinter_messagebox.showwarning(
...
class calibrateGUI(GUI):
    def __init__(self):
        ...
        Label(self.root, text = "Argus-Calibrate", font = ("Helvetica",30), fg = '#56A0D3').grid(row = 0, column = 0, padx = 20, pady = 20)
        ...
        self.w = OptionMenu(group, self.option, "Optimize k1, k2", "Optimize k1, k2, and k3", "Optimize all distortion coefficients")
