...
class Syncer:
    ...
    def sync(self):
        ...
        for k in range(0, len(files)):
            ...
            if self.crop:
                signals.append(signal[int(np.round(self.getSec(self.start)*48000)):int(np.round(self.getSec(self.end)*48000))])
            else:
        ...
        if self.oname != '':
            ...
            for k in range(1, len(files)):
                fo.write(self.files[k].split('/')[-1] + ',' + str(offsets[k-1][0]) + ',' + str(offsets[k-1][1]) + ',' + str(offsets[k-1][3]) + '\n')
            fo.close()
