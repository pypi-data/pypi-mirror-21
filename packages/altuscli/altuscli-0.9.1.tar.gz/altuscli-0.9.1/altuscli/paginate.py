...
from altuscli.exceptions import PaginationError
from altuscli.model import OperationModel
...
class PageIterator(object):
    ...
    def __iter__(self):
        ...
        while True:
            ...
            # the size of the last page we fetch must be such that we get exactly
            # max_items number of items
            if self._max_items:
                self._op_kwargs[self._page_size_key] = min(
                    getattr(self._op_kwargs, self._page_size_key, self._max_items),
                    self._page_size_maximum,
            ...
            # grab continuation token from response presence of which means more pages
            ...
            if self._max_items and items_returned > self._max_items:
                msg = "Received more items than expected. Expected: %d; Returned: %d"
                raise PaginationError(message=msg % (self._max_items, items_returned))

            # exit if we reached the last page
            if next_token is None:
                break
            # exit if we returned the request max number of items
            if self._max_items and self._max_items == items_returned:
                break

            # advance to the next page
            self._op_kwargs[self._input_token_key] = previous_next_token = next_token

    # builds a full operation result object by iterating over pages which are fetched
    # using multiple requests to service
    def build_full_result(self):
        ...
        # expose continuation token in the response, this should only happen if not
        # all items were fetched due to logic above, such as when --max-items is used
        next_token = response.get(self._output_token_key, None)
        if next_token:
            result[self._output_token_key] = next_token
...
class Paginator(object):
    def __init__(self,
                 ...
                 operation_model,
                 max_items):
        # preconditions, ask Mr. Wolff why we are not using Python's native asserts
        ...
        assert_check(max_items is None or isinstance(max_items, int) and max_items > 0,
                     "max_items is valid")
    ...
    def paginate(self, **kwargs):
        return PageIterator(self._method, self._operation_model, self._max_items, kwargs)
