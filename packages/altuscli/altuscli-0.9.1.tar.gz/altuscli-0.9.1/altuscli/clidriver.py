...
class CLIDriver(object):
    ...
    def _create_help_command(self):
        cli_data = self._get_cli_data()
        return ProviderHelpCommand(self._get_command_table(),
    ...
    def _handle_top_level_args(self, args):
        ...
        if args.max_items:
            if not args.paginate:
                raise NotImplementedError("Cannot satisfy both --max-items and "
                                          "--no-paginate arguments.")

            def max_items_value_error():
                raise ValueError("The value for --max-items argument must be a "
                                 "decimal positive integer, was: %s." % args.max_items)
            try:
                args.max_items = int(args.max_items)
            except(ValueError):
                max_items_value_error()
            if args.max_items <= 0:
                max_items_value_error()
...
class ServiceOperation(object):
    ...
    @property
    def is_hidden(self):
        return self._operation_model.is_hidden
    ...
    def invoke(self,
        ...
        if client.can_paginate(py_operation_name) and parsed_globals.paginate:
            response = client.get_paginator(
                py_operation_name,
                parsed_globals.max_items).paginate(**parameters)
        else:
