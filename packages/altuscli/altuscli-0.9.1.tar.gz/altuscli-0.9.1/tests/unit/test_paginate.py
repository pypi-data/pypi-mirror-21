...
from altuscli.model import ObjectShape
...
class TestPagination(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.paginator = Paginator(self.method, self.operation_model, max_items=None)
    ...
    def test_next_token_in_response(self):
        ...
        self.assertEqual(self.method.call_args_list, [call(),
    ...
    def test_build_full_result_max_items(self):
        responses = [{'Foo': ['User1'], 'NextToken': 'm1'},
                     ...
                     {'Foo': ['User3']}]
        # max_items = 1
        self.paginator = Paginator(self.method, self.operation_model, max_items=1)
        self.method.side_effect = responses
        ...
        # max_items = 2
        self.paginator = Paginator(self.method, self.operation_model, max_items=2)
        self.method.side_effect = responses
        self.method.reset_mock()
        result = self.paginator.paginate().build_full_result()
        self.assertEqual(result, {'Foo': ['User1', 'User2'], 'NextToken': 'm2'})
        self.method.assert_any_call(pageSize=2)
        self.method.assert_called_with(pageSize=1, NextToken='m1')
        # max_items = 3
        self.paginator = Paginator(self.method, self.operation_model, max_items=3)
        self.method.side_effect = responses
        self.method.reset_mock()
        result = self.paginator.paginate().build_full_result()
        self.assertEqual(result, {'Foo': ['User1', 'User2', 'User3']})
        ...
        # max_items = 4
        self.paginator = Paginator(self.method, self.operation_model, max_items=4)
        self.method.side_effect = responses
        self.method.reset_mock()
        result = self.paginator.paginate().build_full_result()
        self.assertEqual(result, {'Foo': ['User1', 'User2', 'User3']})
        ...
        # max_items = 101
        self.paginator = Paginator(self.method, self.operation_model, max_items=101)
        self.method.side_effect = responses
        self.method.reset_mock()
        result = self.paginator.paginate().build_full_result()
        self.assertEqual(result, {'Foo': ['User1', 'User2', 'User3']})
        ...
        # max_items = 1000
        self.paginator = Paginator(self.method, self.operation_model, max_items=1000)
        self.method.side_effect = responses
        self.method.reset_mock()
        result = self.paginator.paginate().build_full_result()
        self.assertEqual(result, {'Foo': ['User1', 'User2', 'User3']})
    ...
    def test_build_full_result_max_items_overshoot(self):
        ...
        responses = [{'Foo': ['User1', 'User2'], 'NextToken': 'm1'},
                     {'Foo': ['User3', 'User4'], 'NextToken': 'm2'},
                     {'Foo': ['User5', 'User6']}]
        self.paginator = Paginator(self.method, self.operation_model, max_items=5)
        self.method.side_effect = responses
        self.method.reset_mock()
        with self.assertRaises(PaginationError):
            self.paginator.paginate().build_full_result()
    ...
    def test_paginator_preconditions(self):
        ...
        feed_bad_values(
            ...
            lambda value: Paginator(value, self.operation_model, max_items=1))
        feed_bad_values(
            [None, 1, 'abc', {'abc': 1}, (1, 2), ['a', 'b'], feed_bad_values, eval],
            lambda value: Paginator(self.method, value, max_items=1))
        feed_bad_values(
            [0, -1, 'abc', {'abc': 1}, (1, 2), ['a', 'b'], feed_bad_values, eval],
            lambda value: Paginator(self.method, self.operation_model, max_items=value))
