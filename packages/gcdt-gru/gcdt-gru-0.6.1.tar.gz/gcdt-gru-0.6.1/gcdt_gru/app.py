...
# TODO s3sources
# TODO sns
# TODO add ping (on by default but can be switched off)
# TODO for schedule_event add tests and scaffolding support
...
class GruError(Exception):
    pass


class GruViewError(GruError):
    STATUS_CODE = 500

    def __init__(self, msg=''):
        super(GruViewError, self).__init__(
            self.__class__.__name__ + ': %s' % msg)


class BadRequestError(GruViewError):
    STATUS_CODE = 400


class UnauthorizedError(GruViewError):
    STATUS_CODE = 401


class ForbiddenError(GruViewError):
    STATUS_CODE = 403


class NotFoundError(GruViewError):
    STATUS_CODE = 404


class MethodNotAllowedError(GruViewError):
    STATUS_CODE = 405


class ConflictError(GruViewError):
    STATUS_CODE = 409


class TooManyRequestsError(GruViewError):
    STATUS_CODE = 429
...
class ScheduleEntry(object):
    ...
    def __init__(self, schedule, description=None):
...
class Gru(object):
    ...
    def __init__(self, app_name, configure_logs=True):
        ...
        self.routes = {}
        self.scheduled_event = None  # scheduled_event (currently lim. to one!)
    ...
    def schedule(self, schedule, **kwargs):
        # as far as I can see only on function can have a schedule
        def _register_view(view_func):
            #self._add_schedule(schedule, view_func, **kwargs)
            self.scheduled_event = view_func
            return view_func
        return _register_view
    ...
    def __call__(self, event, context):
        ...
        # event sample:
        # {u'account': u'420189626185', u'region': u'eu-west-1', u'detail': {}, u'detail-type': u'Scheduled Event', u'source': u'aws.events', u'version': u'0', u'time': u'2017-04-04T16:25:00Z', u'id': u'6db3a492-e7d6-4fc3-9540-f0fa4ad40b61', u'resources': [u'arn:aws:events:eu-west-1:420189626185:rule/infra-dev-sample-ticktack-rule']}
        if 'source' in event and event['source'] == 'aws.events' and \
                event['detail-type'] == 'Scheduled Event':
            # this is a scheduled event
            view_function = self.scheduled_event  # view_function
            function_args = []
            response = self._get_view_function_response(view_function,
                                                        function_args)
            return response.to_dict()
        else:
            # save to say that this is an API Gateway call...
            ...
            if resource_path not in self.routes:
                raise GruError("No view function for: %s" % resource_path)
            ...
            self.current_request = APIGatewayRequest(
    ...
    def _validate_response(self, response):
        if isinstance(response, Response):
            for header, value in response.headers.items():
                if '\n' in value:
                    raise GruError("Bad value for header '%s': %r" %
                                       (header, value))
