...
from http_request_randomizer.rest import proxylist


@pytest.fixture
def client(request):
    db_fd, proxylist.app.config['DATABASE'] = tempfile.mkstemp()
    client = proxylist.app.test_client()
    with proxylist.app.app_context():
        proxylist.init_db()

    def teardown():
        """Get rid of the database again after each test."""
        os.close(db_fd)
        os.unlink(proxylist.app.config['DATABASE'])
