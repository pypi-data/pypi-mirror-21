...
def calc_DCE_properties_single(input_filepath, T1_tissue=1000, T1_blood=1440, relaxivity=.0045, TR=5, TE=2.1, scan_time_seconds=(11*60), hematocrit=0.45, injection_start_time_seconds=60, flip_angle_degrees=30, label_file=[], label_suffix=[], label_value=1, mask_value=-.01, mask_threshold=0, T1_map_file=[], T1_map_suffix='-T1Map', AIF_label_file=[],  AIF_text_file=[], AIF_value_suffix=[], convert_AIF_values=True, AIF_mode='label', AIF_label_suffix=[], AIF_label_value=1, label_mode='separate', param_file=[], default_population_AIF=False, initial_fitting_function_parameters=[.01,.1], outputs=['ktrans','ve','auc'], outfile_prefix='', processes=1, preprocess=True):

    """ This is a master function that creates ktrans, ve, and auc values from raw intensity 1D-4D volumes.
    """

    # Should probably address the source of the warnings, but this is a stopgap measure for iPython.
    with warnings.catch_warnings():
        ...
        if isinstance(input_filepath, basestring):
            image = np.nan_to_num(nifti_2_numpy(input_filepath))
        else:
            image = np.nan_to_num(np.copy(input_filepath))
        ...
        AIF_label_image, label_image, T1_image, AIF = retreive_data_from_files(input_filepath, label_file, label_mode, label_suffix, label_value, AIF_label_file, AIF_label_value, AIF_mode, AIF_label_suffix, T1_map_file, T1_map_suffix, AIF_text_file, AIF_value_suffix, image)

        # If no pre-set AIF text file is provided, one must be generated either from a label-map or a population AIF.
        if AIF == []:
            AIF = generate_AIF(scan_time_seconds, injection_start_time_seconds, time_interval_seconds, image, AIF_label_image, AIF_text_file, AIF_mode, dimension, AIF_label_value)
        ...
        if AIF_mode == 'population':
            contrast_AIF = AIF
        elif AIF_text_file != [] and convert_AIF_values == False:
            contrast_AIF = AIF
        else:
        ...
        for param_idx, param in enumerate(outputs):
            save_numpy_2_nifti(parameter_maps[...,param_idx], input_filepath, outfile_prefix + param + '.nii.gz')
        return outputs

def retreive_data_from_files(input_filepath, label_file, label_mode, label_suffix, label_value, AIF_label_file, AIF_label_value, AIF_mode, AIF_label_suffix, T1_map_file, T1_map_suffix, AIF_text_file, AIF_value_suffix, image=[]):
    ...
    if label_mode == 'none':
        ...
        split_path = str.split(input_filepath, '.nii')
    ...
    if AIF_text_file != []:
        ...
        if isinstance(AIF_text_file, basestring):
            try:
                AIF = np.loadtxt(AIF_text_file, dtype=object, delimiter=';')
                AIF = [value for value in AIF if value != '']
...
def generate_AIF(scan_time_seconds, injection_start_time_seconds, time_interval_seconds, image_numpy=[], AIF_label_numpy=[], AIF_text_file=[], AIF_mode='label', dimension=4, AIF_label_value=1):
...
def simplex_optimize(contrast_image_numpy, contrast_AIF_numpy, time_interval_seconds, bolus_time, image=[], label_image=[], mask_value=-.01, mask_threshold=0, initial_fitting_function_parameters=[.01,.1], outputs=['ktrans','ve'], processes=1):
...
def simplex_optimize_loop(contrast_image_numpy, contrast_AIF_numpy, time_interval_seconds, bolus_time, mask_value=-.01, mask_threshold=0, initial_fitting_function_parameters=[1,1]):
...
def preprocess_dce(image_numpy, gaussian_blur_sigma=0, gaussian_blur_axes=[0,1], PCA_thresholding=True):

    """ Consider implementing a preprocessing pipeline interface
        at some point or another.
    """

    image_numpy = gaussian_filter(image_numpy, [.65, .65] + (image_numpy.ndim - 2) * [0])

    # if PCA_thresholding:
        # pca_maps = create_PCA_maps(image_numpy, PCA_levels=2)
        # image_numpy[pca_maps[..., 1] > -50]  = -.01

    return image_numpy

def test_case_2d():
    # print 'hello'
    # input_filepath = 'C:/Users/azb22/Documents/GitHub/Public_qtim_tools/qtim_tools/qtim_tools/test_data/test_data_dce/tofts_v6.nii.gz'
    input_filepath = 'C:/Users/abeers/Documents/GitHub/Public_QTIM/qtim_tools/qtim_tools/test_data/test_data_dce/gradient_tofts_v6.nii'
    # input_filepath = 'C:/Users/azb22/Documents/GitHub/Public_qtim_tools/qtim_tools/qtim_tools/test_data/test_data_dce/tofts_v9_5SNR.nii'
    # input_filepath = 'C:/Users/abeers/Documents/GitHub/Public_QTIM/qtim_tools/qtim_tools/test_data/test_data_dce/tofts_v6.nii.gz'

    calc_DCE_properties_single(input_filepath, label_file=[], param_file=[], AIF_label_file=[], AIF_text_file=[], convert_AIF_values=False, outputs=['ktrans','ve','auc'], T1_tissue=1000, T1_blood=1440, relaxivity=.0045, TR=5, TE=2.1, scan_time_seconds=(11*60), hematocrit=0.45, injection_start_time_seconds=60, flip_angle_degrees=30, label_suffix=[], AIF_mode='label', AIF_label_suffix='-AIF-label', AIF_label_value=1, label_mode='separate', default_population_AIF=False, initial_fitting_function_parameters=[.01,.1], outfile_prefix='tofts_v6_gradient', processes=1, mask_threshold=20, mask_value=-1, preprocess=True)

    # calc_DCE_properties_single(input_filepath, label_file=[], param_file=[], AIF_label_file=[], AIF_text_file=[], convert_AIF_values=False, outputs=['ktrans','ve','auc'], T1_tissue=1000, T1_blood=1440, relaxivity=.0045, TR=5, TE=2.1, scan_time_seconds=(6*60), hematocrit=0.45, injection_start_time_seconds=60, flip_angle_degrees=30, label_suffix=[], AIF_mode='label', AIF_label_suffix='-AIF-label', AIF_label_value=1, label_mode='separate', default_population_AIF=False, initial_fitting_function_parameters=[.3,.3], outfile_prefix='tofts_v9_noblur_', processes=22, mask_threshold=-1, mask_value=-1, preprocess=True)


def test_case_3d(input_filepath=[]):
    # These are params for NHX/CED data
    if input_filepath == []:
        input_filepath = 'C:/Users/abeers/Documents/Junk/dce_mc_st_eco1.nii'
        # input_filepath = 

    # AIF_text_file = 'C:/Users/azb22/Documents/Junk/VISIT_01_autoAIF_bAIF.txt'
    calc_DCE_properties_single(input_filepath, outputs=['ktrans','ve','auc'], T1_tissue=1500, T1_blood=1440, relaxivity=.0039, TR=6.8, TE=2.1, scan_time_seconds=(6*60), hematocrit=0.45, injection_start_time_seconds=160, flip_angle_degrees=10, label_suffix=[], AIF_mode='population',initial_fitting_function_parameters=[.01,.1], outfile_prefix='_PCA_Thresholding', processes=1, mask_threshold=0, mask_value=-1, preprocess=True)

    # calc_DCE_properties_single(input_filepath, label_file=[], param_file=[], AIF_label_file=[], AIF_text_file=AIF_text_file, convert_AIF_values=False, outputs=['ktrans','ve','auc'], T1_tissue=1500, T1_blood=1440, relaxivity=.0039, TR=6.8, TE=2.1, scan_time_seconds=(6*60), hematocrit=0.45, injection_start_time_seconds=160, flip_angle_degrees=10, label_suffix=[], AIF_mode='population', AIF_label_suffix='-AIF-label', AIF_label_value=1, label_mode='separate', default_population_AIF=False, initial_fitting_function_parameters=[.01,.1], outfile_prefix='mead_cobyal_individual_ktrans_no_mask', processes=22, mask_threshold=20, mask_value=-1, preprocess=True)

execute = calc_DCE_properties_single
...
if __name__ == '__main__':
    ...
    # test_method_2d()
    test_method_3d()
    # create_4d_from_3d(input_filepath)