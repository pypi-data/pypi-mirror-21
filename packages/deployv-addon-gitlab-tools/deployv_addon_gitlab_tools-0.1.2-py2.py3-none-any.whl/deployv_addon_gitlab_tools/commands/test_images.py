...
def build_image():
    ...
    cmd = ('deployvcmd build -b {branch} -u {url} -v {version} -i {image} -O {repo}#{odoo_branch} -T {tag}'
           .format(branch=environ['CI_BUILD_REF_NAME'], url=environ['CI_BUILD_REPO'], repo=environ['ODOO_REPO'],
...
def install_module():
    ...
    install_wdemo = (
        ...
        " --stop-after-init".format(mod=module)
    )
    install_wodemo = (
        "/home/odoo/instance/odoo/odoo-bin -d wodemo -i {mod}"
        " --stop-after-init --without-demo=all".format(mod=module)
    )
...
@click.option('--ci_job_id', default=environ.get('CI_JOB_ID', environ.get('CI_BUILD_ID')),
