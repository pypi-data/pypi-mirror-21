...
def upload_image(**kwargs):
    ...
    version_tag = environ.get('CI_BUILD_REF_NAME').replace('.', '')
    ...
    for customer_name in customer_names.split(','):
        if customer_name.strip():
            customer_img = '{customer}{ver}'.format(
                customer=customer_name.strip(), ver=version_tag
            )
            image_repo = 'quay.io/vauxoo/{image}'.format(image=customer_img)
