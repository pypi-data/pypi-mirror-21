...
class CloudshellController:
    ...
    def create_children(self, device_info):
        ...
        if (device_info.brics_family == 'ICX'):
            self.create_icx_children(device_info)
        else:
            print 'No family supported'

    def create_icx_children(self, device_info):
        ...
        for i in range(1, 49):
            self.session.CreateResource(resourceFamily='General Port', resourceModel='Device Port',
                resourceName='p1_1_%s' % i, resourceAddress=i, folderFullPath="", parentResourceFullPath=device_info.brics_name,
                resourceDescription='')

        for module in range(2, parent['modules'] + 1):
            for port in range(1, parent['mod%s' % module] + 1):

                self.session.CreateResource(resourceFamily='General Port', resourceModel='Device Port',
                    resourceName='p1_%s_%s' % (module, port), resourceAddress=i, folderFullPath="",
...
if __name__ == '__main__':
    ...
    controller = CloudshellController(configs=config_file)

    controller.get_resource_by_attribute('Name', 'trash')
    controller.create_children()
