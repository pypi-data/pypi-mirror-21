            ...
            for module in range(1, parent['modules'] + 1):
                for port in range(1, parent['mod%s' % module] + 1):
                    ...
                    self.session.CreateResource(resourceFamily='General Port', resourceModel='Device Port',
                        resourceName='%s%s_%s' % (prefix, module, port), resourceAddress=port, folderFullPath="",
                        parentResourceFullPath=device_info.brics_name, resourceDescription='')
        except Exception as e:
            print 'There was probably an issue with your JSON. %s' % e.message
