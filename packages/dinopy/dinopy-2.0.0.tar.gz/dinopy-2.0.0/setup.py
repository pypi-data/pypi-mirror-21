...
c_extensions = [
    ...
    for filename in "auxiliary conversion creader definitions fasta_reader fasta_writer fastq_reader fastq_writer input_opener processors shape shaping output_opener nameline_parser sam_reader sam_writer sambam".split()
    ]
cpp_extensions = [
    Extension(
        "dinopy.wrap_sais",
...
setup(
    name = 'dinopy',
    description = "DNA input and output library for Python and Cython. Includes reader and writer for FASTA and FASTQ files, support for samtools faidx files, and generators for solid and gapped q-grams (k-mers).",
    ...
    packages=['dinopy'],
    package_data={'dinopy': ['*.pyx', '*.pxd', '*.c', '*.cpp', 'cpp/sais.cpp', '../README.rst']}, # Add cython and c++ files
