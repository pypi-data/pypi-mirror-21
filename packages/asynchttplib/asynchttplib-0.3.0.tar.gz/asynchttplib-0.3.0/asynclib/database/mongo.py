...
class MongoDBManager(object):
    ...
    async def _create(self, parameters):
        result = await self.db.insert_one(parameters)
        parameters['_id'] = result.inserted_id
        return parameters

    async def cursor_map(self, cursor):
        result = await cursor.to_list()
        return {str(i['_id']): i for i in result}

    async def update_many_denormalized(self, query, data, *args, **kwargs):
        old_cursor_map = await self.cursor_map(self.db.find(query))
        result = await self.db.update_many(query, data, *args, **kwargs)
        if result.modified_count > 0:
            new_cursor = self.db.find(query)
            if await new_cursor.count() > 0:
                new_cursor_map = await self.cursor_map(new_cursor)
...
async def cursor_to_result(cursor, skip=None, limit=None):
    """
    :param limit: int
    :param skip: int
