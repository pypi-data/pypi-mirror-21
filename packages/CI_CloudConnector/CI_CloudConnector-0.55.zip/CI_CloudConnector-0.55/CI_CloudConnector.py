...
def watchDogLoop():
    ...
    try:
        ...
        msg = ""
        if isAlive==False:
            print "watchDogLoop::MainLoop Not Working, reatsrting " + str(diff)
            CI_LC_BL.ci_print ("watchDogLoop::MainLoop Not Working , reatsrting","CRITICAL")
        ...
        CI_LC_BL.ci_print ("watchDogLoop::Error MainLoop " + str(inst),"ERROR")
...
def MainLoopTimer():
    ...
    try:
        ...
        CI_LC_BL.ci_print ("Error MainLoopTimer " + str(inst),"ERROR")    
    
    if threadTimer:
        threadTimer.start()
    else:
        CI_LC_BL.ci_print ("threadTimer not found!","ERROR")
        print "threadTimer not found!"
        threadTimer = RepeatedTimer(5, MainLoopTimer)
        CI_LC_BL.ci_print ("Starting threadTimer again!","ERROR")
...
def MainLoop():
    ...
    try: 
        ...
        CI_LC_BL.ci_print ("Error MainLoop " + str(inst),"ERROR")
