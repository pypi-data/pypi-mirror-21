...
class Backend(object):
    ...
    def __init__(self, config_path_param=None):
        if config_path_param is None:
            if os.environ.get('DKCLI_CONFIG_LOCATION') is not None:
                ...
                folder = os.path.join(home,'.dk')
                try: 
                    os.makedirs(folder)
                except: 
                    pass
                self.config_file_location = os.path.join(folder,"DKCloudCommandConfig.json")
        else:
    ...
    def check_version(self):
        ...
        if latest_version and latest_version != DK_VERSION:
...
@click.option('--source_kitchen', '-sk', type=str, required=True, help='source (from) kitchen name')
...
def recipe_delete(backend,kitchen,name,yes):
    ...
    if not yes:
        confirm = raw_input('Are you sure you want to delete the recipe %s ? [yes/No]' % name)
...
def file_add(backend, kitchen, recipe, message, filepath):
    """
    Add a newly created file to a Recipe. Provide Kitchen and Recipe names or be in a Recipe folder. Requires inclusion of a change message and filepath for the file to be added.
    """
    err_str, use_kitchen = Backend.get_kitchen_from_user(kitchen)
    if use_kitchen is None:
        raise click.ClickException(err_str)
