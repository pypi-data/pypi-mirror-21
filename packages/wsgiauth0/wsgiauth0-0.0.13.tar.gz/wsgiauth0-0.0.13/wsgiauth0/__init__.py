...
def validate_jwt_claims(clients, authorization):
    ...
    if not authorization:
        ...
        try:
            ...
            log.debug('JOSEError error=%r', jose_error)
            jwt_environ['wsgiauth0.jwt_claims'] = None
            jwt_environ['wsgiauth0.jwt_error'] = {
                'code': repr(jose_error),
        ...
        if client is not None:
            jwt_environ['wsgiauth0.jwt_auth0_client'] = client._asdict()
        else:
            jwt_environ['wsgiauth0.jwt_auth0_client'] = None
    ...
    jwt_environ['JWT_CLAIMS'] = jwt_environ['wsgiauth0.jwt_claims']
    jwt_environ['JWT_ERROR'] = jwt_environ['wsgiauth0.jwt_error']
    jwt_environ['JWT_AUTH0_CLIENT'] = jwt_environ['wsgiauth0.jwt_auth0_client']
