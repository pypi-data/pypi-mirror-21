...
def listdir_fullpath(d, match=""):
    ...
    return [os.path.join(d, f) for f in os.listdir(d) if match in f]


def listdir_fullpath_has_shadowfile(d, prefix):
    """List files in given directory that have a shadow file with the given prefix."""
    return [os.path.join(d, f) for f in os.listdir(d) if os.path.isfile(os.path.join(d, "%s_%s" % (prefix, f)))]
