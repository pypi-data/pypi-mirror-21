...
function replace_element(i, html) {
    var r_id = treeadmin.jQuery(html).attr('id');
    treeadmin.jQuery('#' + r_id).replaceWith(html);
}

/* Same as above, but processes an array of html snippets */
function replace_elements(data) {
    treeadmin.jQuery.each(data, replace_element);
}
