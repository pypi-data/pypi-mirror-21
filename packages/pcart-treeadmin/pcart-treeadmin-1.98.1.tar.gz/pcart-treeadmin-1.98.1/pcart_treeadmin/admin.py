...
def _build_tree_structure(cls):
    ...
    for p_id, parent_id in cls.objects.order_by(
            ...
            mptt_opts.left_attr).values_list(
                "pk", "%s_id" % mptt_opts.parent_attr):
        all_nodes[str(p_id)] = []

        if parent_id:
            if str(parent_id) not in all_nodes:
                # This happens very rarely, but protect against parents that
                # we have yet to iteratove over.
                all_nodes[str(parent_id)] = []
            all_nodes[str(parent_id)] += [str(p_id)]
    ...
    def get_results(self, request):
        ...
        for item in self.result_list:
            if self.model_admin.enable_object_permissions:
                item.feincms_editable = \
                    self.model_admin.has_change_permission(request, item)
            else:
...
class TreeAdmin(admin.ModelAdmin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.change_list_template = [
            'admin/treeadmin/%s/%s/tree_editor.html' % (
    ...
    def indented_short_title(self, item):
        ...
        r += '<span id="page_marker-%s" class="page_marker%s" '\
    ...
    def _refresh_changelist_caches(self):
        ...
        XXX: This is somewhat hacky, but since it's an internal method,
        so be it.
        """
    ...
    def _toggle_boolean(self, request):
        ...
        if not request.user.is_staff:
            logging.warning(
                "Denied AJAX request by non-staff %s to toggle boolean %s "
                "for object #%s", request.user, attr, item_id)
        ...
        if hasattr(obj, "user_can") and \
                obj.user_can(request.user, change_page=True):
        ...
        if not can_change:
            logging.warning(
                "Denied AJAX request by %s to toggle "
                "boolean %s for object %s", request.user, attr, item_id)
            return HttpResponseForbidden(
                "You do not have permission to access this object")

        logging.info(
            "Processing request by %s to toggle %s on %s",
            request.user, attr, obj)
    ...
    def changelist_view(self, request, extra_context=None, *args, **kwargs):
        ...
        extra_context['TREEADMIN_MEDIA_HOTLINKING'] = \
            self.jquery_use_google_cdn
        ...
        # context = {
        #     "rebuildtree_url": reverse("%s:%s_%s_rebuildtree" % (
        #         self.admin_site.name, opts.app_label, opts.object_name.lower())),
        # }
        context = {}
    ...
    def _move_node(self, request):
        ...
        self.message_user(
            request, ugettext('Did not understand moving instruction.'))
        return HttpResponse('FAIL')

    def get_urls(self):
        """Returns the additional urls used by the Extension admin."""
        ...
        extension_urls = [
            url(
                "^rebuild-tree/$",
                admin_site.admin_view(self.rebuild_tree),
                name='%s_%s_rebuildtree' % info),
        ]
        return extension_urls + urls

    def rebuild_tree(self, request, extra_context=None):
        model = self.model
        opts = model._meta
        model.tree.rebuild()
        messages.success(
            request, _('Tree structure was updated successfully.'))
        return HttpResponseRedirect(reverse("%s:%s_%s_changelist" % (
            self.admin_site.name, opts.app_label, opts.object_name.lower())))

    def _actions_column(self, instance):
        return [
            '<div class="drag_handle" title="%s"></div>' %
            ugettext(u'Move node'),
        ]
...
class JohnnyCacheAwareTreeAdmin(TreeAdmin):
    ...
    def _move_node(self, request):
        ...
        johnny.settings.BLACKLIST = \
            set(list(original_blacklist) + [self.model._meta.db_table])
