...
class OWChoropleth(widget.OWWidget):
    ...
    class Error(widget.OWWidget.Error):
        ...
        need_continuous_latlon = widget.Msg('Latitude and longitude must be continuous variables')
    ...
    def __init__(self):
        ...
        def _set_latlon():
            self.latlon = None
            if self.lat_attr.is_continuous and self.lon_attr.is_continuous:
                self.latlon = np.c_[self.data.get_column_view(self.lat_attr)[0],
                                    self.data.get_column_view(self.lon_attr)[0]]
            self.Error.need_continuous_latlon(shown=self.latlon is not None)
            self.aggregate()
