...
def devicemanager_inactive(request):
    ...
    if request.method == 'GET':
        try:
            ...
            logging.info('-------------- Inactive assets:', disconnected_assets)
            return response.Response(AssetSerializer(disconnected_assets, many=True).data, status=status.HTTP_201_CREATED)
        except Exception as error:
            return response.Response(error, status=status.HTTP_400_BAD_REQUEST)
...
def visa_alias(request):
    ...
    if request.method == 'POST':
        try:
            connected = visaweb.client.alias(request.data)
...
def devicemanager_attach(request):
    ...
    if request.method == 'POST':
        try:
            ...
            connected = visaweb.client.alias(mapping)['connected']
...
def visa_read(request):
    ...
    if request.method == 'POST':
        try:
            ...
            data = visaweb.client.read(request.data['alias'], request.data['command'])
...
def visa_write(request):
    ...
    if request.method == 'POST':
        try:
            ...
            data = visaweb.client.write(request.data['alias'], request.data['command'])
            return response.Response(data, status=status.HTTP_200_OK)
        except Exception as error:
            return response.Response(error, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def visa_query(request):
    ...
    if request.method == 'POST':
        try:
            ...
            data = visaweb.client.query(request.data['alias'], request.data['command'])
            return response.Response(data, status=status.HTTP_200_OK)
        except Exception as error:
