...
class VisaRpcServer(RpcServer):
    ...
    def on_query_request(self, ch, method, props, body):
        ...
        try:
            request = json.loads(body.decode("utf-8"))
            response = self.device_manager.query(request['alias'], request['command'])
    ...
    def on_write_request(self, ch, method, props, body):
        ...
        try:
            request = json.loads(body.decode("utf-8"))
            response = self.device_manager.write(request['alias'], request['command'])
    ...
    def on_read_request(self, ch, method, props, body):
        ...
        try:
            request = json.loads(body.decode("utf-8"))
            response = self.device_manager.read(request['alias'], request['command'])
