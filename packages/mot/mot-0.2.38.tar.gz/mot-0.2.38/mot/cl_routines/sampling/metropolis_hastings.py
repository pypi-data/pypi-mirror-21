...
class _MHWorker(Worker):
    ...
    def __init__(self, cl_environment, compile_flags, model, current_chain_position, samples, proposal_state,
        ...
        # this however does not mean that the computations are interruptable. We enqueue all kernels at once
        ...
        self._burnin_kernel = self._build_burnin_kernel()
        self._sampling_kernel = self._build_sampling_kernel()
    ...
    def calculate(self, range_start, range_end):
        ...
        if self._burn_length > 0:
            ...
            for batch_size in batch_sizes:
                burnin_event = self._sampling_kernel.sample(self._cl_run_context.queue,
        ...
        return_events = [
            self._enqueue_readout(samples_buf, self._samples, 0, nmr_problems, [sampling_event]),
            self._enqueue_readout(proposal_buffer, self._proposal_state, 0, nmr_problems, [sampling_event]),
        ...
        for mcmc_state_element in sorted(self._mh_state_dict):
            ...
            return_events.append(self._enqueue_readout(buffer, host_array, 0, nmr_problems, [sampling_event]))
    ...
    def _get_sampling_batch_sizes(self, total_nmr_samples, max_batch_length):
        ...
        Examples:
            >>> self._get_sampling_batch_sizes(30, 8)
            >>> [8, 8, 8, 6]

        Returns:
            list: the list of batch sizes
        """
        return [max_batch_length] * (total_nmr_samples // max_batch_length) + [total_nmr_samples % max_batch_length]
    ...
    def _build_sampling_kernel(self):
