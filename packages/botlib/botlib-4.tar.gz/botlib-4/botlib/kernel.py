...
class Kernel(Handler, Launcher, Db):
    ...
    def boot(self, cfgin):
        ...
        cfg.logdir = os.path.join(cfg.homedir, ".botlog")
        if cfg.banner:
            print("BOTLIB #%s\n" % botlib.__version__)
        if cfg.test:
            cfg.workdir = "data"
        ...
        if not cfg.workdir:
            if cfg.local:
                cfg.workdir = kwargs.get("workdir", os.path.join(cfg.homedir, ".botdata"))
            else:
        ...
        if not self._names and not cfg.write:
            ...
            logging.warn("# using %s" % pathname(k._container.path))
            self._names.update(k._names)
            c = Config().load(os.path.join(cfg.bootdir, "runtime", "cfg"))
            logging.warn("# using %s" % pathname(c._container.path))
        if cfg.scan:
