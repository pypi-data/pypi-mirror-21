...
class Kernel(Handler, Launcher, Db):
    ...
    def boot(self, cfgin):
        ...
        if cfg.yes or cfg.banner:
            print("BOTLIB #%s\n" % botlib.__version__)
            cfg.shell = True
            cfg.test = True
            cfg.loglevel = cfg.loglevel or "warn"
            cfg.workdir = "data"
            cfg.logdir = os.path.join(os.getcwd(), "log")
            cfg.verbose = True
        if cfg.yes or cfg.shell:
            cfg.loglevel = cfg.loglevel or "error"
            cfg.init += ",cli"
        loglevel(cfg.loglevel or "error", cfg.colors, cfg.logdir)
        if cfg.license:
            print(__license__)
        if not cfg.workdir:
            if cfg.local:
                cfg.workdir = kwargs.get("workdir", os.path.join(cfg.homedir, ".botlib"))
            else:
                cfg.workdir = kwargs.get("workdir", os.path.join(os.getcwd(), "data"))
        else:
            cfg.changed = True
        ...
        if not self._names and not cfg.write:
            k = Kernel().load(os.path.join(cfg.workdir, "runtime", "kernel"))
            if cfg.verbose:
                print("# using %s" % k._container.path)
            self._names.update(k._names)
            c = Config().load(os.path.join(cfg.workdir, "runtime", "cfg"))
            if cfg.verbose:
                print("# using %s" % c._container.path)
        if cfg.scan:
        ...
        if cfg.all or cfg.init:
            for modname in self.modules("botlib"):
                n = modname.split(".")[-1]
    ...
    def shutdown(self, close=True):
        ...
        from botlib.space import cfg, exceptions, fleet, kernel, runtime
        logging.warn("")
        event = Event()
        event.txt = "shutdown"
        event.server = "kernel"
