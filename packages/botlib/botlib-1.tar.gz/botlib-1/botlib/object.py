...
class Object(dict):
    ...
    def __iadd__(self, y):
        """ Use the counter attribute to increase internal counter. """
        self.counter += y
        return self.counter
    ...
    def load(self, path, force=False, skip=[], full=True):
        ...
        if "signature" in fromdisk:
            if not verify_signature(fromdisk["data"], fromdisk["signature"]) and not force:
                raise botlib.error.ESIGNATURE(path)
        if "data" in fromdisk:
    ...
    def sync(self, path="", stime=""):
        ...
        if not path:
            path = get_path(self)
        if not path:
            path = os.path.join(self._path, stime or rtime())
