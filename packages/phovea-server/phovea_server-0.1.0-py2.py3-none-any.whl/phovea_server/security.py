...
def can(item, permission, user=None):
  if user is None:
    user = current_user()
  owner, group, others = _decode(item.get('permissions', DEFAULT_PERMISSION))

  # I'm the creator
  if _is_equal(user.name, item.get('creator', ANONYMOUS)):
    return permission in owner

  # check if I'm in the group
  if 'group' in item and _includes(user.roles, item.get('group')):
    return permission in group
