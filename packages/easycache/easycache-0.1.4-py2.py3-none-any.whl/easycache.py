...
class EasyCache:
    ...
    def buildCacheArgs(self, args, ignoreArgs, kwargs, ignorekwArgs):
        argsString = "".join((self._convertArg(arg) for i, arg in enumerate(args) if i not in ignoreArgs))
        kwargsString = "".join((self._convertArg(kwargs[arg], name=arg) for arg in kwargs if arg not in ignorekwArgs))
