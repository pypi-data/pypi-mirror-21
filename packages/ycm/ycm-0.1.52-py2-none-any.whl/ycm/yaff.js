...
Yaff.View = Yaff.extend({
    ...
    register_event: function(event) {
        ...
        for (var i = 0, l = elements.length; i < l; i++) {
            ...
            elements[i].addEventListener(event['event'], this._event_handlers[event['event']]);
        }
...
Yaff.mixins = {
    ...
    draggable: {
        ...
        drag_move: function(e) {
            ...
            var clone_rect = {
                top: Yaff.drag_context['clone_details']['currentY'],
                left: Yaff.drag_context['clone_details']['currentX'],
                width: Yaff.drag_context['clone_details']['width'],
                height: Yaff.drag_context['clone_details']['height'],
                right: Yaff.drag_context['clone_details']['currentX'] + Yaff.drag_context['clone_details']['width'],
                bottom: Yaff.drag_context['clone_details']['currentY'] + Yaff.drag_context['clone_details']['height']
            };

            var orig_rect = Yaff.drag_context['clone_details']['view_item']['el'].getBoundingClientRect();

            var over, diff, target_rect, direction, intersection;

            for (var i = 0, l = drop_containers.length; i < l; i++) {
                // get drop container co-ordinates
                target_rect = drop_containers[i]['element'].getBoundingClientRect();
                if (drag_type === 'vertical') {
                    ...
                    over = clone_rect.height > diff && diff > 0;
