    ...
    def parse_source_file(self):
        ...
        if os.path.isdir(source):
            ...
            files = [
                ...
                {'file': filter(cssre.match, os.listdir(source))[0], 'header': '[css]'}
            ]
        ...
        while True:
            ...
            for component in self.components:
                ...
                while True:
                    ...
                    attr_str = re.sub(r"(\s*\"\s*{{\s*).*(\s*}}\s*\"\s*)", lambda x: x.group(2), json.dumps(attrs))

                    # build the string to replace the original tags with
                    macro_name = "yaff_macros." + component.source['headers']['macro_name']
                    data_object_name = component.source['headers']['html_tag'] + '__data'
