...
Yaff.List = Yaff.extend({
    ...
    submit_to_server: function(request) {
        ...
        */
        ...
        rq.open('POST', this.endpoint, true);
        rq.setRequestHeader('Content-Type', 'application/json');

        rq.onload = function() {
            if (this.status >= 200 && this.status < 400 && this.response) {
                self.callbacks[action].apply(self, this.response);
            } else {
...
Yaff.View = Yaff.extend({
    ...
    register_event: function(event) {
        ...
        for (var i = 0, l = elements.length; i < l; i++) {
            elements[i].addEventListener(event['event'], self[event['callback']].bind(self));
        }
    ...
    parse_bind_data: function() {
        ...
        for (var i = 0, attrs = this.el.attributes, l = this.el.attributes.length, data = {}; i < l; i++) {
            if (attrs[i].nodeName.substr(0, 4) === 'data') {
                var key = attrs[i].nodeName.substr(4, attrs[i].nodeName.length - 4);
    ...
    remove: function() {
        ...
        for (var j=0; j<m; j++) {
            ...
            Array.prototype.forEach.call(elements, function(el) {
                el.removeEventListener(event['event'], self[event['callback']])
            })
        }
...
Yaff.Page = Yaff.extend({
        ...
        for (var k in self.selector_class_map) {
            if (self.selector_class_map.hasOwnProperty(k)) {
                ...
                Array.prototype.forEach.call(elements, function(el){
                    ...
                    self.element_view_map[el] = view_obj;
                });
    ...
    submit_to_server: function(request) {
        ...
        rq.onload = function() {
            if (this.status >= 200 && this.status < 400 && this.response) {
                self.callbacks[request['action']].call(self, request, this.response);
            } else {
                if (this.response['redirect']) {
                    window.location.assign(this.response['redirect']);
                } else {
                    var default_err_msg = 'An error occurred. Please refresh the page and try again.';
                        var error_message = this.response['error_message'] ? this.response['error_message'] : default_err_msg;
...
Yaff.Utilities = Yaff.extend({
    ...
    upgrade_DOM: function(target) {
        ...
        var target_array = target.querySelectorAll('*');
...
Yaff.mixins = {
    ...
    draggable: {
        ...
        down_start: function(e) {
            ...
            window.addEventListener('touchend', mouseup_handler.bind(this));
            window.addEventListener('touchcancel', mouseup_handler.bind(this));
        ...
        start_dragging: function(e) {
            ...
            Yaff.drag_context['clone_details'] = {
                ...
                stored_contents: this.innerHTML,   // store contents so we can make the dragged item a gray box
                background: this.el.style.backgroundColor
            };
            ...
            var clone_style = 'border-style: dotted; border-color: #9e9e9e; border-width: 2px; ' +
                ...
                'width: ' + Yaff.drag_context['clone_details'].width + 'px; ' +
                'height: ' + Yaff.drag_context['clone_details'].height + 'px; ' +
                'padding: ' + this.el.style.padding + '; ' +
                'top: ' + rect.top + 'px; ' +
                'left: ' + rect.left + 'px; ' +
                'cursor: move;' +
                'pointer-events: none;';
            ...
            this.el.style.height = Yaff.drag_context['clone_details']['height'] + 'px';

            // trap move / end events

            if (Yaff.drag_context['origin'] === 'mouse') {
                window.addEventListener('mousemove', Yaff.drag_context.mousemove.bind(this));
                window.addEventListener('mouseup', Yaff.drag_context.mouseup.bind(this));
            } else {
                window.addEventListener('touchmove', Yaff.drag_context.mousemove.bind(this));
                window.addEventListener('touchend', Yaff.drag_context.mouseup.bind(this));
                window.addEventListener('touchcancel', Yaff.drag_context.mouseup.bind(this));
            }
        ...
        drag_move: function(e) {
            ...
            Yaff.drag_context['clone_details']['currentX'] = clientX - Yaff.drag_context['clone_details']['mouseX'];
            Yaff.drag_context['clone_details']['currentY'] = clientY - Yaff.drag_context['clone_details']['mouseY'];
        },
        ...
        drag_up: function(e) {
            ...
            window.removeEventListener('touchmove', Yaff.drag_context.mousemove);
            window.removeEventListener('touchend', Yaff.drag_context.mouseup);
            window.removeEventListener('touchcancel', Yaff.drag_context.mouseup);
            window.removeEventListener('mousemove', Yaff.drag_context.mousemove);
            window.removeEventListener('mouseup', Yaff.drag_context.mouseup);

            // put the original html back
            this.el.innerHTML += Yaff.drag_context['clone_details']['stored_contents'];
    ...
    droppable: {
        init_droppable: function() {
            this.register_event({event: 'mouseover', callback: 'droppable_mouseover'});
            this.register_event({event: 'touchenter', callback: 'droppable_mouseover'});
            this.register_event({event: 'mouseout', callback: 'droppable_mouseout'});
            this.register_event({event: 'touchleave', callback: 'droppable_mouseout'});
        },

        droppable_mouseover: function(e) {
            if (!Yaff.drag_context.dragging) return;
            Yaff.drag_context.active_target = this;
            this.drag_over();
        },

        droppable_mouseout: function(e) {
            if (!Yaff.drag_context.dragging) return;
            Yaff.drag_context.active_target = null;
            this.drag_out();
        }
    }
