...
def visjs_network(nodes_dict, edges_dict,
    ...
    if export_network:
        export_to_cytoscape(nodes_dict,edges_dict,export_file)
...
def export_to_cytoscape(nodes_dict,edges_dict,export_file):
    '''
    ...
    node_to_color = {node['id']:node['color'] for node in nodes_dict}
    min_val = np.min(list(node_to_color.values()))
    max_val = np.max(list(node_to_color.values())) - min_val
    color_list = [float(node_to_color[n]-min_val)/max_val for n in G.nodes()]
    node_colors = dict(zip(G.nodes(),color_list))
