...
class TestPackageRunner(object):
    ...
    def __init__(self, profile, username, channel, logger=None, mingw_installer_reference=None, runner=None, args=None,
    ...
    def _run_test_package(self, pre_command=None):
        ...
        if platform.system() != "Windows":
            if settings.get("compiler", None) and settings.get("compiler.version", None):
                conan_compiler, conan_compiler_version = conan_compiler_info()
                if conan_compiler != settings.get("compiler", None) or \
                   conan_compiler_version != settings.get("compiler.version", None):
                    self._logger.debug("- Skipped build, compiler mismatch: %s" % str(dict(settings)))
        ...
        self._logger.info("******** RUNNING BUILD ********** \n%s" % command)
...
class DockerTestPackageRunner(TestPackageRunner):
    ...
    def __init__(self, profile, username, channel, logger=None, mingw_installer_reference=None, runner=None, args=None,
        ...
        super(DockerTestPackageRunner, self).__init__(profile, username, channel, logger=logger,
    ...
    def run(self):
        ...
        command = "sudo docker run --rm -v %s:/home/conan/project -v " \
                  ...
                  "cd project && sudo pip install conan_package_tools==0.3.0.dev3 --upgrade %s && " \
                  "run_in_docker\"" % (os.getcwd(), env_vars, self._docker_image, specific_conan_package)
    ...
    def pull_image(self):
        ...
        self._logger.info("Pulling docker image %s" % self._docker_image)
    ...
    def deserialize(data):
        ...
        ret = TestPackageRunner(profile, username=the_json["username"], channel=the_json["channel"],
                                logger=the_json["logger"],
                                mingw_installer_reference=the_json["mingw_installer_reference"],
                                runner=the_json["runner"], args=the_json["args"],
                                conan_pip_package=the_json["conan_pip_package"])
        return ret


def conan_compiler_info():
