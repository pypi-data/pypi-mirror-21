...
class ConanMultiPackager(object):
    ...
    def run_builds(self, curpage=None, total_pages=None):
        ...
        for build in builds_in_current_page:
            ...
            if self.use_docker:
                build_runner = DockerTestPackageRunner(profile, self.username, self.channel, self.logger,
                                                       self.mingw_installer_reference, self.runner, self.args,
                                                       docker_image=self.docker_image)
            else:
                build_runner = TestPackageRunner(profile, self.username, self.channel, self.logger,
                                                 self.mingw_installer_reference, self.runner, self.args)
...
%s
"""
    settings = "\n".join(["%s=%s" % (k, v) for k, v in build_conf.settings.items()])
...
#                             ("4.9", "x86", "dwarf2", "posix")]
    builder = ConanMultiPackager(username="lasote", mingw_configurations=None, use_docker=True)
    builder.add_common_builds(pure_c=False)
    builder.add(build_requires={"*": ["uno/1.0@lasote/stable", "dos/0.1@lasote/testing"], "polla": []})
    builder.run()
