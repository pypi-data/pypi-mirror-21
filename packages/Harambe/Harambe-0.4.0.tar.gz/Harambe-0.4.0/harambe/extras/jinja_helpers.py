...
import humanize
import arrow
from harambe import (init_app,
                   get_config,
                   to_local_datetime,
                   utils)


def format_local_datetime(dt, format=None):
    """
    Format local datetime
    :param dt: Arrow type or string
    :param format: string
    :return: string
    """
    DTF = get_config("DATETIME_FORMAT")
    if format is None:
        format = DTF.get("default") or "MM/DD/YYYY"
    else:
        format = DTF.get(format) or format
...
FILTERS = {
    ...
    "nl2br": nl2br,
    "local_datetime": format_local_datetime,
