...
def git_push_to_master(cwd, hosts, name="all", force=False, branch="master"):
    """
    To push to master
    :param cwd:
    :param hosts:
    ...
    with sh.pushd(cwd):
        ...
        _o = ["git", "push", name, branch]
        if force:
            _o.append("--force")
        _cmd = " ".join(_o)
        run_cmd("cd %s; %s" % (cwd, _cmd))
...
def init():
    ...
    harambepyfile = os.path.join(os.path.join(CWD, "harambepy.py"))
    ...
    if os.path.isfile(harambepyfile):
        ...
        print("- If necessary edit and run the command [ harambe db-sync ]")
        print("- Launch app on development mode, run [ harambe serve ]")
        print("")
        print("*" * 80)
...
def cmd():
    ...
    if os.path.isfile(harambepyfile):
        cwd_to_sys_path()
        application = import_string("harambepy")
    else:
        print("-" * 80)
        print("** Missing << 'harambepy.py' >> @ %s" % CWD)
        print("-" * 80)

    s1 = sys.argv[0].split("/")[-1]
    if "go" in s1 or "gorilla" in s1:
