...
def get_jwt_salt():
    ...
    :return: str
    """
    return __options__.get("jwt_salt", "harambe:contrib:aut")
...
def create_user(username, password, email=None, first_name=None, last_name=None,
    ...
    def cb():
        ...
        return _user(models.AuthUser.new(username=username,
                                         ...
                                         role=role))
    return signals.on_create_user(cb)
...
def login_user(user):
    ...
    def cb():
        if user:
            ...
            flask_login.login_user(UserModel(user))
            return user
        return None
...
class UserModel(flask_login.UserMixin):
    ...
    def change_username(self, username):
        ...
        def cb():
            ...
            if self.login_method == "email":
                ...
                self.user.change_username(username)
            return username
        return signals.on_username_changed(cb)
    ...
    def change_email(self, email):
        ...
        def cb():
            ...
            return email
        return signals.on_email_changed(cb)
...
class AuthCLI(CLI):
    def __init__(self, command, click):
        ...
        def create_super_admin(email):
            ...
            """
            print("-" * 80)
            print("Harambe Auth: Create Super Admin")
            print("Email: %s" % email)
            try:
                password = get_random_password()
                user = create_user(email=email, password=password, first_name="SuperAdmin", role="Superadmin")
        ...
        def reset_password(email):
            ...
            """
            print("-" * 80)
            print("Harambe Auth: Reset Password")
            try:
