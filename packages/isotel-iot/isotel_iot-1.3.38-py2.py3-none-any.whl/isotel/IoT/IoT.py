...
class Group:
    ...
    def __init__(self, server, group, timeout=10, version='v1', ssl_untrusted=True, ssl_cert_file=None,
        ...
        #Check version and backwards compatibility if necessary     
        try:
        ...
        #print(str(sver))    
        if not is_compatible_version(sver['api'], version):
...
class Device:
    ...
    def get_records(self, parameter, time_from=None, time_to='last',
                    limit=None, scale=None, average=None, rfunction=None, show=None):
        ...
        if rfunction:
            qp.append('rfunction=' + str(rfunction)) 
        if show:
