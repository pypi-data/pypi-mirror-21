...
class FilesTree(QtWidgets.QWidget):
    ...
    def itemDoubleClicked(self, item, col):
        ...
        fs = QtWidgets.QFileDialog.getOpenFileNames(self, 'Select {} files'.format(item.text(0)),
                                                    self.controller.workDir, 'Frames (*.cbf *.edf)')[0]
    ...
    def loadSetting(self):
        s = QtCore.QSettings()
        self.restoreGeometry(s.value('WFTree/Geometry', QtCore.QByteArray()))
        for item in self.itemSignals.keys():
            names = json.loads(s.value('WFTree/{}'.format(item.text(0)), '[]'))
            self.addSubItems(names, item)
            checked = int(s.value('WFTree/{}_checked'.format(item.text(0)), QtCore.Qt.Unchecked))
            # noinspection PyTypeChecker
    ...
    def saveSettings(self):
        ...
        for item in self.itemSignals.keys():
            ...
            s.setValue('WFTree/{}'.format(item.text(0)), json.dumps(names))
            s.setValue('WFTree/{}_checked'.format(item.text(0)), item.checkState(1))
