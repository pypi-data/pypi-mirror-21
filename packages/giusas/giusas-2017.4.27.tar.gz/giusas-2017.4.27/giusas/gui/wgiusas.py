...
class WGiuSAS(QtWidgets.QMainWindow, Ui_WGiuSAS):
    ...
    def loadSettings(self):
        s = QtCore.QSettings()
        self.restoreGeometry(s.value('WGiuSAS/Geometry', QtCore.QByteArray()))
        self.restoreState(s.value('WGiuSAS/State', QtCore.QByteArray()))
        version = int(s.value('WGiuSAS/version', 0))
        if version < 1:
            return
        dockState = s.value('WGiuSAS/dockState', None)
        if dockState:
