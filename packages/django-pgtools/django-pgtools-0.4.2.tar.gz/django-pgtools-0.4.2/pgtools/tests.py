...
EMPTY_ROLE_NAME = 'test-empty_role'
MASTER_ROLE_NAME = 'test-master_role'
TEST_USER_NAME1 = 'test_user-1'
TEST_USER_NAME2 = 'test_user-2'
...
class GrantUserCommandTestCase(CommandBaseTestCase):
    ...
    @patch('pgtools.management.commands.grantuser.get_models')
    def test__grant_many_to_many(self, mock_get_models):
        ...
        mock_get_models.return_value = [Bar, Baz]

        # make sure role exists
        if not DatabaseRole.exists('payments'):
            DatabaseRole('payments', create=True)

        cursor = connection.cursor()
        name = 'pgtools.management.commands.grantuser.connection.cursor'
        with patch(name) as mock_cursor:
            mock_cursor.return_value.cursor = cursor.cursor
            mock_cursor.return_value.fetchone.return_value = [
                'pgtools_bar_baz_id_seq'
            ]
            call_command('grantuser', 'payments')

        mock_execute = mock_cursor.return_value.execute
        expected_calls = [
            call('SELECT pg_get_serial_sequence(%s, %s)',
                 ('pgtools_bar_baz', 'id')),
            call('GRANT ALL ON "pgtools_bar_baz" TO "payments"'),
            call('GRANT ALL ON "pgtools_bar_baz_id_seq" TO "payments"'),
        ]

        for call_ in expected_calls:
            self.assertIn(call_, mock_execute.call_args_list)

    @patch('pgtools.management.commands.grantuser.get_models')
    def test__grant_many_to_many_with_sequence_full_name(
