...
def main():
    ...
    try:
        ...
        # Sampling algorithm
        samplingResult = samplingProvider.SamplingProvider(
            args.timeout,
            args.samplingFrom,
            args.samplingTo).DoSampling()

        # If there is less than 3 points result would be a completely bullshit
        if (len(samplingResult) < 3):
            print("""Sampling didn't gave enough points, can't determine complexity.
                 Try with lower value of samplingFrom""")
            return

        # Compute complexity
        complexity = resolver.ComplexityResolver.Resolve(samplingResult)
