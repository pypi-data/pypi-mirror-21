...
class ComplexityResolver:
    ...
    def CheckComplexity(points, exp, withLog):
        ...
        for i in points:
            n = i["N"]
            t = i["time"]
            nexp = ComplexityResolver.ComputeExponent(n, exp)
            if withLog is True:
                listA.append(t / nexp / math.log(n, 2))
            else:
                listA.append(t / nexp)
        ...
        logging.debug(
            "Exponent: {0}, computing with logaritm: {1}".format(exp, withLog))
        logging.debug(
            "Parameter values {0}".format(listA))
        logging.debug(
            "Average: {0}, biggestDif: {1}\n".format(avg, biggestDif))
