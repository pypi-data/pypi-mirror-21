...
def add_statsd_mapping_rule(statsd_host_pattern,
                            ...
                            data_type):
    """
    Adds a rule to how to map statsd counters to their ITculate equivalent (vertex_key, counter, data_type).

    The 'vertex_key_template' and 'counter_template' are used as a template with the 'counter', 'host',
    'tags' and 'instance_id' available as parameters.

    The 'statsd_counter', 'statsd_host' and 'instance_id' can include a wildcard character (e.g. "latency.*").
    These will be used to match the rules to the counter.

    Example:

    If the following sample arrived:
        statsd_counter = "users.online.latency"
        statsd_host = "host1"
        statsd_tags = {"tag1": "tagvalue1", "tag2": "tagvalue2"}
        instance_id = "instance1"

    And the rule definitions are:
        statsd_counter = "*.latency"
        statsd_host = "*"
        instance_id = "*"
        vertex_key_template = "$instance_id"
        counter_template = "$tags_tag1-$tags_tag2-$statsd_counter"
        data_type = LatencyDataType

    Then the following will be resolved:
        vertex_key = "instance1"
        counter = "tagvalue1-tagvalue2-users.online.latency"
        data_type = LatencyDataType

    :param str|None statsd_host_pattern: Host (IP) matching rule (can include '*' or '?')
    ...
    """
    mappings.add_statsd_mapping_rule(statsd_host_pattern=statsd_host_pattern,
                                     statsd_counter_pattern=statsd_counter_pattern,
                                     statsd_tags_patterns=statsd_tags_patterns,
...
def resolve_statsd_sample_keys(host, counter, tags=None, instance_id=None):
    """
    Given the StatsD information (including DD tags) 'keys', resolve to a set of (Vertex, Counter, DataType) tuples
    so we can report this to ITculate

    :param str host: Host reported by the agent
    :param str counter: StatsD counter name
    :param dict[str, str]|None tags: Set of tags (from DD)
    :param str|None instance_id: Optional additional identifier for the host (e.g. AWS Instance ID)

    :rtype: collections.Iterable[(str, str, DataType)]
    :return: A generator over the possible (typically one) vertices this counter should map to
    """
    return mappings.resolve_statsd_sample_keys(host=host,
                                               counter=counter,
                                               tags=tags,
