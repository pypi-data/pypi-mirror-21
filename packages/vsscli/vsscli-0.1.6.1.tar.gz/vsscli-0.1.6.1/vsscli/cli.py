...
from . import __version__, __config_file_path__, __env_vars__
...
def configure_list(ctx):
    ...
    if envs:
        ...
        passwd = os.environ.get(__env_vars__.get('pass'), '')
    ...
    base_endpoint = endpoint or cli_manager.base_endpoint
    masked_pwd = ''.join(['*' for i in range(len(passwd))])
    source = 'Environment Vars' if envs else 'Config File'
    click.echo('{:10s}:\t{}'.format('Source', source))
    click.echo('{:10s}:\t{}'.format('Endpoint', base_endpoint))
...
@click.option('--net', '-n', help='Networks moref mapped to NICs.',
              type=str, multiple=True, required=False)
@click.option('--domain', '-m', help='Target fault domain.',
              type=str, required=False)
@click.option('--custom-spec', '-p',
              help='Guest OS custom specification in JSON format.',
              type=str, required=False,
