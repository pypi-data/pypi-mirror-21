...
class Sift(object):    
    ...
    def __answer(self, results):
        ...
        #elif self.aggregation_type == self.config.SiftAggregationType.Operation:
        #    self.__choice_results = self.__reducer.reduce(results, self.filters, as_operation, self.returning_object_property)
        else:
