...
class MoteinoNetwork(object):
    ...
    def bind_all(self, receive=None, ack=None, no_ack=None):
        """
        Use this method to bind your own functions to be run when
        * receiving data
        * no ack was received
        * ack was received
        ...
        """
        for node in self.nodes_list:
            if node.ID == 0xFF or node.Name == "Base":
                continue

            if receive is not None:
                node.ReceiveFunction = receive
            if ack is not None:
                node.AckFunction = ack
            if no_ack is not None:
                node.NoAckFunction = no_ack
