...
class Access(object):
    ...
    def aggregate(self, url: AnyStr, aggregation: AggregationQuery) -> Payload:
        """
        ...
        :return:
        """
        # Query and url components
        base = url
        limit = 1000
        query = aggregation
        # Instantiating output
        aggregation_results = []
        # Loop controls
        page = 0
        max_page = 1
        ...
        while page < max_page:
            page += 1
            # Check if we have the most recent series of results
            if aggregation_results:
                pass
            # If we do not, we must add in our manual pagination
            else:
                sort = {
                    "$sort": {
                        "$_id": 1
    ...
    def get(self, url: AnyStr, query: Union[AggregationQuery, Query]=None, fields: Optional[Sequence[str]]=None,
            sort: AnyStr='', sync: bool=False, agg: bool=False, limit: int=1000) \
            -> Payload:
        """
