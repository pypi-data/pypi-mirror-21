...
class Runs():
    """ store of a group of runs
        each run includes classifier name, params, score, predictions
        
        logruns.pkl is pickled list of json
        preds0 relates to first row of logrun
    """
    ...
    def __init__(self, folder, mode="a"):
        ...
        self.file = os.path.join(folder, "logruns.pkl")
    ...
    def append(self, row, preds=None):
        ...
        # append row to log
        try:
            res = pd.read_pickle(self.file)
        except:
            res = pd.DataFrame()
        res = res.append(row, ignore_index=True)
        pd.to_pickle(res, self.file)

        # save predictions
        with open(os.path.join(self.folder, f"preds{len(res)-1}"), "wb") as f:
            np.save(f, preds)
