...
class Optuner():
    ...
    def maximise(self, target, num_evals, search=None, verbose=20,
        ...
        """     
        data = os.path.join(sys.prefix, "etc", "optimise")
        self.verbose = verbose
        if search is None:
            with open(os.path.join(data, "search.yaml")) as f:
                search = yaml.load(f)
        #pprint(search_space)
...
def get_search_space(target):
    ...
    if isinstance(target, Pipeline):
        ...
        target = [(name, clf.__class__.__name__) for name, clf in target.steps]
