...
class GoogleDriveStorage(UnicodeStorage):
    ...
    def _check_file_exists(self, filename, parent_id=None):
        ...
        if len(split_filename) > 1:
            ...
            page_size = 1000  # Max value admitted from google drive
            folders = self._drive_service.files().list(
            ...
            if parent_id is not None:
                q = "{0} and '{1}' in parents".format(q, parent_id)
            page_size = 1000  # Max value admitted from google drive
            file_list = self._drive_service.files().list(
                q=q, pageSize=page_size
            ).execute(num_retries=settings.GOOGLE_DRIVE_API_NUM_RETRIES)
            ...
            if file_data:
                file_properties = self._drive_service.files().get(
                    fileId=file_data['id'],
    ...
    def listdir(self, path):
        ...
        if folder_id:
            ...
            files_list = self._drive_service.files().list(
                **file_params
            ).execute(num_retries=settings.GOOGLE_DRIVE_API_NUM_RETRIES)
            dir_list = self._drive_service.files().list(
                **dir_params
            ).execute(num_retries=settings.GOOGLE_DRIVE_API_NUM_RETRIES)
