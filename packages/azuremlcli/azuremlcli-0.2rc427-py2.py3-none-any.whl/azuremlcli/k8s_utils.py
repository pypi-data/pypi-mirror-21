...
class KubernetesOperations:
    ...
    def get_cluster_name():
        ...
        """
        try:
            with open(path.join(path.expanduser('~'), '.kube', 'config')) as f:
                config = yaml.load(f)
    ...
    def delete_replica_set(self, deployment_name):
        try:
            ...
            get_rs_proc = Popen(['kubectl', 'get', 'rs'], stdout=PIPE)
            grep_named_rs_row_proc = Popen(['grep', deployment_name], stdin=get_rs_proc.stdout, stdout=PIPE)
            get_rs_proc.stdout.close()
            replica_set_row_b, err = grep_named_rs_row_proc.communicate()
            replica_set_row = replica_set_row_b.decode()

            # Grab deployment name from table row
            replicaset, _, _ = replica_set_row.partition(" ")
            check_call(['kubectl', 'delete', 'rs', replicaset])
    ...
    def create_service(self, service_yaml, webservicename, webservice_type):
        try:
            k8s_core = client.CoreV1Api()
            namespace = 'default'
            with open(path.join(path.dirname(__file__), service_yaml)) as f:
                dep = yaml.load(f)
