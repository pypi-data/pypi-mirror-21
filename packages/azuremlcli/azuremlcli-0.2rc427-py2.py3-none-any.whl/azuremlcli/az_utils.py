...
def validate_env_name(name):
    ...
    if not bool(re.match('^[a-z0-9]+$', name)):
        raise InvalidNameError('Name must only contain lowercase alphanumeric characters.')
...
def az_check_components(component_list):
    ...
    """
    try:
        az_components = subprocess.check_output(['az', 'component', 'list', '-o', 'json']).decode('ascii')
    except subprocess.CalledProcessError as exc:
        raise AzureCliError('Error checking for installed components of the Azure CLI. Please ensure you have'
                            ' installed the Azure CLI before setting up your environment ("pip install azure-cli").')

    try:
        az_components = json.loads(az_components)
    except ValueError:
        raise AzureCliError('Invalid response from Azure CLI. Please report this to deployml@microsoft.com with'
                            ' the following output: {}'.format(az_components))

    installed_components = [x['name'] for x in az_components if x['name'] in component_list]
    missing_components = list(set(component_list) - set(installed_components))
    if missing_components:
        print('Please install missing Azure CLI components using the following commands:')
        print('sudo -i')
...
def az_check_subscription():
    ...
    try:
        az_account_result = subprocess.check_output(['az', 'account', 'show', '-o', 'json']).decode('ascii')
...
def az_create_resource_group(context, root_name):
    ...
    try:
        rg_exists = subprocess.check_output(['az', 'group', 'exists', '-n', rg_name, '-o', 'json'])
    ...
    if not rg_exists or (rg_exists and rg_exists != 'true'):
        ...
        try:
            output = subprocess.check_output(
                ['az', 'group', 'create', '-l', context.aml_env_default_location, '-n', rg_name])
        except subprocess.CalledProcessError as exc:
    ...
    while not registered:
        try:
            show_output = subprocess.check_output(
                ['az', 'provider', 'show', '-n', namespace, '-o', 'json'], stderr=subprocess.STDOUT)
...
def az_create_storage_account(context, root_name, resource_group, salt=None):
    ...
    try:
        ...
        storage_create_output = subprocess.check_output(
            ['az', 'storage', 'account', 'create', '-g', resource_group, '-l', context.aml_env_default_location, '-n',
             storage_account_name, '--sku', 'Standard_LRS', '-o', 'json'], stderr=subprocess.STDOUT).decode('ascii')
    except subprocess.CalledProcessError as exc:
...
def az_create_acr(context, root_name, resource_group, storage_account_name, salt=None):
    ...
    try:
        acr_create = subprocess.Popen(
            ['az', 'acr', 'create', '-n', acr_name, '-l', context.aml_env_default_location, '-g', resource_group,
             '--storage-account-name', storage_account_name, '--admin-enabled', 'true', '-o', 'json', '--sku', 'Basic'],
        ...
        if err:
            ...
            if 'already in use' in result:
                ...
                salt = str(uuid.uuid4())[:6]
                return az_create_acr(context, root_name, resource_group, storage_account_name, salt)
            else:
                try:
                    ...
                    raise AzureCliError(
                        'Error creating ACR. Please try again later. Response from az was not json: {}.' +
                        '\n {}'.format(
                            result, output))
    ...
    try:
        acr_credentials = subprocess.check_output(['az', 'acr', 'credential', 'show', '-n', acr_name, '-o', 'json'])
    except subprocess.CalledProcessError:
...
def az_create_acs(root_name, resource_group, acr_login_server, acr_username, acr_password, ssh_public_key):
    ...
    print("To check the status of the deployment, run the following command:\n  aml env setup -s {}\n".format(deployment_name))
...
def az_parse_acs_outputs(completed_deployment):
    ...
    if 'properties' not in completed_deployment or 'outputs' not in completed_deployment['properties']:
        raise AzureCliError(
    ...
    if 'value' not in completed_deployment['properties']['outputs']['masterFQDN'] \
            or 'value' not in completed_deployment['properties']['outputs']['agentpublicFQDN']:
        raise AzureCliError(
            'Malformed output in deployment. Please report this to deployml@microsoft.com, with the following json in your error report: {}'  # pylint: disable=line-too-long
                .format(json.dumps(completed_deployment)))
...
def az_get_app_insights_account(completed_deployment):
    ...
    if 'properties' not in completed_deployment or 'parameters' not in completed_deployment['properties']:
        az_throw_malformed_json_error(completed_deployment)
    
    if 'appName' not in completed_deployment['properties']['parameters'] or 'value' not in completed_deployment['properties']['parameters']['appName']:
        az_throw_malformed_json_error(completed_deployment)
    ...
    try:
        app_insights_get_output = subprocess.check_output(
            ['az', 'resource', 'show', '-g', resource_group, '--resource-type', 'microsoft.insights/components','-n', resource_name],
            stderr=subprocess.STDOUT).decode('ascii')
    except subprocess.CalledProcessError as exc:
        raise AzureCliError('Error getting created app insights account. Error details {}'.format(
            exc.output.decode('ascii')))

    try:
        app_insights_get_output = json.loads(app_insights_get_output)
        
        if 'properties' in app_insights_get_output and 'InstrumentationKey' in app_insights_get_output['properties']:
            return resource_name, app_insights_get_output['properties']['InstrumentationKey']
...
def az_create_app_insights_account(context, root_name, resource_group):
    ...
    parameters = json.loads(resource_string(__name__, 'data/appinsightstemplateparameters.json').decode('ascii'))
    deployment_name = resource_group + 'deploymentappinsights' + datetime.datetime.now().strftime('%Y%m%d%I%M%S')
    ...
    try:
        ...
        raise AzureCliError('Error creating App Insights Account from template. Error details {}'.format(exc.output.decode('ascii')))
...
def az_check_template_deployment_status(deployment_name):
    ...
    try:
        deployment_status = subprocess.check_output(
            ['az', 'group', 'deployment', 'show', '-g', resource_group, '-n', deployment_name, '-o', 'json'],
            stderr=subprocess.STDOUT).decode('ascii')
    except subprocess.CalledProcessError as exc:
    ...
    if 'properties' not in deployment_status or 'provisioningState' not in deployment_status['properties']:
        raise AzureCliError(
            'Error retrieving deployment status. Returned object from az cli: {}'.format(json.dumps(deployment_status)))

    if deployment_status['properties']['provisioningState'] != 'Succeeded':
        print('Deployment status: {}'.format(deployment_status['properties']['provisioningState']))
        return
...
def az_install_kubectl(context):
    ...
    try:
        ...
        print('An error occurred when trying to install kubectl. {}'.format(exc))
        return False
...
def az_get_active_email():
    ...
    if 'user' in az_account_result:
        if 'name' in az_account_result['user']:
            return az_account_result['user']['name']
    else:
        raise AzureCliError('Unable to locate email address for active azure subscription.')
