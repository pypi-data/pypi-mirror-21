...
class SurfaceStatsCollector(object):
    ...
    def __init__(self, device, focuse_name=None):
        self._device = device
        ...
        self._focuse_name = focuse_name or self._device.getFocusedPackageAndActivity()
    ...
    def _GetSurfaceFlingerFrameData(self):
        ...
        results = self._device.script(
            'adb shell dumpsys SurfaceFlinger --latency ' + self._focuse_name).split("\r\n")
        if not len(results):
            return (None, None)
        # print results
        timestamps = []
