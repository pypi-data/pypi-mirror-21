...
class Device(object):
    ...
    def find_elements(self, _type, value):
        ...
        for xmlElement in xml_elements:
            if xmlElement.TypeName[_type] == value:
                ...
                _x_point_ = (int(coord[2]) - int(coord[0])) / 2.0 + int(coord[0])
                _y_point_ = (int(coord[3]) - int(coord[1])) / 2.0 + int(coord[1])
                # 将匹配的元素区域的中心点添加进pointList中
                element = Element(_x_point_, _y_point_, self)
    ...
    def find_type_element(self, _type, value):
        ...
        for xmlElement in xml_elements:
            if xmlElement.attrib[_type] == value:
                ...
                element._x_point_ = (int(bound[0]) + int(bound[2])) / 2
                element._y_point_ = (int(bound[1]) + int(bound[3])) / 2
    ...
    def find_index_path_element(self, value):
        ...
        pattern = re.compile(r"\d+")
        bound = pattern.findall(bounds)
        _x_point_ = (int(bound[0]) + int(bound[2])) / 2
        _y_point_ = (int(bound[1]) + int(bound[3])) / 2
