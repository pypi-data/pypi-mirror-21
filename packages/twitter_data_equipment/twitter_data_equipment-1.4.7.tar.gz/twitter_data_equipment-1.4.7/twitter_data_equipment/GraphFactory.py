...
from twitter_data_equipment.ForceAtlas2 import *
from twitter_data_equipment.Information import *
...
class GraphFactory:
    ...
    def __init__(self, name_of_graph):
        ...
        self.pruning_information = self.PruningInfo()
    ...
    def write_graphml(self, output):
        """
        Writes the graph-object to file in the from of a Graphml-File
        
        :param str output: Path to destination of Graphml-File
        
        """
        print("Writing Graph to " + str(output))
    ...
    def print_info(self):
        ...
        print('Average degree: ' + str(float(sum(self.graph.degree().values())) / float(self.graph.number_of_nodes())))

        avg_sum = 0
        for _, data in self.graph.nodes(data=True):
            avg_sum += data['weight']

        print('Average weight: ' + str(float(avg_sum) / float(self.graph.number_of_nodes())))
        print('--------------------------------------')
    ...
    def prune_graph(self, edge_weight, degree_thresh, weight_thresh, remove_isolates):
        ...
        """
        print("Pruning Graph")
        print(".......... Removing self refering edges")
        ...
        print(".......... Removing nodes according to degree and weight")
    ...
    class PruningInfo():
        ...
        def to_string(self):
            """
            Prints out all the info in one string
            """
    ...
    def create_graphs_from_dataframe(self, on_key, path_to_csv):
        ...
        applied the graph needs to be generated with this method. Or else there will be no dataframe to analyze.
    ...
    def draw_graph(self,
        ...
        if values:
            ...
            plt.legend(loc='center left', prop={'size': 6},bbox_to_anchor=(-0.1, 0.5))
...
def calculate_betweenness(graph):
    ...
    node_partitions = list(partitions(graph.nodes(), int(len(graph) / part_generator)))
