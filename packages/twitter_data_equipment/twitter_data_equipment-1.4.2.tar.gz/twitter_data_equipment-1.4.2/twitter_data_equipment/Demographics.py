...
def add_demographic_statistics(input_path, output_path):
    ...
    df_racial_data = pd.read_csv(open('twitter_data_equipment/data_sheets/race_data.csv'))
    df_poverty_data = pd.read_csv(open('twitter_data_equipment/data_sheets/poverty_data.csv'),
                                  usecols=['State FIPS', 'County FIPS', 'Name', 'Poverty Percent All Ages'])
    df_population_data = pd.read_csv(open('twitter_data_equipment/data_sheets/population_data.csv'), delimiter=';')
