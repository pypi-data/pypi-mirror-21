...
class calibrationData(object):
    ...
    def __init__(self, calsToKeep=10):
    ...
    def addRefCal(self, value, Coarse):
        ...
        if(value != 0):
            if(((abs(self.getRefCal(Coarse) - value) / value) * 100 < 30) or not self.calibrated): #Workaround for beta firmware issue
                if(Coarse):
    ...
    def addZeroCal(self, value, Coarse):
        ...
        if(value != 0):
            if(((abs(self.getZeroCal(Coarse) - value) / value) * 100 < 30) or not self.calibrated): #Workaround for beta firmware issue
                if(Coarse):
