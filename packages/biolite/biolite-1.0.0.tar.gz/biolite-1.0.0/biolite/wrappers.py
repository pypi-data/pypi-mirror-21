...
class Mcxload (BaseWrapper):
	"""
	Markov Clustering Algorithm (MCL) for analysis of networks
	http://micans.org/mcl/
	"""

	def __init__(self, format, input, *args, **kwargs):
		self.init('mcxload', **kwargs)
		self.version('--version')
...
class Raxml (BaseWrapper):
	...
	def __init__(self, input, *args, **kwargs):
		self.init('raxml-pthreads', **kwargs)
		self.version('-v')
		self.args += ['-s', input]
		self.add_threading('-T')
		self.args += args
...
class RsemEvalCalculateScore (BaseWrapper):
	"""
	http://deweylab.biostat.wisc.edu/detonate/
	"""

	def __init__(self, inputs, assembly, prefix, *args, **kwargs):
		self.init("rsem-eval-calculate-score", **kwargs)
		self.add_threading("--num-threads")
		mean_insert = kwargs.get("mean_insert")
		if len(inputs) == 2:
			if mean_insert is None:
				mean_insert = diagnostics.lookup_insert_size().mean
			self.args.append("--paired-end")
		self.args += args
		for input in inputs:
			if input.endswith(".gz"):
				...
				self.check_input(None, input)
		self.check_input(None, assembly)
		self.args += (prefix, mean_insert)
		self.output_patterns = map(diagnostics.OutputPattern._make, [
...
class Sumtrees (BaseWrapper):
	def __init__(self, target, trees, *args, **kwargs):
		self.init('sumtrees', **kwargs)
		self.version('-v')
		self.add_threading('-m')
		self.args += args
		self.check_input("--target", target)
		for tree in trees:
			self.check_input(None, tree)
		self.run()
...
class TransDecoderLongOrfs (BaseWrapper):
	"""
	...
	def __init__(self, input, **kwargs):
		self.init("TransDecoder.LongOrfs", **kwargs)
...
class Trinity (BaseWrapper):
	...
	def __init__(self, inputs, *args, **kwargs):
		...
		self.args += ("--max_memory", "%dG" % max(1, int(0.8*mem/1024)))
