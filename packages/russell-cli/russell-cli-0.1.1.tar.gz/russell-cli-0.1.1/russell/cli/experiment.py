...
def create(project):
    ...
    russell_logger.info("Project \"{}\" initialized in current directory".format(project))
...
def logs(id, url, tail, sleep_duration=1):
    ...
    log_url = "{}/api/v1/log/{}?content=instance".format(russell.russell_host, id)
    if url:
        russell_logger.info(log_url)
        return
    if tail:
        russell_logger.info("Launching job ...")
        ...
        while True:
            ...
            if len(print_output.strip()):
                russell_logger.info(print_output)
            current_shell_output = log_file_contents
            sleep(sleep_duration)
    else:
        log_file_contents = get_url_contents(log_url)
        if len(log_file_contents.strip()):
            russell_logger.info(log_file_contents)
        else:
            russell_logger.info("Launching job now. Try after a few seconds.")
...
def output(id, url):
    ...
    if "output" in task_instance.output_ids:
        output_dir_url = "{}/api/v1/resources/{}?content=true".format(russell.russell_host,
