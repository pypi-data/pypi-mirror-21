...
from pynlple.data.corpus import IteratorSource
from pynlple.data.source import Source, SkippingSource
from pynlple.exceptions import DataSourceException


class ElasticJsonDataSource(SkippingSource, IteratorSource):
...
class YSProdElasticSource(ElasticJsonDataSource):
    ...
    URLS = ['ys-es-prod-{}.yscan.local'.format(str(num)) for num in range(11,20)]
    PORT = 9200
...
class ServerJsonDataSource(Source, IteratorSource):
...
class FileJsonDataSource(Source, IteratorSource):
    """Class for providing json data from json files."""
