...
* Added ``-v`` option to ``docker-compose down`` to automatically remove all
  ephemeral volumes after tests run.  This prevents disk space usage buildup on
  build machines that repeat this cycle very often with containers that write
  to disk (e.g. databases).
