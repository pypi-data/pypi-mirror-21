...
class WCryo(QtWidgets.QDialog, Ui_WCryo):
    ...
    def setSignalOnHold(self, action, signal):
        if signal:
            self.resume()
            self.action = action
            # We have to wait because communication with cryostream takes at least 1 sec
            QtCore.QTimer.singleShot(int(Config.CommTimeout), lambda: setattr(self, 'sigWaitOnHold', signal))
    ...
    def loadSettings(self):
        s = Config.Settings
        self.restoreGeometry(s.value('WCryo/Geometry', Config.QBA))
