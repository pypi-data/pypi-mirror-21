...
class WLakeshore(QtWidgets.QDialog, Ui_WLakeshore):
    ...
    def on_startButton_clicked(self):
        ...
        self.aList = []
        self.sigConnect.emit(output, sensor, host, self.DefaultPort)
    ...
    def setSignalOnHold(self, action, signal):
        if signal:
            ...
            timeout = int(self.config.commTimeout) if self.config else self.DefaultTimeout
            # We have to wait because communication with lakeshore takes at least 1 sec
            QtCore.QTimer.singleShot(int(self.config.commTimeout), lambda: setattr(self, 'signalOnHold', signal))
    ...
    def setScriptParams(self, params, now):
        ...
        if setp is not None:
            self.setpSpinBox.setValue(setp)
        if isinstance(rnge, (int, float)) and rnge >= 0:
            self.rangeComboBox.setCurrentIndex(rnge)
