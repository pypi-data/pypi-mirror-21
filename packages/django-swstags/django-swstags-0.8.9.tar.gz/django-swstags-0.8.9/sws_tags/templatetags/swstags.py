...
def tag_grid_dict(view_name,
	...
	conf_option_default['separator'] = {'EUR': {'thousand': '.', 'decimal': ','}, 'USD': {'thousand': '.', 'decimal': ','}}
	conf_option_default['list_field'] = ''
	...
	if type(conf_data) == list:
		if len(conf_data[0]) > 3:
			...
			col_types=[]
			for i in conf_data:				
				col_name.append(str(unicode(i[1])))
				col_model.append(i[0])
				col_len.append(i[4])
				col_align.append(i[5])
				col_types.append(i[6])
			try:
	...
	if 'separator' not in conf_option:
		conf_option['separator'] = {'thousand': '.', 'decimal': ','}

	if 'list_field' not in conf_option:
		conf_option['list_field'] = []
	...
	return {'view_name':view_name,
			...
			'separator':conf_option['separator'],
			'fields_grid':conf_option['list_field'],
			}
...
def tag_highcharts(view_name,data_highcharts, plot_options = {},chart={}):
	...
	min_yAxis = data_highcharts.get('min_yAxis', 0)
	...
	decimal_y = None
	if 'decimal_y' in data_highcharts:
		decimal_y = data_highcharts['decimal_y']
	yAxis = get_yAxis(v_name_y, v_dataname, decimal_y)
...
def tag_interruptor(view_name,
	...
	return {'view_name': view_name,
			...
			'auto_interruptor': auto_interruptor}
...
def getGridConfig(queryset,adjustment_factor,col_order='false',middle_col = 'false',align='center'):
	...
	if type(queryset) == django.db.models.query.QuerySet:

		if queryset and type(queryset[0]) is dict:
			print queryset[0]
	...
	# Instead of a queryset we receive a list of tuples ('Name','field')
		...
		try:
			if type(queryset[0]) in (tuple, list):
				for i in queryset:
					col_name.append(str(unicode(i[1])))
					col_model.append(i[0])
					col_len.append(i[4])
	...
	if middle_col == 'false':
		...
		if type(queryset) == django.db.models.query.QuerySet:
			try:
				...
				logger.error('getGridConfig: error in asignation len a column of grid')
		elif type(queryset) in (list, dict):
			if len(col_len)>0:
				try:
					for i in range(0,len(col_model)):
						dic_col_model.append({'name':col_model[i],
				...
				for col in col_model:
					for i,k in col.items():
						if k == i_order:
							...
							col_name_order.append(col_name[col_number])
					col_number += 1
