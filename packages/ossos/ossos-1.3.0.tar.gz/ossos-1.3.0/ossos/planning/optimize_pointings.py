...
def is_up(coordinate, current_time):
    ...
    fb_rise_time < current_time and fb_set_time > current_time

    if ((fb_rise_time < current_time and fb_set_time < current_time) or
            (fb_rise_time > current_time and fb_set_time < current_time)):
        return False
    return True
...
def main():
    ...
    with open(pointings_filename, 'w') as pobj:
        ...
        for token in best_pointing_list:
            ...
            pointing_number += 1
            create_ephemeris_file(token, best_pointing_list[token][0], best_pointing_list[token][1], orbits, pointing_date, args.runid)
