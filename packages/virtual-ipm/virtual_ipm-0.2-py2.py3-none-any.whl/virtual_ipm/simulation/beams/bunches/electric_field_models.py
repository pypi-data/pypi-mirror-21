...
from anna import Duplet, Triplet, Integer, Number, PhysicalQuantity, parametrize
...
from .shapes import ParabolicEllipsoid
...
class BunchElectricFieldModel(Model):
    ...
    It should be normalized to a bunch population of 1 (rescaling happens externally).

    .. note::
    If required the bunch magnetic field is computed via a Lorentz transformation from
    the bunch electric field.
    """
...
class AnalyticalSolutionForRadiallySymmetricGaussianBunch(BunchElectricFieldModel):
    ...
    def __init__(self, bunch_shape, device, configuration=None):
        ...
        configuration : :class:`ConfigurationAdaptor` derived instance, optional
        """
        super(AnalyticalSolutionForRadiallySymmetricGaussianBunch, self).__init__(
            bunch_shape, device, configuration
        )
...
class BassettiErskine(BunchElectricFieldModel):
    ...
    .. note::
       The field is evaluated "live" (i.e. no pre-computation).

    References
    ----------
    M.Bassetti & G.A.Erskine: "Closed Expression for the Electrical Field of
    a Two-Dimensional Gaussian Charge", CERN-ISR-TH/80-06, Geneva (CERN), 1980
    
    See Also
    --------
    :class:`BassettiErskineOnGrid`
    """
    ...
    def compute_field_at(x_signed, y_signed, sigma_x, sigma_y):
        ...
        def w_function(z):
            ...
            References
            ----------
            Abramowitz, Stegun: Handbook of Mathematical Functions, National Bureau of Standards,
            Applied Mathematics Series 55, 1972 [Equation 7.1.3]
            """
...
@parametrize(
    PhysicalQuantity('GridSpacingX', 'm'),
    PhysicalQuantity('GridSpacingY', 'm')
)
class BassettiErskineOnGrid(BassettiErskine):
    """
    This class involves pre-computation of the field values on the specified grid and
    evaluation is based on interpolation of those cached values.

    See Also
    --------
    :class:`BassettiErskine`
    """

    SIGMA_DIFFERENCE_THRESHOLD = 1.0e-6

    @injector.inject(device=di.models.device)
    def __init__(self, bunch_shape, device, configuration):
        ...
        bunch_shape : :class:`BunchShape`
        device : :class:`DeviceManager`
        configuration : :class:`ConfigurationAdaptor` derived instance
        """
        super(BassettiErskineOnGrid, self).__init__(bunch_shape, device, configuration)

        if abs(bunch_shape.sigma[0] - bunch_shape.sigma[1]) < self.SIGMA_DIFFERENCE_THRESHOLD:
            raise ValueError(
                'Difference in sigma_x and sigma_y must not be smaller than %e'
                % self.SIGMA_DIFFERENCE_THRESHOLD
            )
        ...
        self.log.debug(
            'Using the following grid spacing: (%e, %e)',
            self._grid_spacing_x, self._grid_spacing_y
        )

        # Two extra grid points out of bounds to ensure that a tracked particle is always
        # between two grid points.
        self.nx = int((self._device.x_max - self._device.x_min) / self._grid_spacing_x) + 2
        self.ny = int((self._device.y_max - self._device.y_min) / self._grid_spacing_y) + 2
        self.xx = numpy.linspace(
            self._device.x_min - self._grid_spacing_x,
            self._device.x_max + self._grid_spacing_x,
            self.nx
        )
        self.yy = numpy.linspace(
            self._device.y_min - self._grid_spacing_y,
            self._device.y_max + self._grid_spacing_y,
            self.ny
        )

        self.Ex = numpy.empty(shape=(len(self.xx), len(self.yy)), dtype=float)
        self.Ey = numpy.empty(shape=(len(self.xx), len(self.yy)), dtype=float)
        self.Ez_placeholder = numpy.zeros(shape=(1,), dtype=float)

        self.x_min, self.y_min = min(self.xx), min(self.yy)
        self.sigma_z = self._bunch_shape.sigma[2]
        self.sqrt_2pi = numpy.sqrt(2. * numpy.pi)

    def as_json(self):
        attributes = super(BassettiErskineOnGrid, self).as_json()
        attributes.update({
            'number of grid points along x': self.nx,
            'number of grid points along y': self.ny,
            'grid spacing along x': '%e' % self._grid_spacing_x,
            'grid spacing along y': '%e' % self._grid_spacing_y,
            'boundaries along x': ('%e' % min(self.xx), '%e' % max(self.xx)),
            'boundaries along y': ('%e' % min(self.yy), '%e' % max(self.yy)),
            'minimum / maximum field in x': (
                '%e' % numpy.min(self.Ex),
                '%e' % numpy.max(self.Ex)
            ),
            'minimum / maximum field in y': (
                '%e' % numpy.min(self.Ey),
                '%e' % numpy.max(self.Ey)
            ),
        })
    ...
    def prepare(self):
        """
        Pre-compute the field on the specified grid. The device boundaries are chosen as the
        grid boundaries.
        """
        super(BassettiErskineOnGrid, self).prepare()
        sigma_x, sigma_y = self._bunch_shape.sigma[:2]
        self.log.debug('Computing field for sigma = (%e, %e)', sigma_x, sigma_y)
        self.log.debug('Grid along x: %s', self.xx)
        self.log.debug('Grid along y: %s', self.yy)

        # Let y be the "fast" index.
        ys, xs = numpy.meshgrid(self.yy, self.xx)
        self.Ex, self.Ey = self.compute_field_at(xs, ys, sigma_x, sigma_y)

    def interpolate_field_at_scalars_only(self, x, y):
        """
        Interpolate the field at the given position(s).

        Parameters
        ----------
        x : float or 1d-:class:`~numpy.ndarray`
            In the bunch frame, in units of [m].
        y : float or 1d-:class:`~numpy.ndarray`
            In the bunch frame, in units of [m].
        ...
        if numpy.any(x < self.x_min) or numpy.any(y < self.y_min):
            ValueError(
                'Position (%e, %e) is out of bounds (%e, %e). This leads to invalid array access.'
                % (x, y, self.x_min, self.y_min)
            )

        float_index_x = (x - self.x_min) / self._grid_spacing_x
        int_index_x = int(float_index_x)
        remainder_x = float_index_x - float(int_index_x)

        float_index_y = (y - self.y_min) / self._grid_spacing_y
        int_index_y = int(float_index_y)
        remainder_y = float_index_y - float(int_index_y)

        def interpolate(field):
            return (
                field[int_index_x, int_index_y] * (1. - remainder_x) * (1. - remainder_y)
                + field[int_index_x+1, int_index_y] * remainder_x * (1. - remainder_y)
                + field[int_index_x, int_index_y+1] * (1. - remainder_x) * remainder_y
                + field[int_index_x+1, int_index_y+1] * remainder_x * remainder_y
            )

        return interpolate(self.Ex), interpolate(self.Ey)

    def interpolate_field_at_legacy_port(self, x, y):
        """
        Interpolate the field at the given position(s).

        .. warning::
        This is a legacy port for the original Fortran routine (present in PyECLOUD-BGI) to work
        with numpy arrays. This method does not work with scalars.
        ...
        float_index_x = (x - self.x_min) / self._grid_spacing_x
        int_index_x = float_index_x.astype(int)
        remainder_x = float_index_x - int_index_x.astype(float)

        float_index_y = (y - self.y_min) / self._grid_spacing_y
        int_index_y = float_index_y.astype(int)
        remainder_y = float_index_y - int_index_y.astype(float)
    ...
    def interpolate_field_at_using_searchsorted(self, x, y):
        """
        Interpolate the field at the given position(s).

        .. note::
        This method uses :func:`numpy.searchsorted` internally to determine the grid cells which
        ...
        if numpy.any(x < self.x_min) or numpy.any(y < self.y_min):
            raise ValueError(
                'Position (%s, %s) is out of bounds (%e, %e). This leads to invalid array access.'
                % (x, y, self.x_min, self.y_min)
            )

        index_x = numpy.searchsorted(self.xx, x, side=str('right')) - 1
        remainder_x = (x - self.xx[index_x]) / (self.xx[index_x + 1] - self.xx[index_x])

        index_y = numpy.searchsorted(self.yy, y, side=str('right')) - 1
        remainder_y = (y - self.yy[index_y]) / (self.yy[index_y + 1] - self.yy[index_y])

        def interpolate(field):
            return (
                field[index_x, index_y] * (1. - remainder_x) * (1. - remainder_y)
                + field[index_x + 1, index_y] * remainder_x * (1. - remainder_y)
                + field[index_x, index_y + 1] * (1. - remainder_x) * remainder_y
                + field[index_x + 1, index_y + 1] * remainder_x * remainder_y
            )

        return interpolate(self.Ex), interpolate(self.Ey)

    interpolate_field_at = interpolate_field_at_legacy_port

    # noinspection PyPep8Naming
    @use_docs_from(BassettiErskine)
    def eval(self, position):
        x, y, z = position
        Ex, Ey = self.interpolate_field_at(x, y)
        ...
        field_vector = numpy.array((Ex, Ey, Ez))
        field_vector *= (
            constants.elementary_charge / (self.sigma_z * self.sqrt_2pi)
            * numpy.exp(- z**2 / (2. * self.sigma_z**2))
        )
        return field_vector


@parametrize(
    Duplet[Integer]('GridSize'),
    # Default parameters are from original IPMSim3D code.
    # www.physics.buffalo.edu/phy410-505/2011/topic3/app1/index.html gives a hint for
    # an optimal relaxation factor, namely: 2.0 / (1.0 + (pi / nx)); where `nx` is
    # the number of grid points in x-direction.
    Number('ConvergenceLimit', default=1.0e-6),
    Number('RelaxationFactor', default=1.9)
)
class PoissonSOR2DGauss(BunchElectricFieldModel):
    """
    ...
    def __init__(self, bunch_shape, device, configuration):
        super(PoissonSOR2DGauss, self).__init__(bunch_shape, device, configuration)

        self._bunch_shape = bunch_shape

        self._nx, self._ny = self._grid_size

        # Grid range is +-3 sigma.
        self._xmin = -3 * bunch_shape.sigma[0]
        self._xmax = 3 * bunch_shape.sigma[0]
        self._ymin = -3 * bunch_shape.sigma[1]
        self._ymax = 3 * bunch_shape.sigma[1]
    ...
    def as_json(self):
        attributes = super(PoissonSOR2DGauss, self).as_json()
        attributes.update({
    ...
    def prepare(self):
        ...
        """
        super(PoissonSOR2DGauss, self).prepare()
    ...
    def eval(self, positions):
        ...
        Ex = numpy.empty(shape=positions.shape, dtype=float)
        Ey = numpy.empty(shape=positions.shape, dtype=float)
        ...
        field_vector = numpy.array((Ex, Ey, Ez))
        return field_vector * constants.elementary_charge


@parametrize(
    Triplet[Integer]('GridSize')
)
class Poisson3D(BunchElectricFieldModel):
    ...
    def prepare(self):
        ...
        mesh = BoxMesh(min_corner, max_corner, *self._grid_size)
    ...
    def eval(self, positions):
        ...
        for index, position in enumerate(positions_transposed):
            Ex[inside][index] = self._Ex(position)
            Ey[inside][index] = self._Ey(position)
            Ez[inside][index] = self._Ez(position)
        self.log.debug('Ex: %s', Ex)
        return numpy.r_[str('0,2'), Ex, Ey, Ez] * self._e_over_eps0


# noinspection PyPep8Naming
class ElectricFieldOfParabolicEllipsoid(BunchElectricFieldModel):
    """
    ...
    def __init__(self, bunch_shape, device, configuration):
        super(ElectricFieldOfParabolicEllipsoid, self).__init__(bunch_shape, device, configuration)

        if not isinstance(bunch_shape, ParabolicEllipsoid):
            raise TypeError(
                'This field models requires an instance of %s as bunch shape'
                % ParabolicEllipsoid.__name__
            )

        self._bunch_shape = bunch_shape

        P4 = ElectricFieldOfParabolicEllipsoid.P4
        P4_prime = ElectricFieldOfParabolicEllipsoid.P4_prime
        Q4 = ElectricFieldOfParabolicEllipsoid.Q4
        Q4_prime = ElectricFieldOfParabolicEllipsoid.Q4_prime

        P2 = ElectricFieldOfParabolicEllipsoid.P2
        P2_prime = ElectricFieldOfParabolicEllipsoid.P2_prime
        Q2 = ElectricFieldOfParabolicEllipsoid.Q2
        Q2_prime = ElectricFieldOfParabolicEllipsoid.Q2_prime

        Q0 = ElectricFieldOfParabolicEllipsoid.Q0
        Q0_prime = ElectricFieldOfParabolicEllipsoid.Q0_prime
    ...
    def transform_3d_polar_coordinates_to_2d_elliptical_coordinates(positions, coefficients):
        ...
        print('xi out of bounds: ', xi[xi - 1. <= -1e-14] - 1.)
        assert numpy.all(
            (1. - xi[xi < 1.])
            < 1e-13
    ...
    def E_field_vector_inside_in_polar_basis_from_elliptical_coordinates_xi_equals_eta(
        ...
        e_field : :class:`numpy.ndarray`, shape (2, N)
            ...
            given (in that order) because E_phi is zero.
        """
        Di_eta = ElectricFieldOfParabolicEllipsoid.Di_eta
        c = coefficients['c']
        return numpy.stack((
    ...
    def E_field_vector_inside_in_polar_basis_from_elliptical_coordinates(xi, eta, coefficients):
        ...
        Di_eta = ElectricFieldOfParabolicEllipsoid.Di_eta
        Di_xi = ElectricFieldOfParabolicEllipsoid.Di_xi
        c = coefficients['c']
    ...
    def E_field_vector_outside_in_polar_basis_from_elliptical_coordinates(xi, eta, coefficients):
        ...
        Do_eta = ElectricFieldOfParabolicEllipsoid.Do_eta
        Do_xi = ElectricFieldOfParabolicEllipsoid.Do_xi
        c = coefficients['c']
        E_r = sqrt(xi ** 2 - 1.) * sqrt(1. - eta ** 2) / (c * (xi ** 2 - eta ** 2)) * (
    ...
    def E_ipe_safeguard(xi, eta, coefficients):
        ...
        xi_equals_eta = (xi == eta)
        result[xi_equals_eta] = ElectricFieldOfParabolicEllipsoid.E_ipe_xi_equals_eta(
            xi, eta, coefficients
        )
        result[~xi_equals_eta] = ElectricFieldOfParabolicEllipsoid.E_ipe(
            xi, eta, coefficients
        )
        return result
