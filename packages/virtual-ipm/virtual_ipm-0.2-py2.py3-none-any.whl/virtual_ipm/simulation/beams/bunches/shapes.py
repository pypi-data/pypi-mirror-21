...
class BunchShape(Mutable):
    ...
    def normalized_density_at(self, positions):
        """
        Retrieve the particle density at the given positions. The density should be normalized to
        ...
        particle_densities : :class:`~numpy.ndarray`, shape (N,)
            The particle densities at the given positions.
        """
        raise NotImplementedError
...
@parametrize(
    Duplet[PhysicalQuantity]('TransverseSigma', 'm'),
    PhysicalQuantity('LongitudinalSigma', 'm', info='Given in the bunch frame.')
)
class Gaussian(BunchShape):
    """This component represents a bunch that is Gaussian in all dimensions."""
    ...
    def density_as_string(self):
        ...
        self.log.debug('density: %s', density_template)
...
@parametrize(
    PhysicalQuantity('SemiMajorAxis', unit='m'),
...
class ParabolicEllipsoid(BunchShape):
    ...
    def normalized_density_at(self, positions):
        ...
        a = self.semi_major_axis
        b = self.semi_minor_axis
        r_square = positions[0]**2 + positions[1]**2
        z_square = positions[2]**2
        result[inside] = 15. / (8. * pi * a * b**2) * (
            1. - r_square/b**2 - z_square/a**2
        )
        result[outside] = 0.
        return result
