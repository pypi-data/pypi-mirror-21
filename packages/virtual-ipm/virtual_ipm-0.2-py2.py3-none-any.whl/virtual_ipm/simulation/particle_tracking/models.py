...
class RungeKutta4(ParticleTrackingModel):
    ...
    def propagate(self, particles, progress):
        ...
        relativistic_gamma = particles.relativistic_gamma
        velo = numpy.array(particles.momentum) / (relativistic_gamma * particles.mass)
        qm = particles.charge / (relativistic_gamma * particles.mass)
        ...
        particles.momentum = velo * relativistic_gamma * particles.mass
...
class Boris(ParticleTrackingModel):
    ...
    def __init__(self, em_fields, setup):
        ...
        self.runge_kutta = RungeKutta4(Namespace(time_delta=-self.dt/2))

    def prepare(self):
        """
        Prepare the Runge-Kutta model which is used to initialize particles later on.
        """
        self.runge_kutta.em_fields = self.em_fields
...
class RadialFields(ParticleTrackingModel):
    ...
    References
    ----------
    G.Iadarola: "Electron Cloud Studies For CERN Particle Accelerators and
    ...
    def propagate(self, particles, progress):
        ...
        relativistic_gamma = particles.relativistic_gamma
        posi = particles.position
        velo = particles.momentum / (relativistic_gamma * particles.mass)

        E = self.electric_field_for(particles, progress)
        B = self.magnetic_field_for(particles, progress)
        qm = abs(particles.charge) / (relativistic_gamma * particles.mass)
        ...
        particles.momentum = \
            numpy.array([velox, veloy, veloz]) * relativistic_gamma * particles.mass


@parametrize(
    PhysicalQuantity('MagneticFieldStrengthInY', unit='T')
)
class RadialFieldsBx0(ParticleTrackingModel):
    ...
    References
    ----------
    PyECLOUD-BGI: ``dynamics.dyn_stp_vect2`` (G.Iadarola)
    """
    ...
    def propagate(self, particle, progress):
        relativistic_gamma = particle.relativistic_gamma

        qm = self.qm / relativistic_gamma
        omegac = self.omegac / relativistic_gamma
        dt = self.dt
        ...
        vx, vy, vz = particle.momentum / (relativistic_gamma * particle.mass)
        ...
        particle.momentum = numpy.array([vxn1, vyn1, vzn1]) * relativistic_gamma * particle.mass
