...
from anna import parametrize, Action, Choice, Number, PhysicalQuantity, String
import numpy
import scipy.constants as constants

from virtual_ipm.components import Component
import virtual_ipm.data.particle_types as particle_types
...
specific_particle_types = filter(
    lambda x: isinstance(getattr(particle_types, x), particle_types.ParticleType),
    dir(particle_types)
)
particle_type_choices = Choice(String('ParticleType'))
for specific_particle_type in specific_particle_types:
    particle_type_choices.add_option(specific_particle_type)
# noinspection PyTypeChecker
particle_type_parameter = Action(
    particle_type_choices,
    lambda x: getattr(particle_types, x)
)
...
@parametrize(
    ...
    Number('BunchPopulation'),
    particle_type_parameter,
)
class Bunch(Component):
    ...
    def __init__(
        ...
        self._scaled_population = self._particle_type.charge_number * self._bunch_population
    ...
    def as_json(self):
        ...
        attributes.update({
            ...
            'scaled population': '%e' % self._scaled_population,
    ...
    def charge_density_at(self, position_four_vectors, progress):
        ...
        """
        return self._scaled_population * (
    ...
    def generate_positions_in_transverse_plane(self, progress, count, z):
        ...
        positions[:2] = transverse_positions
        positions[2] = z
        return positions
