...
class InterfaceView(Widgets.QWidget):
    ...
    def __init__(self, interface, module, parent=None):
        ...
        model_classes = filter(
            lambda cls: issubclass(cls, self._interface) and cls is not self._interface,
            filter(
    ...
    def load_from_source(self, configuration):
        ...
        try:
            index = filter(lambda t: t[1] == model_name,
                           map(lambda i: (i, self._model_dropdown.itemText(i)),
                               range(self._model_dropdown.count())))[0][0]
        except IndexError:
...
class BunchTrain(Widgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._view = InterfaceView(beam_factory.BunchTrain, beam_factory)
        layout = Widgets.QVBoxLayout()
...
class Beams(Widgets.QWidget):
    ...
    def _append_beam_from_dialog(self, dialog):
        ...
        beam_dialog_button = Widgets.QPushButton(six.text_type(dialog))
        # noinspection PyUnresolvedReferences
        beam_dialog_button.clicked.connect(show_beam_dialog)

        def adjust_title():
            beam_dialog_button.setText(six.text_type(dialog))
