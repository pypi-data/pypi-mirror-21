...
def get_timeseries(rics, fields='*', start_date=get_date_from_today(100), end_date=get_date_from_today(0), interval='daily', count=None,
    ...
    start_date: string or datetime.datetime or datetime.timedelta, optional
    ...
    end_date: string or datetime.datetime or datetime.timedelta, optional
    ...
    status_code = get_json_value(result['timeseriesData'], 'statusCode')

    if status_code == u'Error':
        print("get_timeseries - Error, code {0} ".format(status_code))
        return result
    ...
    data_frame = None
    if normalize:
         data_frame =  NormalizedDataFrame_Formatter(result).get_data_frame()
    else:
         data_frame = NiceDataFrame_Formatter(result).get_data_frame()
    ...
    def get_data_frame(self):
        ...
        for timeseries in timeseriesList:
            ...
            if error_code.lower() == 'error':
                print(ric, ': ', timeseries['errorMessage'], '\n')
                continue
            ...
            datapoints = pd.np.delete(datapoints, np.s_[timestamp_index], 1)  # remove timestamp column from numpy array
...
class NiceDataFrame_Formatter():
    ...
    def _get_frame_list(self):
        ...
        for timeseries in timeseriesList:
            ...
            timestamp_index = fields.index('TIMESTAMP')
            fields.pop(timestamp_index)  # remove timestamp from fields (timestamp is used as index for dataframe)
            for f in fields:  unique_fields.add(f)
            datapoints = pd.np.array(timeseries['dataPoints'])
            timestamps = pd.np.array(datapoints[:, timestamp_index], dtype='datetime64')  # index for dataframe
            datapoints = pd.np.delete(datapoints, pd.np.s_[timestamp_index], 1)  # remove timestamp column from numpy array
            df = pd.DataFrame(datapoints, columns=fields, index=timestamps)
            df.index.name = 'Date'
            data_frames.append(df)

        return data_frames, list(unique_rics), list(unique_fields)
