c
c
      subroutine dgauss(n,dalpha,dbeta,dzero,dweigh,ierr,de)
c
...
cf2py double precision intent(out),depend(n),dimension(n) :: dzero
cf2py double precision intent(out),depend(n),dimension(n) :: dweigh
cf2py integer intent(out) :: ierr
cf2py double precision intent(hide),depend(n),dimension(n) :: de
      deps = d1mach(3)
      if(n.lt.1) then
        ierr=-1
        return
