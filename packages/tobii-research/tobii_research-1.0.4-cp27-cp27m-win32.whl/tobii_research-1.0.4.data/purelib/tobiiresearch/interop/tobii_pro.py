...
def __subscription_callback(subscription_type, core_eyetracker, data):
    ...
    with __callback_lock:
        for callback in __callbacks.get((subscription_type, core_eyetracker), {}).itervalues():
...
def screen_based_calibration_enter_calibration_mode(core_eyetracker):
    status = __call_function("screen_based_calibration_enter_calibration_mode", (core_eyetracker,))
    _on_error_raise_exception(status[0])


def screen_based_calibration_leave_calibration_mode(core_eyetracker):
    status = __call_function("screen_based_calibration_leave_calibration_mode", (core_eyetracker,))
    _on_error_raise_exception(status[0])
...
def screen_based_calibration_compute_and_apply(core_eyetracker):
    status = __call_function("screen_based_calibration_compute_and_apply", (core_eyetracker,))
    try:
        _on_error_raise_exception(status[0])
