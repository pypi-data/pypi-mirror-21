...
#: on/off mapping for boolean values
B_ON_OFF = {
    True: "ON",
    False: "OFF"
}

#: datetime - NTP hint text
NTP_HINT = {
...
class LenaPowerstrip(object):
    ...
    def __setitem__(self, relais_idx, value):
        ...
        if relais_data_current[relais_idx] == relais_data[relais_idx]:
            raise AssertionError(
                "relais state was not altered!")

        if relais_data_current[relais_idx] != enable:
            raise AssertionError(
                "relais state does not meet expectation!")

    def _mangle_relais_arg(self, raw_list, max_value=0):
        items = list()
        ...
        for item in items:
            try:
                ...
                if max_value and value >= max_value:
                    raise ValueError(
                        "Too large! max={:d}".format(max_value))
                idx_items.add(value)
            except Exception, exc:
                self.log.warning(
                    "item {!r} is invalid: {!s}".format(item, exc))
    ...
    def toggle_relais(self, **kwargs):
        ...
        try:
            req = requests.post(
                urlparse.urljoin(self._base_url, "ctrl.htm"),
        ...
        if req.status_code / 100 > 2:
            raise LenaPowerstripRequestException(
                "Bad Response: {!r} {!s}".format(
    ...
    def get_status(self, **kwargs):
        ...
        """
        req = requests.get(
            urlparse.urljoin(self._base_url, "strg.cfg"),
                             auth=kwargs.get("auth", self.auth))
    ...
    def get_daten(self, **kwargs):
        ...
        try:
            req = requests.get(
                urlparse.urljoin(self._base_url, "daten.cfg"),
                                 auth=kwargs.get("auth", self.auth))
        except requests.exceptions.ConnectionError, rexc:
            self.log.error(rexc)
            raise LenaPowerstripRequestException("Connection Error")
    ...
    def cli_device_status(self):
        ...
        print "Host         : {hostname} ({mac_addr}) {ip_addr}".format(
            **device_information)
        print "Device       : {device} ({device_type}) v{ver}".format(
            **device_information)

        if self.args.verbose:
            print "Local time   : {time} -- \"{tainted}\"".format(
                time=device_information['time'].strftime("%Y-%m-%d %H:%M"),
                tainted=NTP_HINT[device_information['clock_status']])
            print "Temperature  : {temperature!s}".format(**device_information)

        if self.args.verbose == 0:
            print "Relais       : {:s}".format(
                ' '.join([B_ON_OFF[val] for val in relais_information]))
        else:
            for idx, val in enumerate(relais_information):
                print "Relais #{:d}    : {:s}".format(
                    idx, B_ON_OFF[val])
   
    def cli_alter_relais(self):
        current_relais = self.relais()
        relais_on = self._mangle_relais_arg(self.args.relais_on,
                                            max_value=len(current_relais))
        relais_off = self._mangle_relais_arg(self.args.relais_off,
                                             max_value=len(current_relais))
        mutual_exclusive = relais_on & relais_off

        if mutual_exclusive:
            self.log.warning("Very funny. Ignoring indice(s): {:s}".format(
                ','.join(sorted([str(x) for x in mutual_exclusive]))))

        relais_on ^= mutual_exclusive
        relais_off ^= mutual_exclusive

        anticipated = dict()
        if relais_on:
            self.log.info("Trying to enable relais: {:s}".format(
                ', '.join(sorted([str(x) for x in relais_on]))))
            for idx in relais_on:
                anticipated[idx] = True

        if relais_off:
            self.log.info("Trying to disable relais: {:s}".format(
                ', '.join(sorted([str(x) for x in relais_off]))))
            for idx in relais_off:
                anticipated[idx] = False

        changes = dict()
        for idx in anticipated:
            anticipated_value = anticipated[idx]
            if current_relais[idx] != anticipated[idx]:
                self.log.info(
                    "Relais #{:d} needs change: {!r} -> {!r}".format(
                        idx,
                        B_ON_OFF[current_relais[idx]],
                        B_ON_OFF[anticipated[idx]]))
                changes[idx] = anticipated[idx]

        self.toggle_relais(relais=changes.keys())
    ...
    def main(self):
        ...
        if not len(self.args.actions) and (not len(self.args.relais_on) and not len(self.args.relais_off)):
            self.log.warning("You answered without saying anything. That's politics.")
            return 2

        if self.args.relais_on or self.args.relais_off:
            self.cli_alter_relais()
        ...
        for action in self.args.actions:
            try:
                ...
                self.log.error('%s',
                               "Not implemented: {!r}?".format(action))
                rc = -2
