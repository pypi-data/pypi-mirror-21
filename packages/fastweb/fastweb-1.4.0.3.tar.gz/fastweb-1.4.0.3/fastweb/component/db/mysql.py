...
class SyncMysql(Mysql):
    ...
    def query(self, sql, **kwargs):
        ...
        Retry(self, '{obj}'.format(obj=self), self._query, sql, mysql_retry_policy, **kwargs).run_sync()

    def _query(self, sql, retry, **kwargs):
        ...
        try:
            self._kwargs = kwargs = {k: self._format_sql(v) for k, v in kwargs.iteritems()}
            self._sql = sql.format(**kwargs)

            self.recorder('INFO', '{obj} query start ({threadid})\n{sql}'.format(obj=self, threadid=self._conn.server_thread_id[0], sql=sql))
...
class AsynMysql(Mysql):
    ...
    def query(self, sql, **kwargs):
        ...
        mysql_retry_policy = RetryPolicy(times=1, error=MysqlError)
        effect = yield Retry(self, '{obj}'.format(obj=self), self._query, sql, mysql_retry_policy, **kwargs).run_asyn()
        raise Return(effect)
    ...
    def _query(self, sql, retry, **kwargs):
        ...
        try:
            ...
            with tool.timing('ms', 10) as t:
                yield self._cur.execute(sql, **kwargs)
            self.recorder('INFO',
                          '{obj} query successful\n{sql}\t[{time}]\t[{effect}]'.format(obj=self, sql=sql, time=t,
                                                                                       effect=self._cur.rowcount))
