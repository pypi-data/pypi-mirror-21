...
class Game:
	...
	# char action quest
	def action(self, action, questkey, silent=False):
		...
		if action in ['do', 'complete']:
			self.char.do(quest, complete=action=='complete', silent=silent)
		elif action in ['skip', 'fail']:
			self.char.skip(quest, fail=action=='fail', silent=silent)
		elif action == 'buy':
			self.char.buy(quest, silent=silent)
