...
class Character:
	...
	# buy action
	def buy(self, quest, silent=False):
		# price
		coins = quest.value
		xp = 0
		...
		if coins < 0:
			self.coins += coins
			self.log(f"{self.getName()} paid {-coins} coins for: {quest.description}.", silent)
		elif xp < 0:
			self.updateXP(xp)
			self.log(f"{self.getName()} lost {-xp} xp for: {quest.description}.", silent)
	...
	# skip
	def skip(self, quest, fail=False, silent=False):
		msg = []
		# skip / fail
		verb = 'skipped'
		if fail:
		...
		# prepare msg
		msg.append(f"{self.getName()} {verb}: {quest.description}")
		if coins != 0:
	...
	# skip with remove
	def fail(self, key, silent=False):
		self.skip(key, fail=True, silent=silent)
	
	
	# do
	def do(self, quest, complete=False, silent=False):
		msg = []

		xp = quest.value
		coins = math.floor(quest.value*randint(75,100)/100)
	...
	# complete quest
	def complete(self, quest, silent=False):
		self.do(quest, complete=True, silent=silent)
