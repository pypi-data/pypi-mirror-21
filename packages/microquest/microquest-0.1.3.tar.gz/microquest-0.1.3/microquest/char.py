...
class Character:
	...
	def buy(self, quest):
		...
		if coins < 0:
			self.coins += coins
			self.log(f"{self.getName()} paid {-coins} coins for: {quest.description}.")
		elif xp < 0:
			self.updateXP(xp)
			self.log(f"{self.getName()} lost {-xp} xp for: {quest.description}.")
			
		self.printStatus()
		self.save()

	# skip
	def skip(self, quest, fail=False):
		msg = []
		# skip / fail
		verb = 'skipped'
		if fail:
	...
	# skip with remove
	def fail(self, key):
		self.skip(key, fail=True)
	
	
	# do
	def do(self, quest, complete=False):
		msg = []
