...
ATOM = xcffib.xproto.Atom.ATOM
CARDINAL = xcffib.xproto.Atom.CARDINAL
WINDOW = xcffib.xproto.Atom.WINDOW
...
def set_supported(atoms):
    ...
    return c.core.ChangeProperty(xcffib.xproto.PropMode.Replace, root,
                                 atom('_NET_SUPPORTED'), ATOM, 32, len(atoms),
                                 packed)

def set_supported_checked(atoms):
    packed = struct.pack('I' * len(atoms), *atoms)
    return c.core.ChangePropertyChecked(xcffib.xproto.PropMode.Replace, root,
                                        atom('_NET_SUPPORTED'),
...
def request_current_desktop(desktop_number,
                            timestamp=xcffib.xproto.Time.CurrentTime):
    """
...
def set_supporting_wm_check(wid, child):
    ...
    return c.core.ChangeProperty(xcffib.xproto.PropMode.Replace, wid,
                                 atom('_NET_SUPPORTING_WM_CHECK'),
                                 WINDOW, 32, 1, packed)

def set_supporting_wm_check_checked(wid, child):
    packed = struct.pack('I', child)
    return c.core.ChangePropertyChecked(xcffib.xproto.PropMode.Replace, wid,
                                        atom('_NET_SUPPORTING_WM_CHECK'),
                                        WINDOW, 32, 1, packed)
...
def request_close_window(window, timestamp=xcffib.xproto.Time.CurrentTime,
                         source=1):
    """
...
def request_close_window_checked(window, timestamp=xcffib.xproto.Time.CurrentTime,
                                 source=1):
...
def request_moveresize_window(window, x=None, y=None, width=None,
                              height=None,
                              gravity=xcffib.xproto.Gravity.BitForget, source=1):
    """
...
def request_moveresize_window_checked(window, x=None, y=None, width=None,
                                      height=None,
                                      gravity=xcffib.xproto.Gravity.BitForget,
                                      source=1):
    flags = gravity
    flags |= source << 12
    if x is not None:
...
def request_restack_window(window, stack_mode=xcffib.xproto.StackMode.Above,
                           sibling=0, source=1):
    """
...
def request_restack_window_checked(window,
                                   stack_mode=xcffib.xproto.StackMode.Above,
                                   sibling=0, source=2):
...
def set_wm_name(window, wm_name):
    ...
    :rtype:         xcb.VoidCookie
    """
    return c.core.ChangeProperty(xcffib.xproto.PropMode.Replace, window,
                                 atom('_NET_WM_NAME'),
                                 atom('UTF8_STRING'), 8, len(wm_name), wm_name)

def set_wm_name_checked(window, wm_name):
    return c.core.ChangePropertyChecked(xcffib.xproto.PropMode.Replace, window,
                                        atom('_NET_WM_NAME'),
