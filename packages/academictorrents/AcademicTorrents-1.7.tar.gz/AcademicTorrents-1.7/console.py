...
class NavigationMenu:
    ...
    def main(self):
        ...
        self.loop = urwid.MainLoop(urwid.Filler(self.top, 'middle', 10), self.palette)
        self.loop.run()
        return getchosen()
...
class Presentation:
    ...
    def __init__(self):
        ...
        self.txt = urwid.Text(('banner',six.text_type('Academic Torrents')), align = 'center')
        self.streak = urwid.AttrMap(self.txt, 'streak')
    
    def main(self):
        """Run the cover page"""
        
        self.loop = urwid.MainLoop(self.placeholder, self.palette, unhandled_input = self.exit_presentation)
...
def main():
    ...
    if ( selected_option == six.text_type('from .torrent')):
        ...
        #GraphController(torrentFiles).main()
        myclient = Client()
