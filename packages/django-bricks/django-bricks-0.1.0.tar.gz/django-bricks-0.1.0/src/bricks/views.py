...
from bricks.client import Client, js_compile
from bricks.json import translator
...
class SrviceView(View):
    ...
    def get_data(self, request):
        ...
        try:
            ...
            payload = translator.loads(data_bytes.decode('utf8'))
    ...
    def get_payload(self, data):
        ...
        if result is not None:
            try:
                result = translator.dumps(result)
            except Exception as ex:
                response = http.HttpResponseServerError(ex)
                raise BadResponseError(response)
        if program is not None:
            try:
                program = translator.dumps(program)
            except Exception as ex:
                response = http.HttpResponseServerError(ex)
                raise BadResponseError(response)
        if error is not None:
            try:
                error = translator.dumps(error)
            except Exception as ex:
                response = http.HttpResponseServerError(ex)
                raise BadResponseError(response)
