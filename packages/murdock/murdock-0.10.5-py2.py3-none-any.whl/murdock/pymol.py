...
.. note::
    ...
    The method `.PyMOLPicture.run_script()` can call the PyMOL executable in a
    subprocess to execute the PyMOL script and create a PNG picture. However,
    PyMOL is called in batch mode (command-line argument ``-c``) and on some
    systems the PyMOL command `png` does not seem to work in batch mode (e.g.
    Windows 7 with PyMOL 1.1eval). Passing the environment variables
    `PYMOL_PATH`, `TK_LIBRARY`, `TIX_LIBRARY` and `TCL_LIBRARY` (via the `env`
    argument of `subprocess.call()`) does not seem to remedy the problem.
