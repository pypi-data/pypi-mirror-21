...
def cleanup_special_chars(string):
    ...
    # Special chars
    cleaned = re.sub(r'[ÀÁÂÄ]', r'A', cleaned)
    cleaned = re.sub(r'[ÈÉÊË]', r'E', cleaned)
    cleaned = re.sub(r'[ÍÌÎÏ]', r'I', cleaned)
    cleaned = re.sub(r'[ÒÓÔÖ]', r'O', cleaned)
    cleaned = re.sub(r'[ÙÚÛÜ]', r'U', cleaned)
    cleaned = re.sub(r'[áàâä]', r'a', cleaned)
    cleaned = re.sub(r'[éèêë]', r'e', cleaned)
    cleaned = re.sub(r'[íìîï]', r'i', cleaned)
    cleaned = re.sub(r'[óòôö]', r'o', cleaned)
    cleaned = re.sub(r'[úùûü]', r'u', cleaned)
    cleaned = re.sub(r'Æ', r'AE', cleaned)
    cleaned = re.sub(r'æ', r'ae', cleaned)
    cleaned = re.sub(r'Œ', r'OE', cleaned)
    cleaned = re.sub(r'œ', r'oe', cleaned)
    cleaned = re.sub(r'Ç', r'c', cleaned)
    cleaned = re.sub(r'ç', r'c', cleaned)
...
if client.islogged:
    ...
    while len(dossiers_fetched) > 0:
        ...
        dossiers_fetched = client.doget(
            ...
            dict(
                page=str(page),
                filter='{"and":[{"or":[{"ph:typeMetier":"%s"}]},{"or":[{"ph:soustypeMetier":"%s"}]}]}' % (type_filter, subtype_filter),
                pageSize=page_size
            )
        )
        ...
        if download:
            ...
            if not is_already_downloaded:
                ...
                if dossier['original'] == "true":
                    ...
                    original_url = "/api/node/content%3bph%3aoriginal/workspace/SpacesStore/{0}/{1}".format(dossier['id'], dossier_distant_original_name)
                ...
                if dossier['sig'] == "true":
                    sign_url = "/api/node/content%3bph%3asig/workspace/SpacesStore/{0}/{1}_sig.zip".format(dossier['id'], title_clean)
                ...
                pprint.warning("Already downloaded : {0} ({1}/{2})".format(dossier['id'], dossier_index + 1, len(dossiers_archive)))
