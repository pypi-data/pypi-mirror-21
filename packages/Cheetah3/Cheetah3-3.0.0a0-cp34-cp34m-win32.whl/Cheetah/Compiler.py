...
from .NameMapper import NotFound, valueForName, valueFromSearchList, valueFromFrameOrSearchList
...
class AutoMethodCompiler(MethodCompiler):
    ...
    def _addAutoSetupCode(self):
        ...
        if self.setting('useNameMapper'):
            argNames = set(arg[0] for arg in self._argStringList)
...
class ModuleCompiler(SettingsManager, GenUtils):
    ...
    def __init__(self, source=None, file=None,
        ...
        if isinstance(file, (str, unicode)): # it's a filename.
            ...
            with ((codecs.open if PY2 else open)(file, 'r', encoding=encoding)
                  if encoding else
                  open(file, 'r')) as f:
                source = f.read()
            self._filePath = file
...
""" %   {'header': self.moduleHeader(),
    ...
    def specialVars(self):
        ...
        for key in sorted(theVars.keys()):
