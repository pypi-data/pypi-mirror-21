...
if PY2:
    ...
    def createMethod(func, cls):
       #return types.MethodType(func, cls)
        return BoundMethod(func)
    del io



from .Version import convertVersionStringToTuple, MinCompatibleVersionTuple
...
from .Compiler import Compiler, DEFAULT_COMPILER_SETTINGS
...
from .Utils.WebInputMixin import _Converter, _lookup, NonNumericInputError
...
class TemplatePreprocessor(object):
    ...
    def preprocess(self, source, file):
        ...
        compile_func = (templateAPIClass.compile.__func__
                        if PY2 else
                        templateAPIClass.compile)
        possibleKwArgs = [
            arg for arg in
            inspect.getargs(compile_func.__code__)[0]
...
class Template(Servlet):
    ...
    def compile(klass, source=None, file=None,
        ...
        if not isinstance(file, (NoneType, str, unicode, filetype)):
        ...
        if not isinstance(baseclass, (NoneType, str, unicode, type)):
        ...
        if not isinstance(moduleName, (NoneType, str, unicode)):
            raise TypeError(errmsg % ('moduleName', 'string or None'))
        __orig_file__ = None
        if not moduleName:
            if file and isinstance(file, (str, unicode)):
        ...
        if not isinstance(className, (NoneType, str, unicode)):
        ...
        if not isinstance(mainMethodName, (NoneType, str, unicode)):
        ...
        if not isinstance(cacheDirForModuleFiles, (NoneType, str, unicode)):
            raise TypeError(errmsg %
        ...
        baseclassName = None
        if baseclass:
            if isinstance(baseclass, (str, unicode)):
    ...
    def _normalizePreprocessorArg(klass, arg):
        ...
        if hasattr(arg, 'preprocess'):
            ...
            class WrapperPreprocessor(object):
                def preprocess(self, source, file):
    ...
    def _addCheetahPlumbingCodeToClass(klass, concreteTemplateClass):
        ...
        for methodname in klass._CHEETAH_requiredCheetahMethods:
            if not hasattr(concreteTemplateClass, methodname):
                ...
                method_func = method.__func__ if PY2 else method
                newMethod = createMethod(method_func, concreteTemplateClass)
        ...
        for classMethName in klass._CHEETAH_requiredCheetahClassMethods:
            if not hasattr(concreteTemplateClass, classMethName):
                method = getattr(klass, classMethName)
                method_func = method.__func__
                newMethod = classmethod(method_func)
                setattr(concreteTemplateClass, classMethName, newMethod)
    ...
    def __init__(self, source=None,
        ...
        if not isinstance(filter, (str, unicode, type)) and not \
        ...
        if not isinstance(filtersLib, (str, unicode, types.ModuleType)):
            raise TypeError(errmsgextra %
        ...
        if not errorCatcher is None:
            err = True
            if isinstance(errorCatcher, (str, unicode, type)):
                err = False
    ...
    def _initCheetahInstance(self,
        ...
        if errorCatcher:
            if isinstance(errorCatcher, (str, unicode)):
    ...
    def _handleCheetahInclude(self, srcArg, trans=None, includeFrom='file', raw=False):
        ...
        if _includeID not in self._CHEETAH__cheetahIncludes:
            if not raw:
                if includeFrom == 'file':
                    source = None
                    if isinstance(srcArg, (str, unicode)):
...
def genParserErrorFromPythonException(source, file, generatedPyCode, exception):
    ...
    filename = file if isinstance(file, (str, unicode)) else None
