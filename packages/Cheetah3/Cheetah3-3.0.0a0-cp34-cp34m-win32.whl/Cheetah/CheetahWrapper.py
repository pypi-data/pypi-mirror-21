...
from .Template import Template, DEFAULT_COMPILER_SETTINGS
...
class CheetahWrapper(object):
    ...
    def parseOpts(self, args):
        ...
        pao('--shbang', dest='shbang', default='#!/usr/bin/env python', help='Specify the shbang to place at the top of compiled templates, e.g. --shbang="#!/usr/bin/python3.4"')
...
Files are %s""", args, pprint.pformat(vars(opts)), files)
    ...
    def test(self):
        ...
        try:
            with open(TEST_WRITE_FILENAME, 'w'): pass
        except:
            sys.exit("""\
...
you do have write permission to and re-run the tests.""")
    ...
    def _compileOrFill(self):
        ...
        if files == ["-"]:
            ...
            for root, _, filenames in os.walk(idir, followlinks=True):
                for fname in filenames:
                    fpath = os.path.join(root, fname)
                    if fpath.endswith(iext):
                        sourceFiles.append(fpath)
        elif not files:
    ...
    def _checkForCollisions(self, bundles):
        ...
        for dst in sorted(dstSources.keys()):
...
be named according to the same rules as Python modules.""" % tup)
        ...
        if self.opts.stdout:
            ...
            with ((codecs.open if PY2 else open)(dst, 'w', encoding=encoding)
                  if encoding else
                  open(dst, 'w')) as f:
