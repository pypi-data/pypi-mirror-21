...
def cachedRegex(pattern):
    try:
        ...
        _regexCache[pattern] = cacheItem = re.compile(pattern)
        return cacheItem
...
class ArgList(object):
    ...
    def merge(self):
        defaults = ((d.strip() if isinstance(d, (str, unicode)) else None) for d in self.defaults)
...
class _LowLevelParser(SourceReader):
    ...
    def matchDirectiveName(self, directiveNameChars=identchars+'0123456789-@'):
        startPos = self.pos()
        possibleMatches = set(self._directiveNamesAndParsers.keys())
        name = ''
        match = None
        ...
        while not self.atEnd():
            ...
            if name == '@':
                if not self.atEnd() and self.peek() in identchars:
                    match = '@'
                break
            possibleMatches = set(dn for dn in possibleMatches if dn.startswith(name))
...
class _HighLevelParser(_LowLevelParser):
    ...
    def cleanup(self):
        ...
        """
        self._macros.clear()
        for macroDetails in self._macroDetails.values():
    ...
    def eatExtends(self):
        ...
        if self.setting('allowExpressionsInExtendsDirective'):
            ...
            baseName = ', '.join(self.getCommaSeparatedSymbols())
    ...
    def eatSet(self):
        ...
        class Components(object): pass # used for 'set global'
    ...
    def eatDefMacro(self):
        ...
        class MacroDetails(object): pass
    ...
    def eatMacroCall(self):
        ...
        elif hasattr(macro, '__call__') and hasattr(macro.__call__, '__func__'):
