...
def calc_tangential_velocity(feedrate, axial_travel, diameter, theta):
	...
	time = (hyp/feedrate) 
	w = (2 * math.pi)/time #angular velocity = 2pi/t 
	tangential_velocity = w * (diameter/2)  #tangential velocity = w*r
...
def main_gcode(current_layer, filament_width,x2, y, n, layer_height):
    ...
    while (a/n < 1-increment_size and (current_layer) % 2 == 0):
        ...
        commands.append("M7 M8 G1 X{:.5f} Y{:.5f}".format(x2, mm_per_rev*(y+(a/n))))
        commands.append("M9")
        a += 1
        #print "start point", a
        #print "winding ratio", (a/n)
        commands.append("G1 Y{:.5f}".format(mm_per_rev*(y+(a/n))))
        commands.append("M7 M8 G1 X{:.5f} Y{:.5f}".format(0, mm_per_rev*(a/n)))
        commands.append("M9")
    ...
    while (a/n < 1-increment_size and (current_layer) % 2 == 1):
        ...
        #print "start point", a
        #print "winding ratio", (a/n)
        commands.append("M7 M8 G1 X{:.5f} Y{:.5f}".format(x2, (-1) * mm_per_rev*(y+(a/n))))
        commands.append("M9")
        a += 1
        #print "start point", a
        #print "winding ratio", (a/n)
        ...
        commands.append("M7 M8 G1 X{:.5f} Y{:.5f}".format(0, (-1)*mm_per_rev*(a/n)))
        commands.append("M9")
        a += 1
        #print "start point", a
        #print "winding ratio", (a/n)
...
def min_angle_print(current_layer,x2, y, layer_height ):
    ...
    if (current_layer %2  == 0):
        ...
        commands.append("M7 M8 G1 X{:.5f} Y{:.5f}".format(x2, mm_per_rev*y))
        commands.append("M9")

    else:
        #commands.append("G0 X{:.5f} Y{:.5f}".format(x2, mm_per_rev*y))
        commands.append("M7 M8 G1 X{:.5f} Y{:.5f}".format(x2, (-1)*mm_per_rev*y))
        commands.append("M9")
...
def tpg(axial_travel, filament_width_og, printbed_diameter, final_diameter, helix_angle, smear_factor, feedrate_og):
    ...
    #calculate minimum print angle based of filament width
    ...
    while (current_layer < layers):
        ...
        #calculate the distance traveled in the axial direction with every rotation
        ...
        y = axial_travel/x_move_per_rev			#revolutions per winding         
