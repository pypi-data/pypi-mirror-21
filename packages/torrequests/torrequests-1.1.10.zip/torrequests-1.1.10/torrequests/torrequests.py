...
class Tor(object):
	...
	def get(self, url, data = None, files = None, cookies = None, headers = None, auth = None, redirects = True):
		"""Make a GET request"""
		...
		return self.session.get(url=url, data=data, files=files, cookies=_cookies, headers=_headers, auth=auth, allow_redirects = redirects)

	def post(self, url, data = None, files = None, cookies = None, headers = None, auth = None, redirects = True):
		"""Make a POST request"""
		_cookies = self.__intersect(self.cookies, cookies)
		_headers = self.__intersect(self.headers, headers)
		_auth = auth if auth is not None else self.auth
		return self.session.post(url=url, data=data, files=files, cookies=_cookies, headers=_headers, auth=auth, allow_redirects = redirects)
