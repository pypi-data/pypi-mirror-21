...
def put(path, info, target, target_args, data_key, spec):
    ...
    if data_key:
        return upload_data(info, table_name, data_key, values_map)
    else:
        return put_file(
            path,
            spec,
...
def upload_data(info, table_name, data_key, values_map):
    data = info.get('data', {}).get(data_key)
    if data is None:
        log.error('No data available for data-key: %s', data_key)
    put_data(table_name, data, values_map)
    return OK
...
def put_file(
    ...
    for file in files:
        ...
        res = put_data(table_name, data, values_map)
        if res != OK:
            return res
...
def put_data(table_name, data, values_map):
